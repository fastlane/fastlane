  ...
  describe FastlaneCore::ItunesTransporter do
    def shell_upload_command(provider_short_name = nil)
      ...
      unless FastlaneCore::Helper.windows?
        escaped_password = escaped_password.gsub("\\'") { "'\"\\'\"'" }
    ...
    def java_upload_command(provider_short_name = nil)
      [
        ...
        "-Djava.ext.dirs=#{FastlaneCore::Helper.transporter_java_ext_dir
          .shellescape}",
        '-XX:NewSize=2m',
        '-Xms32m',
        '-Xmx1024m',
        ...
        "-classpath #{FastlaneCore::Helper.transporter_java_jar_path
          .shellescape}",
    ...
    describe 'with Xcode 7.x installed' do
      ...
      describe 'by default' do
        describe 'upload command generation' do
          it 'generates a call to java directly' do
            ...
            expect(transporter.upload('my.app.id', '/tmp')).to eq(
                        java_upload_command
                      )
          end
        end
        ...
        describe 'download command generation' do
          it 'generates a call to java directly' do
            transporter = FastlaneCore::ItunesTransporter.new(email, password)
            expect(transporter.download('my.app.id', '/tmp')).to eq(
                        java_download_command
                      )
          end
        end
      ...
      describe 'use_shell_script is false with a itc_provider short name set' do
        ...
        describe 'download command generation' do
          it 'generates a call to java directly' do
            ...
            expect(transporter.download('my.app.id', '/tmp')).to eq(
                        java_download_command('abcd1234')
                      )
          end
        end
      ...
      describe 'use_shell_script is true with a itc_provider short name set' do
        ...
        describe 'download command generation' do
          it 'generates a call to java directly' do
            ...
            expect(transporter.download('my.app.id', '/tmp')).to eq(
                        shell_download_command('abcd1234')
                      )
          end
        end
      ...
      describe 'use_shell_script is true' do
        describe 'upload command generation' do
          it 'generates a call to the shell script' do
            transporter =
              FastlaneCore::ItunesTransporter.new(email, password, true)
            expect(transporter.upload('my.app.id', '/tmp')).to eq(
                        shell_upload_command
                      )
          end
      ...
      describe 'use_shell_script is false' do
        describe 'upload command generation' do
          it 'generates a call to java directly' do
            transporter =
              FastlaneCore::ItunesTransporter.new(email, password, false)
            expect(transporter.upload('my.app.id', '/tmp')).to eq(
                        java_upload_command
                      )
          end
    ...
    describe 'with `FASTLANE_ITUNES_TRANSPORTER_USE_SHELL_SCRIPT` set' do
      before(:each) do
        ...
        if FastlaneCore::Helper.windows?
          allow(File).to receive(:exist?).with('C:/Program Files (x86)/itms')
                     .and_return(true)
        end
      end
    ...
    describe 'with no special configuration' do
      ...
      describe 'upload command generation' do
        it 'generates the correct command' do
          ...
          if FastlaneCore::Helper.is_mac? &&
             FastlaneCore::Helper.xcode_version.start_with?('6.')
            command = shell_upload_command
          end
          # If we are on Mac with Xcode >= 9, switch to newer java command
          if FastlaneCore::Helper.is_mac? &&
             FastlaneCore::Helper.xcode_at_least?(9)
            command = java_upload_command_9
          end
      ...
      describe 'download command generation' do
        it 'generates the correct command' do
          ...
          if FastlaneCore::Helper.is_mac? &&
             FastlaneCore::Helper.xcode_version.start_with?('6.')
            command = shell_download_command
          end
          # If we are on Mac with Xcode >= 9, switch to newer java command
          if FastlaneCore::Helper.is_mac? &&
             FastlaneCore::Helper.xcode_at_least?(9)
            command = java_download_command_9
          end
    ...
    describe 'with simulated no-test environment' do
      ...
      describe 'and faked command execution' do
        ...
        it 'handles exceptions' do
          expect(FastlaneCore::FastlanePty).to receive(:spawn).and_raise(
                      StandardError,
                      "It's all broken now."
                    )
