describe FastlaneCore do
  describe FastlaneCore::PrintTable do
    before do
      ...
      @values = { cert_name: 'asdf', output: '..', a_bool: true, a_hash: {} }
    ...
    it 'prints out all the information in a nice table' do
      ...
      value =
        FastlaneCore::PrintTable.print_values(
          config: @config, title: title, hide_keys: %i[a_sensitive]
        )
      expect(value[:title]).to eq(title.green)
      expect(value[:rows]).to eq(
                  [%w[cert_name asdf], %w[output ..], ['a_bool', true]]
                )
    end
    ...
    it 'supports mask_keys property with symbols and strings' do
      value =
        FastlaneCore::PrintTable.print_values(
          config: @config, mask_keys: [:cert_name, 'a_bool']
        )
      expect(value[:rows]).to eq(
                  [
                    %w[cert_name ********],
    ...
    it 'supports hide_keys property with symbols and strings' do
      value =
        FastlaneCore::PrintTable.print_values(
          config: @config, hide_keys: [:cert_name, 'a_bool', :a_sensitive]
        )
      expect(value[:rows]).to eq([%w[output ..]])
    end
    ...
    it 'recurses over hashes' do
      ...
      value =
        FastlaneCore::PrintTable.print_values(
          config: @config, hide_keys: %i[cert_name a_bool]
        )
      expect(value[:rows]).to eq(
                  [
                    %w[output ..],
                    %w[a_hash.foo bar],
                    %w[a_hash.bar.foo bar],
                    %w[a_sensitive ********]
                  ]
                )
    end
    ...
    it 'supports hide_keys property in hashes' do
      ...
      value =
        FastlaneCore::PrintTable.print_values(
          config: @config,
          hide_keys: [:cert_name, :a_bool, 'a_hash.foo', 'a_hash.bar.foo']
        )
      expect(value[:rows]).to eq([%w[output ..], %w[a_sensitive ********]])
    end
    ...
    it 'supports printing default values and ignores missing unset ones ' do
      ...
      value = FastlaneCore::PrintTable.print_values(config: @config)
      expect(value[:rows]).to eq(
                  [%w[output .], ['a_bool', true], %w[a_sensitive ********]]
                )
    end
    ...
    describe 'Breaks down lines' do
      ...
      it 'middle truncate' do
        expect(FastlaneCore::PrintTable).to receive(:should_transform?)
                    .and_return(true)
        value =
          FastlaneCore::PrintTable.print_values(
            config: @config,
            hide_keys: %i[output a_bool a_sensitive],
            transform: :truncate_middle
          )
      ...
      it "doesn't break with long strings with no space" do
        ...
        value =
          FastlaneCore::PrintTable.print_values(
            config: rows, hide_keys: [], transform: :newline
          )
        expect(value[:rows].count).to eq(1)
      ...
      it 'raises an exception for invalid transform' do
        ...
        expect do
          FastlaneCore::PrintTable.print_values(
            config: @config, hide_keys: [], transform: :something_random
          )
    ...
    it 'supports non-Configuration prints' do
      value =
        FastlaneCore::PrintTable.print_values(
          config: { key: 'value' }, title: 'title'
        )
      expect(value[:rows]).to eq([%w[key value]])
    end
  end
end
