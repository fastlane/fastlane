  ...
  describe FastlaneCore::UpdateChecker do
    ...
    describe '#update_available?' do
      it 'no update is available' do
        ...
        expect(
          FastlaneCore::UpdateChecker.update_available?(name, '0.9.11')
        ).to eq(false)
      end
      ...
      it 'same version' do
        ...
        expect(
          FastlaneCore::UpdateChecker.update_available?(name, Fastlane::VERSION)
        ).to eq(false)
      end

      it 'new pre-release' do
        FastlaneCore::UpdateChecker.server_results[name] =
          [Fastlane::VERSION, 'pre'].join('.')
        expect(
          FastlaneCore::UpdateChecker.update_available?(name, Fastlane::VERSION)
        ).to eq(false)
      end
      ...
      it 'current: Pre-Release - new official version' do
        ...
        expect(
          FastlaneCore::UpdateChecker.update_available?(name, '0.9.1.pre')
        ).to eq(true)
      end
      ...
      it 'a new pre-release when pre-release is installed' do
        ...
        expect(
          FastlaneCore::UpdateChecker.update_available?(name, '0.9.1.pre1')
        ).to eq(true)
      end
    end
    ...
    describe '#update_command' do
      ...
      it 'works a custom gem name' do
        expect(
          FastlaneCore::UpdateChecker.update_command(gem_name: 'gym')
        ).to eq('sudo gem install gym')
      end

      it 'works with system ruby' do
        expect(FastlaneCore::UpdateChecker.update_command).to eq(
