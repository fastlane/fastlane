    ...
    describe 'ConfigItem input validation' do
      ...
      describe 'raises an error if short option is invalid' do
        ...
        it 'long string' do
          expect do
            FastlaneCore::ConfigItem.new(
              key: :foo, short_option: '-abc', description: 'foo'
            )
          end.to raise_error(
                      'short_option for key :foo must be a string of length 1'
                    )
        end
      end
          ...
          expect do
            FastlaneCore::ConfigItem.new(
              key: :foo, short_option: '-f', description: 'foo.'
                      ...
                      "Do not let descriptions end with a '.', since it's used for user inputs as well for key :foo"
                    )
        end
      end
    end
