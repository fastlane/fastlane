  ...
  describe FastlaneCore::ConfigurationFile do
    describe 'Properly loads and handles various configuration files' do
      let(:options) do
        [
          FastlaneCore::ConfigItem.new(
            key: :devices, description: 'desc', is_string: false, optional: true
          ),
          FastlaneCore::ConfigItem.new(
            key: :a_boolean,
            description: 'desc',
          ...
          FastlaneCore::ConfigItem.new(
            key: :ios_version, description: 'desc', default_value: '123'
          ),
          FastlaneCore::ConfigItem.new(
            key: :app_identifier,
            description: 'Mac',
            verify_block:
              proc do |value|
                unless value.split('.').count == 3
                  UI.user_error!("Invalid identifier '#{value}'")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :apple_id, description: 'yo', default_value: '123'
          )
        ]
      end
      ...
      it 'overwrites existing values' do
        ...
        config =
          FastlaneCore::Configuration.create(
            options,
            { app_identifier: 'detlef.app.super' }
          )
      ...
      it 'prints a warning if no value is provided' do
        important_message =
          "In the config file './fastlane_core/spec/fixtures/ConfigFileEmptyValue' you have the line apple_id, but didn't provide any value. Make sure to append a value right after the option name. Make sure to check the docs for more information"
        expect(FastlaneCore::UI).to receive(:important).with(important_message)
        expect(FastlaneCore::UI).to receive(:important).with(
                    "No values defined in './fastlane_core/spec/fixtures/ConfigFileEmptyValue'"
                  )
      ...
      describe 'Handling invalid broken configuration files' do
        it 'automatically corrects invalid quotations' do
          ...
          config.load_configuration_file(
            './fastlane_core/spec/fixtures/ConfigInvalidQuotation'
          )
        ...
        it 'properly shows an error message when there is a syntax error in the Fastfile' do
          ...
          expect do
            config.load_configuration_file(
              './fastlane_core/spec/fixtures/ConfigSytnaxError'
            )
          end.to raise_error(
      ...
      describe 'Prints out a table of summary' do
        it 'shows a warning when no values were found' do
          expect(FastlaneCore::UI).to receive(:important).with(
                      "No values defined in './fastlane_core/spec/fixtures/ConfigFileEmpty'"
                    )
          ...
          expect(Terminal::Table).to receive(:new).with(
                      {
                        rows: [
                          ...
                          [:apple_id, 'from_le_block']
                        ],
                        title:
                          "Detected Values from './fastlane_core/spec/fixtures/ConfigFileValid'"
                      }
                    )
      ...
      it 'allows using a custom block to handle special callbacks' do
        config = FastlaneCore::Configuration.create(options, {})
        config.load_configuration_file(
          'ConfigFileUnhandledBlock',
          proc do |method_sym, arguments, block|
            if method_sym == :some_custom_block
              if arguments == %w[parameter]
                expect { block.call(arguments.first, 'custom') }.to raise_error(
                            'Yeah: parameter custom'
                          )
              else
                UI.user_error!('no')
              end
      ...
      describe 'for_lane and for_platform support' do
        ...
        it 'reads global keys when platform and lane dont match' do
          with_env_values(
            'FASTLANE_PLATFORM_NAME' => 'osx', 'FASTLANE_LANE_NAME' => 'debug'
          ) do
            config = FastlaneCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')
        ...
        it "reads lane setting when platform doesn't match or no for_platform" do
          with_env_values(
            'FASTLANE_PLATFORM_NAME' => 'osx',
            'FASTLANE_LANE_NAME' => 'enterprise'
          ) do
            config = FastlaneCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')
        ...
        it "reads platform setting when lane doesn't match or no for_lane" do
          with_env_values(
            'FASTLANE_PLATFORM_NAME' => 'ios', 'FASTLANE_LANE_NAME' => 'debug'
          ) do
            config = FastlaneCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')
        ...
        it 'reads platform and lane setting' do
          with_env_values(
            'FASTLANE_PLATFORM_NAME' => 'ios', 'FASTLANE_LANE_NAME' => 'release'
          ) do
            config = FastlaneCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')

            expect(config[:app_identifier]).to eq(
                        'com.forplatformios.forlanerelease'
                      )
          end
        end
        ...
        it 'allows exceptions in blocks to escape but the configuration is still intact' do
          with_env_values(
            'FASTLANE_PLATFORM_NAME' => 'ios', 'FASTLANE_LANE_NAME' => 'explode'
          ) do
            config = FastlaneCore::Configuration.create(options, {})

            expect do
              config.load_configuration_file('ConfigFileForLane')
            end.to raise_error('oh noes!')
