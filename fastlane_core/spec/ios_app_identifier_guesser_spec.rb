...
describe FastlaneCore::IOSAppIdentifierGuesser do
  it 'returns nil if no clues' do
    ...
    expect(
      FastlaneCore::IOSAppIdentifierGuesser.guess_app_identifier([])
    ).to be_nil
  end

  describe 'guessing from command line args' do
    it 'returns iOS app_identifier if specified with -a' do
      args = %w[-a com.krausefx.app]
      expect(
        FastlaneCore::IOSAppIdentifierGuesser.guess_app_identifier(args)
      ).to eq('com.krausefx.app')
    end

    it 'returns iOS app_identifier if specified with --app_identifier' do
      args = %w[--app_identifier com.krausefx.app]
      expect(
        FastlaneCore::IOSAppIdentifierGuesser.guess_app_identifier(args)
      ).to eq('com.krausefx.app')
    end
  ...
  describe 'guessing from Appfile' do
    it 'returns iOS app_identifier found in Appfile' do
      expect(CredentialsManager::AppfileConfig).to receive(:try_fetch_value)
                  .with(:app_identifier)
                  .and_return('Appfile.bundle.id')
      expect(
        FastlaneCore::IOSAppIdentifierGuesser.guess_app_identifier([])
      ).to eq('Appfile.bundle.id')
    end
  end

  describe 'guessing from configuration files' do
    def allow_non_target_configuration_file(file_name)
      allow_any_instance_of(FastlaneCore::Configuration).to receive(
                                 :load_configuration_file
                               )
                                 .with(
                                 file_name,
                                 any_args
                               ) do |configuration, config_file_name|
        nil
      end
    end
    ...
    it 'catches common permuations of application id with swift matcher' do
      ...
      valid_id_strings.zip(expected_app_ids).each do |line, expected_app_id|
        app_id =
          FastlaneCore::IOSAppIdentifierGuesser.match_swift_application_id(
            text_line: line
          )
