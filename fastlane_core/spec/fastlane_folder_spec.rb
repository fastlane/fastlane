describe FastlaneCore::FastlaneFolder do
  ...
  describe '#path' do
    ...
    it "returns nil if the fastlane path isn't available" do
      Dir.chdir('/') { expect(FastlaneCore::FastlaneFolder.path).to eq(nil) }
    end
  ...
  describe '#fastfile_path' do
    it "uses a Fastfile that's inside a fastlane directory" do
      expect(FastlaneCore::FastlaneFolder.fastfile_path).to eq(
                  File.join('.', 'fastlane', fastfile_name)
                )
    end
  ...
  describe '#swift?' do
    it 'returns false if nil fastfile_path' do
      allow(FastlaneCore::FastlaneFolder).to receive(:fastfile_path).and_return(
                 nil
               )
