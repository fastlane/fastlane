class FeatureHelper
  def self.disabled_class_method; end

  def self.enabled_class_method; end
  ...
  describe FastlaneCore::Feature do
    describe 'Register a Feature' do
      it 'registers a feature successfully with environment variable and description' do
        expect do
          FastlaneCore::Feature.register(
            env_var: 'TEST_ENV_VAR_VALID', description: 'A valid test feature'
          )
      ...
      it 'raises an error if no description specified' do
        expect do
          FastlaneCore::Feature.register(
            env_var: 'TEST_ENV_VAR_INVALID_NO_DESCRIPTION'
          )
        end.to raise_error('Invalid Feature')
      end
    end
    ...
    describe '#enabled?' do
      before(:all) do
        FastlaneCore::Feature.register(
          env_var: 'TEST_ENV_VAR_FOR_ENABLED_TESTS',
          description: 'Test feature for enabled? tests'
        )
      end

      it 'reports unregistered features as not enabled' do
        expect(
          FastlaneCore::Feature.enabled?('TEST_ENV_VAR_NOT_REGISTERED')
        ).to be_falsey
      end
      ...
      it 'reports features for missing environment variables as disabled' do
        expect(
          FastlaneCore::Feature.enabled?('TEST_ENV_VAR_FOR_ENABLED_TESTS')
        ).to be_falsey
      end
    ...
    describe 'Register feature methods' do
      ...
      it 'Calls disabled instance method with disabled environment variable' do
        with_env_values('TEST_ENV_VAR_FOR_METHOD_TESTS' => '0') do
          ...
          FastlaneCore::Feature.register_instance_method(
            klass: FeatureHelper,
            symbol: :test_method,
