    ...
    describe 'Create a new Configuration Manager' do
      ...
      it 'raises an error if no array is given' do
        expect do
          FastlaneCore::Configuration.create('string', {})
                    ...
                    'available_options parameter must be an array of ConfigItems but is String'
                  )
      end

      it 'raises an error if array contains invalid elements' do
        expect do
          FastlaneCore::Configuration.create(%w[string], {})
        end.to raise_error(
                    'available_options parameter must be an array of ConfigItems. Found String.'
                  )
      end
      ...
      it 'raises an error if the option of a given value is not available' do
        expect do
          FastlaneCore::Configuration.create([], { cert_name: 'value' })
                    ...
                    "Could not find option 'cert_name' in the list of available options: "
                  )
      end
        ...
        expect do
          FastlaneCore::Configuration.create(
            ...
            {}
                    ...
                    "Do not let descriptions end with a '.', since it's used for user inputs as well for key :cert_name"
                  )
      end
      ...
      describe 'config conflicts' do
        it 'raises an error if a key was used twice' do
          expect do
            FastlaneCore::Configuration.create(
              [
                FastlaneCore::ConfigItem.new(key: :cert_name, env_name: 'asdf'),
                FastlaneCore::ConfigItem.new(key: :cert_name, env_name: 'asdf')
              ],
              {}
            )
          end.to raise_error(
                      "Multiple entries for configuration key 'cert_name' found!"
                    )
        end
        ...
        it 'raises an error if a short_option was used twice' do
          conflicting_options = [
            FastlaneCore::ConfigItem.new(
              key: :foo, short_option: '-f', description: 'foo'
            ),
            FastlaneCore::ConfigItem.new(
              key: :bar, short_option: '-f', description: 'bar'
            )
          ]
        ...
        it 'raises an error for unresolved conflict between options' do
          conflicting_options = [
            FastlaneCore::ConfigItem.new(
              key: :foo, conflicting_options: %i[bar oof]
            ),
          ...
          expect do
            FastlaneCore::Configuration.create(conflicting_options, values)
                      ...
                      "Unresolved conflict between options: 'foo' and 'bar'"
                    )
        end
        ...
        it 'calls custom conflict handler when conflict happens between two options' do
          conflicting_options = [
            FastlaneCore::ConfigItem.new(
              key: :foo,
              conflicting_options: %i[bar oof],
              conflict_block:
                proc do |value|
                  UI.user_error!(
                    "You can't use option '#{value.key}' along with 'foo'"
                  )
                end
            ),
            FastlaneCore::ConfigItem.new(key: :bar),
            FastlaneCore::ConfigItem.new(
              ...
              conflict_block:
                proc do |value|
                  UI.user_error!(
                    "You can't use option '#{value.key}' along with 'oof'"
                  )
                end
            )
          ]
      ...
      describe 'data_type' do
        it 'sets the data type correctly if `is_string` is not set but type is specified' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', type: Array
            )
        ...
        it 'sets the data type correctly if `is_string` is set but the type is specified' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', is_string: true, type: Array
            )
        ...
        it 'sets the data type correctly if `is_string` is set but the type is not specified' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', is_string: true
            )
      ...
      describe 'arrays' do
        it 'returns Array default values correctly' do
          ...
          expect(config[:foo]).to eq(%w[5 4 3 2 1])
        end
        ...
        it 'returns Array environment variable values correctly' do
          ...
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', env_name: 'FOO', type: Array
            )
          config = FastlaneCore::Configuration.create([config_item], {})
      ...
      describe 'auto_convert_value' do
        it 'auto converts string values to Integers' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', type: Integer
            )
        ...
        it 'auto converts string values to Floats' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', type: Float
            )
        ...
        it 'auto converts Array values to Strings if allowed' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :xcargs, description: 'xcargs', type: :shell_string
            )
        ...
        it 'does not auto convert Array values to Strings if not allowed' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :xcargs, description: 'xcargs', type: String
            )
        ...
        it 'auto converts nil to nil when type is not specified' do
          config_item =
            FastlaneCore::ConfigItem.new(key: :foo, description: 'foo')
        ...
        it 'auto converts strings to integers' do
          c = [FastlaneCore::ConfigItem.new(key: :int_value, type: Integer)]
          config = FastlaneCore::Configuration.create(c, { int_value: '10' })
        ...
        it "auto converts '0' to the integer 0" do
          c = [FastlaneCore::ConfigItem.new(key: :int_value, type: Integer)]
          config = FastlaneCore::Configuration.create(c, { int_value: '0' })
      ...
      describe 'validation' do
        it 'raises an exception if the data type is not as expected' do
          ...
          expect { config_item.valid?('ABC') }.to raise_error(
                      FastlaneCore::Interface::FastlaneError,
                      "'foo' value must be a Float! Found String instead."
                    )
        end
        ...
        it 'verifies the default value as well' do
          ...
          expect do
            @config = FastlaneCore::Configuration.create([c], {})
                      ...
                      "Invalid default value for output, doesn't match verify_block"
                    )
        end
        ...
        it 'calls verify_block for non nil values' do
          c =
            FastlaneCore::ConfigItem.new(
              ...
              type: Object,
              verify_block:
                proc { |value| UI.user_error!("'#{value}' is not valid!") }
            )
          [
            true,
            false,
            ...
            expect { c.valid?(value) }.to raise_error(
                        "'#{value}' is not valid!"
                      )
          end
        end
        ...
        it 'verifies type' do
          ...
          expect { c.valid?('a string') }.to raise_error(
                      "'param' value must be a Float! Found String instead."
                    )
        end
      ...
      describe 'deprecation', focus: true do
        it 'deprecated message changes the description' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo', deprecated: 'replaced by bar'
            )
          expect(config_item.description).to eq(
                      '**DEPRECATED!** replaced by bar - foo'
                    )
        end
        ...
        it 'deprecated boolean changes the description' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, description: 'foo. use bar instead', deprecated: true
            )
          expect(config_item.description).to eq(
        ...
        it 'deprecated messages replaces empty description' do
          config_item =
            FastlaneCore::ConfigItem.new(
              key: :foo, deprecated: 'replaced by bar'
            )
          expect(config_item.description).to eq(
            ...
            config_item =
              FastlaneCore::ConfigItem.new(
                ...
                deprecated: 'replaced by bar'
                      ...
                      FastlaneCore::Interface::FastlaneCrash,
                      'Deprecated option must be optional'
                    )
        end
        ...
        it "doesn't display a deprecation message when loading a config if a deprecated option doesn't have a value" do
          ...
          expect(FastlaneCore::UI).to receive(:deprecated).with(
                      "Using deprecated option: '--foo' (replaced by bar)"
                    )
      ...
      describe 'misc features' do
        it 'makes it non optional by default' do
          c = FastlaneCore::ConfigItem.new(key: :test, default_value: '123')
        ...
        it 'takes the values from the environment if available' do
          c = FastlaneCore::ConfigItem.new(key: :test, env_name: 'FL_TEST')
          config = FastlaneCore::Configuration.create([c], {})
        ...
        it 'can push and pop configuration values' do
          ...
          config =
            FastlaneCore::Configuration.create([name, other, platform], {})
      ...
      describe 'Automatically removes the --verbose flag' do
        ...
        it "doesn't remove --verbose if it's a valid option" do
          options = [
            FastlaneCore::ConfigItem.new(key: :verbose, is_string: false)
          ]
          config =
            FastlaneCore::Configuration.create(options, { verbose: true })
      ...
      describe 'Use a valid Configuration Manager' do
        before do
            ...
            FastlaneCore::ConfigItem.new(
              ...
              verify_block:
                proc do |value|
                  unless value.to_i > 0
                    UI.user_error!(
                      'Please enter a valid positive number of seconds'
                    )
                  end
                end
            )
          ...
          @values = {
            cert_name: 'asdf', output: '..', wait_processing_interval: 10
          }
        ...
        describe '#keys' do
          it 'returns all available keys' do
            expect(@config.all_keys).to eq(
                        %i[cert_name output wait_processing_interval]
                      )
          end
        end
        ...
        describe 'fetch' do
          it 'raises an error if a non symbol was given' do
            expect { @config.fetch(123) }.to raise_error(
                        "Key '123' must be a symbol. Example :app_id."
                      )
          end

          it 'raises an error if this option does not exist' do
            expect { @config[:asdfasdf] }.to raise_error(
                        "Could not find option 'asdfasdf' in the list of available options: cert_name, output, wait_processing_interval"
                      )
          end
          ...
          it 'auto converts the value after asking the user for one' do
            ...
            config_item =
              FastlaneCore::ConfigItem.new(
                ...
                is_string: false,
                type: Array,
                # we actually allow String and Array here
                skip_type_validation: true,
            ...
            expect(FastlaneCore::UI).to receive(:input).and_return(
                        'app.identifier'
                      )
            expect(config[:app_identifiers].class).to eq(Array)
            expect(config[:app_identifiers]).to eq(%w[app.identifier])
          end
        end

        describe 'verify_block' do
          it "throws an error if the key doesn't exist" do
            expect { @config.set(:non_existing, 'value') }.to raise_error(
                        "Could not find option 'non_existing' in the list of available options: cert_name, output, wait_processing_interval"
                      )
          end

          it "throws an error if it's invalid" do
            expect { @config.set(:output, 132) }.to raise_error(
                        "'output' value must be a String! Found #{123
                          .class} instead."
                      )
            expect do
        ...
        describe 'Parameter priority order' do
          it 'prioritizes CLI values over everything else' do
            ...
            config_item =
              FastlaneCore::ConfigItem.new(
                key: :item, env_name: 'abc', default_value: 'val default'
              )
            config =
              FastlaneCore::Configuration.create(
                [config_item],
          ...
          it 'asks if no other option' do
            ...
            config_item =
              FastlaneCore::ConfigItem.new(
                key: :item, env_name: 'abc', optional: false
              )
            config = FastlaneCore::Configuration.create([config_item], {})
