  ...
  describe FastlaneCore::Helper do
    ...
    describe '#json_file?' do
      it 'should return false on invalid json file' do
        expect(
          FastlaneCore::Helper.json_file?(
            './fastlane_core/spec/fixtures/json_file/broken'
          )
        ).to be(false)
      end
      it 'should return true on valid json file' do
        expect(
          FastlaneCore::Helper.json_file?(
            './fastlane_core/spec/fixtures/json_file/valid'
          )
        ).to be(true)
      end
    end
    ...
    describe '#keychain_path' do
      it 'finds file in current directory' do
        ...
        expect(FastlaneCore::Helper.keychain_path('test.keychain')).to eq(
      ...
      it 'finds file in current directory with spaces and \"' do
        ...
        expect(
          FastlaneCore::Helper.keychain_path('\\"\\ test\\ \\".keychain')
        ).to eq(File.expand_path(found))
      end
    end
    ...
    describe 'Xcode' do
      ...
      it '#xcode_path', requires_xcode: true do
        ...
        expect(FastlaneCore::Helper.xcode_path).to match(
                    %r{/Applications/Xcode.*.app/Contents/Developer/}
                  )
      end

      it '#transporter_path', requires_xcode: true do
        expect(FastlaneCore::Helper.transporter_path).to match(
                    %r{/Applications/Xcode.*.app/Contents/Applications/Application Loader.app/Contents/itms/bin/iTMSTransporter}
                  )
      end
    ...
    describe '#zip_directory' do
      ...
      it 'creates correct zip command with contents_only set to false with default print option (true)' do
        expect(FastlaneCore::Helper).to receive(:backticks).with(
                    "cd '#{directory}' && zip -r '#{the_zip}' 'to_zip'",
                    print: true
                  )
                    .exactly(1)
                    .times

        FastlaneCore::Helper.zip_directory(
          directory_to_zip,
          the_zip,
          contents_only: false
        )
      ...
      it 'creates correct zip command with contents_only set to true with print set to false' do
        expect(FastlaneCore::Helper).to receive(:backticks).with(
                    "cd '#{directory_to_zip}' && zip -r '#{the_zip}' *",
                    print: false
                  )
                    .exactly(1)
                    .times
        ...
        FastlaneCore::Helper.zip_directory(
          directory_to_zip,
          the_zip,
          contents_only: true, print: false
        )
      end
    end
  end
