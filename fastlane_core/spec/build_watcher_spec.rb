...
describe FastlaneCore::BuildWatcher do
  context '.wait_for_build_processing_to_be_complete' do
    ...
    it 'returns an already-active build' do
      expect(Spaceship::TestFlight::Build).to receive(:all_processing_builds)
                  .and_return([])
      expect(Spaceship::TestFlight::Build).to receive(:latest).and_return(
                  active_build
                )
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([active_build])

      expect(UI).to receive(:success).with(
                  "Build #{active_build.train_version} - #{active_build
                    .build_version} is already being tested"
                )
      found_build =
    ...
    it 'returns a ready to submit build' do
      ...
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([ready_build])

      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{ready_build
                    .train_version} - #{ready_build.build_version}"
                )
      found_build =
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
          app_id: 'some-app-id',
    ...
    it 'returns a export-compliance-missing build' do
      expect(Spaceship::TestFlight::Build).to receive(:all_processing_builds)
                  .and_return([])
      expect(Spaceship::TestFlight::Build).to receive(:latest).and_return(
                  export_compliance_required_build
                )
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([export_compliance_required_build])

      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{export_compliance_required_build
                    .train_version} - #{export_compliance_required_build
                    .build_version}"
                )
      found_build =
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
    ...
    it 'returns an review rejected build' do
      ...
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([review_rejected_build])

      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{review_rejected_build
                    .train_version} - #{review_rejected_build.build_version}"
                )
      found_build =
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
          app_id: 'some-app-id',
    ...
    it 'waits when a build is still processing' do
      expect(Spaceship::TestFlight::Build).to receive(:all_processing_builds)
                  .and_return([processing_build])
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([processing_build], [ready_build])
      expect(FastlaneCore::BuildWatcher).to receive(:sleep)

      expect(UI).to receive(:message).with(
                  "Waiting for App Store Connect to finish processing the new build (#{ready_build
                    .train_version} - #{ready_build.build_version})"
                )
      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{ready_build
    ...
    it 'watches the latest build when more than one build is processing' do
      expect(Spaceship::TestFlight::Build).to receive(:all_processing_builds)
                  .and_return([processing_build, old_processing_build])
      ...
      expect(UI).to_not(
        receive(:important).with(
          "Started watching build #{ready_build.train_version} - #{ready_build
            .build_version} but expected 1.0 - 0"
        )
      )
      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{ready_build
    ...
    it 'watches the latest build and warns about expected build difference' do
      ...
      expect(UI).to receive(:important).with(
                  "Started watching build #{ready_build
                    .train_version} - #{ready_build
                    .build_version} but expected #{old_processing_build
                    .train_version} - #{old_processing_build.build_version}"
                )
      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{ready_build
                    .train_version} - #{ready_build.build_version}"
      ...
      found_build =
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
          app_id: 'some-app-id',
          platform: :ios,
          train_version: old_processing_build.train_version,
          build_version: old_processing_build.build_version
        )
    ...
    it 'waits for specified build to be processed when strict watch is enabled' do
      expect(Spaceship::TestFlight::Build).to_not(
        receive(:all_processing_builds)
      )
      expect(Spaceship::TestFlight::Build).to receive(:builds_for_train)
                  .and_return([ready_build], [old_ready_build])

      expect(UI).to_not(receive(:important))
      expect(UI).to receive(:success).with(
                  "Successfully finished processing the build #{old_ready_build
                    .train_version} - #{old_ready_build.build_version}"
                )
      found_build =
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
          app_id: 'some-app-id',
    ...
    it 'crashes if it cannot find a build to watch' do
      ...
      expect(UI).to receive(:crash!).with(
                  'Could not find a build for app: some-app-id on platform: ios'
                )
                  .and_call_original
      expect do
        FastlaneCore::BuildWatcher.wait_for_build_processing_to_be_complete(
