...
def before_each_fastlane_core
  ...
  ['invalid', '', 0, '284882215', %w[338986109 FR]].each do |current|
    if current.kind_of?(Array)
      id = current[0]
      country = current[1]
      url = "https://itunes.apple.com/lookup?id=#{id}&country=#{country}"
      body_file =
        "fastlane_core/spec/responses/itunesLookup-#{id}_#{country}.json"
    else
      id = current
  ...
  stub_request(
    :get,
    'https://itunes.apple.com/lookup?bundleId=com.facebook.Facebook'
  )
    .with(
    headers: {
      'Accept' => '*/*',
    ...
    body:
      File.read(
        'fastlane_core/spec/responses/itunesLookup-com.facebook.Facebook.json'
      ),
    headers: {}
  )
  ...
  stub_request(
    :get,
    'https://itunes.apple.com/lookup?bundleId=net.sunapps.invalid'
  )
    .with(
    headers: {
      'Accept' => '*/*',
    ...
    body:
      File.read(
        'fastlane_core/spec/responses/itunesLookup-net.sunapps.invalid.json'
      ),
    headers: {}
  )
end
...
def with_env_values(hash)
  hash.each { |k, v| stub_const('ENV', ENV.to_h.merge(k => v)) }
end
