...
describe FastlaneCore do
  describe FastlaneCore::DeviceManager do
    before(:all) do
      @simctl_output =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerSimctlOutputXcode7'
        )
      @system_profiler_output =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerSystem_profilerOutput'
        )
      @instruments_output =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerInstrumentsOutput'
        )
      @system_profiler_output_items_without_items =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerSystem_profilerOutputItemsWithoutItems'
        )
      @system_profiler_output_usb_hub =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerSystem_profilerOutputUsbHub'
        )
    ...
    it 'raises an error if xcrun CLI prints garbage simulator' do
      ...
      s = StringIO.new
      s.puts(response)
      expect(Open3).to receive(:popen3).with('xcrun simctl list devices')
                  .and_yield(nil, s, nil, nil)
      allow(Open3).to receive(:popen3).with('xcrun simctl list runtimes')
                 .and_yield(nil, s, nil, nil)

      expect { devices = FastlaneCore::Simulator.all }.to raise_error(
    ...
    describe 'properly parses the simctl output and generates Device objects for iOS simulator' do
      ...
      it 'Xcode 8' do
        ...
        simctl_output =
          File.read(
            './fastlane_core/spec/fixtures/DeviceManagerSimctlOutputXcode8'
          )
        expect(response).to receive(:read).and_return(simctl_output)
        expect(Open3).to receive(:popen3).with('xcrun simctl list devices')
                    .and_yield(nil, response, nil, nil)
      ...
      it 'Xcode 9' do
        ...
        simctl_output =
          File.read(
            './fastlane_core/spec/fixtures/DeviceManagerSimctlOutputXcode9'
          )
        expect(response).to receive(:read).and_return(simctl_output)
        expect(Open3).to receive(:popen3).with('xcrun simctl list devices')
                    .and_yield(nil, response, nil, nil)
    ...
    it 'properly parses the simctl output with unavailable devices and generates Device objects for all simulators' do
      ...
      simctl_output =
        File.read(
          './fastlane_core/spec/fixtures/DeviceManagerSimctlOutputXcode10BootedUnavailable'
        )
      expect(response).to receive(:read).and_return(simctl_output)
      expect(Open3).to receive(:popen3).with('xcrun simctl list devices')
                  .and_yield(nil, response, nil, nil)
    ...
    it 'properly parses system_profiler and instruments output and generates Device objects for iOS' do
      ...
      expect(Open3).to receive(:popen3).with('instruments -s devices')
                  .and_yield(nil, response, nil, nil)
    ...
    it 'parses runtime information properly to get the exact version information' do
      ...
      expect(Open3).to receive(:popen3).with('xcrun simctl list devices')
                  .and_yield(nil, response, nil, nil)
      thing = {}
      expect(thing).to receive(:read).and_return(
