...
module FastlaneCore
  ...
  class TransporterInputError < StandardError; end
  # The TransporterTransferError occurs when some error happens
  # while uploading or downloading something from/to iTC
  class TransporterTransferError < StandardError; end

  # Used internally
  class TransporterRequiresApplicationSpecificPasswordError < StandardError; end
  ...
  class TransporterExecutor
    ...
    SKIP_ERRORS = [
      'ERROR: An exception has occurred: Scheduling automatic restart in 1 minute'
    ]
    ...
    def execute(command, hide_output)
      ...
      if hide_output
        ...
        UI.success(
          'iTunes Transporter progress... this might take a few minutes...'
        )
      end
      ...
      unless exit_status.zero?
        @errors <<
          "The call to the iTMSTransporter completed with a non-zero exit status: #{exit_status}. This indicates a failure."
      end
    ...
    def parse_line(line, hide_output)
      ...
      if line.match(re)
        ...
        if $1.include?('Your Apple ID or password was entered incorrectly') ||
          ...
          unless Helper.test?
            CredentialsManager::AccountManager.new(user: @user)
              .invalid_credentials
          ...
          UI.error($1)
          UI.error(
            'You have to change the build number of your app to upload your ipa file'
          )
        end
      ...
      if line =~ RETURN_VALUE_REGEX
        if $1.to_i != 0
          ...
          UI.error(@errors.join("\n"))
          UI.crash!(
            "Return status of iTunes Transporter was #{$1}: #{@errors.join(
              '\n'
            )}"
          )
        else
      ...
      if !hide_output && line =~ OUTPUT_REGEX
        ...
        UI.verbose("[Transporter]: #{$1}") unless output_done
      end
    end
    ...
    def additional_upload_parameters
      ...
      if ENV['DELIVER_ITMSTRANSPORTER_ADDITIONAL_UPLOAD_PARAMETERS'].to_s
         .length ==
         0
  ...
  class ShellScriptTransporterExecutor < TransporterExecutor
    def build_upload_command(
      username, password, source = '/tmp', provider_short_name = ''
    )
      [
        ...
        '-t Signiant',
        '-k 100000',
        ('-WONoPause true' if Helper.windows?),
        (
          unless provider_short_name.to_s.empty?
            # Windows only: process instantly returns instead of waiting for key press
    ...
    def handle_error(password)
      ...
      UI.error(
        "Could not download/upload from App Store Connect! It's probably related to your password or your internet connection."
      )
    end
  ...
  class JavaTransporterExecutor < TransporterExecutor
    ...
    def java_code_option
      if Helper.mac? && Helper.xcode_at_least?(9)
        ...
        return "-classpath #{Helper.transporter_java_jar_path
          .shellescape} com.apple.transporter.Application"
      end
    end
    ...
    def handle_error(password)
      unless File.exist?(Helper.transporter_java_jar_path)
        UI.error(
          "The iTMSTransporter Java app was not found at '#{Helper
            .transporter_java_jar_path}'."
        )
        UI.error(
          "If you're using Xcode 6, please select the shell script executor by setting the environment variable " \
    ...
    def execute(command, hide_output)
      ...
      FileUtils.cd(Helper.itms_path) { return super(command, hide_output) }
    end
  end
  ...
  class ItunesTransporter
    ...
    def initialize(
      ...
      @transporter_executor =
        if use_shell_script
          ShellScriptTransporterExecutor.new
        else
          JavaTransporterExecutor.new
        end
    ...
    def download(app_id, dir = nil)
      ...
      command =
        @transporter_executor.build_download_command(
          @user,
          @password,
          app_id,
          dir,
      ...
      if successful
        UI.success(
          "✅ Successfully downloaded the latest package from App Store Connect to #{itmsp_path}"
        )
      else
    ...
    def upload(app_id, dir)
      ...
      command =
        @transporter_executor.build_upload_command(
          @user,
          @password,
          actual_dir,
          @provider_short_name
    ...
    def provider_ids
      command =
        @transporter_executor.build_provider_ids_command(@user, @password)
      UI.verbose(
        @transporter_executor.build_provider_ids_command(@user, 'YourPassword')
      )
      lines = []
      begin
        result =
    ...
    def load_password_for_transporter
      ...
      if ENV[TWO_FACTOR_ENV_VARIABLE].to_s.length > 0
        UI.message(
          "Fetching password for transporter from environment variable named `#{TWO_FACTOR_ENV_VARIABLE}`"
        )
