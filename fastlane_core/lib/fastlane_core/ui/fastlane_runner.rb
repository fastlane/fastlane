...
module Commander
  ...
  class Runner
    ...
    def run!
      ...
      begin
        unless FastlaneCore::Helper.xcode_at_least?(
      ...
      begin
        if xcode_outdated
          ...
          FastlaneCore::UI.user_error!(
            "fastlane requires a minimum version of Xcode #{Fastlane::MINIMUM_XCODE_RELEASE}, please upgrade and make sure to use `sudo xcode-select -s /Applications/Xcode.app`"
          )
        end
        ...
        action_launch_context =
          FastlaneCore::ActionLaunchContext.context_for_action_name(
            @program[:name],
            fastlane_client_language: fastlane_client_language, args: ARGV
          )
        ...
        if FastlaneCore::Helper.test?
          ...
          if self.active_command.name == 'help' && @default_command == :help # need to access directly via @
            ...
            abort(
              "Please ensure to use one of the available commands (#{self
                .commands
                .keys
                .join(', ')})"
                .red
    ...
    def rescue_file_error(e)
      ...
      FastlaneCore::UI.important(
        'Check out https://docs.fastlane.tools/advanced/#directory-behavior for more details'
      )
      puts('')
      raise e
    end
    ...
    def suggest_ruby_reinstall(e)
      ...
      if Gem::Version.new(RUBY_VERSION) < Gem::Version.new('2.1')
        ui.error(
          'Apple has recently changed their servers to require TLS 1.2, which may'
        )
        ui.error(
          "not be available to your system installed Ruby (#{RUBY_VERSION})"
        )
      end
      ui.error('')
      ui.error(
        'The best solution is to use the self-contained fastlane version.'
      )
      ui.error(
        "Which ships with a bundled OpenSSL,ruby and all gems - so you don't depend on system libraries"
      )
      ...
      ui.error('-----------------------------------------------------------')
      ui.error(
        'for more details on ways to install fastlane please refer the documentation:'
      )
      ui.error('-----------------------------------------------------------')
      ...
      ui.error('')
      ui.error(
        '- Make sure OpenSSL is installed with Homebrew: `brew update && brew upgrade openssl`'
      )
      ui.error('- If you use system Ruby:')
      ui.error('  - Run `brew update && brew install ruby`')
      ui.error('- If you use rbenv with ruby-build:')
      ui.error(
        '  - Run `brew update && brew upgrade ruby-build && rbenv install 2.3.1`'
      )
      ui.error(
        '  - Run `rbenv global 2.3.1` to make it the new global default Ruby version'
      )
      ui.error('- If you use rvm:')
      ui.error('  - First run `rvm osx-ssl-certs update all`')
      ui.error(
        '  - Then run `rvm reinstall ruby-2.3.1 --with-openssl-dir=/usr/local`'
      )
      ui.error('')
      ui.error(
        "If that doesn't fix your issue, please google for the following error message:"
      )
      ui.error("  '#{e}'")
      ui.error(
        '-----------------------------------------------------------------------'
    ...
    def handle_unknown_error!(e)
      ...
      error_info =
        e.respond_to?(:preferred_error_info) ? e.preferred_error_info : nil
      should_show_github_issues =
        e.respond_to?(:show_github_issues) ? e.show_github_issues : true
    ...
    def reraise_formatted!(e, message)
      backtrace =
        FastlaneCore::Env.truthy?('FASTLANE_HIDE_BACKTRACE') ? [] : e.backtrace
