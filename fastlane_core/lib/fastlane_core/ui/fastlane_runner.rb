...
module Commander
  ...
  class Runner
    ...
    def suggest_ruby_reinstall(e)
      ...
      ui.error("  - Run `brew update && brew upgrade ruby-build && rbenv install 2.4.9`")
      ui.error("  - Run `rbenv global 2.4.9` to make it the new global default Ruby version")
      ui.error("- If you use rvm:")
      ui.error("  - First run `rvm osx-ssl-certs update all`")
      ui.error("  - Then run `rvm reinstall ruby-2.4.9 --with-openssl-dir=/usr/local`")
      ui.error("")
