      ...
      Open3.popen3(command) do |stdin, stdout, stderr, thrd|
        ...
        if thrd.value.success?
          set_partition_list(
            path,
            keychain_path,
            keychain_password: keychain_password, output: output
          )
          ...
          if err.include?('SecKeychainItemImport') &&
             err.include?('The specified item already exists in the keychain')
            UI.verbose(
              "'#{File.basename(path)}' is already installed on this machine"
            )
          else
            UI.error(err)
          end
        ...
        Open3.popen3(command) do |stdin, stdout, stderr, thrd|
          unless thrd.value.success?
            ...
            if err.include?('SecKeychainItemSetAccessWithPassword')
              ...
              UI.important('')
              UI.important(
                'Please look at the following docs to see how to set a keychain password:'
              )
              UI.important(
