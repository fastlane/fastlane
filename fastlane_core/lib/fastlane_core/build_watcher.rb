    ...
    class << self
      ...
      def wait_for_build_processing_to_be_complete(
        ...
        unless strict_build_watch
          ...
          if watched_build.nil?
            UI.crash!(
              "Could not find a build for app: #{app_id} on platform: #{platform}"
            )
          end

          unless watched_build.train_version == train_version &&
                 watched_build.build_version == build_version
            UI.important(
              "Started watching build #{watched_build
                .train_version} - #{watched_build
                .build_version} but expected #{train_version} - #{build_version}"
            )
          end
        ...
        loop do
          matched_build =
            matching_build(
              watched_train_version: train_version,
              watched_build_version: build_version,
              app_id: app_id,
              platform: platform
            )
          ...
          return matched_build if matched_build && matched_build.processed?
      ...
      def watching_build(app_id: nil, platform: nil)
        processing_builds =
          Spaceship::TestFlight::Build.all_processing_builds(
            app_id: app_id, platform: platform, retry_count: 2
          )
      ...
      def matching_build(
        watched_train_version: nil,
        watched_build_version: nil,
        app_id: nil,
        platform: nil
      )
        matched_builds =
          Spaceship::TestFlight::Build.builds_for_train(
            app_id: app_id,
            platform: platform,
            train_version: watched_train_version,
            retry_count: 2
          )
        matched_builds.find do |build|
          build.build_version == watched_build_version
        end
      end
      ...
      def report_status(build: nil)
        ...
        if build.nil?
          ...
          UI.success(
            "Build #{build.train_version} - #{build
              .build_version} is already being tested"
          )
        elsif build.ready_to_submit? || build.export_compliance_missing? ||
              build.review_rejected?
          UI.success(
            "Successfully finished processing the build #{build
              .train_version} - #{build.build_version}"
          )
        else
          UI.message(
            "Waiting for App Store Connect to finish processing the new build (#{build
              .train_version} - #{build.build_version})"
          )
        end
      end
