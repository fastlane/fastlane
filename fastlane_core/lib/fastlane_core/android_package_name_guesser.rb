  ...
  class AndroidPackageNameGuesser
    class << self
      def android_package_name_arg?(gem_name, arg)
        return arg == '--package_name' || arg == '--app_package_name' ||
      ...
      def guess_package_name_from_environment
        package_name = nil
        if FastlaneCore::Env.truthy?('SUPPLY_PACKAGE_NAME')
          package_name ||= ENV['SUPPLY_PACKAGE_NAME']
        end
        if FastlaneCore::Env.truthy?('SCREENGRAB_APP_PACKAGE_NAME')
          package_name ||= ENV['SCREENGRAB_APP_PACKAGE_NAME']
        end
        package_name
      end
      ...
      def fetch_package_name_from_file(file_name, package_name_key)
        ...
        genericfile_options = [
          FastlaneCore::ConfigItem.new(key: package_name_key)
        ]
      ...
      def guess_package_name_from_config_files
        package_name = nil
        package_name ||=
          fetch_package_name_from_file('Supplyfile', :package_name)
        package_name ||=
          fetch_package_name_from_file('Screengrabfile', :app_package_name)
        package_name
      end
