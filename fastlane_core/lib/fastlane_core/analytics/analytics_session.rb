...
module FastlaneCore
  class AnalyticsSession
    ...
    def initialize(
      analytics_ingester_client: AnalyticsIngesterClient.new(GA_TRACKING)
    )
    ...
    def action_launched(launch_context: nil)
      ...
      return if @launch_event_sent || launch_context.p_hash.nil?

      @launch_event_sent = true
      builder =
        AnalyticsEventBuilder.new(
      ...
      @threads << post_thread unless post_thread.nil?
    end

    def action_completed(completion_context: nil); end
