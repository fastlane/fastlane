...
module FastlaneCore
  class AppIdentifierGuesser
    ...
    def generate_p_hash(app_id)
      ...
      return Digest::SHA256.hexdigest("p#{app_id}fastlan3_SAlt") # hashed + salted the bundle identifier # we don't want this method to cause a crash
    ...
    def ios_app_identifier(args)
      return FastlaneCore::IOSAppIdentifierGuesser.guess_app_identifier(args) # we don't want this method to cause a crash
    rescue StandardError
      nil
    end
    ...
    def android_app_identifier(args, gem_name)
      ...
      app_identifier ? "android_project_#{app_identifier}" : nil # we don't want this method to cause a crash
    rescue StandardError
      nil
    end
  end
