  ...
  class TestParser
    ...
    def initialize(path)
      ...
      unless File.exist?(path)
        UI.user_error!("File not found at path '#{path}'")
      end
    ...
    def ensure_file_valid!
      ...
      unless supported_versions.include?(format_version)
        UI.user_error!(
          "Format version '#{format_version}' is not supported, must be #{supported_versions
            .join(', ')}"
        )
      end
    end
    ...
    def parse_content
      self.data =
        self.raw_json['TestableSummaries'].collect do |testable_summary|
          summary_row = {
            project_path: testable_summary['ProjectPath'],
            target_name: testable_summary['TargetName'],
            test_name: testable_summary['TestName'],
            duration:
              testable_summary['Tests'].map do |current_test|
                current_test['Duration']
              end.inject(:+),
            tests:
              unfold_tests(testable_summary['Tests']).collect do |current_test|
                current_row = {
                  identifier: current_test['TestIdentifier'],
                  test_group:
                    current_test['TestIdentifier'].split('/')[0..-2].join('.'),
                ...
                if current_test['FailureSummaries']
                  current_row[:failures] =
                    current_test['FailureSummaries']
                      .collect do |current_failure|
                      {
                        file_name: current_failure['FileName'],
                        line_number: current_failure['LineNumber'],
                        message: current_failure['Message'],
                        performance_failure:
                          current_failure['PerformanceFailure'],
                        failure_message:
                          "#{current_failure['Message']} (#{current_failure[
                            'FileName'
                          ]}:#{current_failure['LineNumber']})"
                      }
                    end
                end
                current_row
          ...
          summary_row[:number_of_failures] =
            summary_row[:tests].find_all do |a|
              (a[:failures] || []).count > 0
            end.count
          summary_row
        end
      self.data.first[:run_destination_name] =
        self.raw_json['RunDestination']['Name']
      return self.data
    end
  end
end
