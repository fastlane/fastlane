    ...
    def self.available_options
      ...
      [
        FastlaneCore::ConfigItem.new(
          ...
          is_string: false,
          default_value: false,
          conflicting_options: %i[developer_id development],
          conflict_block:
            proc do |option|
        ...
        FastlaneCore::ConfigItem.new(
          ...
          is_string: false,
          default_value: false,
          conflicting_options: %i[adhoc development],
          conflict_block:
            proc do |option|
              UI.user_error!(
                "You can't enable both :#{option.key} and :developer_id"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          is_string: false,
          default_value: false,
          conflicting_options: %i[adhoc developer_id],
          conflict_block:
            proc do |option|
              UI.user_error!(
                "You can't enable both :#{option.key} and :development"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          key: :ignore_profiles_with_different_name,
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless value.end_with?('.mobileprovision')
                UI.user_error!('The output name must end with .mobileprovision')
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
          key: :skip_fetch_profiles,
        ...
        FastlaneCore::ConfigItem.new(
          key: :skip_certificate_verification,
          short_option: '-z',
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              value = value.to_s
              pt = %w[macos tvos ios]
              unless pt.include?(value)
                UI.user_error!("Unsupported platform, must be: #{pt}")
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
