    ...
    def run
      ...
      command :download_all do |c|
        ...
        c.option(
          ...
          'Only works with `fastlane sigh download_all` command: Also download Xcode managed provisioning profiles'
        )
      ...
      command :resign do |c|
        ...
        c.description =
          'Resigns an existing ipa file with the given provisioning profile'
        c.option(
          '-i',
          '--signing_identity STRING',
          String,
          'The signing identity to use. Must match the one defined in the provisioning profile.'
        )
        c.option(
          '-x',
          '--version_number STRING',
          String,
          'Version number to force binary and all nested binaries to use. Changes both CFBundleShortVersionString and CFBundleVersion.'
        )
        c.option(
          '-p',
          '--provisioning_profile PATH',
          String,
          '(or BUNDLE_ID=PATH) The path to the provisioning profile which should be used. ' \
        ...
        c.option(
          ...
          '--entitlements PATH',
          String,
          'The path to the entitlements file to use.'
        )
        c.option(
          '--short_version STRING',
          String,
          'Short version string to force binary and all nested binaries to use (CFBundleShortVersionString).'
        )
        c.option(
          '--bundle_version STRING',
          String,
          'Bundle version to force binary and all nested binaries to use (CFBundleVersion).'
        )
        c.option(
        ...
        c.option(
          ...
          String,
          'New application bundle ID (CFBundleIdentifier)'
        )
        c.option(
          '--keychain_path STRING',
          String,
          'Path to the keychain that /usr/bin/codesign should use'
        )

        c.action { |args, options| Sigh::Resign.new.run(options, args) }
      end
      ...
      command :manage do |c|
        ...
        c.option(
          '-f',
          '--force',
          'Force remove all expired provisioning profiles. Required on CI.'
        )
        c.option(
          '-e',
          '--clean_expired',
        ...
        c.option(
          ...
          String,
          'Remove any provisioning profiles that matches the regular expression.'
        )
        c.example(
          'Remove all "iOS Team Provisioning" provisioning profiles',
          'fastlane sigh manage -p "iOS\ ?Team Provisioning Profile"'
        )

        c.action { |args, options| Sigh::LocalManage.start(options, args) }
      end
