...
module Sigh
  class Runner
    ...
    def run
      ...
      unless profile
        UI.user_error!('Something went wrong fetching the latest profile')
      end
    ...
    def profile_type
      ...
      if Spaceship.client.in_house?
        @profile_type = Spaceship.provisioning_profile.in_house
      end
      if Sigh.config[:adhoc]
        @profile_type = Spaceship.provisioning_profile.ad_hoc
      end
      if Sigh.config[:developer_id]
        @profile_type = Spaceship.provisioning_profile.direct
      end
    ...
    def fetch_profiles
      ...
      results =
        profile_type.find_by_bundle_id(
          bundle_id: Sigh.config[:app_identifier],
          mac: Sigh.config[:platform].to_s == 'macos',
          sub_platform: Sigh.config[:platform].to_s == 'tvos' ? 'tvOS' : nil
        )
      results =
        results.find_all do |current_profile|
          if current_profile.valid? || Sigh.config[:force]
            true
          else
            UI.message(
              "Provisioning Profile '#{current_profile
                .name}' is not valid, skipping this one..."
            )
            false
          end
        end
      ...
      if Sigh.config[:provisioning_name].to_s.length > 0
        results = filter_profiles_by_name(results)
      end
      ...
      return results.find_all do |current_profile|
        ...
        raw_certs.each do |current_cert|
          ...
          if FastlaneCore::CertChecker.installed?(file.path)
            ...
            UI.message(
              "Certificate for Provisioning Profile '#{current_profile
                .name}' not available locally: #{current_cert[:cert]
                .id}, skipping this one..."
            )
          end
        end
    ...
    def create_profile!
      ...
      name =
        Sigh.config[:provisioning_name] ||
          [bundle_id, profile_type.pretty_type].join(' ')
      ...
      UI.important(
        "Creating new provisioning profile for '#{Sigh.config[
          :app_identifier
        ]}' with name '#{name}' for '#{Sigh.config[:platform]}' platform"
      )
      profile =
        profile_type.create!(
          name: name,
    ...
    def certificates_for_profile_and_platform
      ...
      when 'ios', 'tvos'
        ...
        elsif profile_type == Spaceship.provisioning_profile.AdHoc &&
              Spaceship.client &&
    ...
    def certificate_to_use
      ...
      if certificates.count == 0
        filters = ''
        if Sigh.config[:cert_owner_name]
          filters << "Owner Name: '#{Sigh.config[:cert_owner_name]}' "
        end
        if Sigh.config[:cert_id]
          filters << "Certificate ID: '#{Sigh.config[:cert_id]}' "
        end
        if filters.length > 0
          UI.important("No certificates for filter: #{filters}")
        end
        message =
          "Could not find a matching code signing identity for type '#{profile_type
            .to_s
            .split(':')
            .last}'. "
        message +=
          'It is recommended to use match to manage code signing for you, more information on https://codesigning.guide. '
        message +=
          "If you don't want to do so, you can also use cert to generate a new one: https://fastlane.tools/cert"
        UI.user_error!(message)
      end

      return certificates if Sigh.config[]
    ...
    def download_profile(profile)
      ...
      profile_name += '_tvos' if Sigh.config[:platform].to_s == 'tvos'
      ...
      File.open(output_path, 'wb') { |f| f.write(profile.download) }
    ...
    def ensure_app_exists!
      if Spaceship::App.find(
         Sigh.config[:app_identifier],
         mac: Sigh.config[:platform].to_s == 'macos'
       )
        return
      end
      ...
      UI.user_error!(
        "Could not find App with App Identifier '#{Sigh.config[
          :app_identifier
        ]}'"
      )
    end
    ...
    def print_produce_command(config)
      ...
      UI.message('=========================================='.yellow)
      UI.message(
        "Could not find App ID with bundle identifier '#{config[
          :app_identifier
        ]}'"
      )
      UI.message(
      ...
      UI.message('')
      UI.message(
        "fastlane produce -u #{config[:username]} -a #{config[
          :app_identifier
        ]} --skip_itc"
          .yellow
      )
      ...
      UI.message(
        'If the app should also be created on App Store Connect, remove the ' +
          '--skip_itc'.yellow +
