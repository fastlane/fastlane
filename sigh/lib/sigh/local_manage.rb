...
module Sigh
  class LocalManage
    ...
    def self.list_profiles
      ...
      profiles_valid =
        profiles.select do |profile|
          profile['ExpirationDate'] > now && profile['ExpirationDate'] > soon
        end
      ...
      profiles_expired =
        profiles.select { |profile| profile['ExpirationDate'] < now }
      ...
      UI.message(
        "#{profiles_soon.count} are valid but will expire within 30 days".yellow
      )
      UI.message("#{profiles_valid.count} are valid".green)

      if profiles_expired.count > 0
        UI.message(
          'You can remove all expired profiles using `fastlane sigh manage -e`'
        )
      end
    end
    ...
    def self.cleanup_profiles(expired = false, pattern = nil, force = nil)
      ...
      profiles =
        load_profiles.select do |profile|
          (expired && profile['ExpirationDate'] < now) ||
            (!pattern.nil? && profile['Name'] =~ pattern)
        end

      UI.message(
        'The following provisioning profiles are either expired or matches your pattern:'
      )
      profiles.each { |profile| UI.message(profile['Name'].red) }

      delete = force
      unless delete
        if Helper.ci?
          UI.user_error!(
            'On a CI server, cleanup cannot be used without the --force option'
          )
        else
          delete =
            UI.confirm("Delete these provisioning profiles #{profiles.length}?")
        end
      end

      if delete
        profiles.each { |profile| File.delete(profile['Path']) }
    ...
    def self.load_profiles
      UI.message(
        'Loading Provisioning profiles from ~/Library/MobileDevice/Provisioning Profiles/'
      )
      profiles_path =
        File.expand_path('~') +
      ...
      profile_paths.each do |profile_path|
        profile =
          Plist.parse_xml(`security cms -D -i '#{profile_path}' 2> /dev/null`)
