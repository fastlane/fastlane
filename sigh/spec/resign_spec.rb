   ...
   3 valid identities found"
    ...
    it 'Create provisioning options from hash' do
      ...
      provisioning_options =
        @resign.create_provisioning_options(provisioning_profiles)
      expect(provisioning_options).to eq(
                  "-p at.fastlane=#{tmp_path}/folder/mobile.mobileprovision -p at.fastlane.today=#{tmp_path}/folder/mobile.today.mobileprovision"
                )
    end
    ...
    it 'Create provisioning options from array' do
      ...
      provisioning_options =
        @resign.create_provisioning_options(provisioning_profiles)
      expect(provisioning_options).to eq(
                  "-p #{tmp_path}/folder/mobile.mobileprovision"
                )
    end
