    ...
    class << self
      def start
        FastlaneCore::PrintTable.print_values(
          config: PEM.config,
          hide_keys: %i[new_profile],
          title: "Summary for PEM #{Fastlane::VERSION}"
        )
        login

        existing_certificate =
          certificate_sorted.detect do |c|
            c.owner_name == PEM.config[:app_identifier]
          end

        if existing_certificate
          remaining_days =
            (existing_certificate.expires - Time.now) / 60 / 60 / 24
          UI.message(
            "Existing push notification profile for '#{existing_certificate
              .owner_name}' is valid for #{remaining_days.round} more days."
          )
          if remaining_days > PEM.config[:active_days_limit]
            if PEM.config[:force]
              UI.success(
                'You already have an existing push certificate, but a new one will be created since the --force option has been set.'
              )
            else
              UI.success(
                "You already have a push certificate, which is active for more than #{PEM
                  .config[
                  :active_days_limit
                ]} more days. No need to create a new one"
              )
              UI.success(
                'If you still want to create a new one, use the --force option when running PEM.'
              )
              return false
            end
          end
      ...
      def create_certificate
        UI.important(
          "Creating a new push certificate for app '#{PEM.config[
            :app_identifier
          ]}'."
        )
        ...
        begin
          cert =
            certificate.create!(
              csr: csr, bundle_id: PEM.config[:app_identifier]
            )
        rescue => ex
          if ex.to_s.include?('You already have a current')
            ...
            UI.message(ex.to_s)
            UI.user_error!(
              "You already have 2 active push profiles for this application/environment. You'll need to revoke an old certificate to make room for a new one"
            )
          else
            raise ex
          end
        ...
        filename_base =
          PEM.config[:pem_name] ||
            "#{certificate_type}_#{PEM.config[:app_identifier]}"
        ...
        if PEM.config[:save_private_key]
          ...
          UI.message(
            'Private key: '.green + Pathname.new(private_key_path).realpath.to_s
          )
        end
        ...
        if PEM.config[:generate_p12]
          ...
          UI.message(
            'p12 certificate: '.green +
              Pathname.new(p12_cert_path).realpath.to_s
          )
        end
