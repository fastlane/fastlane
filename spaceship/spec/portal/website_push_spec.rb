describe Spaceship::Portal::WebsitePush do
  ...
  describe 'successfully loads and parses all website pushes' do
    ...
    it 'inspect works' do
      expect(Spaceship::Portal::WebsitePush.all.first.inspect).to include(
                  'Portal::WebsitePush'
                )
    end
  ...
  describe '#create' do
    it 'creates a website push' do
      expect(client).to receive(:create_website_push!).with(
                  'Fastlane Website Push',
                  'web.com.fastlane.example',
                  mac: false
                )
                  .and_return({})
      website_push =
        Spaceship::Portal::WebsitePush.create!(
          bundle_id: 'web.com.fastlane.example', name: 'Fastlane Website Push'
        )
    end
  end
  ...
  describe '#delete' do
    ...
    it 'deletes the website push by a given website_id' do
      expect(client).to receive(:delete_website_push!).with(
                  'R7878HDXC3',
                  mac: false
                )
