describe Spaceship::Portal::Passbook do
  ...
  describe 'successfully loads and parses all passbooks' do
    ...
    it 'inspect works' do
      expect(Spaceship::Portal::Passbook.all.first.inspect).to include(
                  'Portal::Passbook'
                )
    end
  ...
  describe '#create' do
    it 'creates a passbook' do
      expect(client).to receive(:create_passbook!).with(
                  'Fastlane Passbook',
                  'pass.com.fastlane.example'
                )
                  .and_return({})
      passbook =
        Spaceship::Portal::Passbook.create!(
          bundle_id: 'pass.com.fastlane.example', name: 'Fastlane Passbook'
        )
    end
  end
