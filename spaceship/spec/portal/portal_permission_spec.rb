  ...
  describe 'InsufficientPermissions' do
    ...
    it "raises an appropriate Developer Portal error when user doesn't have enough permission to do something" do
      ...
      expected_error_message =
        "User spaceship@krausefx.com (Team ID XXXXXXXXXX) doesn't have enough permission for the following action: download_certificate (You are not permitted to download this certificate.)"

      cert = Spaceship::Certificate.all.first
      expect { cert.download }.to raise_exception(
                  Spaceship::Client::InsufficientPermissions,
                  expected_error_message
                )
    end
  end
