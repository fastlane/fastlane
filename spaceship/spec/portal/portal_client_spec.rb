  ...
  context 'authenticated' do
    ...
    describe 'csrf_tokens' do
      it 'uses the stored token for all upcoming requests' do
        ...
        stub_request(
          ...
          headers: { 'csrf' => 'top_secret', 'csrf_ts' => '123123' }
        )
          .to_return(
          status: 200,
          body: PortalStubbing.adp_read_fixture_file('listDevices.action.json'),
          headers: { 'Content-Type' => 'application/json' }
        )

        # Hard code the tokens
        allow(subject).to receive(:csrf_tokens).and_return(
                   { csrf: 'top_secret', csrf_ts: '123123' }
                 )
      ...
      it 'returns a list of apps' do
        ...
        expect(apps.first.keys).to eq(
                    %w[
                      ...
                      isDevPushEnabled
                      isProdPushEnabled
                      associatedApplicationGroupsCount
                      associatedCloudContainersCount
    ...
    describe '#app_groups' do
      ...
      it 'returns a list of apps' do
        ...
        expect(app_groups.first.keys).to eq(
                    %w[name prefix identifier status applicationGroup]
                  )
      end
    end
    ...
    describe '#create_app' do
      ...
      it 'should strip non ASCII characters' do
        response =
          subject.create_app!(
            :explicit,
            'pp Test 1ed9e25c93ac7142ff9df53e7f80e84c',
            'tools.fastlane.spaceship.some-explicit-app'
          )
        expect(response['isWildCard']).to eq(false)
      ...
      it 'should make a request create an explicit app id with push feature' do
        payload = {}
        payload[Spaceship.app_service.push_notification.on.service_id] =
          Spaceship.app_service.push_notification.on
        response =
          subject.create_app!(
            :explicit,
            'Production App',
            'tools.fastlane.spaceship.some-explicit-app',
            enable_services: payload
          )
    ...
    describe '#create_app_group' do
      it 'should make a request create an app group' do
        response =
          subject.create_app_group!(
            'Production App Group',
            'group.tools.fastlane.spaceship'
          )
    ...
    describe '#devices' do
      ...
      it 'returns a list of device hashes' do
        ...
        expect(devices.first.keys).to eq(
                    %w[
                      deviceId
                      name
                      deviceNumber
    ...
    describe '#certificates' do
      let(:certificates) { subject.certificates(%w[5QPB9NHCEI]) }
    ...
    describe '#create_device' do
      it 'works as expected when the name is free' do
        ...
        expect(device['deviceNumber']).to eq(
                    '7f6c8dc83d77134b5a3a1c53f1202b395b04482b'
                  )
    ...
    describe '#provisioning_profiles' do
      it 'makes a call to the developer portal API' do
        ...
        expect(profiles.sample.keys).to include(
    ...
    describe '#provisioning_profiles_via_xcode_api' do
      it 'makes a call to the developer portal API' do
        ...
        expect(
          a_request(:post, /developerservices2.apple.com/)
        ).to have_been_made
      end
    end
    ...
    describe '#create_provisioning_profile' do
      it 'works when the name is free' do
        response =
          subject.create_provisioning_profile!(
            'net.sunapps.106 limited',
            'limited',
            'R9YNDTPLJX',
            %w[C8DL7464RQ],
            %w[C8DLAAAARQ]
      ...
      it 'works when template name is specified' do
        ...
        response =
          subject.create_provisioning_profile!(
            ...
            %w[C8DL7464RQ],
            [],
            mac: false, sub_platform: nil, template_name: template_name
          )
        expect(response.keys).to include(
                    'name',
                    'status',
    ...
    describe '#revoke_certificate' do
      it 'makes a revoke request and returns the revoked certificate' do
        ...
        expect(response.first.keys).to include(
                    'certificateId',
                    'certificateType',
                    'certificate'
                  )
    ...
    describe '#fetch_program_license_agreement_messages' do
      it 'makes a request to fetch all Program License Agreement warnings from Olympus' do
        ...
        expect(
          a_request(
            :get,
            'https://olympus.itunes.apple.com/v1/contractMessages'
          )
        ).to have_been_made
        ...
        expected_first_message =
          '<b>Review the updated Paid Applications \
...
the developer website<a/>.<br />'
  ...
  describe 'keys api' do
    let(:api_root) do
      'https://developer.apple.com/services-account/QH65B2/account/auth/key'
    end
    before do
      MockAPI::DeveloperPortalServer.post(
        '/services-account/QH65B2/account/auth/key/:action'
      ) { { keys: [] } }
    end
    ...
    describe '#download_key' do
      it 'downloads a key' do
        MockAPI::DeveloperPortalServer.get(
          '/services-account/QH65B2/account/auth/key/download'
        ) { '----- BEGIN PRIVATE KEY -----' }
        subject.download_key(id: '123')
        expect(WebMock).to have_requested(
                    :get,
                    api_root + '/download?keyId=123&teamId=XXXXXXXXXX'
                  )
      end
    end
  ...
  describe 'merchant api' do
    let(:api_root) do
      'https://developer.apple.com/services-account/QH65B2/account/ios/identifiers/'
    end
    before do
      MockAPI::DeveloperPortalServer.post(
        '/services-account/QH65B2/account/ios/identifiers/:action'
      ) { { identifierList: [], omcId: [] } }
    end
    ...
    describe '#create_merchant!' do
      it 'creates a merchant' do
        subject.create_merchant!(
          'ExampleApp Production',
          'merchant.com.example.app.production'
        )
