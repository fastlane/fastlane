describe Spaceship::Portal::AppGroup do
  ...
  describe 'successfully loads and parses all app groups' do
    ...
    it 'inspect works' do
      expect(Spaceship::Portal::AppGroup.all.first.inspect).to include(
                  'Portal::AppGroup'
                )
    end
  ...
  describe '#create' do
    it 'creates an app group' do
      expect(client).to receive(:create_app_group!).with(
                  'Production App Group',
                  'group.tools.fastlane'
                )
                  .and_return({})
      group =
        Spaceship::Portal::AppGroup.create!(
          group_id: 'group.tools.fastlane', name: 'Production App Group'
        )
    end
  end
