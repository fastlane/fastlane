describe Spaceship::Portal::App do
  ...
  describe '#create' do
    it 'creates an app id with an explicit bundle_id' do
      expect(client).to receive(:create_app!).with(
                  :explicit,
                  'Production App',
                  'tools.fastlane.spaceship.some-explicit-app',
                  mac: false, enable_services: {}
                ) { { 'isWildCard' => true } }
      app =
        Spaceship::Portal::App.create!(
          bundle_id: 'tools.fastlane.spaceship.some-explicit-app',
          name: 'Production App'
        )
      expect(app.is_wildcard).to eq(true)
    end
    ...
    it 'creates an app id with an explicit bundle_id and no push notifications' do
      expect(client).to receive(:create_app!).with(
                  ...
                  'Production App',
                  'tools.fastlane.spaceship.some-explicit-app',
                  mac: false, enable_services: { push_notification: 'off' }
                ) { { 'enabledFeatures' => %w[inAppPurchase] } }
      app =
        Spaceship::Portal::App.create!(
          bundle_id: 'tools.fastlane.spaceship.some-explicit-app',
          name: 'Production App',
          enable_services: { push_notification: 'off' }
        )
    ...
    it 'creates an app id with a wildcard bundle_id' do
      ...
      app =
        Spaceship::Portal::App.create!(
          bundle_id: 'tools.fastlane.spaceship.*', name: 'Development App'
        )
      expect(app.is_wildcard).to eq(false)
    end
  end
  ...
  describe '#update_name' do
    ...
    it 'updates the name of the app by given bundle_id' do
      stub_request(
        :post,
        'https://developer.apple.com/services-account/QH65B2/account/ios/identifiers/updateAppIdName.action'
      )
        .with(
        body: {
          'appIdId' => 'B7JBD8LHAA',
        ...
        status: 200,
        body:
          PortalStubbing.adp_read_fixture_file('updateAppIdName.action.json'),
