...
describe Spaceship::Portal::Merchant do
  ...
  before do
    allow(Spaceship::Portal::Merchant).to receive(:client).and_return(
               mock_client
             )
  end
  ...
  describe '.find' do
    it 'works with specific Merchant IDs' do
      ...
      merchant =
        Spaceship::Portal::Merchant.find('merchant.com.example.app.sandbox')
  ...
  describe '.create' do
    it 'creates a merchant' do
      allow(mock_client).to receive(:create_merchant!).with(
      ...
      merchant =
        Spaceship::Portal::Merchant.create!(
          bundle_id: 'merchant.com.example.app.production',
          name: 'ExampleApp Production',
          mac: false
        )
      expect(merchant).to be_instance_of(Spaceship::Portal::Merchant)
  ...
  describe '.delete' do
    it 'deletes the merchant by a given merchant_id' do
      ...
      allow(mock_client).to receive(:delete_merchant!).with(
                 'LM3IY56BXC',
                 mac: anything
               )
