  ...
  describe 'UI' do
    describe '#select_team' do
      ...
      describe 'Multiple Teams' do
        before { PortalStubbing.adp_stub_multiple_teams }

        it 'Lets the user select the team if in multiple teams' do
          allow($stdin).to receive(:gets).and_return('2')
          expect(Spaceship::Client::UserInterface).to receive(:interactive?)
                      .and_return(true)
        ...
        it "Let's the user specify the team name using method parameters" do
          expect(subject.select_team(team_name: 'SecondTeamProfiName')).to eq(
                      'SecondTeam'
                    )
        end
        ...
        it 'Raises an Error if shell is non interactive' do
          ...
          expect { subject.select_team }.to raise_error(
                      'Multiple Teams found; unable to choose, terminal not interactive!'
                    )
        end
