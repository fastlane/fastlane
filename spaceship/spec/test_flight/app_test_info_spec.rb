...
describe Spaceship::TestFlight::AppTestInfo do
  ...
  it 'gets the TestInfo' do
    expect(app_test_info.test_info).to be_instance_of(
  ...
  it 'gets beta review info' do
    expect(app_test_info.beta_review_info).to be_instance_of(
    ...
    expect(app_test_info.beta_review_info.contact_email).to eq(
                'contact@email.com'
              )
    expect(app_test_info.beta_review_info.demo_account_name).to eq('User Name')
    expect(app_test_info.beta_review_info.demo_account_password).to eq(
                'Password'
              )
  ...
  it 'sets beta review info' do
    ...
    expect(app_test_info.raw_data['betaReviewInfo']).to eq(
                beta_review_info.raw_data
              )
  end
  ...
  context 'client interactions' do
    it 'find app test info on the server' do
      mock_client_response(
        :get_app_test_info,
        with: hash_including(app_id: 'app-id')
      ) { app_test_info.raw_data }

      found_app_test_info =
        Spaceship::TestFlight::AppTestInfo.find(app_id: 'app-id')
      expect(found_app_test_info).to be_instance_of(
                  Spaceship::TestFlight::AppTestInfo
                )

      # use to_h.to_s to compare raw_data, since they are different instances
      expect(found_app_test_info.raw_data.to_h.to_s).to eq(
                  app_test_info.raw_data.to_h.to_s
                )
    end

    RSpec::Matchers.define(:same_app_test_info) do |other_app_test_info|
      match do |args|
        args[:app_test_info].raw_data.to_h.to_s ==
          other_app_test_info.raw_data.to_h.to_s
      end
    end

    it 'updates app test info on the server' do
      expect(app_test_info.client).to receive(:put_app_test_info).with(
                  same_app_test_info(app_test_info)
                )
