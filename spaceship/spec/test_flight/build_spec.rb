  ...
  context '.find' do
    it 'finds a build by a build_id' do
      mock_client_response(:get_build) do
        { id: 456, bundleId: 'com.foo.bar', trainVersion: '1.0' }
      end
    ...
    it 'returns raises when the build cannot be found' do
      mock_client_response(:get_build).and_raise(
  ...
  context 'collections' do
    ...
    context '.all' do
      it 'contains all of the builds across all build trains' do
        ...
        expect(builds.map(&:train_version).uniq).to eq(%w[1.0 1.1])
      end
    end
    ...
    context '.builds_for_train' do
      it 'returns the builds for a given train version' do
        builds =
          Spaceship::TestFlight::Build.builds_for_train(
            app_id: 10, platform: 'ios', train_version: '1.0'
          )
        expect(builds.size).to eq(1)
        expect(builds.map(&:train_version)).to eq(%w[1.0])
      end
    end
    ...
    context '.all_processing_builds' do
      it 'returns a collection of builds that are processing' do
        builds =
          Spaceship::TestFlight::Build.all_processing_builds(
            app_id: 10, platform: 'ios'
          )
        expect(builds.size).to eq(1)
    ...
    context '.latest' do
      it 'returns the latest build across all build trains' do
        latest_build =
          Spaceship::TestFlight::Build.latest(app_id: 10, platform: 'ios')
  ...
  context 'instances' do
    let(:build) do
      Spaceship::TestFlight::Build.find(
        app_id: 'some-app-id', build_id: 'some-build-id'
      )
    end
      ...
      mock_client_response(:get_build) do
        {
          ...
          exportCompliance: { usesEncryption: true, encryptionUpdated: false },
          testInfo: [
            {
              locale: 'en-US',
    ...
    it 'reloads a build' do
      ...
      build.id = 1
      build.app_id = 2
      expect { build.reload }.to change(build, :bundle_id).from(nil).to(
                  'some-bundle-id'
                )
    end
    ...
    context 'lazy loading attribute' do
      let(:build) do
        Spaceship::TestFlight::Build.new('bundleId' => 1, 'appAdamId' => 1)
      end
      it 'loads TestInfo' do
        expect(build).to receive(:reload).once.and_call_original
        expect(build.test_info).to be_instance_of(
                    Spaceship::TestFlight::TestInfo
                  )
      end
      it 'loads BetaReviewInfo' do
        expect(build).to receive(:reload).once.and_call_original
        expect(build.beta_review_info).to be_instance_of(
                    Spaceship::TestFlight::BetaReviewInfo
                  )
      end
      it 'loads ExportCompliance' do
        expect(build).to receive(:reload).once.and_call_original
        expect(build.export_compliance).to be_instance_of(
                    Spaceship::TestFlight::ExportCompliance
                  )
      end
    end
    ...
    context '#save!' do
      it 'saves via the client' do
        expect(build.client).to receive(:put_build).with(
                    app_id: 'some-app-id',
                    build_id: 1,
                    build: instance_of(Spaceship::TestFlight::Build)
                  )
    ...
    RSpec::Matchers.define(:same_test_info) do |other_test_info|
      match { |args| args[:build].test_info.to_s == other_test_info.to_s }
    end
    ...
    context '#update_build_information!' do
      it 'updates description' do
        ...
        expect(build.client).to receive(:put_build).with(
                    same_test_info(updated_test_info)
                  )
