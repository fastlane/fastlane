...
describe Spaceship::TestFlight::Group do
  ...
  context 'collections' do
    before do
      mock_client_response(:get_groups, with: { app_id: 'app-id' }) do
        [
          { id: 1, name: 'Group 1', isDefaultExternalGroup: true },
          { id: 2, name: 'Group 2', isDefaultExternalGroup: false }
        ]
      end
      mock_client_response(
        :create_group_for_app,
        with: { app_id: 'app-id', group_name: 'group-name' }
      ) { { id: 3, name: 'group-name', isDefaultExternalGroup: false } }
    end
    ...
    context '.find' do
      it 'returns a Group by group_name' do
        group =
          Spaceship::TestFlight::Group.find(
            app_id: 'app-id', group_name: 'Group 1'
          )
      ...
      it 'returns nil if no group matches' do
        group =
          Spaceship::TestFlight::Group.find(
            app_id: 'app-id', group_name: 'Group NaN'
          )
    ...
    context '.create!' do
      it 'returns an existing group with the same name' do
        group =
          Spaceship::TestFlight::Group.create!(
            app_id: 'app-id', group_name: 'Group 1'
          )
        expect(group.name).to eq('Group 1')
        expect(group.id).to eq(1)
      ...
      it 'creates the group for correct parameters' do
        group =
          Spaceship::TestFlight::Group.create!(
            app_id: 'app-id', group_name: 'group-name'
          )
    ...
    context '.delete!' do
      it 'removes a group for correct parameters' do
        expect(mock_client).to receive(:delete_group_for_app).with(
                    app_id: 'app-id', group_id: 2
                  )
        Spaceship::TestFlight::Group.delete!(
          app_id: 'app-id', group_name: 'Group 2'
        )
      end
    end
    ...
    context '.default_external_group' do
      it 'returns the default external group' do
        group =
          Spaceship::TestFlight::Group.default_external_group(app_id: 'app-id')
        expect(group).to be_instance_of(Spaceship::TestFlight::Group)
        expect(group.id).to eq(1)
      end
    end
  ...
  context 'instances' do
    let(:group) do
      Spaceship::TestFlight::Group.new(
        'appAdamId' => 1, 'id' => 2, 'isDefaultExternalGroup' => true
      )
    end
    let(:tester) do
      double(
        'Tester',
        tester_id: 'some-tester-id',
    ...
    context '#add_tester!' do
      it 'adds a tester via client' do
        expect(mock_client).to receive(:create_app_level_tester).with(
                    app_id: 1,
        ...
        expect(mock_client).to receive(:post_tester_to_group).with(
                    group_id: 2,
                    email: tester.email,
                    first_name: tester.first_name,
                    last_name: tester.last_name,
    ...
    context '#remove_tester!' do
      it 'removes a tester via client' do
        expect(mock_client).to receive(:delete_tester_from_group).with(
                    group_id: 2, tester_id: 'some-tester-id', app_id: 1
                  )
    ...
    context '#builds' do
      ...
      it 'gets the builds for this group via client' do
        expect(mock_client).to receive(:builds_for_group).with(
                    app_id: 1, group_id: 2
                  )
        builds = group.builds
