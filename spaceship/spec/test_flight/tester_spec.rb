...
describe Spaceship::TestFlight::Tester do
  ...
  context 'collections' do
    before do
      mock_client_response(:testers_for_app, with: { app_id: 'app-id' }) do
        [
          { id: 1, email: 'email_1@domain.com' },
          { id: 2, email: 'email_2@domain.com' }
        ]
      end

      mock_client_response(
        :search_for_tester_in_app,
        with: { app_id: 'app-id', text: 'email_1@domain.com' }
      ) { [{ id: 1, email: 'email_1@domain.com' }] }

      mock_client_response(
        :search_for_tester_in_app,
        with: { app_id: 'app-id', text: 'EmAiL_3@domain.com' }
      ) do
        [
          { id: 3, email: 'EmAiL_3@domain.com' },
          { id: 4, email: 'tacos_email_3@domain.com' }
        ]
      end

      mock_client_response(
        :search_for_tester_in_app,
        with: { app_id: 'app-id', text: 'taquito' }
      ) do
        [
          { id: 1, email: 'taquito@domain.com' },
          { id: 2, email: 'taquitos@domain.com' }
        ]
      end

      mock_client_response(
        :search_for_tester_in_app,
        with: { app_id: 'app-id', text: 'NaN@domain.com' }
      ) { [] }
    end
    ...
    context '.find' do
      it 'returns a Tester by email address' do
        tester =
          Spaceship::TestFlight::Tester.find(
            app_id: 'app-id', email: 'email_1@domain.com'
          )
      ...
      it 'returns nil if no Tester matches' do
        tester =
          Spaceship::TestFlight::Tester.find(
            app_id: 'app-id', email: 'NaN@domain.com'
          )
    ...
    context '.search' do
      it 'returns a Tester by email address' do
        testers =
          Spaceship::TestFlight::Tester.search(
            app_id: 'app-id', text: 'email_1@domain.com'
          )
      ...
      it 'returns empty array if no Tester matches' do
        testers =
          Spaceship::TestFlight::Tester.search(
            app_id: 'app-id', text: 'NaN@domain.com'
          )
  ...
  context 'instances' do
    let(:tester) do
      Spaceship::TestFlight::Tester.new(
        'id' => 2, 'email' => 'email@domain.com'
      )
    end

    context '.remove_from_app!' do
      it 'removes a tester from the app' do
        expect(mock_client).to receive(:delete_tester_from_app).with(
                    app_id: 'app-id', tester_id: 2
                  )
    ...
    context '.remove_from_testflight!' do
      it 'removes a tester from TestFlight' do
        expect(mock_client).to receive(:remove_testers_from_testflight).with(
                    app_id: 'app-id', tester_ids: [2]
                  )
  ...
  context 'invites' do
    ...
    context '.resend_invite' do
      it 'suceeds' do
        expect(mock_client).to receive(:resend_invite_to_external_tester).with(
                    app_id: 'app-id', tester_id: 'tester-id'
                  )
