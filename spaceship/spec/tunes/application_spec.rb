describe Spaceship::Application do
  ...
  describe 'successfully loads and parses all apps' do
    it 'inspect works' do
      expect(Spaceship::Application.all.first.inspect).to include(
                  'Tunes::Application'
                )
    end
    ...
    it 'parses application correctly' do
      ...
      expect(app.app_icon_preview_url).to eq(
                  'https://is5-ssl.mzstatic.com/image/thumb/Purple3/v4/78/7c/b5/787cb594-04a3-a7ba-ac17-b33d1582ebc9/mzl.dbqfnkxr.png/340x340bb-80.png'
                )
    ...
    it '#url' do
      expect(Spaceship::Application.all.first.url).to eq(
    ...
    describe '#create!' do
      it 'works with valid data and defaults to English' do
        Spaceship::Tunes::Application.create!(
          name: 'My name', sku: 'SKU123', bundle_id: 'net.sunapps.123'
        )
      end
        ...
        expect do
          Spaceship::Tunes::Application.create!(
            name: 'My Name', sku: 'SKU123', bundle_id: 'net.sunapps.123'
                    ...
                    'You must choose a primary language. You must choose a primary language.'
                  )
      end
      ...
      it 'raises an error if bundle is wildcard and bundle_id_suffix has not specified' do
        ...
        expect do
          Spaceship::Tunes::Application.create!(
            name: 'My Name', sku: 'SKU123', bundle_id: 'net.sunapps.*'
          )
        end.to raise_error(
                    'You must enter a Bundle ID Suffix. You must enter a Bundle ID Suffix.'
                  )
      end
    end
        ...
        expect do
          Spaceship::Tunes::Application.create!(
            name: 'My Name', sku: 'SKU123', bundle_id: 'net.sunapps.123'
                    ...
                    'You must provide a company name to use on the App Store. You must provide a company name to use on the App Store.'
                  )
      end
    end
    ...
    describe '#resolution_center' do
      it 'when the app was rejected' do
        ...
        expect(
          result['appNotes']['threads'].first['messages'].first['body']
        ).to include(
                    'Your app declares support for audio in the UIBackgroundModes'
                  )
      end

      it 'when the app was not rejected' do
        TunesStubbing.itc_stub_resolution_center_valid
        expect(Spaceship::Tunes::Application.all.first.resolution_center).to eq(
                    {
                      'sectionErrorKeys' => [],
                      'sectionInfoKeys' => [],
                      'sectionWarningKeys' => [],
    ...
    describe '#builds' do
      ...
      before do
        ...
        mock_client_response(:get_build_trains) { %w[1.0 1.1] }
    ...
    describe 'Access app_versions' do
      ...
      describe 'BUNDLES' do
        ...
        it 'fails to find the edit_version of a bundle' do
          expect { bundle.edit_version }.to raise_error(
                      'We do not support BUNDLE types right now'
                    )
        end

        it 'fails to find the live_version of a bundle' do
          expect { bundle.live_version }.to raise_error(
                      'We do not support BUNDLE types right now'
                    )
        end
      end
    ...
    describe 'Create new version' do
      it 'raises an exception if there already is a new version' do
        app = Spaceship::Application.all.first
        expect { app.create_version!('0.1') }.to raise_error(
                    'Cannot create a new version for this app as there already is an `edit_version` available'
                  )
      end
    end
    ...
    describe 'Promo codes' do
      ...
      it 'fetches remaining promocodes' do
        ...
        expect(promocodes[0].contract_file_name).to eq(
