describe Spaceship::TunesClient do
    ...
    it 'raises an exception if authentication failed' do
      expect do
        subject.login('bad-username', 'bad-password')
                  ...
                  "Invalid username and password combination. Used 'bad-username' as the username."
                )
    end
  end
  ...
  describe 'exception if Apple ID & Privacy not acknowledged' do
    ...
    it 'has authType is sa' do
      ...
      allow_any_instance_of(Spaceship::Client).to receive(:request).and_return(
                                 response
                               )

      expect { Spaceship::Tunes.login(username, password) }.to raise_exception(
  ...
  describe 'Logged in' do
    ...
    before { Spaceship::Tunes.login(username, password) }
    ...
    it '#hostname' do
      expect(subject.class.hostname).to eq(
    ...
    describe '#handle_itc_response' do
      it 'raises an exception if something goes wrong' do
        data =
          JSON.parse(
            TunesStubbing.itc_read_fixture_file(
              'update_app_version_failed.json'
            )
          )[
            'data'
          ]
        expect { subject.handle_itc_response(data) }.to raise_error(
      ...
      it 'does nothing if everything works as expected and returns the original data' do
        data =
          JSON.parse(
            TunesStubbing.itc_read_fixture_file(
              'update_app_version_success.json'
            )
          )[
            'data'
          ]
      ...
      it 'identifies try again later responses' do
        data =
          JSON.parse(
            TunesStubbing.itc_read_fixture_file(
              'update_app_version_temporarily_unable.json'
            )
          )[
            'data'
          ]
    ...
    describe 'associated to multiple teams' do
      let(:associated_teams) do
        [
          {
            'contentProvider' => {
              'name' => 'Tom', 'contentProviderId' => '1234'
            }
          },
          {
            'contentProvider' => {
              'name' => 'Harry', 'contentProviderId' => '5678'
            }
          }
        ]
      end
    ...
    describe 'in-app-purchase methods' do
      before do
        stub_request(
          :post,
          'https://appstoreconnect.apple.com/WebObjects/iTunesConnect.woa/ra/apps/123/iaps/abc/submission'
        )
          .and_return(
          status: 200,
          body: TunesStubbing.itc_read_fixture_file('iap_submission.json'),
    ...
    describe 'iOS app bundle methods' do
      before do
        stub_request(
          :get,
          'https://appstoreconnect.apple.com/WebObjects/iTunesConnect.woa/ra/appbundles/metadetail/1234567890'
        )
          .and_return(
          status: 200,
          body: TunesStubbing.itc_read_fixture_file('bundle_details.json'),
          headers: { 'Content-Type' => 'application/json' }
        )
      end
  ...
  describe 'CI' do
    it 'crashes when running in non-interactive shell' do
      ...
      expect { subject.select_team }.to raise_error(
