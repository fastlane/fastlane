describe Spaceship::Tunes::IAP do
  ...
  describe 'returns all purchases' do
    it 'returns as IAPList' do
      expect(app.in_app_purchases.all.first.class).to eq(
    ...
    it 'Finds a specific product' do
      ...
      expect(app.in_app_purchases.find('go.find.me').reference_name).to eq(
                  'localizeddemo'
                )
    end

    it 'Finds families' do
      expect(app.in_app_purchases.families.class).to eq(
                  Spaceship::Tunes::IAPFamilies
                )
      expect(app.in_app_purchases.families.all.first.class).to eq(
                  Spaceship::Tunes::IAPFamilyList
                )
      expect(app.in_app_purchases.families.all.first.name).to eq(
                  'Product name1234'
                )
    end
    ...
    describe 'Create new IAP' do
      it 'create consumable' do
        ...
        expect(Spaceship::UploadFile).to receive(:from_path).with(
                    'ftl_FAKEMD5_screenshot1024.jpg'
                  )
                    .and_return(du_uploadimage_correct_screenshot)
        app.in_app_purchases.create!(
          ...
          pricing_intervals: [
            { country: 'WW', begin_date: nil, end_date: nil, tier: 1 }
          ]
        )
      end
