describe Spaceship::Tunes::IAPDetail do
  ...
  describe 'Details of an IAP' do
    ...
    it 'Read language versions' do
      expect(detailed.versions['de-DE'.to_sym][:name]).to eq(
                  'test name german1'
                )
    end
    ...
    describe 'pricing_info' do
      ...
      context 'when iap is not cleared for sale yet' do
        before do
          allow(detailed).to receive(:cleared_for_sale).and_return(false)
        end
      ...
      context 'when iap is a non-subscription product' do
        ...
        before do
          expect(detailed).to receive(:pricing_intervals).at_least(:once)
                      .and_return([interval])
        end
      ...
      context 'when iap is a subscription product with territorial pricing' do
        ...
        before do
          allow(detailed).to receive(:pricing_intervals).at_least(:once)
                     .and_return(intervals)
        end

        it 'returns pricing infos for each country' do
          expect(subject).to all(
                      be_an(Spaceship::Tunes::IAPSubscriptionPricingInfo)
                    )
        ...
        it 'returns the matching entries from the subscription price tier matrix' do
          qa =
            pricing_tiers.find { |t| t.tier_stem == '22' }.pricing_info
              .find { |p| p.country_code == 'QA' }
          cl =
            pricing_tiers.find { |t| t.tier_stem == '10' }.pricing_info
  ...
  describe 'modification' do
    it 'saved' do
      ...
      expect(client).to receive(:update_iap!).with(
                  app_id: '898536088',
                  purchase_id: '1195137656',
                  data: detailed.raw_data
                )
      detailed.save!
    end
    ...
    it 'saved and changed screenshot' do
      ...
      expect(client.du_client).to receive(:upload_purchase_review_screenshot)
                  .and_return(
                  {
                    'token' => 'tok',
                    'height' => 100,
      ...
      expect(client.du_client).to receive(:get_picture_type).and_return(
                  'MZPFT.SortedScreenShot'
                )
      expect(Spaceship::Utilities).to receive(:content_type).and_return(
                  'image/jpg'
                )
      expect(client).to receive(:update_iap!).with(
                  app_id: '898536088',
    ...
    it 'saved with subscription pricing goal' do
      ...
      price_goal =
        TunesStubbing.itc_read_fixture_file('iap_price_goal_calc.json')

      transformed_pricing_intervals =
        JSON.parse(price_goal)['data'].map do |language_code, value|
          existing_interval =
            edited.pricing_intervals.find do |interval|
      ...
      expect(client).to receive(:update_iap!).with(
                  app_id: '898536088',
                  purchase_id: '1195137657',
                  data: edited.raw_data
                )
      expect(client).to receive(:update_recurring_iap_pricing!).with(
                  app_id: '898536088',
                  purchase_id: '1195137657',
    ...
    it 'saved with changed pricing detail' do
      ...
      edited.pricing_intervals = [
        { country: 'WW', begin_date: nil, end_date: nil, tier: 4 }
      ]
      edited.save!
      expect(edited.pricing_intervals).to eq(
                  [
                    {
                      tier: 4,
                      begin_date: nil,
    ...
    it 'saved with changed pricing detail for recurring product' do
      ...
      expect(client).to receive(:update_recurring_iap_pricing!).with(
                  app_id: '898536088',
                  purchase_id: '1195137657',
                  pricing_intervals: edited.raw_data['pricingIntervals']
                )
      edited.save!
    end
    ...
    it 'saved with changed versions' do
      ...
      edited.versions = {
        'en-US' => {
          name: 'Edit It', description: 'Description has at least 10 characters'
        }
      }
      edited.save!
      expect(edited.versions).to eq(
                  {
                    :"en-US" => {
                      name: 'Edit It',
  ...
  describe 'Deletion' do
    it 'delete' do
      expect(client).to receive(:delete_iap!).with(
                  app_id: '898536088', purchase_id: '1195137656'
                )
      detailed.delete!
    end
  end
