describe Spaceship::Tunes::Availability do
  ...
  describe 'update_availability!' do
    it 'inspect works' do
      ...
      availability = Spaceship::Tunes::Availability.from_territories(%w[US])
    ...
    describe 'with countries as strings' do
      it 'correctly adds a country' do
        ...
        # currently countries in list are US and GB
        availability =
          Spaceship::Tunes::Availability.from_territories(%w[FR GB US])
        availability = client.update_availability!(app.apple_id, availability)
      ...
      it 'correctly sets preorder with no app available date' do
        ...
        availability =
          Spaceship::Tunes::Availability.from_territories(
            %w[US],
            cleared_for_preorder: true
          )
        availability = client.update_availability!(app.apple_id, availability)
      ...
      it 'correctly sets preorder with app available date' do
        ...
        availability =
          Spaceship::Tunes::Availability.from_territories(
            %w[US],
            cleared_for_preorder: true, app_available_date: '2020-02-20'
          )
        availability = client.update_availability!(app.apple_id, availability)
    ...
    describe 'with countries as Territories' do
      it 'correctly adds a country' do
        ...
        # currently countries in list are US and GB
        new_territories_codes = %w[FR GB US]
        all_territories = client.supported_territories
        territories =
          all_territories.select do |territory|
            new_territories_codes.include?(territory.code)
          end

        availability =
          Spaceship::Tunes::Availability.from_territories(territories)
        availability = client.update_availability!(app.apple_id, availability)
    ...
    describe 'enable_b2b_app!' do
      it 'throws exception if b2b cannot be enabled' do
        ...
        availability = client.availability(app.apple_id)
        expect { availability.enable_b2b_app! }.to raise_error(
                    'Not possible to enable b2b on this app'
                  )
      end
    ...
    describe 'add_b2b_users' do
      it 'throws exception if b2b app not enabled' do
        availability = client.availability(app.apple_id)
        expect { availability.add_b2b_users(%w[abc@def.com]) }.to raise_error(
                    'Cannot add b2b users if b2b is not enabled'
                  )
      end
      ...
      it 'works correctly' do
        TunesStubbing.itc_stub_app_pricing_intervals_vpp
        availability = client.availability(app.apple_id)
        new_availability = availability.add_b2b_users(%w[abc@def.com])
        expect(new_availability).to be_an_instance_of(
    ...
    describe 'update_b2b_users' do
      ...
      it 'does not do anything for same b2b_user_list' do
        ...
        new_availability =
          availability.update_b2b_users(%w[b2b1@abc.com b2b2@def.com])
        expect(new_availability).to be_an_instance_of(
                    Spaceship::Tunes::Availability
                  )
      ...
      it 'adds new user' do
        ...
        availability = client.availability(app.apple_id)
        new_availability =
          availability.update_b2b_users(
            %w[b2b1@abc.com b2b2@def.com jkl@mno.com]
          )
        expect(new_availability).to be_an_instance_of(
                    Spaceship::Tunes::Availability
                  )
      ...
      it 'adds and removes appropriate users' do
        ...
        availability = client.availability(app.apple_id)
        new_availability =
          availability.update_b2b_users(%w[b2b1@abc.com jkl@mno.com])
        expect(new_availability).to be_an_instance_of(
                    Spaceship::Tunes::Availability
                  )
        expect(new_availability.b2b_users.length).to eq(3)
