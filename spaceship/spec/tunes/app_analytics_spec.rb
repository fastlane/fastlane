...
describe Spaceship::Tunes::AppAnalytics do
  ...
  describe 'App Analytics Grabbed Properly' do
    it 'accesses live analytics details' do
      ...
      val = sales['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      paying_users = analytics.app_paying_users
      expect(paying_users['size']).to eq(1)
      val =
        paying_users['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      installs = analytics.app_installs
      expect(installs['size']).to eq(1)
      val = installs['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      sessions = analytics.app_sessions
      expect(sessions['size']).to eq(1)
      val = sessions['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      active_devices = analytics.app_active_devices
      expect(active_devices['size']).to eq(1)
      val =
        active_devices['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      crashes = analytics.app_crashes
      expect(crashes['size']).to eq(1)
      val = crashes['results'].find { |a| a['adamId'].include?('898536088') }
      expect(val['meetsThreshold']).to eq(true)

      measure_installs =
        analytics.app_measure_interval(start_time, end_time, 'installs')
      expect(measure_installs['size']).to eq(1)
      val =
        measure_installs['results'].find do |a|
          a['adamId'].include?('898536088')
        end
      expect(val['meetsThreshold']).to eq(true)
    end
    ...
    it 'grabs live analytics split by view_by' do
      ...
      TunesStubbing.itc_stub_analytics(start_time, end_time)
      analytics = app.analytics
      measure_installs_by_source =
        analytics.app_measure_interval(
          start_time,
          end_time,
          'installs',
          'source'
      ...
      val =
        measure_installs_by_source['results'].find do |a|
          a['adamId'].include?('898536088')
        end
      expect(val['meetsThreshold']).to eq(true)
    end
    ...
    it 'accesses non-live analytics details' do
      ...
      expect { analytics = app.analytics }.to raise_error(
                  'Analytics are only available for live apps.'
                )
    end
  end
end
