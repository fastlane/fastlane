describe Spaceship::DUClient, :du do
  ...
  describe '#upload_large_icon and #upload_watch_icon' do
    ...
    it 'handles successful upload request using #upload_large_icon' do
      ...
      data =
        subject.upload_large_icon(
          version,
          correct_jpg_image,
          contentProviderId,
          ssoTokenForImage
        )
      expect(data['token']).to eq(
    ...
    it 'handles failed upload request using #upload_watch_icon' do
      ...
      error_text =
        '[IMG_ALPHA_NOT_ALLOWED] Alpha is not allowed. Please edit the image to remove alpha and re-save it.'
      expect do
        data =
          subject.upload_watch_icon(
            version,
            bad_png_image,
            contentProviderId,
            ssoTokenForImage
  ...
  describe '#upload_geojson' do
    ...
    it 'handles failed upload request' do
      ...
      error_text =
        '[FILE_GEOJSON_FIELD_NOT_SUPPORTED] The routing app coverage file is in the wrong format. For more information, see the Location and Maps Programming Guide in the iOS Developer Library.'
      expect do
        data =
          subject.upload_geojson(
            version,
