describe Spaceship::ProvisioningProfile do
  ...
  describe "#repair" do
    ...
    it "repairs an existing profile with added devices" do
      ...
      expect(client).to receive(:repair_provisioning_profile!).with('PP00000001', '1 Gut Altentann Ad Hoc', 'store', '2UMR2S6P4L', [cert_id], ["AAAAAAAAAA", "BBBBBBBBBB", "CCCCCCCCCC", "DDDDDDDDDD"], mac: false, sub_platform: nil).and_return({})
      profile.repair!
    end

    it "update the certificate if the current one doesn't exist" do
      profile.certificates = []
      expect(client).to receive(:repair_provisioning_profile!).with('PP00000001', '1 Gut Altentann Ad Hoc', 'store', '2UMR2S6P4L', [cert_id], [], mac: false, sub_platform: nil).and_return({})
      profile.repair!
    end
    ...
    describe "Different Environments" do
      it "Development" do
        ...
        expect(client).to receive(:repair_provisioning_profile!).with('475ESRP5F3', 'Development Leobersdorf 140', 'limited', '2UMR2S6P4L', [cert_id], devices, mac: false, sub_platform: nil).and_return({})
        profile.repair!
      end
    end
  end
  ...
  describe "#update!" do
    ...
    it "updates an existing tvOS profile" do
      expect(client).to receive(:repair_provisioning_profile!).with('PP00000001', '1 Gut Altentann Ad Hoc', 'store', '2UMR2S6P4L', [cert_id], [], mac: false, sub_platform: 'tvOS').and_return({})
      tvOSProfile.update!
    end
  end
