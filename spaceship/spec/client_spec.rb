describe Spaceship::Client do
  ...
  def stub_client_request(error, times, status, body)
    stub_request(:get, test_uri).to_raise(error).times(times).then.to_return(
      status: status, body: body
    )
  end

  def stub_client_retry_auth(status_error, times, status_ok, body)
    stub_request(:get, test_uri).to_return(status: status_error, body: body)
      .times(times)
      .then
      .to_return(status: status_ok, body: body)
  end
  ...
  describe 'detect_most_common_errors_and_raise_exceptions' do
    ...
    it 'raises Spaceship::InsufficientPermissions for InsufficentPermissions' do
      ...
      expect { subject.req_home }.to raise_error(
    ...
    it 'raises Spaceship::InternalServerError' do
      stub_client_request(
        ...
        6,
        504,
        '<html>Internal Server - Read</html>'
      )

      expect { subject.req_home }.to raise_error(Spaceship::GatewayTimeoutError)
    end
    ...
    it 'raises Spaceship::GatewayTimeoutError' do
      stub_client_request(
        ...
        6,
        504,
        '<html>Gateway Timeout - In Read</html>'
      )
  ...
  describe 'retry' do
    [
      ...
      it "re-raises when retry limit reached throwing #{thrown}" do
        ...
        expect { subject.req_home }.to raise_error(thrown)
      end
    ...
    it "raises AppleTimeoutError when response contains '302 Found'" do
      ...
      expect { subject.req_home }.to raise_error(
                  Spaceship::Client::AppleTimeoutError
                )
    end
...
BODY
      ...
      expect { subject.req_home }.to raise_error(
                  Spaceship::Client::BadGatewayError
                )
    end
    ...
    it 'fails to retry request if loggin fails in retry block when UnauthorizedAccess Error raised' do
      ...
      expect { subject.req_home }.to raise_error(
                  Spaceship::Client::UnauthorizedAccessError
                )
    end
  ...
  describe '#log_response' do
    it 'handles ASCII-8BIT to UTF-8 encoding gracefully' do
      response =
        TestResponse.new([130, 5, 3120, 130, 4, 171, 160, 3, 2].pack('C*'))
      expect(
        subject.send(:log_response, :get, TestClient.hostname, response)
      ).to be_truthy
    end
  end
  ...
  describe '#persistent_cookie_path' do
    ...
    after { ENV.delete('SPACESHIP_COOKIE_PATH') }

    it 'uses $SPACESHIP_COOKIE_PATH when set' do
      tmp_path = Dir.mktmpdir
      ENV['SPACESHIP_COOKIE_PATH'] = "#{tmp_path}/custom_path"
      expect(subject.persistent_cookie_path).to eq(
                  "#{tmp_path}/custom_path/spaceship/username/cookie"
                )
    end

    it 'uses home dir by default' do
      allow(subject).to receive(:directory_accessible?).with(
                 File.expand_path('~/.fastlane')
               )
                 .and_return(true)
      expect(subject.persistent_cookie_path).to eq(
                  File.expand_path('~/.fastlane/spaceship/username/cookie')
                )
    end
    ...
    it 'supports legacy .spaceship path' do
      ...
      allow(subject).to receive(:directory_accessible?).with(
                 ...
                 .and_return(true)
      expect(subject.persistent_cookie_path).to eq(
                  File.expand_path('~/.spaceship/username/cookie')
                )
    end
    ...
    it 'uses /var/tmp if home not available' do
      ...
      allow(subject).to receive(:directory_accessible?).with(
                 ...
                 .and_return(false)
      allow(subject).to receive(:directory_accessible?).with(
                 File.expand_path('/var/tmp')
               )
                 .and_return(true)
      expect(subject.persistent_cookie_path).to eq(
                  File.expand_path('/var/tmp/spaceship/username/cookie')
                )
    end
    ...
    it 'falls back to Dir.tmpdir as last resort' do
      ...
      allow(subject).to receive(:directory_accessible?).with(
                 ...
                 .and_return(false)
      allow(subject).to receive(:directory_accessible?).with(Dir.tmpdir)
                 .and_return(true)
      expect(subject.persistent_cookie_path).to eq(
                  "#{Dir.tmpdir}/spaceship/username/cookie"
                )
    end
  end
end
