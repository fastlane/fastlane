...
module Spaceship
  class ConnectAPI
    class SubscriptionAppStoreReviewScreenshot
      ...
      def self.type
        return 'subscriptionAppStoreReviewScreenshots'
      end
      ...
      def update(client: nil, source_file_checksum:, uploaded:)
        client ||= Spaceship::ConnectAPI
        resp = client.update_subscription_app_store_review_screenshot(screenshot_id: id, source_file_checksum: source_file_checksum, uploaded: uploaded)
        resp.to_models.first # self
      end

      def delete(client: nil)
        client ||= Spaceship::ConnectAPI
        client.delete_subscription_app_store_review_screenshot(screenshot_id: id)
      end
    end
  end
end
