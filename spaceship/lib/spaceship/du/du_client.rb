  ...
  class DUClient < Spaceship::Client #:nodoc:
    ...
    def upload_screenshot(
      ...
      upload_file(
        ...
        sso_token: sso_token_for_image,
        du_validation_rule_set: screenshot_picture_type(device, is_messages)
      )
    end
    ...
    def upload_purchase_review_screenshot(
      ...
      upload_file(
        ...
        content_provider_id: content_provider_id,
        sso_token: sso_token_for_image,
        du_validation_rule_set: get_picture_type(upload_file)
      )
    end

    def upload_large_icon(
      app_version, upload_file, content_provider_id, sso_token_for_image
    )
      upload_file(
        app_version: app_version,
        upload_file: upload_file,
        ...
        content_provider_id: content_provider_id,
        sso_token: sso_token_for_image,
        du_validation_rule_set: 'MZPFT.LargeApplicationIcon'
      )
    end

    def upload_watch_icon(
      app_version, upload_file, content_provider_id, sso_token_for_image
    )
      upload_file(
        app_version: app_version,
        ...
        content_provider_id: content_provider_id,
        sso_token: sso_token_for_image,
        du_validation_rule_set: 'MZPFT.GizmoAppIcon'
      )
    end
    ...
    def upload_trailer(
      app_version, upload_file, content_provider_id, sso_token_for_video
    )
      upload_file(
        app_version: app_version,
        upload_file: upload_file,
        path: '/upload/purple-video',
        content_provider_id: content_provider_id,
        sso_token: sso_token_for_video
      )
    end

    def upload_trailer_preview(
      app_version, upload_file, content_provider_id, sso_token_for_image, device
    )
      upload_file(
        app_version: app_version,
        upload_file: upload_file,
        ...
        content_provider_id: content_provider_id,
        sso_token: sso_token_for_image,
        du_validation_rule_set: screenshot_picture_type(device, nil)
      )
    end
    ...
    def get_picture_type(upload_file)
      ...
      result =
        device_resolution_map.find do |key, resolutions|
    ...
    def upload_file(
      ...
      sso_token: nil,
      du_validation_rule_set: nil,
      app_id: nil
    )
      if upload_file.file_size == 0
        raise "File #{upload_file.file_path} is empty"
      end
      ...
      r =
        request(:post) do |req|
          ...
          if app_type
            req.headers['X-Apple-Jingle-Correlation-Key'] =
              "#{app_type}:AdamId=#{app_id}:Version=#{version}"
          end
          ...
          if du_validation_rule_set
            req.headers['X-Apple-Upload-Validation-RuleSets'] =
              du_validation_rule_set
          end
      ...
      if r.status == 500 && r.body.include?('Server Error')
        return upload_file(
          ...
          content_provider_id: content_provider_id,
          sso_token: sso_token,
          du_validation_rule_set: du_validation_rule_set,
          app_id: app_id
        )
      end
    ...
    def parse_upload_response(response)
      ...
      if !content['statusCode'].nil? && content['statusCode'] != 200
        error_codes = ''
        unless content['errorCodes'].nil?
          error_codes = content['errorCodes'].join(',')
        end
