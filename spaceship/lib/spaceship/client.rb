...
module Spaceship
  ...
  class Client
    ...
    AUTH_TYPES = %w[sa hsa non-sa hsa2]
    ...
    def team_id=(team_id)
      ...
      result =
        available_teams.find do |available_team|
          team_id.to_s == available_team[:team_id].to_s
        end

      unless result
        error_string =
          "Could not set team ID to '#{team_id}', only found the following available teams:\n\n#{available_teams
            .map { |team| "- #{team[:team_id]} (#{team[:team_name]})" }.join(
            "\n"
          )}\n"
      ...
      response =
        request(:post) do |req|
          ...
          req.body = {
            contentProviderId: team_id, dsId: user_detail_data.ds_id # https://github.com/fastlane/fastlane/issues/6711
          }.to_json
    ...
    def self.client_with_authorization_from(another_client)
      self.new(
        cookie: another_client.instance_variable_get(:@cookie),
        current_team_id: another_client.team_id
      )
    end
    ...
    def initialize(cookie: nil, current_team_id: nil)
      ...
      @client =
        Faraday.new(self.class.hostname, options) do |c|
          ...
          if ENV['SPACESHIP_DEBUG']
            ...
            if ENV['SPACESHIP_PROXY_SSL_VERIFY_NONE']
              c.ssl[:verify_mode] = OpenSSL::SSL::VERIFY_NONE
            end
          end
    ...
    def logger
      unless @logger
        if ENV['VERBOSE']
          ...
          path =
            "/tmp/spaceship#{Time.now.to_i}_#{Process.pid}_#{Thread.current
              .object_id}.log"
    ...
    def persistent_cookie_path
      if ENV['SPACESHIP_COOKIE_PATH']
        ...
        [
          File.join(self.fastlane_user_dir, 'spaceship'),
          '~/.spaceship',
          '/var/tmp/spaceship',
          "#{Dir.tmpdir}/spaceship"
        ].each do |dir|
    ...
    def login(user = nil, password = nil)
      if user.to_s.empty? || password.to_s.empty?
        ...
        keychain_entry =
          CredentialsManager::AccountManager.new(user: user, password: password)
    ...
    def send_shared_login_request(user, password)
      ...
      data = { accountName: user, password: password, rememberMe: true }
      ...
      begin
        ...
        important_cookie =
          @cookie.store.entries.find { |a| a.name.include?('DES') }
        if important_cookie
          modified_cookie = self.cookie # returns a string of all cookies
          unescaped_important_cookie =
            "#{important_cookie.name}=#{important_cookie.value}"
          escaped_important_cookie =
            "#{important_cookie.name}=\"#{important_cookie.value}\""
          modified_cookie.gsub!(
            unescaped_important_cookie,
            escaped_important_cookie
          )
        end
        ...
        raise InvalidUserCredentialsError.new,
              "Invalid username and password combination. Used '#{user}' as the username."
      end
      ...
      when 409
        ...
        if (response.body || '').include?('invalid="true"')
          ...
          # Looking for status of 412 might be enough but might be safer to keep looking only at what is being reported
              ...
              AUTH_TYPES.include?(response.body['authType'])
          raise AppleIDAndPrivacyAcknowledgementNeeded.new,
    ...
    def fetch_olympus_session
      ...
      if body
        ...
        user_map = body['user']
        self.user_email = user_map['emailAddress'] if user_map
    ...
    def itc_service_key
      ...
      if File.exist?(itc_service_key_path)
        return File.read(itc_service_key_path)
      end
    ...
    def load_session_from_env
      ...
      if Spaceship::Globals.verbose?
        puts('Loading session from environment variable')
      end
    ...
    def fetch_program_license_agreement_messages
      ...
      messages_request =
        request(:get, 'https://olympus.itunes.apple.com/v1/contractMessages')
      body = messages_request.body
      if body
        body = JSON.parse(body) if body.kind_of?(String)
        body.map { |messages| all_messages.push(messages['message']) }
      end
    ...
    def with_retry(tries = 5, &_block)
      return yield
           ...
           # New Faraday version: Faraday::TimeoutError => ex
           BadGatewayError,
           AppleTimeoutError,
      ...
      unless tries.zero?
        msg =
          "Timeout received: '#{ex.class}', '#{ex
            .message}'. Retrying after 3 seconds (remaining: #{tries})..."
      ...
      raise ex # re-raise the exception
           ...
           # <h2>Internal Server Error</h2> with content type json
           InternalServerError => ex
      tries -= 1
      unless tries.zero?
        msg =
          "Internal Server Error received: '#{ex.class}', '#{ex
            .message}'. Retrying after 3 seconds (remaining: #{tries})..."
        puts(msg) if Spaceship::Globals.verbose?
        logger.warn(msg)
      ...
      if @loggedin && !(tries -= 1).zero?
        msg =
          "Auth error received: '#{ex.class}', '#{ex
            .message}'. Login in again then retrying after 3 seconds (remaining: #{tries})..."
        puts(msg) if Spaceship::Globals.verbose?
        logger.warn(msg)

        if self.class.spaceship_session_env.to_s.length > 0
          raise UnauthorizedAccessError.new,
    ...
    def parse_response(response, expected_key = nil)
      if response.body
        ...
        if response.body['userString']
          response.body['userString'] =
            CGI.unescapeHTML(response.body['userString'])
        end
        if response.body['resultString']
          response.body['resultString'] =
            CGI.unescapeHTML(response.body['resultString'])
        end
      ...
      if content.nil?
        if response.body
          detect_most_common_errors_and_raise_exceptions(response.body)
        end
        raise UnexpectedResponse, response.body
        # else if it is a hash and `resultString` includes `NotAllowed`
      elsif content.kind_of?(Hash) &&
            (content['resultString'] || '').include?('NotAllowed')
        ...
        raise_insufficient_permission_error!(
          additional_error_string: content['userString']
        )
      else
    ...
    def detect_most_common_errors_and_raise_exceptions(body)
      ...
      if body['messages'] && body['messages']['error'].include?('Forbidden')
        ...
        # With the default location the error would say that `parse_response` is the caller
            ...
            body['messages']['error'].include?('insufficient privileges')
        ...
        raise GatewayTimeoutError,
              'Received a gateway timeout error from App Store Connect / Developer Portal, please try again later'
      elsif (body['resultString'] || '').include?('Program License Agreement')
        raise ProgramLicenseAgreementUpdated,
              "#{body[
                'userString'
              ]} Please manually log into your Apple Developer account to review and accept the updated agreement."
      end
    end

    # This also gets called from subclasses
    def raise_insufficient_permission_error!(
      additional_error_string: nil, caller_location: 2
    )
      ...
      calling_method_name =
        caller_locations(caller_location, 2).first.label.sub('block in', '')
          .delete('!')
          .strip
      ...
      error_message =
        "User #{self
          .user} #{team_id}doesn't have enough permission for the following action: #{calling_method_name}"
      if additional_error_string.to_s.length > 0
        error_message += " (#{additional_error_string})"
      end
    ...
    def store_csrf_tokens(response)
      if response && response.headers
        ...
        @csrf_tokens = tokens if tokens && !tokens.empty?
      end
    end
    ...
    def log_request(method, url, params, headers = nil, &block)
      ...
      if body
        begin
          ...
          if body.kind_of?(Hash) && body.key?('password')
            body['password'] = '***'
          end
      ...
      params_to_log =
        params_to_log.collect { |key, value| "{#{key}: #{value}}" }
      logger.info(
        ">> #{method.upcase} #{url}: #{body_to_log} #{params_to_log.join(', ')}"
      )
    end
    ...
    def send_request(method, url_or_path, params, headers, &block)
      with_retry do
        ...
        if response.body.to_s.include?('<title>302 Found</title>')
          raise AppleTimeoutError.new,
                'Apple 302 detected - this might be temporary server error, check https://developer.apple.com/system-status/ to see if there is a known downtime'
        end

        if response.body.to_s.include?('<h3>Bad Gateway</h3>')
          raise BadGatewayError.new,
                'Apple 502 detected - this might be temporary server error, try again later'
        end
    ...
    def encode_params(params, headers)
      ...
      headers =
        { 'Content-Type' => 'application/x-www-form-urlencoded' }.merge(headers)
