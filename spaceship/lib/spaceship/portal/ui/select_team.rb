    ...
    class UserInterface
      ...
      def self.ci?
        if Object.const_defined?('FastlaneCore') &&
           FastlaneCore.const_defined?('Helper')
      ...
      def select_team(team_id: nil, team_name: nil)
        ...
        if team_id.length > 0
          ...
          teams.each_with_index do |team, i|
            if team['teamId'].strip == team_id
              return # There are 2 different values - one from the login page one from the Dev Team Page
              team[
                'teamId'
              ]
            end
            if team['currentTeamMember']['teamMemberId'].to_s.strip == team_id
              return team['teamId']
            end
          end
          ...
          puts(
            "Couldn't find team with ID '#{team_id}'. Make sure your are using the correct App Store Connect team ID and have the proper permissions for this team"
          )
        end
        ...
        if team_name.length > 0
          ...
          # https://github.com/fastlane/fastlane/issues/14228
          puts(
            "Couldn't find team with Name '#{team_name}. Make sure you have the proper permissions for this team'"
          )
        end
        ...
        unless self.class.interactive?
          ...
          teams.each_with_index do |team, i|
            puts(
              "#{i + 1}) #{team['teamId']} \"#{team['name']}\" (#{team[
                'type'
              ]})"
            )
          end
        ...
        loop do
          ...
          puts(
            'Multiple teams found on the ' + 'Developer Portal'.yellow +
              ', please enter the number of the team you want to use: '
          )
          teams.each_with_index do |team, i|
            puts(
              "#{i + 1}) #{team['teamId']} \"#{team['name']}\" (#{team[
