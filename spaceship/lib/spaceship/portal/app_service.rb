  ...
  module Portal
    ...
    class AppService
      ...
      def self.new_service(id, values: { on: true, off: false })
        m = Module.new
        values.each do |k, v|
          m.define_singleton_method(k) { AppService.new(id, v) }
        end
        return m
      end
      ...
      CloudKit =
        AppService.new_service(
          'cloudKitVersion',
          values: { xcode5_compatible: 1, cloud_kit: 2 }
        )
      ...
      constants.each do |c|
        name =
          c.to_s.gsub(/([A-Z0-9]+)([A-Z][a-z])/, '\1_\2').gsub( # ABBRVString -> ABBRV_String
            /([a-z0-9])([A-Z])/,
            '\1_\2'
          ) # CamelCase -> Camel_Case
            .downcase
        self.class.send(:define_method, name) { AppService.const_get(c) }
      end

      def ==(other)
        self.class == other.class && self.service_id == other.service_id &&
