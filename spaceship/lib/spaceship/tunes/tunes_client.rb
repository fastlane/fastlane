  ...
  class TunesClient < Spaceship::Client
    ...
    def select_team(team_id: nil, team_name: nil)
      ...
      if t_name.length > 0 && t_id.length.zero? # we prefer IDs over names, they are unique
        if Spaceship::Globals.verbose?
          puts("Looking for App Store Connect Team with name #{t_name}")
        end

        teams.each do |t|
          if t['contentProvider']['name'].casecmp(t_name).zero?
            t_id = t['contentProvider']['contentProviderId'].to_s
          end
        end

        if t_id.length.zero?
          puts(
            "Could not find team with name '#{t_name}', trying to fallback to default team"
          )
        end
      end

      if teams.count == 1
        t_id = teams.first['contentProvider']['contentProviderId'].to_s
      end

      if t_id.length > 0
        if Spaceship::Globals.verbose?
          puts("Looking for App Store Connect Team with ID #{t_id}")
        end
      ...
      loop do
        puts(
          "Multiple #{'App Store Connect teams'
            .yellow} found, please enter the number of the team you want to use: "
        )
        ...
        teams.each_with_index do |team, i|
          puts(
            "#{i + 1}) \"#{team['contentProvider']['name']}\" (#{team[
              'contentProvider'
            ][
              'contentProviderId'
            ]})"
        ...
        if team_to_use
          self.team_id =
            team_to_use['contentProvider']['contentProviderId'].to_s # actually set the team id here
          return self.team_id
        end
      end
    end
    ...
    def fetch_errors_in_data(
      data_section: nil, sub_section_name: nil, keys: nil
    )
    ...
    def handle_itc_response(raw, flaky_api_call: false)
      ...
      error_keys = %w[sectionErrorKeys validationErrors serviceErrors]
      info_keys = %w[sectionInfoKeys sectionWarningKeys]
      ...
      errors_in_version_info =
        fetch_errors_in_data(
          data_section: data,
          sub_section_name: 'versionInfo',
          keys: error_and_info_keys_to_check
        )
      ...
      handle_response_hash =
        lambda do |hash, current_language = nil|
          errors = []
          if hash.kind_of?(Hash)
            ...
            hash.each do |key, value|
              ...
              unless key == 'errorKeys' && value.kind_of?(Array) &&
                     value.count > 0
                next
              end
              # Prepend the error with the language so it's easier to understand for the user
              errors +=
                value.collect do |current_error_message|
                  if current_language
                    "[#{current_language}]: #{current_error_message}"
                  else
            ...
            hash.each { |value| errors += handle_response_hash.call(value) }
      ...
      errors_in_data =
        fetch_errors_in_data(data_section: data, keys: error_keys)
      errors_in_version_info =
        fetch_errors_in_data(
          data_section: data, sub_section_name: 'versionInfo', keys: error_keys
        )
      ...
      info_in_version_info =
        fetch_errors_in_data(
          data_section: data, sub_section_name: 'versionInfo', keys: info_keys
        )

      info_in_data.each { |info_key, info_value| puts(info_value) }

      info_in_version_info.each { |info_key, info_value| puts(info_value) }
    ...
    def create_application!(
      ...
      if version
        puts(
          'The `version` parameter is deprecated. Use `Spaceship::Tunes::Application.ensure_version!` method instead'
        )
      end
      ...
      unless itunes_connect_users.nil?
        ...
        data['iTunesConnectUsers']['grantedUsers'] =
          data['iTunesConnectUsers']['availableUsers'].select do |user|
            itunes_connect_users.include?(user['username'])
          end
      end
    ...
    def create_version!(app_id, version_number, platform = 'ios')
      r =
        request(:post) do |req|
          ...
          req.body = { version: { value: version_number.to_s } }.to_json
          req.headers['Content-Type'] = 'application/json'
        end
    ...
    def get_resolution_center(app_id, platform)
      r =
        request(
          :get,
          "ra/apps/#{app_id}/platforms/#{platform}/resolutionCenter?v=latest"
        )
      parse_response(r, 'data')
    end
    ...
    def get_reviews(app_id, platform, storefront, version_id, upto_date = nil)
      ...
      loop do
        ...
        last_review_date =
          Time.at(all_reviews[-1]['value']['lastModified'] / 1000)

        if upto_date && last_review_date < upto_date
          all_reviews =
            all_reviews.select do |review|
              Time.at(review['value']['lastModified'] / 1000) > upto_date
            end
          break
        end
    ...
    def app_version(app_id, is_live, platform: nil)
      ...
      version_id =
        Spaceship::Tunes::AppVersionCommon.find_version_id(platform, is_live)
      ...
      app_version_data(
        app_id,
        version_platform: version_platform, version_id: version_id
      )
    end
    ...
    def app_version_data(app_id, version_platform: nil, version_id: nil)
      ...
      r =
        request(
          :get,
          "ra/apps/#{app_id}/platforms/#{version_platform}/versions/#{version_id}"
        )
      parse_response(r, 'data')
    end
    ...
    def delete_member!(user_id, email)
      payload = []
      payload << { dsId: user_id, email: email }
      request(:post) do |req|
    ...
    def create_member!(
      firstname: nil, lastname: nil, email_address: nil, roles: [], apps: []
    )
      ...
      if apps.length == 0
        data['user']['userSoftwares'] = {
          value: { grantAllSoftware: true, grantedSoftwareAdamIds: [] }
        }
      else
        data['user']['userSoftwares'] = {
          value: { grantAllSoftware: false, grantedSoftwareAdamIds: apps }
        }
      end
    ...
    def time_series_analytics(
      app_ids, measures, start_time, end_time, frequency, view_by
    )
      data = {
        adamId: app_ids,
        dimensionFilters: [],
    ...
    def update_price_tier!(app_id, price_tier)
      ...
      data['pricingIntervalsFieldTO']['value'].first['priceTierEffectiveDate'] =
        effective_date
      ...
      if first_price # first price, need to set all countries
        data['countries'] =
          supported_countries.collect do |c|
    ...
    def transform_to_raw_pricing_intervals(
      app_id = nil,
      purchase_id = nil,
      ...
      if subscription_price_target
        pricing_calculator =
          iap_subscription_pricing_target(
            app_id: app_id,
            purchase_id: purchase_id,
            currency: subscription_price_target[:currency],
            tier: subscription_price_target[:tier]
          )
        intervals_array =
          pricing_calculator.map do |language_code, value|
            existing_interval =
              if pricing_intervals
                pricing_intervals.find do |interval|
                  interval[:country] == language_code
                end
              end
            grandfathered =
    ...
    def update_availability!(app_id, availability)
      ...
      data['countries'] =
        availability.territories.map do |territory|
          { 'code' => territory.code }
        end
      data['theWorld'] =
        if availability.include_future_territories.nil?
          true
        else
          availability.include_future_territories
        end
      ...
      app_available_date =
        cleared_for_preorder ? availability.app_available_date : nil
      ...
      data['preOrder']['clearedForPreOrder'] = {
        'value' => cleared_for_preorder,
        'isEditable' => true,
        'isRequired' => true,
        'errorKeys' => nil
      }
      data['preOrder']['appAvailableDate'] = {
        'value' => app_available_date,
        'isEditable' => true,
        'isRequired' => true,
        'errorKeys' => nil
      }
      data['b2bUsers'] =
        if availability.b2b_app_enabled
          availability.b2b_users.map do |user|
            {
              'value' => {
                'add' => user.add,
                'delete' => user.delete,
                'dsUsername' => user.ds_username
              }
            }
          end
        else
    ...
    def available_languages
      ...
      response.flat_map do |region|
        region['storeFronts']
      end.flat_map { |storefront| storefront['supportedLocaleCodes'] }.uniq
    end
    ...
    def upload_purchase_review_screenshot(app_id, upload_image)
      data =
        du_client.upload_purchase_review_screenshot(
          app_id,
          upload_image,
          content_provider_id,
          sso_token_for_image
    ...
    def upload_messages_screenshot(app_version, upload_image, device)
      ...
      du_client.upload_messages_screenshot(
        app_version,
        upload_image,
        content_provider_id,
        sso_token_for_image,
    ...
    def user_detail_data
      @_cached_user_detail_data ||=
        Spaceship::Tunes::UserDetail.factory(user_details_data, self)
    end
    ...
    def candidate_builds(app_id, version_id)
      r =
        request(
          :get,
          "ra/apps/#{app_id}/versions/#{version_id}/candidateBuilds"
        )
    ...
    def build_trains(app_id, testing_type, tries = 5, platform: nil)
      ...
      if retry_error_messages.any? { |message| ex.to_s.include?(message) }
        ...
        if tries > 0
          logger.warn(
            'Received temporary server error from App Store Connect. Retrying the request...'
          )
      ...
      raise Spaceship::Client::UnexpectedResponse,
            "Temporary App Store Connect error: #{ex}"
    end
    ...
    def remove_testflight_build_from_review!(
      app_id: nil, train: nil, build_number: nil, platform: 'ios'
    )
      r =
        request(:post) do |req|
          req.url(
            "ra/apps/#{app_id}/platforms/#{platform}/trains/#{train}/builds/#{build_number}/reject"
          )
          req.body = {}.to_json
          req.headers['Content-Type'] = 'application/json'
        end
    ...
    def all_builds_for_train(app_id: nil, train: nil, platform: 'ios')
      ...
      r =
        request(
          :get,
          "ra/apps/#{app_id}/trains/#{train}/buildHistory?platform=#{platform}"
        )
      handle_itc_response(r.body)
    end
    ...
    def build_details(app_id: nil, train: nil, build_number: nil, platform: nil)
      r =
        request(
          :get,
          "ra/apps/#{app_id}/platforms/#{platform ||
            'ios'}/trains/#{train}/builds/#{build_number}/details"
        )
      handle_itc_response(r.body)
    end
    ...
    def update_build_information!(
      ...
      build_info['reviewAccountRequired']['value'] =
        (review_user_name.to_s + review_password.to_s).length > 0

      # Now send everything back to iTC
      r =
        request(:post) do |req|
          # same URL, but a POST request
          req
            .url(url)
    ...
    def submit_testflight_build_for_review!(
      ...
      privacy_policy_url: # Optional Metadata:
      nil,
      ...
      build_info['details'].each do |current|
        ...
        current['pageLanguageValue'] = current[]
      end

      review_info = {
        'significantChange' => { 'value' => significant_change },
        'buildTestInformationTO' => build_info,
        'exportComplianceTO' => {
          'usesEncryption' => { 'value' => encryption },
          'encryptionUpdated' => { 'value' => encryption_updated },
          'isExempt' => { 'value' => is_exempt },
          'containsProprietaryCryptography' => { 'value' => proprietary },
          'containsThirdPartyCryptography' => { 'value' => third_party }
        }
      }
      ...
      r =
        request(:post) do |req|
          ...
          req
            .url(
            "ra/apps/#{app_id}/platforms/#{platform}/trains/#{train}/builds/#{build_number}/review/submit"
          )
    ...
    def send_app_submission(app_id, version, data)
      ...
      ad_id_error_keys =
        r.body.fetch('data').fetch('adIdInfo').fetch('sectionErrorKeys')
      export_error_keys =
        r.body.fetch('data').fetch('exportCompliance').fetch('sectionErrorKeys')
    ...
    def release_to_all_users!(app_id, version)
      ...
      r =
        request(:post) do |req|
          req.url(
            "ra/apps/#{app_id}/versions/#{version}/phasedRelease/state/COMPLETE"
          )
          req.headers['Content-Type'] = 'application/json'
          req.body = app_id.to_s
        end
    ...
    def subscription_pricing_tiers(app_id)
      @subscription_pricing_tiers ||=
        begin
          ...
          data = parse_response(r, 'data')['pricingTiers']
          data.map do |tier|
            Spaceship::Tunes::IAPSubscriptionPricingTier.factory(tier)
          end
        end
    end
    ...
    def update_recurring_iap_pricing!(
      app_id: nil, purchase_id: nil, pricing_intervals: nil
    )
      with_tunes_retry do
        r =
          request(:post) do |req|
            pricing_data = {}
            req.url(
              "ra/apps/#{app_id}/iaps/#{purchase_id}/pricing/subscriptions"
            )
    ...
    def create_iap_family(
      app_id: nil, name: nil, product_id: nil, reference_name: nil, versions: []
    )
    ...
    def iap_subscription_pricing_target(
      app_id: nil, purchase_id: nil, currency: nil, tier: nil
    )
      r =
        request(
          :get,
          "ra/apps/#{app_id}/iaps/#{purchase_id}/pricing/equalize/#{currency}/#{tier}"
        )
      parse_response(r, 'data')
    end
    ...
    def create_iap!(
      ...
      if subscription_duration
        data['pricingDurationType'] = { value: subscription_duration }
      end
      if subscription_free_trial
        data['freeTrialDurationType'] = { value: subscription_free_trial }
      end
    ...
    def delete_sandbox_testers!(tester_class, emails)
      ...
      request(:post) do |req|
        req.url(url)
        req.body =
          emails.map { |email| { emailAddress: { value: email } } }.to_json
        req.headers['Content-Type'] = 'application/json'
      end
      true
    end
    ...
    def version_states_history(app_id, platform, version_id)
      r =
        request(
          :get,
          "ra/apps/#{app_id}/versions/#{version_id}/stateHistory?platform=#{platform}"
        )
      parse_response(r, 'data')
    end
    ...
    def generate_app_version_promocodes!(
      app_id: nil, version_id: nil, quantity: nil
    )
      data = [
        {
          numberOfCodes: quantity, agreedToContract: true, versionId: version_id
        }
      ]
    ...
    def with_tunes_retry(tries = 5, potential_server_error_tries = 3, &_block)
      ...
      unless (tries -= 1).zero?
        msg =
          "App Store Connect temporary error received: '#{ex
            .message}'. Retrying after #{seconds_to_sleep} seconds (remaining: #{tries})..."
        puts(msg)
        logger.warn(msg)
      ...
      unless (potential_server_error_tries -= 1).zero?
        msg =
          "Potential server error received: '#{ex
            .message}'. Retrying after 10 seconds (remaining: #{potential_server_error_tries})..."
        puts(msg)
        logger.warn(msg)
        sleep(seconds_to_sleep) unless Object.const_defined?('SpecHelper')
        retry
    ...
    def update_tester_from_app!(tester, app_id, testing)
      ...
      data = {
        users: [
          {
            emailAddress: { value: tester.email },
            firstName: { value: tester.first_name },
            lastName: { value: tester.last_name },
