    ...
    class IAPDetail < TunesBase
      ...
      def setup
        ...
        if @raw_pricing_data
          @raw_data.set(
            %w[pricingIntervals],
            @raw_pricing_data['subscriptions']
          )
        end
      end
      ...
      def pricing_intervals=(value = [])
        ...
        raw_data.set(%w[pricingIntervals], raw_pricing_intervals)
        if @raw_pricing_data
          @raw_pricing_data['subscriptions'] = raw_pricing_intervals
        end
      end
      ...
      def review_screenshot
        unless raw_data && raw_data['versions'] && raw_data['versions'].first &&
               raw_data['versions'].first['reviewScreenshot'] &&
      ...
      def save!
        ...
        if @review_screenshot
          ...
          screenshot_data =
            client.upload_purchase_review_screenshot(
              application.apple_id,
              upload_file
            )
      ...
      def world_wide_pricing_info
        client.pricing_tiers.find do |p|
          p.tier_stem == pricing_intervals.first[:tier].to_s
        end.pricing_info
      end
      ...
      def territorial_pricing_info
        ...
        pricing_intervals.map do |interval|
          pricing_matrix.find do |p|
            p.tier_stem == interval[:tier].to_s
          end.pricing_info
