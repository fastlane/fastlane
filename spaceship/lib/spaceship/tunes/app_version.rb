  ...
  module Tunes
    ...
    class AppVersion < TunesBase
      ...
      attr_mapping(
        {
          ...
          'appStoreInfo.addressLine1.value' =>
            :trade_representative_address_line_1,
          'appStoreInfo.addressLine2.value' =>
            :trade_representative_address_line_2,
          'appStoreInfo.addressLine3.value' =>
      ...
      def create_languages(languages)
        ...
        copy_from =
          self.languages.find { |a| a['language'] == 'en-US' } ||
            self.languages.first

        languages.each do |language|
          # First, see if it's already available
          found =
            self.languages.find { |local| local['language'] == language }
          next if found
      ...
      def candidate_builds
        res =
          client.candidate_builds(self.application.apple_id, self.version_id)
        builds = []
        res.each do |attrs|
      ...
      def select_build(build)
        raw_data.set(
          %w[preReleaseBuildVersionString value],
          build.build_version
        )
        raw_data.set(%w[preReleaseBuildTrainVersionString], build.train_version)
        raw_data.set(%w[preReleaseBuildUploadDate], build.upload_date)
        true
      end
      ...
      def update_rating(hash)
        ...
        hash.each do |key, value|
          to_edit =
            self.raw_data['ratings']['nonBooleanDescriptors'].find do |current|
          ...
          if to_edit
            ...
            unless to_set
              raise "Invalid value '#{value}' for '#{key}', must be 0-2"
            end
            ...
            to_edit =
              self.raw_data['ratings']['booleanDescriptors'].find do |current|
                current['name'].include?(key)
              end
      ...
      def url
        url =
          "https://appstoreconnect.apple.com/WebObjects/iTunesConnect.woa/ra/ng/app/#{application
            .apple_id}/#{self.platform}/versioninfo/"
      ...
      def upload_large_icon!(icon_path)
        ...
        raw_data['largeAppIcon']['value'] =
          generate_image_metadata(image_data, upload_image.file_name)
      end
      ...
      def upload_geojson!(geojson_path)
        ...
        if @transit_app_file.nil?
          @transit_app_file = Tunes::TransitAppFile.factory({})
        end
      ...
      def upload_screenshot!(
        screenshot_path, sort_order, language, device, is_messages
      )
        ...
        device_lang_screenshots =
          screenshots_data_for_language_and_device(
            language,
            device,
            is_messages
          )[
        ...
        existing_sort_orders =
          device_lang_screenshots.map { |s| s['value']['sortOrder'] }
        if screenshot_path # adding / replacing
          upload_file = UploadFile.from_path(screenshot_path)
          screenshot_data =
            client.upload_screenshot(self, upload_file, device, is_messages)
          ...
          language_details =
            raw_data_details.find { |d| d['language'] == language }[
              'displayFamilies'
            ][
              'value'
            ]
          device_language_details =
            language_details.find do |display_family|
              display_family['name'] == device
            end
          ...
          if existing_sort_orders.include?(sort_order) # replace
            device_lang_screenshots[existing_sort_orders.index(sort_order)] =
              new_screenshot
          else
            # add
          ...
          # removing
          unless existing_sort_orders.include?(sort_order)
            raise 'cannot remove screenshot with non existing sort_order'
          end
          device_lang_screenshots.delete_at(
            existing_sort_orders.index(sort_order)
          )
        end
        setup_screenshots
      end
      ...
      def upload_trailer!(
        ...
        device_lang_trailers =
          trailer_data_for_language_and_device(language, device)['value']
        existing_sort_orders =
          device_lang_trailers.map { |s| s['value']['sortPosition'] }

        if trailer_path # adding / replacing trailer
          if (timestamp =~ /^[0-9][0-9].[0-9][0-9]$/).nil?
          ...
          video_preview_data =
            client.upload_trailer_preview(self, video_preview_file, device)
          ...
          new_trailer = {
            'value' => {
              ...
              'descriptionXML' =>
                trailer_data['responses'][0]['descriptionDoc'],
          ...
          if existing_sort_orders.include?(sort_order) # replace
            device_lang_trailers[existing_sort_orders.index(sort_order)] =
              new_trailer
          else
            # add
          ...
          # removing trailer
          unless existing_sort_orders.include?(sort_order)
            raise 'cannot remove trailer with non existing sort_order'
          end
      ...
      def generate_promocodes!(quantity)
        data =
          client.generate_app_version_promocodes!(
      ...
      def setup_large_app_icon
        ...
        @large_app_icon = nil
        if large_app_icon
          @large_app_icon = Tunes::AppImage.factory(large_app_icon)
        end
      end
      ...
      def setup_watch_app_icon
        ...
        @watch_app_icon = nil
        if watch_app_icon
          @watch_app_icon = Tunes::AppImage.factory(watch_app_icon)
        end
      end
      ...
      def setup_transit_app_file
        ...
        @transit_app_file = nil
        if transit_app_file
          @transit_app_file = Tunes::TransitAppFile.factory(transit_app_file)
        end
      end

      def screenshots_data_for_language_and_device(
        language, device, is_messages
      )
      ...
      def container_data_for_language_and_device(data_field, language, device)
        unless DeviceType.exists?(device)
          raise "#{device} isn't a valid device name"
        end
        ...
        unless languages.count > 0
          raise "#{language} isn't an activated language"
        end
        lang_details = languages[0]
        display_families = lang_details['displayFamilies']['value']
        device_details =
          display_families.find do |display_family|
            display_family['name'] == device
          end
        raise "Couldn't find device family for #{device}" if device_details.nil?
        unless device_details.key?(data_field)
          raise "Unexpected state: missing device details for #{device}"
        end
      ...
      def setup_screenshots
        ...
        language_details =
          raw_data_details.each do |current_language|
            ...
            (language_details || []).each do |device_language_details|
              ...
              if device_language_details['scaled']
                device_language_details['scaled']['value'] = true
              end
              if device_language_details['messagesScaled']
        ...
        raw_data_details.each do |row|
          ...
          @screenshots[
            ...
            setup_screenshots_for(row) + setup_messages_screenshots_for(row)
        end
      end
      ...
      def setup_screenshots_for(row)
        ...
        display_families.each do |display_family|
          ...
          display_family
            ...
            data =
              {
                device_type: display_family['name'], language: row['language']
      ...
      def setup_messages_screenshots_for(row)
        ...
        display_families.each do |display_family|
          display_family_screenshots =
            display_family.fetch('messagesScreenshots', {})
            ...
            data =
              {
                ...
                language: row['language'], # to identify imessage screenshots later on (e.g: during download)
                is_imessage: true
              }.merge(screenshot_data)
            result << Tunes::AppScreenshot.factory(data)
          end
      ...
      def setup_trailers_for(row)
        ...
        display_families.each do |display_family|
          display_family.fetch('trailers', {}).fetch('value', [])
            .each do |trailer|
      ...
      def check_preview_screenshot_resolution(preview_screenshot_path, device)
        ...
        unless actual_resolution == expected_resolution
          raise "Invalid #{orientation} screenshot resolution for device #{device}. Should be #{expected_resolution}"
        end
      end
