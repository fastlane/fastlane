    ...
    class AppVersionCommon
      class << self
        def find_version_id(platform, is_live)
          version =
            platform[(is_live ? 'deliverableVersion' : 'inFlightVersion')]
        ...
        def find_platform(versions, search_platform: nil)
          ...
          platform =
            versions.detect do |p|
              %w[ios osx appletvos].include?(p['platformString'])
            end
          ...
          if versions.length > 1 && search_platform.nil?
            platform =
              versions.detect { |p| p['platformString'].to_sym == :ios }
          elsif !search_platform.nil?
            platform =
              versions.detect do |p|
                p['platformString'].to_sym == search_platform
              end
          end
          platform
        end
