  ...
  class Client
    def handle_two_step_or_factor(response)
      ...
      if r.body.kind_of?(Hash) && r.body['trustedDevices'].kind_of?(Array)
        handle_two_step(r)
      elsif r.body.kind_of?(Hash) &&
            r.body['trustedPhoneNumbers'].kind_of?(Array) &&
            r.body['trustedPhoneNumbers'].first.kind_of?(Hash)
        handle_two_factor(r)
      else
        raise "Although response from Apple indicated activated Two-step Verification or Two-factor Authentication, spaceship didn't know how to handle this response: #{r
                .body}"
      end
    end

    def handle_two_step(response)
      if response.body.fetch('securityCode', {})['tooManyCodesLock'].to_s
         .length >
         0
      ...
      puts(
        "Two-step Verification (4 digits code) is enabled for account '#{self
          .user}'"
      )
      puts(
      ...
      available =
        response.body['trustedDevices'].collect do |current|
          "#{current['name']}\t#{current['modelName'] || 'SMS'}\t(#{current[
            'id'
          ]})"
        end
    ...
    def handle_two_step_for_device(device_id)
      ...
      r =
        request(:put) do |req|
          req.url(
            "https://idmsa.apple.com/appleauth/auth/verify/device/#{device_id}/securitycode"
          )
          update_request_headers(req)
        end
    ...
    def handle_two_factor(response, depth = 0)
      ...
      if code == 'sms'
        ...
        body =
          request_two_factor_code_from_phone(
            response.body['trustedPhoneNumbers'],
            code_length
          )
      end
      ...
      r =
        request(:post) do |req|
          req.url(
            "https://idmsa.apple.com/appleauth/auth/verify/#{code_type}/securitycode"
          )
          req.headers['Content-Type'] = 'application/json'
          req.body = body
          update_request_headers(req)
    ...
    def request_two_factor_code_from_phone(phone_numbers, code_length)
      ...
      available =
        phone_numbers.collect { |current| current['numberWithDialCode'] }
      result = choose(*available)
      ...
      r =
        request(:put) do |req|
          ...
          req.headers['Content-Type'] = 'application/json'
          req.body = {
            'phoneNumber' => { 'id' => phone_id }, 'mode' => 'sms'
          }.to_json
          update_request_headers(req)
        end
      ...
      code =
        ask(
          "Please enter the #{code_length} digit code you received at #{result}:"
        )
      {
        'securityCode' => { 'code' => code.to_s },
