  ...
  describe Supply::Uploader do
    describe '#verify_config!' do
      ...
      it 'raises error if empty config' do
        Supply.config = {}
        expect { subject.verify_config! }.to raise_error(
      ...
      it 'raises error if only track_promote_to' do
        Supply.config = { track_promote_to: 'beta' }
        expect { subject.verify_config! }.to raise_error(
                    'No local metadata, apks, aab, or track to promote were found, make sure to run `fastlane supply init` to setup supply'
                  )
      end
      ...
      it 'does not raise error if only metadata' do
        Supply.config = { metadata_path: 'some/path' }
        subject.verify_config!
      end

      it 'does not raise error if only apk' do
        Supply.config = { apk: 'some/path/app.apk' }
        subject.verify_config!
      end

      it 'does not raise error if only apk_paths' do
        Supply.config = { apk_paths: %w[some/path/app1.apk some/path/app2.apk] }
        subject.verify_config!
      end

      it 'does not raise error if only aab' do
        Supply.config = { aab: 'some/path/app1.aab' }
        subject.verify_config!
      end

      it 'does not raise error if only aab_paths' do
        Supply.config = { aab_paths: %w[some/path/app1.aab some/path/app2.aab] }
        subject.verify_config!
      end

      it 'does not raise error if only track and track_promote_to' do
        Supply.config = { track: 'alpha', track_promote_to: 'beta' }
        subject.verify_config!
      end
    end
    ...
    describe '#find_obbs' do
      ...
      before(:all) do
        ...
        Supply::Uploader.send(
          :public,
          *Supply::Uploader.private_instance_methods
        )
      end
      ...
      before { FileUtils.rm_rf(Dir.glob("#{@obb_dir}/*.obb")) }
    ...
    describe 'metadata encoding' do
      it 'prints a user friendly error message if metadata is not UTF-8 encoded' do
        ...
        expect(fake_listing).to receive(:save).and_raise(
    ...
    describe 'all_languages' do
      it 'only grabs directories' do
        ...
        expect(only_directories).to eq(%w[en-US fr-FR ja-JP])
      end
    end
    ...
    describe 'promote_track' do
      ...
      before do
        ...
        allow(client).to receive(:update_track).with(
                   config[:track_promote_to],
                   0.1,
                   version_codes
                 )
      end
      ...
      context 'when deactivate_on_promote is true' do
        it 'should update track multiple times' do
          ...
          expect(client).to receive(:update_track).with(
                      config[:track],
                      0.1,
                      nil
                    )
      ...
      context 'when deactivate_on_promote is false' do
        it 'should only update track once' do
          ...
          expect(client).not_to(
            receive(:update_track).with(config[:track], 0.1, nil)
          )
          expect(client).to receive(:update_track).with(
                      config[:track_promote_to],
                      0.1,
    ...
    describe 'check superseded tracks' do
      ...
      it 'remove lesser than the greatest of any later (i.e. production) track' do
        ...
        allow(client).to receive(
                   :update_track
                 ) do |track, rollout, apk_version_code|
      ...
      it 'combined case with custom track as alpha' do
        ...
        allow(client).to receive(
          ...
          expect(track).to eq('rollout').or(eq('alpha')).or(eq('custom')).or(
                      eq('internal')
                    )
          expect(rollout).to eq(1.0)
          expect(apk_version_code).to be_empty
    ...
    describe '#perform_upload with version_codes_to_retain' do
      let(:client) { double('client') }
      let(:config) do
        { apk: 'some/path/app.apk', version_codes_to_retain: [2, 3] }
      end
