    ...
    module PluginScoresHelper
      ...
      class FastlanePluginScore
        ...
        def initialize(hash, cache_path)
          if ENV['GITHUB_USER_NAME'].to_s.length == 0 ||
             ENV['GITHUB_API_TOKEN'].to_s.length == 0
          ...
          self.data = {
            ...
            has_gnu_license:
              includes_license?('GNU') || includes_license?('GPL'),
            major_release:
              Gem::Version.new(hash['version']) >= Gem::Version.new('1.0.0'),
            actions: [],
            sha: hash['sha']
          }
          ...
          File.open(cache_path, 'w') { |file| file.write(self.cache.to_yaml) }

          self.data[:overall_score] = 0
          self.data[:ratings] =
            self.ratings.collect do |current_rating|
              [current_rating.key, current_rating.value]
            end.to_h
        ...
        def load_cache
          ...
          if self.data[:sha] == cache_data[:sha]
            ...
            self.data[:age_in_days] =
              (DateTime.now - self.data[:initial_commit]).to_i
        ...
        def append_git_data
          Dir.mktmpdir('fastlane-plugin') do |tmp|
            ...
            `
              ...
              #{self.homepage.shellescape}

              #{clone_folder.shellescape}
            `
            ...
            Dir.chdir(clone_folder) do
              ...
              self.data[:initial_commit] =
                DateTime.parse(
                  `
                    git rev-list --all|tail -n1|xargs git show|grep -v diff|head -n3|tail -1|cut -f2-8 -d' '
                  `.strip
                )
                  .to_date
              self.data[:age_in_days] =
              ...
              Dir['spec/**/*_spec.rb'].each do |spec_file|
                ...
                tests_count +=
                  File.read(spec_file).scan(/ it /).count
              end
              self.data[:tests] = tests_count

              actions =
                Dir['**/actions/*.rb'].map do |action_file|
        ...
        def append_github_data
          ...
          self.data[:github_subscribers] =
            repo_details['subscribers_count'].to_i
      ...
      class FastlaneActionFileParser
          ...
          lines.each do |line|
            ...
            when :in_class
              ...
              if is_end?(line)
                ...
                unless name.nil?
                  actions <<
                    FastlanePluginAction.new(
                      name: name.fastlane_underscore,
                      category: category,
            ...
            when :in_method
              ...
              self.state << :in_block if is_block?(line)
              if is_end?(line)
                if last_method_name == 'category'
                  category = last_string_statement
                end
                if last_method_name == 'description'
                  description = last_string_statement
                end
            ...
            puts(
              "Current action name: #{name}, category: #{category}, description: \"#{description}\", details: \"#{details}\""
            )
            puts(
              "Last string statement: #{last_string_statement}, last method name: #{last_method_name}"
            )
          end
          actions
        end
        ...
        def action_name_from_line(line)
          ...
          ) { |matching| matching.captures.first }
        end
        ...
        def string_statement_from_line(line)
          ...
          line.match(regex) { |matching| matching.captures.compact.last }
        end
      end
    end
  end
