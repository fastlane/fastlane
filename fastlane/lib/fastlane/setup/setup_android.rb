module Fastlane
  class SetupAndroid < Setup
    ...
    def fetch_information_for_appfile
      UI.message('')
      UI.message(
        "To avoid re-entering your package name and issuer every time you run fastlane, we'll store those in a so-called Appfile."
      )

      self.package_name = UI.input('Package Name (com.krausefx.app): ')
      puts('')
      puts(
        'To automatically upload builds and metadata to Google Play, fastlane needs a service action json secret file'
          .yellow
      )
      puts(
    ...
    def init_supply
      UI.message('')
      question =
        'Do you plan on uploading metadata, screenshots, and builds to Google Play using fastlane?'
          .yellow
      UI.message(question)
      UI.message(
        'We will now download your existing metadata and screenshots into the `fastlane` folder so fastlane can manage it'
      )
      if UI.confirm('Download existing metadata and setup metadata management?')
        begin
          ...
          supply_config = {
            json_key: self.json_key_file, package_name: self.package_name
          }
          Supply.config =
          ...
          UI.error(ex.to_s)
          UI.error(
            "Setting up `supply` (metadata management action) failed, but don't worry, you can try setting it up again using `fastlane supply init` whenever you want."
          )
        end
      else
        UI.success(
