  ...
  class Setup
    ...
    def self.start(user: nil, is_swift_fastfile: false)
      if FastlaneCore::FastlaneFolder.setup? && !Helper.test?
        ...
        UI.important('------------------')
        UI.important(
          "fastlane is already set up at path `#{FastlaneCore::FastlaneFolder
            .path}`, see the available lanes above"
        )
        UI.message('')
      ...
      spinner =
        TTY::Spinner.new(
          '[:spinner] Looking for iOS and Android projects in current directory...',
          format: :dots
        )
      spinner.auto_spin
      ...
      ios_projects.delete_if do |path|
        Gem.path.any? do |gem_path|
          File.expand_path(path).start_with?(gem_path)
        end
      end
      ios_projects.delete_if do |path|
        path.match(
      ...
      if ios_projects.count > 0
        current_directory =
          ios_projects.find_all do |current_project_path|
        ...
        if current_directory.count == 1
          ...
          if current_directory.count == 2
            ...
            extensions = [
              File.extname(current_directory[0]),
              File.extname(current_directory[1])
            ]
            if extensions.sort == %w[.xcodeproj .xcworkspace].sort
              # Yep, that's this kind of setup
              chosen_project =
                current_directory.find { |d| d.end_with?('.xcworkspace') }
            end
          end
          chosen_project ||=
            UI.select(
              'Multiple iOS projects found in current directory',
              current_directory
            )
          had_multiple_projects_to_choose_from = true
        else
          UI.error(
            'It looks like there is no iOS project in the current directory, though we did find one in a sub-directory'
          )
          UI.error(
            'Please `cd` into the directory of the intended Xcode project you wish to use.'
          )
          UI.user_error!(
        ...
        if chosen_project == 'Pods.xcodeproj'
          unless UI.confirm(
                 "Found '#{chosen_project}', which usually isn't normally what you want. Make sure to switch to the directory containing your intended Xcode project. Would you still like to continue with #{chosen_project}?"
               )
            UI.user_error!(
              'Make sure to `cd` into the directory containing the Xcode project you intend to use and then use `fastlane init` again'
            )
          end
        end
        UI.message(
          "Detected an iOS/macOS project in the current directory: '#{chosen_project}'"
        )
        ...
        UI.error(
          "No iOS or Android projects were found in directory '#{Dir.pwd}'"
        )
        UI.error(
          'Make sure to `cd` into the directory containing your iOS or Android app'
        )
        if UI.confirm(
           'Alternatively, would you like to manually setup a fastlane config in the current directory instead?'
         )
          SetupIos.new(
            is_swift_fastfile: is_swift_fastfile,
            user: user,
          ...
          UI.user_error!(
            'Make sure to `cd` into the directory containing your project and then use `fastlane init` again'
          )
        end
      end
    end
    ...
    def initialize(
      is_swift_fastfile: nil,
      user: nil,
      project_path: nil,
      had_multiple_projects_to_choose_from: nil,
      preferred_setup_method: nil
    )
      self.is_swift_fastfile = is_swift_fastfile
      self.user = user
      self.project_path = project_path
      self.had_multiple_projects_to_choose_from =
        had_multiple_projects_to_choose_from
    ...
    def write_fastfile!
      ...
      fastfile_path =
        File.join(FastlaneCore::FastlaneFolder.path, fastfile_file_name)
      ...
      appfile_path =
        File.join(FastlaneCore::FastlaneFolder.path, appfile_file_name)
    ...
    def setup_gemfile!
      ...
      FastlaneCore::CommandExecutor.execute(
        ...
        print_command: true,
        error:
          proc do |error_output|
            UI.error(
              'Something went wrong when running `bundle update` for you'
            )
            UI.error(
              "Please take a look at your Gemfile at path `#{gemfile_path}`"
            )
            UI.error(
              'and make sure you can run `bundle update` on your machine.'
            )
          end
      )
    end
    ...
    def ensure_gemfile_valid!(update_gemfile_if_needed: false)
      ...
      unless gemfile_content.include?('https://rubygems.org')
        UI.error(
          "You have a local Gemfile, but RubyGems isn't defined as source"
        )
        UI.error(
          "Please update your Gemfile at path `#{gemfile_path}` to include"
        )
        UI.important('')
      ...
      unless gemfile_content.include?('fastlane')
        if update_gemfile_if_needed
          gemfile_content << "\n\ngem \"fastlane\""
          UI.message(
            "Adding `fastlane` to your existing Gemfile at path '#{gemfile_path}'"
          )

          File.write(gemfile_path, gemfile_content)
        else
          UI.error(
            "You have a local Gemfile, but it doesn't include \"fastlane\" as a dependency"
          )
    ...
    def add_or_update_gemfile(update_gemfile_if_needed: false)
      if gemfile_exists?
        ensure_gemfile_valid!(
          update_gemfile_if_needed: update_gemfile_if_needed
        )
      else
        if update_gemfile_if_needed ||
           UI.confirm(
             'It is recommended to run fastlane with a Gemfile set up, do you want fastlane to create one for you?'
           )
          setup_gemfile!
        end
      end
    ...
    def setup_swift_support
      ...
      destination_path =
        File.expand_path('swift', FastlaneCore::FastlaneFolder.path)
      FileUtils.cp_r(runner_source_resources, destination_path)
      UI.success(
        "Copied Swift fastlane runner project to '#{destination_path}'."
      )
    ...
    def explain_concepts
      ...
      UI.message(
        'fastlane uses a ' + '`Fastfile`'.yellow +
          ' to store the automation configuration'
      )
      ...
      UI.message(
        'To get a list of all the available actions, open ' +
          'https://docs.fastlane.tools/actions'.cyan
      )
      continue_with_enter
    end
    ...
    def suggest_next_steps
      ...
      if self.platform == :android
        ...
        UI.message(
          "\t\thttps://docs.fastlane.tools/codesigning/getting-started/".cyan
        )
      end
    ...
    def show_analytics_note
      UI.message(
        'fastlane will collect the number of errors for each action to detect integration issues'
      )
      UI.message(
        'No sensitive/private information will be uploaded, more information: ' +
          'https://docs.fastlane.tools/#metrics'.cyan
      )
    end
  end
end
