module Fastlane
  ...
  class SetupIos < Setup
    ...
    def setup_ios
      ...
      selected =
        UI.select('What would you like to use fastlane for?', options.keys)
      ...
      begin
        ...
        if ex.kind_of?(Spaceship::Client::BasicPreferredInfoError) ||
           ex.kind_of?(Spaceship::Client::UnexpectedResponse)
    ...
    def ios_app_store
      ...
      if self.app_exists_on_itc
        ...
        UI.message(
          "Would you like to have fastlane manage your app's metadata?"
        )
        UI.message(
          'If you enable this feature, fastlane will download your existing metadata and screenshots.'
        )
        UI.message(
          "This way, you'll be able to edit your app's metadata in local `.txt` files."
        )
        UI.message(
          'After editing the local `.txt` files, just run fastlane and all changes will be pushed up.'
        )
        UI.message(
          "If you don't want to use this feature, you can still use fastlane to upload and distribute new builds to the App Store"
        )
        include_metadata =
        ...
        if include_metadata
          ...
          deliver_options =
            FastlaneCore::Configuration.create(
              Deliver::Options.available_options,
              {
                ...
                username:
                  # precheck doesn't need to run during init
                  self
                    .user,
          ...
          Deliver::Setup.new.run(
            deliver_options,
            is_swift: self.is_swift_fastfile
          )
        end
      end
      ...
      if self.is_swift_fastfile
        ...
        if include_metadata
          lane <<
            "\tuploadToAppStore(username: \"#{self.user}\", app: \"#{self
              .app_identifier}\")"
        else
          lane <<
            "\tuploadToAppStore(username: \"#{self.user}\", app: \"#{self
              .app_identifier}\", skipScreenshots: true, skipMetadata: true)"
        end
        lane << '}'
      else
        lane = [
    ...
    def ios_screenshots
      ...
      UI.message(
        'fastlane uses UI Tests to automate generating localized screenshots of your iOS app'
      )
      UI.message(
        'fastlane will now create 2 helper files that are needed to get the setup running'
      )
      UI.message(
        'For more information on how this works and best practices, check out'
      )
      UI.message(
        "\thttps://docs.fastlane.tools/getting-started/ios/screenshots/".cyan
      )
      continue_with_enter
      ...
      ui_testing_scheme =
        UI.select(
          "Which is your UI Testing scheme? If you can't find it in this list, make sure it's marked as `Shared` in the Xcode scheme list",
          available_schemes
        )

      UI.header('Automatically upload to iTC?')
      UI.message(
        'Would you like fastlane to automatically upload all generated screenshots to App Store Connect'
      )
      UI.message('after generating them?')
      UI.message(
        "If you enable this feature you'll need to provide your App Store Connect credentials so fastlane can upload the screenshots to App Store Connect"
      )
      automatic_upload =
        UI.confirm(
          'Enable automatic upload of localized screenshots to App Store Connect?'
        )
      if automatic_upload
        ask_for_credentials(adp: true, itc: true)
        verify_app_exists_itc!
      ...
      if self.is_swift_fastfile
        ...
        if automatic_upload
          lane <<
            "\tuploadToAppStore(username: \"#{self.user}\", app: \"#{self
              .app_identifier}\", skipBinaryUpload: true, skipMetadata: true)"
        end
        lane << '}'
      else
        lane = [
    ...
    def find_and_setup_xcode_project(ask_for_scheme: true)
      ...
      if ask_for_scheme
        self.scheme =
          self.project.select_scheme(
            preferred_to_include: self.project.project_name
          )
      end

      self.app_identifier = self.project.default_app_identifier # These two vars need to be accessed in order to be set
      ask_for_bundle_identifier if self.app_identifier.to_s.length == 0
    end
    ...
    def ask_for_credentials(itc: true, adp: false)
      ...
      UI.message(
        'To use App Store Connect and Apple Developer Portal features as part of fastlane,'
      )
      UI.message('we will ask you for your Apple ID username and password')
      UI.message(
        'This is necessary for certain fastlane features, for example:'
      )
      UI.message('')
      UI.message(
        '- Create and manage your provisioning profiles on the Developer Portal'
      )
      UI.message(
        '- Upload and manage TestFlight and App Store builds on App Store Connect'
      )
      UI.message('- Manage your App Store Connect app metadata and screenshots')
      UI.message('')
      UI.message(
        'Your Apple ID credentials will only be stored in your Keychain, on your local machine'
      )
      ...
      if itc
        ...
        if self.is_swift_fastfile
          self.append_team(
            "var itcTeam: String? { return \"#{self
              .itc_team_id}\" } // App Store Connect Team ID"
          )
        else
          self.append_team(
            "itc_team_id(\"#{self.itc_team_id}\") # App Store Connect Team ID"
          )
        end
      end
      ...
      if adp
        ...
        if self.is_swift_fastfile
          self.append_team(
            "var teamID: String { return \"#{self
              .adp_team_id}\" } // Apple Developer Portal Team ID"
          )
        else
          self.append_team(
            "team_id(\"#{self.adp_team_id}\") # Developer Portal Team ID"
          )
        end
      end
    ...
    def apple_xcode_project_versioning_enabled
      ...
      result =
        Fastlane::Actions::GetBuildNumberAction.run(
          {
            project: paths.first,
            # most of the times, there will only be one project in there
    ...
    def show_information_about_version_bumps
      UI.important(
        "It looks like your project isn't set up to do automatic version incrementing"
      )
      UI.important(
        'To enable fastlane to handle automatic version incrementing for you, please follow this guide:'
      )
      UI.message(
      ...
      UI.important(
        'Afterwards check out the fastlane docs on how to set up automatic build increments'
      )
      UI.message(
    ...
    def verify_app_exists_adp!
      if self.app_identifier.to_s.length == 0
        UI.user_error!('No app identifier provided')
      end
      UI.message(
        "Checking if the app '#{self
          .app_identifier}' exists in your Apple Developer Portal..."
      )
      app = Spaceship::Portal::App.find(self.app_identifier)
      if app.nil?
        UI.error(
          "It looks like the app '#{self
            .app_identifier}' isn't available on the #{'Apple Developer Portal'
            .bold
            .underline}"
        )
        UI.error(
          "for the team ID '#{self.adp_team_id}' on Apple ID '#{self.user}'"
        )
        ...
        if UI.confirm(
          ...
          UI.important(
            "Alright, we won't create the app for you. Be aware, the build is probably going to fail when you try it"
          )
        end
      else
        UI.success(
          "✅  Your app '#{self
            .app_identifier}' is available in your Apple Developer Portal"
        )
      end
    end
    ...
    def verify_app_exists_itc!
      ...
      UI.message(
        "Checking if the app '#{self
          .app_identifier}' exists on App Store Connect..."
      )
      app = Spaceship::Tunes::Application.find(self.app_identifier)
      if app.nil?
        UI.error(
          "Looks like the app '#{self
            .app_identifier}' isn't available on #{'App Store Connect'.bold
            .underline}"
        )
        UI.error(
          "for the team ID '#{self.itc_team_id}' on Apple ID '#{self.user}'"
        )
        if UI.confirm(
           'Would you like fastlane to create the App on App Store Connect for you?'
         )
        ...
        UI.success(
          "✅  Your app '#{self
            .app_identifier}' is available on App Store Connect"
        )
        self.app_exists_on_itc = true
      end
    end
    ...
    def increment_build_number_if_applicable
      ...
      relative_project_path =
        project_path_name.relative_path_from(current_path_name)
    ...
    def create_app_online!(mode: nil)
      ...
      loop do
        ...
        begin
          ...
          UI.important(
            'It looks like something went wrong when we tried to create your app on the Apple Developer Portal'
          )
          unless UI.confirm(
                 'Would you like to try again (y)? If you enter (n), fastlane will fall back to the manual setup'
               )
            raise ex
          end
        end
