  ...
  class ConfigurationHelper
    def self.parse(action, params)
      ...
      if first_element && first_element.kind_of?(FastlaneCore::ConfigItem)
        # default use case
        return FastlaneCore::Configuration.create(
          action.available_options,
          params
        )
      elsif first_element
        if Helper.test?
          UI.error("Old configuration format for action '#{action}'")
        end
        return params
      else
      ...
      if action.respond_to?(:action_name)
        ...
        UI.error(
          "Check out the error below and available options by running `fastlane action #{action
            .action_name}`"
        )
      end
      raise ex
