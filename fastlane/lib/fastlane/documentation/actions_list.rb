module Fastlane
  class ActionsList
    def self.run(filter: nil, platform: nil)
      ...
      filter ? show_details(filter: filter) : print_all(platform: platform)
    end
      ...
      all_actions(platform) do |action, name|
        ...
        if action < Action
          if action.description
            current << action.description.to_s.remove_markdown
          end
    ...
    def self.show_details(filter: nil)
      ...
      if action
        UI.user_error!(action_subclass_error(filter)) unless action < Action
        ...
        if Fastlane::Actions.is_deprecated?(action)
          ...
          if action.deprecated_notes
            puts(action.deprecated_notes.to_s.remove_markdown.deprecated)
          end
          puts("==========================================\n".deprecated)
        end

        puts(
          "More information can be found on https://docs.fastlane.tools/actions/#{filter}"
        )
        puts('')
      else
    ...
    def self.print_suggestions(filter)
      if !filter.nil? && filter.length > 1
        ...
        unless corrections.flatten.empty?
          puts("Did you mean: #{corrections.flatten.uniq.join(', ')}?".green)
        end
      end
    end
    ...
    def self.print_options(action, name)
      if action.available_options
        options = parse_options(action.available_options)
      end
        ...
        Terminal::Table.new(
          ...
          rows:
            FastlaneCore::PrintTable.transform_output(
              output.map { |key, desc| [key.yellow, desc] }
            )
        )
      )
      puts(
    ...
    def self.print_return_value(action, name)
      ...
      puts(
        Terminal::Table.new(
          title: "#{name} Return Value".green,
          rows:
            FastlaneCore::PrintTable.transform_output([[action.return_value]])
        )
      )
      puts('')
    end
    ...
    def self.all_actions(platform = nil)
      action_symbols =
        Fastlane::Actions.constants.select do |c|
          Fastlane::Actions.const_get(c).kind_of?(Class) &&
            c != :TestSampleCodeAction
        end
      action_symbols.sort.each do |symbol|
        ...
        if platform && action.respond_to?(:is_supported?) &&
           !action.is_supported?(platform.to_sym)
          next
        end
    ...
    def self.find_action_named(name)
      all_actions { |action, action_name| return action if action_name == name }
      ...
      if options.kind_of?(Array)
        options.each do |current|
          if current.kind_of?(FastlaneCore::ConfigItem)
            rows <<
              [
                current.key.to_s.yellow,
                if current.deprecated
                  current.description.red
                else
                  current.description
                end,
                current.env_name,
                current.help_default_value
              ]
          elsif current.kind_of?(Array)
            # Legacy actions that don't use the new config manager
            unless [2, 3].include?(current.count)
              UI.user_error!(
                "Invalid number of elements in this row: #{current}. Must be 2 or 3"
              )
            end
            rows << current
