    ...
    def trigger_action_by_name(method_sym, custom_dir, *arguments)
      ...
      if class_ref
        ...
        begin
          ...
          if PluginManager.new.pluginsfile_content.to_s.include?(method_sym.to_s)
            # That's a plugin, but for some reason we can't find it
            # TODO: Add actual link
            UI.user_error!("Plugin '#{method_sym}' couldn't be found, make sure to follow the docs for troubleshooting")
          else
