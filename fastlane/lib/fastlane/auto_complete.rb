    ...
    def self.execute(args, options)
      ...
      if shell.end_with?('fish')
        ...
        if UI.interactive?
          confirm =
            UI.confirm(
              "This will copy a fish script into #{fish_completions_dir} that provides the command tab completion. If the directory does not exist it will be created. Sound good?"
            )
        ...
        final_completion_script_path =
          File.join(fish_completions_dir, 'fastlane.fish')
        ...
        if UI.interactive?
          confirm =
            UI.confirm(
              "This will copy a shell script into #{fastlane_conf_dir} that provides the command tab completion. Sound good?"
            )
          return unless confirm
        end
        ...
        completion_script_path =
          File.join(Fastlane::ROOT, 'lib', 'assets', 'completions')
        ...
        Fastlane::SHELLS.each do |shell_name|
          open(
            "#{fastlane_conf_dir}/completions/completion.#{shell_name}",
            'a'
          ) do |file|
            default_line_prefix = Helper.bundler? ? 'bundle exec ' : ''

            file.puts(
              self.get_auto_complete_line(
                shell_name,
                "#{default_line_prefix}fastlane"
              )
            )

            custom_commands.each do |command|
              auto_complete_line =
                self.get_auto_complete_line(shell_name, command)
        ...
        UI.success(
          'Copied! To use auto complete for fastlane, add the following line to your favorite rc file (e.g. ~/.bashrc)'
        )
