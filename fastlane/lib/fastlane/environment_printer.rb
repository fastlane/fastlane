module Fastlane
  class EnvironmentPrinter
    def self.output
      ...
      FastlaneCore::Configuration.sensitive_strings.compact
        .each do |sensitive_element|
      ...
      if FastlaneCore::Helper.mac? && UI.interactive? &&
         UI.confirm(
           "🙄  Wow, that's a lot of markdown text... should fastlane put it into your clipboard, so you can easily paste it on GitHub?"
         )
        copy_to_clipboard(env_info)
        UI.success('Successfully copied markdown into your clipboard 🎨')
      end
      UI.success(
        'Open https://github.com/fastlane/fastlane/issues/new to submit a new issue ✅'
      )
    end

    def self.get
      UI.important(
        'Generating fastlane environment output, this might take a few seconds...'
      )
    ...
    def self.print_loaded_plugins
      ...
      if plugin_manager.available_plugins.length <= 0
        ...
        plugin_manager.available_plugins.each do |plugin|
          begin
            installed_version =
              Fastlane::ActionCollector.determine_version(plugin)
            latest_version = FastlaneCore::UpdateChecker.fetch_latest(plugin)
            if Gem::Version.new(installed_version) ==
               Gem::Version.new(latest_version)
    ...
    def self.gems_to_check
      Helper.contained_fastlane? ? Gem::Specification : Gem.loaded_specs.values
    end
    ...
    def self.print_loaded_fastlane_gems
      ...
      table << "|-----|---------|------------|\n"
      fastlane_tools = Fastlane::TOOLS + %i[fastlane_core credentials_manager]
      ...
      gems_to_check.each do |current_gem|
        ...
        begin
          latest_version =
            FastlaneCore::UpdateChecker.fetch_latest(current_gem.name)
          if Gem::Version.new(current_gem.version) >=
             Gem::Version.new(latest_version)
            update_status = '✅ Up-To-Date'
          else
            update_status = '🚫 Update available'
    ...
    def self.print_system_locale
      ...
      found_one = false
      env_table = ''
      %w[LANG LC_ALL LANGUAGE].each do |e|
        env_icon = '🚫'
        if ENV[e] && ENV[e].end_with?('UTF-8')
          env_icon = '✅'
          found_one = true
        end
        env_icon = '' if ENV[e].nil?
