module Fastlane
  ...
  class Lane
    ...
    def initialize(
      platform: nil, name: nil, description: nil, block: nil, is_private: false
    )
      unless description.kind_of?(Array)
        UI.user_error!('description must be an array')
      end
      if name.to_s.include?(' ')
        UI.user_error!('lane name must not contain any spaces')
      end
    ...
    class << self
      ...
      def verify_lane_name(name)
        if self.black_list.include?(name.to_s)
          UI.error(
            "Lane name '#{name}' is invalid! Invalid names are #{self.black_list
              .join(', ')}."
          )
        ...
        if self.gray_list.include?(name.to_sym)
          UI.error('------------------------------------------------')
          UI.error(
            "Lane name '#{name}' should not be used because it is the name of a fastlane tool"
          )
          UI.error(
            "It is recommended to not use '#{name}' as the name of your lane"
          )
          UI.error('------------------------------------------------')
      ...
      def ensure_name_not_conflicts(name)
        ...
        UI.error('------------------------------------------------')
        UI.error(
          "Name of the lane '#{name}' is already taken by the action named '#{name}'"
        )
        UI.error('------------------------------------------------')
      end
    end
