    ...
    class FastlaneVersionAction < Action
      def self.run(params)
        ...
        if Gem::Version.new(Fastlane::VERSION) < defined_version
          update_message = nil
          if Helper.bundler?
            update_message = "Please update using `bundle update fastlane`."
          else
            update_message = "Please update using `sudo gem update fastlane`."
          end
          UI.user_error!("The Fastfile requires a fastlane version of >= #{defined_version}. You are on #{Fastlane::VERSION}. #{update_message}")
        end
