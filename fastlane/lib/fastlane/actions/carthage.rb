  ...
  module Actions
    class CarthageAction < Action
      ...
      def self.run(params)
        ...
        elsif %w[update build bootstrap].include?(command_name) &&
              params[:dependencies].count > 0
        ...
        if params[:configuration]
          cmd << "--configuration #{params[:configuration]}"
        end
        ...
        if params[:project_directory]
          cmd << "--project-directory #{params[:project_directory]}"
        end
      ...
      def self.validate(params)
        ...
        if params[:log_path] &&
           !%w[build bootstrap update].include?(command_name)
          UI.user_error!(
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            description:
              "Carthage command (one of: #{available_commands.join(', ')})",
            default_value: 'bootstrap',
            verify_block:
              proc do |value|
                unless available_commands.include?(value)
                  UI.user_error!(
                    "Please pass a valid command. Use one of the following: #{available_commands
                      .join(', ')}"
                  )
                end
              end
            ...
            verify_block:
              proc do |value|
                value.split(',').each do |platform|
                  unless available_platforms.map(&:downcase).include?(
                         platform.downcase
                       )
                    UI.user_error!(
                      "Please pass a valid platform. Use one of the following: #{available_platforms
                        .join(', ')}"
                    )
                  end
                end
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value.end_with?('framework.zip')
                  UI.user_error!(
                    'Please pass a valid string for output. Use following format *.framework.zip'
                  )
                end
              end
          ),
