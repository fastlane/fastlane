    ...
    class UploadSymbolsToCrashlyticsAction < Action
      def self.run(params)
        ...
        if !params[:api_token] && !params[:gsp_path]
          UI.user_error!(
            'Either Fabric API key or path to Firebase Crashlytics GoogleService-Info.plist must be given.'
          )
        end
        ...
        if max_worker_threads > 1
          UI.message(
            "Using #{max_worker_threads} threads for Crashlytics dSYM upload 🏎"
          )
        end
        ...
        if current_path.end_with?('.dSYM')
          ...
          Dir.mktmpdir do |dir|
            Dir.chdir(dir) do
              ...
              work_q = Queue.new
              Dir['*.dSYM'].each { |sub| work_q.push(sub) }
      ...
      def self.upload_dsym(params, path)
        ...
        command <<
          "-p #{params[:platform] == 'appletvos' ? 'tvos' : params[:platform]}"
      ...
      def self.find_api_token(params)
        ...
        unless params[:api_token].to_s.length > 0
          Dir['./**/Info.plist'].each do |current|
            result =
              Actions::GetInfoPlistValueAction.run(path: current, key: 'Fabric')
            next unless result
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            default_value:
              ENV[SharedValues::DSYM_OUTPUT_PATH.to_s] ||
                (Dir['./**/*.dSYM'] + Dir['./**/*.dSYM.zip']).sort_by do |f|
                  File.mtime(f)
                end.last,
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!(
                    "Couldn't find file at path '#{File.expand_path(value)}'"
                  )
                end
                unless value.end_with?('.zip', '.dSYM')
                  UI.user_error!('Symbolication file needs to be dSYM or zip')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                ...
                if value.to_s.length == 0
                  UI.user_error!(
                    "No Path to GoogleService-Info.plist for Firebase Crashlytics given, pass using `gsp_path: 'path'`"
                  )
                end
              end
          ),
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                available = %w[ios appletvos mac]
                unless available.include?(value)
                  UI.user_error!(
                    "Invalid platform '#{value}', must be #{available.join(
                      ', '
                    )}"
                  )
                end
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                min_threads = 1
                unless value >= min_threads
                  UI.user_error!(
                    "Too few threads (#{value}) minimum number of threads: #{min_threads}"
                  )
                end
              end
          )
