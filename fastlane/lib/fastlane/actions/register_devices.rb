    ...
    class RegisterDevicesAction < Action
      ...
      def self.run(params)
        ...
        if devices
          ...
          devices_file =
            CSV.read(File.expand_path(File.join(devices_file)), col_sep: "\t")
          unless devices_file.first == ['Device ID', 'Device Name']
            UI.user_error!(
              'Please provide a file according to the Apple Sample UDID file (https://devimages.apple.com.edgekey.net/downloads/devices/Multiple-Upload-Samples.zip)'
            )
          end

          device_objs =
            devices_file.drop(1).map do |device|
              next if existing_devices.map(&:udid).include?(device[0])

              unless device.count == 2
                UI.user_error!(
                  'Invalid device line, please provide a file according to the Apple Sample UDID file (http://devimages.apple.com/downloads/devices/Multiple-Upload-Samples.zip)'
                )
              end
          ...
          UI.user_error!(
            'You must pass either a valid `devices` or `devices_file`. Please check the readme.'
          )
        end
      ...
      def self.available_options
        ...
        platform =
          Actions.lane_context[Actions::SharedValues::PLATFORM_NAME].to_s
        ...
        [
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Could not find file '#{value}'")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless "'ios', 'mac'".include?(value)
                  UI.user_error!('The platform can only be ios or mac')
                end
              end
          )
        ]
