      ...
      def self.run(params)
        ...
        if devices
          device_objs = devices.map do |k, v|
            UI.user_error!("Passed invalid UDID: #{v} for device: #{k}") unless UDID_REGEXP.match?(v)
          ...
          device_objs = devices_file.drop(1).map do |device|
            ...
            UI.user_error!("Passed invalid UDID: #{device[0]} for device: #{device[1]}") unless UDID_REGEXP.match?(device[0])
