    ...
    class UpdateUrbanAirshipConfigurationAction < Action
      def self.run(params)
        ...
        begin
          ...
          UI.error(ex)
          UI.error(
            "Unable to update Urban Airship configuration for plist file at '#{path}'"
          )
        end
      end
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_errror!('Could not find Urban Airship plist file')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :development_app_key,
            optional: true,
