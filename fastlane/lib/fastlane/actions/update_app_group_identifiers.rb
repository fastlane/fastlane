  ...
  module Actions
    ...
    class UpdateAppGroupIdentifiersAction < Action
      ...
      def self.run(params)
        ...
        UI.message(
          "New App Group Identifiers: #{params[:app_group_identifiers]}"
        )

        entitlements_file = params[:entitlements_file]
        unless File.exist?(entitlements_file)
          UI.user_error!(
            "Could not find entitlements file at path '#{entitlements_file}'"
          )
        end
        ...
        result['com.apple.security.application-groups'] =
          params[:app_group_identifiers]

        # save entitlements file
        result.save_plist(entitlements_file)
        UI.message(
          "New App Group Identifiers set: #{result[
            'com.apple.security.application-groups'
          ]}"
        )

        Actions.lane_context[SharedValues::APP_GROUP_IDENTIFIERS] =
          result['com.apple.security.application-groups']
      end
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            env_name: 'FL_UPDATE_APP_GROUP_IDENTIFIER_ENTITLEMENTS_FILE_PATH',
