  ...
  module Actions
    class GetManagedPlayStorePublishingRightsAction < Action
      def self.run(params)
        ...
        callback_uri =
          'https://fastlane.github.io/managed_google_play-callback/callback.html'
        uri =
          "https://play.google.com/apps/publish/delegatePrivateApp?service_account=#{credentials[
            'client_email'
          ]}&continueUrl=#{URI.escape(callback_uri)}"

        UI.message(
          'To obtain publishing rights for custom apps on Managed Play Store, open the following URL and log in:'
        )
        UI.message('')
        UI.important(uri)
        UI.message(
          '([Cmd/Ctrl] + [Left click] lets you open this URL in many consoles/terminals/shells)'
        )
        UI.message('')
        UI.message(
          'After successful login you will be redirected to a page which outputs some information that is required for usage of the `create_app_on_managed_play_store` action.'
        )
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            short_option: '-j',
            conflicting_options: %i[json_key_data],
            optional: true,
            # optional until it is possible specify either json_key OR json_key_data are required
            description:
