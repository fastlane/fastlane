    ...
    class ArtifactoryAction < Action
      def self.run(params)
        ...
        if File.exist?(file_path)
          ...
          Actions.lane_context[SharedValues::ARTIFACTORY_DOWNLOAD_URL] =
            upload.uri
          Actions.lane_context[SharedValues::ARTIFACTORY_DOWNLOAD_SIZE] =
            upload.size
      ...
      def self.connect_to_artifactory(params)
        ...
        config = params.values.select { |key| config_keys.include?(key) }
