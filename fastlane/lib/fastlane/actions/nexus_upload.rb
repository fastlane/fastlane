    ...
    class NexusUploadAction < Action
      ...
      def self.upload_url(params)
        ...
        if params[:nexus_version] == 2
          ...
          url <<
            "/#{params[:repo_project_name]}-#{params[:repo_project_version]}"
      ...
      def self.upload_options(params)
        ...
        options <<
          "-u #{params[:username].shellescape}:#{params[:password].shellescape}"
      ...
      def self.ssl_options(params)
        options = []
        options << '--insecure' unless params[:ssl_verify]
      ...
      def self.proxy_options(params)
        options = []
        if params[:proxy_address] && params[:proxy_port] &&
           params[:proxy_username] &&
           params[:proxy_password]
          options <<
            "-x #{params[:proxy_address].shellescape}:#{params[:proxy_port]
              .shellescape}"
          options <<
            "--proxy-user #{params[:proxy_username].shellescape}:#{params[
              :proxy_password
            ]
              .shellescape}"
        end
            ...
            verify_block:
              proc do |value|
                ...
                unless File.exist?(file_path)
                  UI.user_error!("Couldn't find file at path '#{file_path}'")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                min_version = 2
                max_version = 3
                unless value >= min_version
                  UI.user_error!(
                    "Unsupported version (#{value}) min. supported version: #{min_version}"
                  )
                end
                unless value <= max_version
                  UI.user_error!(
                    "Unsupported version (#{value}) max. supported version: #{max_version}"
                  )
                end
              end
          ),
