  ...
  module Actions
    class AppaloosaAction < Action
      ...
      def self.run(params)
        ...
        binary_url =
          get_binary_link(binary, api_key, store_id, params[:group_ids])
        return if binary_url.nil?
        screenshots_url =
          get_screenshots_links(
            api_key,
            store_id,
            params[:screenshots],
            params[:locale],
      ...
      def self.get_s3_url(api_key, store_id, path)
        uri =
          URI(
            "#{APPALOOSA_SERVER}/#{store_id}/upload_services/url_for_download"
          )
        params = { store_id: store_id, api_key: api_key, key: path }
        uri.query = URI.encode_www_form(params)
        http = Net::HTTP.new(uri.host, uri.port)
        http.use_ssl = true
        url_for_download_response =
          http.request(Net::HTTP::Get.new(uri.request_uri))
        if invalid_response?(url_for_download_response)
          UI.user_error!(
            'ERROR: A problem occurred with your API token and your store id. Please try again.'
          )
        end
      ...
      def self.upload_screenshots(screenshots, api_key, store_id)
        ...
        list = []
        list <<
          screenshots.map { |screen| upload_on_s3(screen, api_key, store_id) }
      end
      ...
      def self.get_uploaded_links(uploaded_screenshots, api_key, store_id)
        ...
        urls = []
        urls <<
          uploaded_screenshots.flatten.map do |url|
            get_s3_url(api_key, store_id, url)
          end
      end

      def self.get_screenshots_links(
        api_key, store_id, screenshots_path, locale, device
      )
      ...
      def self.get_screenshots(screenshots_path, locale, device)
        ...
        if screenshots_path.strip.empty?
          ...
          screenshots_list(screenshots_path, locale, device)
        end
      end

      def self.screenshots_list(path, locale, device)
        unless Dir.exist?("#{path}/#{locale}")
          return warning_detected('screenshots folder not found')
        end
        list = Dir.entries("#{path}/#{locale}") - %w[. ..]
        list.map do |screen|
          next if screen.match(device).nil?
          unless Dir.exist?("#{path}/#{locale}#{screen}")
            "#{path}/#{locale}#{screen}"
          end
        end.compact
      end
      ...
      def self.upload_on_appaloosa(
        api_key, store_id, binary_path, screenshots, group_ids, description
      )
        screenshots = all_screenshots_links(screenshots)
        uri =
          URI(
            "#{APPALOOSA_SERVER}/#{store_id}/mobile_application_updates/upload"
          )
        http = Net::HTTP.new(uri.host, uri.port)
        http.use_ssl = true
        req =
      ...
      def self.all_screenshots_links(screenshots)
        if screenshots.nil?
          ...
          screenshots = screens.map { |_k, _v| '' }
        else
          missings = 5 - screenshots.count
          (1..missings).map { |_i| screenshots << '' }
        end
        screenshots
      end

      def self.get_env_value(option)
        available_options.map { |opt| opt if opt.key == option.to_sym }.compact[
          0
        ]
          .default_value
      end
      ...
      def self.error_detected(errors)
        errors ? UI.user_error!("ERROR: #{errors}") : false
      end
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!(
                    "Couldn't find ipa || apk file at path '#{value}'"
                  )
                end
              end
          ),
