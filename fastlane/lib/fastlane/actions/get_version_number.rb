  ...
  module Actions
    class GetVersionNumberAction < Action
      ...
      def self.get_target!(project, target_name)
        ...
        unless target_name
          ...
          non_test_targets =
            targets.reject do |t|
              ...
              t
                .respond_to?(:test_target_type?) &&
                t.test_target_type?
            end

          # Returns if only one non-test target
          return targets.first if non_test_targets.count == 1
        ...
        unless target
          UI.user_error!("Cannot find target named '#{target_name}'")
        end
      ...
      def self.get_plist!(folder, target, configuration = nil)
        ...
        if configuration
          ...
          selected =
            UI.select(
              'What build configuration would you like to use?',
              options
            )
        ...
        unless File.exist?(plist_file)
          UI.user_error!("Cannot find plist file: #{plist_file}")
        end
            ...
            verify_block:
              proc do |value|
                ...
                if !File.exist?(value) && !Helper.test?
                  UI.user_error!(
                    "Could not find Xcode project at path '#{File.expand_path(
                      value
                    )}'"
                  )
                end
