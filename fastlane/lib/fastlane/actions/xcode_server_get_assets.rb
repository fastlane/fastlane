module Fastlane
  module Actions
    module SharedValues
      ...
      XCODE_SERVER_GET_ASSETS_ARCHIVE_PATH =
        :XCODE_SERVER_GET_ASSETS_ARCHIVE_PATH
    end
    ...
    class XcodeServerGetAssetsAction < Action
      ...
      def self.run(params)
        ...
        if found_bots.count == 0
          UI.user_error!(
            "Failed to find a Bot with name #{bot_name} on server #{host}, only available Bots: #{bot_names}"
          )
        end
        ...
        integrations =
          xcs.fetch_integrations(bot['_id']).select do |i|
            i['currentStep'] == 'completed'
          end
        if (integrations || []).count == 0
          UI.user_error!(
            "Failed to find any completed integration for Bot \"#{bot_name}\""
          )
        end
        ...
        if integration_number_override
          integration =
            integrations.find { |i| i['number'] == integration_number_override }
          unless integration
            UI.user_error!(
              "Specified integration number #{integration_number_override} does not exist."
            )
          end
        else
        ...
        unless assets_path
          UI.user_error!(
            "Failed to fetch assets for integration #{integration['number']}."
          )
        end

        asset_entries =
          Dir.entries(assets_path).map { |i| File.join(assets_path, i) }

        UI.success(
          "Successfully downloaded #{asset_entries
            .count} assets to file #{assets_path}!"
        )

        # now find the archive and unzip it
        zipped_archive_path =
          asset_entries.find { |i| i.end_with?('xcarchive.zip') }
        ...
        if zipped_archive_path
          ...
          archive_file_path =
            File.basename(
              zipped_archive_path,
              File.extname(zipped_archive_path)
            )
          ...
          if File.exist?(archive_path)
            ...
            UI.important(
              "Archive #{archive_path} already exists, not unzipping again..."
            )
          else
            # unzip the archive
          ...
          unless keep_all_assets
            files_to_delete =
              asset_entries.select do |i|
                File.extname(i) != '.xcarchive' &&
                  !%w[. ..].include?(File.basename(i))
              end

            files_to_delete.each { |i| FileUtils.rm_rf(i) }
          end
        ...
        Actions.lane_context[SharedValues::XCODE_SERVER_GET_ASSETS_PATH] =
          assets_path
      ...
      class XcodeServer
        ...
        def fetch_all_bots
          ...
          if response.status == 401
            UI.user_error!(
              "You are unauthorized to access data on #{@host}, please check that you're passing in a correct username and password."
            )
          end
          if response.status != 200
            UI.user_error!(
              "Failed to fetch Bots from Xcode Server at #{@host}, response: #{response
        ...
        def fetch_integrations(bot_id)
          ...
          if response.status != 200
            UI.user_error!(
              "Failed to fetch Integrations for Bot #{bot_id} from Xcode Server at #{@host}, response: #{response
                .status}: #{response.body}"
            )
          end
          JSON.parse(response.body)['results']
        ...
        def fetch_assets(integration_id, target_folder, action)
          ...
          Dir.mktmpdir do |dir|
            ...
            streamer =
              lambda do |chunk, remaining_bytes, total_bytes|
                if remaining_bytes && total_bytes
                  UI.important(
                    "Downloading: #{100 -
                      (100 * remaining_bytes.to_f / total_bytes.to_f).to_i}%"
                  )
                else
                  UI.error(chunk.to_s)
                end
            ...
            if response.status == 500
              UI.user_error!(
                "Integration doesn't have any assets (it probably never ran)."
              )
            end
            if response.status != 200
              UI.user_error!(
                "Failed to fetch Assets zip for Integration #{integration_id} from Xcode Server at #{@host}, response: #{response
                  .status}: #{response.body}"
              )
            end
            ...
            action.sh(
              "cd \"#{out_folder}\"; cat \"#{temp_file}\" | gzip -d | tar -x"
            )

            # then pull the real name from headers
            asset_filename =
              response.headers['Content-Disposition'].split(';')[1].split('=')[
                1
              ]
                .delete('"')
            ...
            found_folder =
              Dir.entries(out_folder).select do |item|
                item != '.' && item != '..'
              end[
                0
              ]
            ...
            if found_folder.nil?
              UI.user_error!("Internal error, couldn't find unzipped folder")
            end
        ...
        def headers
          require 'base64'
          headers = {
            'User-Agent' => 'fastlane-xcode_server_get_assets',
            # XCS wants user agent. for some API calls. not for others. sigh.
            'X-XCSAPIVersion' =>
              1 # XCS API version with this API, Xcode needs this otherwise it explodes in a 500 error fire. Currently Xcode 7 Beta 5 is on Version 5.
          }

          if @username && @password
            userpass = "#{@username}:#{@password}"
            headers['Authorization'] =
              "Basic #{Base64.strict_encode64(userpass)}"
          end
