  ...
  module Actions
    ...
    class HgCommitVersionBumpAction < Action
      def self.run(params)
        ...
        xcodeproj_path = '/tmp/Test.xcodeproj' if Helper.test?
        ...
        if Helper.test?
          ...
          info_plist_files =
            project.objects.select do |object|
              ...
              Pathname.new(
                File.expand_path(
                  File.join(xcodeproj_path, '..', info_plist_path)
                )
              )
