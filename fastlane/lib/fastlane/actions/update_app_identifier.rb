    ...
    class UpdateAppIdentifierAction < Action
      def self.run(params)
        ...
        unless File.exist?(info_plist_path)
          UI.user_error!(
            "Couldn't find info plist file at path '#{params[:plist_path]}'"
          )
        end
        ...
        if plist['CFBundleIdentifier'] == "$(#{identifier_key})"
          ...
          configs =
            project.objects.select do |obj|
              obj.isa == 'XCBuildConfiguration' &&
                !obj.build_settings[identifier_key].nil?
            end
          ...
          configs =
            configs.select do |obj|
              resolve_path(
                obj.build_settings[info_plist_key],
                params[:xcodeproj]
              ) ==
                info_plist_path
            end
          unless configs.count > 0
            UI.user_error!(
              "Xcodeproj doesn't have configuration with info plist #{params[
                :plist_path
              ]}."
            )
          end
