  ...
  module Actions
    ...
    class DeploygateAction < Action
      ...
      def self.upload_build(api_token, user_name, binary, options)
        ...
        connection =
          Faraday.new(
            url: DEPLOYGATE_URL_BASE, request: { timeout: 120 }
          ) do |builder|
      ...
      def self.run(options)
        ...
        UI.success(
          'Starting with app upload to DeployGate... this could take some time ⏳'
        )
        ...
        unless binary
          UI.user_error!(
            'missing `ipa` and `apk`. deploygate action needs least one.'
          )
        end
        ...
        response =
          self.upload_build(api_token, user_name, binary, upload_options)
        if parse_response(response)
          UI.message(
            "DeployGate URL: #{Actions.lane_context[
              SharedValues::DEPLOYGATE_URL
            ]}"
          )
          UI.success(
            "Build successfully uploaded to DeployGate as revision \##{Actions
              .lane_context[
      ...
      def self.parse_response(response)
        if response.body && response.body.key?('error')
          if response.body['error']
            UI.error(
              "Error uploading to DeployGate: #{response.body['message']}"
            )
            ...
            Actions.lane_context[SharedValues::DEPLOYGATE_REVISION] =
              res['revision']
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.user_error!(
                    "No API Token for DeployGate given, pass using `api_token: 'token'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.user_error!(
                    "No User for DeployGate given, pass using `user: 'user'`"
                  )
                end
              end
          ),
