  ...
  module Actions
    ...
    class BuildIosAppAction < Action
      def self.run(values)
        ...
        unless Actions.lane_context[SharedValues::SIGH_PROFILE_TYPE].to_s ==
               'development'
          values[:export_method] ||=
            Actions.lane_context[SharedValues::SIGH_PROFILE_TYPE]
        end

        if Actions.lane_context[
          ...
          if values[:export_options].kind_of?(Hash)
            match_mapping =
              (
                ...
                .dup
            existing_mapping =
              (values[:export_options][:provisioningProfiles] || {}).dup
          ...
          if values[:export_options].kind_of?(Hash)
            ...
            Actions.lane_context[SharedValues::SIGH_PROFILE_PATHS]
              .each do |profile_path|
              begin
                profile = FastlaneCore::ProvisioningProfile.parse(profile_path)
                app_id_prefix = profile['ApplicationIdentifierPrefix'].first
                bundle_id =
                  profile['Entitlements']['application-identifier'].gsub(
                    "#{app_id_prefix}.",
                    ''
                  )
                values[:export_options][:provisioningProfiles][bundle_id] =
                  profile['Name']
              rescue => ex
      ...
      def self.show_xcode_9_warning
        ...
        UI.message(
          'You passed a path to a custom plist file for exporting the binary.'
        )
        UI.message(
          'Make sure to include information about what provisioning profiles to use with Xcode 9'
        )
        UI.message(
          'More information: https://docs.fastlane.tools/codesigning/xcode-project/#xcode-9-and-up'
        )
      end
    end
  end
