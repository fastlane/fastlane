    ...
    class MailgunAction < Action
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem
            .new(
            key: :mailgun_sandbox_postmaster,
            env_name: 'MAILGUN_SANDBOX_POSTMASTER',
            optional: true,
            description:
              'Mailgun sandbox domain postmaster for your mail. Please use postmaster instead'
      ...
      def self.handle_params_transition(options)
        if options[:mailgun_sandbox_postmaster]
          options[:postmaster] = options[:mailgun_sandbox_postmaster]
        end
        if options[:mailgun_sandbox_postmaster]
          puts(
            "\nUsing :mailgun_sandbox_postmaster is deprecated, please change to :postmaster"
              .yellow
          )
        end

        options[:apikey] = options[:mailgun_apikey] if options[:mailgun_apikey]
        if options[:mailgun_apikey]
          puts(
            "\nUsing :mailgun_apikey is deprecated, please change to :apikey"
              .yellow
          )
        end
      end
      ...
      def self.mailgunit(options)
        ...
        unless options[:attachment].nil?
          ...
          attachments =
            attachment_filenames.map { |filename| File.new(filename, 'rb') }
        ...
        RestClient.post(
          "https://api:#{options[
            :apikey
          ]}@api.mailgun.net/v3/#{sandbox_domain}/messages",
          params
        )
