    ...
    class GradleAction < Action
      def self.run(params)
        ...
        unless File.exist?(gradle_path)
          UI.user_error!(
            "Couldn't find gradlew at path '#{File.expand_path(gradle_path)}'"
          )
        end
        ...
        unless params[:properties].nil?
          flags <<
            params[:properties].map do |k, v|
              "-P#{k.to_s.shellescape}=#{v.to_s.shellescape}"
            end.join(' ')
        end
        unless params[:system_properties].nil?
          flags <<
            params[:system_properties].map do |k, v|
              "-D#{k.to_s.shellescape}=#{v.to_s.shellescape}"
            end.join(' ')
        end
        ...
        if build_type
          Actions.lane_context[SharedValues::GRADLE_BUILD_TYPE] = build_type
        end
        ...
        apk_search_path =
          File.join(project_dir, '**', 'build', 'outputs', 'apk', '**', '*.apk')
        aab_search_path =
          File.join(
            project_dir,
            '**',
        ...
        new_apks =
          Dir[apk_search_path].reject { |path| path =~ /^.*-unaligned.apk$/i }
        ...
        if last_apk_path
          Actions.lane_context[SharedValues::GRADLE_APK_OUTPUT_PATH] =
            File.expand_path(last_apk_path)
        end
        if last_aab_path
          Actions.lane_context[SharedValues::GRADLE_AAB_OUTPUT_PATH] =
            File.expand_path(last_aab_path)
        end
        ...
        if new_apks.empty? && new_aabs.empty?
          UI.message("Couldn't find any new signed apk files...")
        end
      ...
      def self.authors
        %w[KrauseFx lmirosevic]
      end

      def self.is_supported?(platform)
        %i[ios android].include?(platform) # we support iOS as cross platforms apps might want to call `gradle` also
      end
