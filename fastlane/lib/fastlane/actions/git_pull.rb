  ...
  module Actions
    class GitPullAction < Action
      def self.run(params)
        ...
        commands += ['git pull &&'] unless params[:only_tags]
            ...
            verify_block:
              proc do |value|
                unless value.kind_of?(TrueClass) || value.kind_of?(FalseClass)
                  UI.user_error!(
                    'Please pass a valid value for only_tags. Use one of the following: true, false'
                  )
                end
              end
          )
      ...
      def self.example_code
        ['git_pull', 'git_pull(only_tags: true) # only the tags, no commits']
      end
