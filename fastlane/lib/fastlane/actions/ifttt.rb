  ...
  module Actions
    class IftttAction < Action
      def self.run(options)
        ...
        uri =
          URI.parse(
            "https://maker.ifttt.com/trigger/#{options[
              :event_name
            ]}/with/key/#{options[:api_key]}"
          )
        ...
        unless response.code == '200'
          UI.user_error!(
            "Failed to make a request to IFTTT. #{response.message}."
          )
        end
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                if value.to_s.empty?
                  raise UI.error(
                          "No API key given, pass using `api_key: 'key'`"
                        )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                if value.to_s.empty?
                  raise UI.error(
                          "No event name given, pass using `event_name: 'name'`"
                        )
                end
              end
          ),
