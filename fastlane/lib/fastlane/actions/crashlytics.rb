module Fastlane
  module Actions
    class CrashlyticsAction < Action
      def self.run(params)
        if params[:groups].kind_of?(Array)
          params[:groups] = params[:groups].join(',')
        end
        if params[:emails].kind_of?(Array)
        ...
        if params[:notes]
          if params[:notes_path]
            UI.error('Overwriting :notes_path, because you specified :notes')
          end

          changelog =
            Helper::CrashlyticsHelper.write_to_tempfile(
              params[:notes],
              'changelog'
            )
          tempfiles << changelog
          params[:notes_path] = changelog.path
        elsif Actions.lane_context[SharedValues::FL_CHANGELOG] &&
              !params[:notes_path]
          UI.message(
            'Sending FL_CHANGELOG as release notes to Beta by Crashlytics'
          )
        ...
        if params[:ipa_path]
          ...
          android_manifest =
            Helper::CrashlyticsHelper.generate_android_manifest_tempfile
          tempfiles << android_manifest
          command =
            Helper::CrashlyticsHelper.generate_android_command(
              params,
              android_manifest.path
            )
        else
          UI.user_error!(
            'You have to either pass an ipa or an apk file to the Crashlytics action'
          )
        end

        UI.success(
          'Uploading the build to Crashlytics Beta. Time for some ☕️.'
        )

        sanitizer =
          proc do |message|
            message.gsub(params[:api_token], '[[API_TOKEN]]').gsub(
              params[:build_secret],
              '[[BUILD_SECRET]]'
            )
          end
        ...
        if FastlaneCore::Globals.verbose?
          UI.verbose(sanitizer.call(command.join(' ')))
        end
        ...
        UI.success(
          'Visit https://fabric.io/_/beta to add release notes and notify testers.'
        )
      end
        ...
        [
            ...
            verify_block:
              proc do |value|
                unless File.exist?(File.expand_path(value))
                  UI.user_error!(
                    "Couldn't find crashlytics at path '#{File.expand_path(
                      value
                    )}'`"
                  )
                end
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No API token for Crashlytics given, pass using `api_token: 'token'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No build secret for Crashlytics given, pass using `build_secret: 'secret'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Path '#{value}' not found")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
