      ...
      def self.run(params)
        ...
        if required_version.to_s.length == 0
          ...
          if xcode_version_paths.first
            UI.verbose(
              "Loading required version from #{xcode_version_paths.first}"
            )
            required_version = File.read(xcode_version_paths.first).strip
          else
            UI.user_error!(
              'No version: provided when calling the `ensure_xcode_version` action'
            )
          end
        end
        ...
        begin
          ...
          UI.user_error!(
            "Invalid version number provided, make sure it's valid: #{ex}"
          )
        end
        ...
        if selected_version == required_version
          ...
          UI.message(
            "Selected Xcode version is not correct: #{selected_version}. You expected #{required_version}."
          )
          UI.message(
            "To correct this, use: `xcode_select(version: #{required_version})`."
          )
