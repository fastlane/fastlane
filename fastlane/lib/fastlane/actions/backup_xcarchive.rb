      ...
      def self.run(params)
        ...
        if zipped
          Dir.mktmpdir('backup_xcarchive') do |dir|
            ...
            # Create zip
            Actions.sh(
              "cd \"#{xcarchive_folder}\" && zip -r -X \"#{zip_file}\" \"#{xcarchive_file}\" > /dev/null"
            )

            # Moved to its final destination
            FileUtils.mv(zip_file, full_destination)

            Actions.lane_context[SharedValues::BACKUP_XCARCHIVE_FILE] =
              "#{full_destination}/#{File.basename(zip_file)}"
          end
        else
          # Copy xcarchive file
          ...
          Actions.lane_context[SharedValues::BACKUP_XCARCHIVE_FILE] =
            "#{full_destination}/#{File.basename(xcarchive)}"
        end
      end
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                if !Helper.test? && !File.exist?(value)
                  UI.user_error!(
                    "Couldn't find xcarchive file at path '#{value}'"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                if !Helper.test? && !File.directory?(value) &&
                   !File.exist?(value)
                  UI.user_error!(
                    "Couldn't find the destination folder at '#{value}'"
                  )
                end
              end
          ),
