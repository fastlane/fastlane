            ...
            verify_block:
              proc do |value|
                if value.to_s.length == 0
                  UI.user_error!("No value found for 'apk'")
                end
            ...
            verify_block:
              proc do |language|
                unless Supply::Languages::ALL_LANGUAGES.include?(language)
                  UI.user_error!(
                    "Please enter one of the available languages: #{Supply::Languages::ALL_LANGUAGES}"
                  )
                end
              end
          ),
...
class PlaycustomappClient < Supply::AbstractGoogleServiceClient
  ...
  def create_app(
    app_title: nil, language_code: nil, developer_account: nil, apk_path: nil
  )
    custom_app =
      Google::Apis::PlaycustomappV1::CustomApp.new(
        title: app_title, language_code: language_code
      )
