    ...
    class CreateKeychainAction < Action
      def self.run(params)
        ...
        if !exists?(keychain_path)
          commands <<
            Fastlane::Actions.sh(
              "security create-keychain -p #{escaped_password} #{keychain_path}",
              log: false
            )
        elsif params[:require_create]
          UI.abort_with_message!(
            "`require_create` option passed, but found keychain '#{keychain_path}', failing create_keychain action"
          )
        else
        ...
        if params[:default_keychain]
          ...
          begin
            Actions.lane_context[
              Actions::SharedValues::ORIGINAL_DEFAULT_KEYCHAIN
            ] =
              Fastlane::Actions.sh('security default-keychain', log: false)
                .strip
        ...
        if params[:unlock]
          commands <<
            Fastlane::Actions.sh(
              "security unlock-keychain -p #{escaped_password} #{keychain_path}",
              log: false
            )
        end
