  ...
  module Actions
    class SlackAction < Action
      ...
      def self.run(options)
        ...
        options[:message] =
          Slack::Notifier::Util::LinkFormatter.format(options[:message])

        if options[:channel].to_s.length > 0
          channel = options[:channel]
          unless %w[# @].include?(channel[0]) # send message to channel by default
            channel = ('#' + options[:channel])
          end
        end

        username =
          if options[:use_webhook_configured_username_and_icon]
            nil
          else
            options[:username]
          end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            key: :use_webhook_configured_username_and_icon,
      ...
      def self.generate_slack_attachments(options)
        ...
        should_add_payload =
          lambda do |payload_name|
            options[:default_payloads].nil? ||
              options[:default_payloads].map(&:to_sym).include?(
                payload_name.to_sym
              )
          end
        ...
        slack_attachment = {
          ...
          color: color,
          mrkdwn_in: %w[pretext text fields message],
          fields: []
        }

        # custom user payloads
        slack_attachment[:fields] +=
          options[:payload].map do |k, v|
            {
              title: k.to_s,
        ...
        if Actions.git_branch && should_add_payload[:git_branch]
          slack_attachment[:fields] <<
            { title: 'Git Branch', value: Actions.git_branch, short: true }
        end

        # git_author
        if Actions.git_author_email && should_add_payload[:git_author]
          if FastlaneCore::Env.truthy?(
             'FASTLANE_SLACK_HIDE_AUTHOR_ON_SUCCESS'
           ) &&
             options[:success]
        ...
        # last_git_commit
        if Actions.last_git_commit_message &&
           should_add_payload[:last_git_commit]
          slack_attachment[:fields] <<
            {
              title: 'Git Commit',
        ...
        if Actions.last_git_commit_hash(true) &&
           should_add_payload[:last_git_commit_hash]
          slack_attachment[:fields] <<
            {
