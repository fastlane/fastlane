    ...
    class AppetizeAction < Action
      ...
      def self.run(options)
        ...
        if options[:path]
          ...
          if options[:url].nil?
            UI.user_error!(
              'url parameter is required if no file path is specified'
            )
          end
        ...
        UI.message(
          "App URL: #{Actions.lane_context[SharedValues::APPETIZE_APP_URL]}"
        )
        UI.message(
          "Manage URL: #{Actions.lane_context[
            SharedValues::APPETIZE_MANAGE_URL
          ]}"
        )
        UI.message(
          "Public Key: #{Actions.lane_context[
      ...
      def self.appetize_url(options)
        Actions.lane_context[SharedValues::APPETIZE_API_HOST] =
          options[:api_host]
            ...
            verify_block:
              proc do |value|
                if value.start_with?('https')
                  UI.user_error!(
                    'API host should not contain the scheme e.g. `https`'
                  )
                end
              end
          ),
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.user_error!(
                    "No API Token for Appetize.io given, pass using `api_token: 'token'`"
                  )
                end
              end
          ),
            ...
            verify_block:
              proc do |value|
                if value.start_with?('private_')
                  UI.user_error!(
                    'You provided a private key to appetize, please provide the public key'
                  )
                end
              end
          ),
      ...
      def self.authors
        %w[klundberg giginet steprescott]
      end
