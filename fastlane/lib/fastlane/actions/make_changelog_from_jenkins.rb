      ...
      def self.run(params)
        ...
        if Helper.ci? || Helper.test?
          ...
          jenkins_api_url =
            URI(
              ENV['BUILD_URL'] +
                'api/json\?wrapper\=changes\&xpath\=//changeSet//comment'
            )
          begin
            ...
            UI.error(
              "Unable to read/parse changelog from jenkins: #{ex.message}"
            )
          end
        end

        Actions.lane_context[SharedValues::FL_CHANGELOG] =
          changelog.strip.length > 0 ? changelog : params[:fallback_changelog]
      end
