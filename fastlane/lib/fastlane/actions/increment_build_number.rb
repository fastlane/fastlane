  ...
  module Actions
    ...
    class IncrementBuildNumberAction < Action
      ...
      def self.run(params)
        ...
        agv_enabled =
          system(
            [command_prefix, 'agvtool what-version', command_suffix].join(' ')
          )
        ...
        command =
          [
            ...
            if params[:build_number]
              "new-version -all #{params[:build_number].to_s.strip}"
            else
              'next-version -all'
            end,
            command_suffix
        ...
        build_number =
          Actions.sh("#{command_prefix} agvtool what-version", log: false)
            .split("\n")
            .last
            .strip
