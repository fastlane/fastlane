    ...
    class OclintAction < Action
      ...
      def self.run(params)
        ...
        if `which #{oclint_path}`.to_s.empty? && !Helper.test?
          UI.user_error!(
            'You have to install oclint or provide path to oclint binary. Fore more details: '
          ) +
            'http://docs.oclint.org/en/stable/intro/installation.html'.yellow
        end
        ...
        unless File.exist?(compile_commands)
          UI.user_error!(
            "Could not find json compilation database at path '#{compile_commands}'"
          )
        end
        ...
        if File.file?(compile_commands_dir)
          ...
          compile_commands =
            File.join(compile_commands_dir, 'compile_commands.json')
        end

        if params[:select_reqex]
          UI.important(
            "'select_reqex' parameter is deprecated. Please use 'select_regex' instead."
          )
          select_regex = params[:select_reqex]
        end

        select_regex = params[:select_regex] if params[]
        ...
        files =
          JSON.parse(File.read(compile_commands)).map do |compile_command|
            file = compile_command['file']
            if File.exist?(file)
              file
            else
              File.join(compile_command['directory'], file)
            end
          end
        ...
        command_prefix =
          ['cd', File.expand_path('.').shellescape, '&&'].join(' ')
        ...
        if params[:rc]
          UI.important(
            "It's recommended to use 'thresholds' instead of deprecated 'rc' parameter"
          )
          oclint_args << "-rc=#{params[:rc]}" if params[]
        end

        if params[:thresholds]
          oclint_args <<
            ensure_array_is_not_string!(params[:thresholds]).map do |t|
              "-rc=#{t}"
            end
        end
        # Escape ' in rule names with \' when passing on to shell command
        if params[:enable_rules]
          oclint_args <<
            params[:enable_rules].map { |r| "-rule #{r.shellescape}" }
        end
        if params[:disable_rules]
          oclint_args <<
            params[:disable_rules].map { |r| "-disable-rule #{r.shellescape}" }
        end

        if params[:max_priority_1]
          oclint_args << "-max-priority-1=#{params[:max_priority_1]}"
        end
        if params[:max_priority_2]
          oclint_args << "-max-priority-2=#{params[:max_priority_2]}"
        end
        if params[:max_priority_3]
          oclint_args << "-max-priority-3=#{params[:max_priority_3]}"
        end

        if params[:enable_clang_static_analyzer]
          oclint_args << '-enable-clang-static-analyzer'
        end
        if params[:enable_global_analysis]
          oclint_args << '-enable-global-analysis'
        end
        if params[:allow_duplicated_violations]
          oclint_args << '-allow-duplicated-violations'
        end
        ...
        Actions.lane_context[SharedValues::FL_OCLINT_REPORT_PATH] =
          File.expand_path(report_path)
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            key: :select_reqex,
            # select_reqex is deprecated, remove as soon as possible
          ...
          FastlaneCore::ConfigItem.new(
            key: :enable_clang_static_analyzer,
          ...
          FastlaneCore::ConfigItem.new(
            key: :allow_duplicated_violations,
