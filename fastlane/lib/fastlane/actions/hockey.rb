  ...
  module Actions
    ...
    class HockeyAction < Action
      def self.connection(options)
        ...
        base_url =
          if options.delete(:bypass_cdn)
            'https://rink.hockeyapp.net'
          else
            'https://upload.hockeyapp.net'
          end
        foptions = { url: base_url }
      ...
      def self.upload_build(api_token, ipa, options)
        ...
        if dsym_filename
          options[:dsym] =
            Faraday::UploadIO.new(dsym_filename, 'application/octet-stream')
        end
      ...
      def self.create_and_update_build(api_token, ipa, options)
        %i[public_identifier bundle_short_version bundle_version].each do |key|
          unless options[key]
            UI.user_error!(
              "To use the 'create_update' upload mechanism you need to pass the '#{key
                .to_sym}' option."
            )
          end
        end
        ...
        if ipa && File.exist?(ipa)
          ipaio = Faraday::UploadIO.new(ipa, 'application/octet-stream')
        end
        dsym = options.delete(:dsym)

        if dsym
          if dsym && File.exist?(dsym)
            dsym_io = Faraday::UploadIO.new(dsym, 'application/octet-stream')
          end
        end
        ...
        when 200...300
          ...
          UI.user_error!(
            "Error trying to create app version:  #{response
      ...
      def self.run(options)
        build_file =
          [options[:ipa], options[:apk]].detect { |e| !e.to_s.empty? }
        ...
        if options[:dsym]
          ...
          if build_file.nil?
            UI.user_error!(
              "You have to provide a build file (params 'apk' or 'ipa')"
            )
          end
          ...
          if options[:ipa].to_s.end_with?('.ipa')
            ...
            if File.exist?(dsym_path)
              ...
              UI.important(
                "Symbols not found on path #{File.expand_path(
                  dsym_path
                )}. Crashes won't be symbolicated properly"
              )
              dsym_filename = nil
            end
          end
        ...
        if dsym_filename && !File.exist?(dsym_filename)
          UI.user_error!(
            "Symbols on path '#{File.expand_path(dsym_filename)}' not found"
          )
        end

        if options[:upload_dsym_only]
          UI.success(
            'Starting with dSYM upload to HockeyApp... this could take some time.'
          )
        else
          UI.success(
            'Starting with file(s) upload to HockeyApp... this could take some time.'
          )
        end
        ...
        when 200...300
          ...
          Actions.lane_context[SharedValues::HOCKEY_BUILD_INFORMATION] =
            response.body
        ...
        [
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Couldn't find apk file at path '#{value}'")
                end
              end,
            ...
            conflict_block:
              proc do |value|
                UI.user_error!(
                  "You can't use 'apk' and '#{value.key}' options in one run"
                )
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No API token for Hockey given, pass using `api_token: 'token'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Couldn't find ipa file at path '#{value}'")
                end
              end,
            conflicting_options: %i[apk],
            conflict_block:
              proc do |value|
                UI.user_error!(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            description:
              'Set true if you want to create then update your app as opposed to just upload it.' \
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless %w[add replace].include?(value)
                  UI.user_error!(
                    "Invalid value '#{value}' for key 'strategy'. Allowed values are 'add', 'replace'."
                  )
                end
              end
          ),
      ...
      def self.author
        %w[KrauseFx modzelewski lacostej]
      end
