  ...
  module Actions
    ...
    class OnesignalAction < Action
      def self.run(params)
        ...
        unless android_gcm_sender_id.nil?
          payload['android_gcm_sender_id'] = android_gcm_sender_id
        end
        ...
        Actions.lane_context[SharedValues::ONE_SIGNAL_APP_ID] =
          response_body['id']
        Actions.lane_context[SharedValues::ONE_SIGNAL_APP_AUTH_KEY] =
          response_body['basic_auth_key']
      ...
      def self.check_response_code(response)
        ...
        when 200, 204
          ...
          UI.user_error!(
            "Unexpected #{response.code} with response: #{response.body}"
          )
        end
      end
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.error(
                    "Please add 'ENV[\"ONE_SIGNAL_AUTH_KEY\"] = \"your token\"' to your Fastfile's `before_all` section."
                  )
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.error(
                    "Please add 'ENV[\"ONE_SIGNAL_APP_NAME\"] = \"Your app name\"' to your Fastfile's `before_all` section."
                  )
      ...
      def self.authors
        %w[timothybarraclough smartshowltd]
      end
