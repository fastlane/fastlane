  ...
  module Actions
    ...
    class VerifyBuildAction < Action
      ...
      def self.app_path(params, dir)
        build_path =
          params[:ipa_path] || params[:build_path] ||
            Actions.lane_context[SharedValues::IPA_OUTPUT_PATH] ||
            ''
        unless File.exist?(build_path)
          UI.user_error!("Unable to find file '#{build_path}'")
        end
        ...
        unless app_path && File.exist?(app_path)
          UI.user_error!('Unable to find app file')
        end
        app_path
      end
      ...
      def self.gather_cert_info(app_path)
        ...
        parts.each do |part|
          if part =~ /\AAuthority=i(Phone|OS)/
            ...
            values['provisioning_type'] =
              type.downcase =~ /distribution/i ? 'distribution' : 'development'
          end
      ...
      def self.update_with_profile_info(app_path, values)
        profile =
          `
            ...
            /embedded.mobileprovision | security cms -D
          `
        ...
        full_bundle_identifier =
          "#{application_identifier_prefix}.#{values['bundle_identifier']}"

        unless plist['Entitlements']['application-identifier'] ==
               full_bundle_identifier
          UI.user_error!(
            "Inconsistent identifier found; #{plist['Entitlements'][
              'application-identifier'
            ]}, found in the embedded.mobileprovision file, should match #{full_bundle_identifier}, which is embedded in the codesign identity"
          )
        end
        unless plist['Entitlements']['com.apple.developer.team-identifier'] ==
               values['team_identifier']
          UI.user_error!('Inconsistent identifier found')
        end
      ...
      def self.print_values(values)
        FastlaneCore::PrintTable.print_values(
          config: values, title: "Summary for verify_build #{Fastlane::VERSION}"
        )
      end

      def self.evaulate(params, values)
        if params[:provisioning_type]
          unless params[:provisioning_type] == values['provisioning_type']
            UI.user_error!(
              "Mismatched provisioning_type. Required: '#{params[
                :provisioning_type
              ]}''; Found: '#{values['provisioning_type']}'"
            )
          end
        end
        if params[:provisioning_uuid]
          unless params[:provisioning_uuid] == values['provisioning_uuid']
            UI.user_error!(
              "Mismatched provisioning_uuid. Required: '#{params[
                :provisioning_uuid
              ]}'; Found: '#{values['provisioning_uuid']}'"
            )
          end
        end
        if params[:team_identifier]
          unless params[:team_identifier] == values['team_identifier']
            UI.user_error!(
              "Mismatched team_identifier. Required: '#{params[
                :team_identifier
              ]}'; Found: '#{values['team_identifier']}'"
            )
          end
        end
        if params[:team_name]
          unless params[:team_name] == values['team_name']
            UI.user_error!(
              "Mismatched team_name. Required: '#{params[
                :team_name
              ]}'; Found: 'values['team_name']'"
            )
          end
        end
        if params[:app_name]
          unless params[:app_name] == values['app_name']
            UI.user_error!(
              "Mismatched app_name. Required: '#{params[
                :app_name
              ]}'; Found: '#{values['app_name']}'"
            )
          end
        end
        if params[:bundle_identifier]
          unless params[:bundle_identifier] == values['bundle_identifier']
            UI.user_error!(
              "Mismatched bundle_identifier. Required: '#{params[
                :bundle_identifier
              ]}'; Found: '#{values['bundle_identifier']}'"
            )
          end
        end
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                ...
                unless av.include?(value)
                  UI.user_error!(
                    "Unsupported provisioning_type, must be: #{av}"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :provisioning_uuid,
          ...
          FastlaneCore::ConfigItem.new(
            ...
            conflicting_options: %i[build_path],
            optional: true
          ),
          FastlaneCore::ConfigItem.new(
            key: :build_path,
            ...
            conflicting_options: %i[ipa_path],
            optional: true
          )
        ]
      end
