      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            sensitive: true,
            default_value:
              Actions.lane_context[SharedValues::APPETIZE_PUBLIC_KEY],
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                available = %w[25 50 75 100]
                unless available.include?(value)
                  UI.user_error!(
                    "Invalid scale, available: #{available.join(', ')}"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                available = %w[portrait landscape]
                unless available.include?(value)
                  UI.user_error!(
                    "Invalid device, available: #{available.join(', ')}"
                  )
                end
              end
          ),
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                available = %w[black white silver gray]
                unless available.include?(value)
                  UI.user_error!(
                    "Invalid device color, available: #{available.join(', ')}"
                  )
                end
              end
          ),
