  ...
  module Actions
    ...
    class AppetizeViewingUrlGeneratorAction < Action
      def self.run(params)
        ...
        url_params << "params=#{URI.encode_www_form_component(params[:params])}" if params[:params]
        url_params << "proxy=#{URI.encode_www_form_component(params[:proxy])}" if params[:proxy]
