    ...
    class AppledocAction < Action
      ...
      def self.run(params)
        ...
        if params_hash[:output]
          Actions.lane_context[SharedValues::APPLEDOC_DOCUMENTATION_OUTPUT] =
            File.expand_path(params_hash[:output])
      ...
      def self.params_hash_to_cli_args(params)
        ...
        params.each do |key, value|
          args = ARGS_MAP[key]
          if args.empty?
            cli_args << value if key != :input
          elsif value.kind_of?(Array)
            value.each { |v| cli_args << cli_param(args, v) }
          else
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            key: :no_create_docset,
            env_name: 'FL_APPLEDOC_NO_CREATE_DOCSET',
            description: 'Create HTML and skip creating a DocSet',
            is_string: false,
            default_value: false
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            key: :docset_platform_family,
            env_name: 'FL_APPLEDOC_DOCSET_PLATFORM_FAMILY',
            description: 'DocSet platform family',
            is_string: true,
            optional: true
          ),
          FastlaneCore::ConfigItem.new(
            key: :docset_cert_issuer,
