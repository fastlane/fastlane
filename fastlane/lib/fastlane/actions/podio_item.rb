    ...
    class PodioItemAction < Action
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No Client ID for Podio given, pass using `client_id: 'id'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No Client Secret for Podio given, pass using `client_secret: 'secret'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No App ID for Podio given, pass using `app_id: 'id'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No App token for Podio given, pass using `app_token: 'token'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :identifying_field,
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No Identifying field given, pass using `identifying_field: 'field name'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :identifying_value,
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No Identifying value given, pass using `identifying_value: 'unique value'`"
                  )
                end
              end
          ),
      ...
      def self.authenticate(client_id, client_secret, app_id, app_token)
        ...
        if auth_response.code != 200
          UI.user_error!('Failed to authenticate with Podio API')
        end
      ...
      def self.get_item(
        auth_config, identifying_field, identifying_value, app_id
      )
        item_id, item_url =
          get_existing_item(auth_config, identifying_value, app_id)
      ...
      def self.get_existing_item(auth_config, identifying_value, app_id)
        filter_request_body = {
          query: identifying_value, limit: 1, ref_type: 'item'
        }.to_json
        filter_response =
          RestClient.post(
            "#{BASE_URL}/search/app/#{app_id}/",
            filter_request_body,
            auth_config
          )
        if filter_response.code != 200
          UI.user_error!(
            "Failed to search for already existing item #{identifying_value}"
          )
        end
      ...
      def self.create_item(
        ...
        item_request_body = {
          fields: { identifying_field => identifying_value }
        }.to_json
        item_response =
          RestClient.post(
            "#{BASE_URL}/item/app/#{app_id}",
            item_request_body,
            auth_config
          )
        if item_response.code != 200
          UI.user_error!("Failed to create item \"#{identifying_value}\"")
        end
      ...
      def self.update_item(auth_config, item_id, fields)
        if fields.length > 0
          ...
          item_response =
            RestClient.put(
              "#{BASE_URL}/item/#{item_id}",
              item_request_body,
              auth_config
            )
          unless item_response.code != 200 || item_response.code != 204
            UI.user_error!("Failed to update item values \"#{fields}\"")
          end
        end
      end
      ...
      def self.get_embed_id(auth_config, url)
        ...
        embed_response =
          RestClient.post("#{BASE_URL}/embed/", embed_request_body, auth_config)
        if embed_response.code != 200
          UI.user_error!("Failed to create embed for link #{link}")
        end
