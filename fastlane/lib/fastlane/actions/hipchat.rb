  ...
  module Actions
    ...
    class HipchatAction < Action
      def self.run(options)
        ...
        if %w[yellow red green purple gray random].include?(
        ...
        if (message_format == 'html') && (options[:include_html_header] == true)
          message =
            "<table><tr><td><img src='https://s3-eu-west-1.amazonaws.com/fastlane.tools/fastlane.png' width='50' height='50'></td><td>#{message[
              0..9999
            ]}</td></tr></table>"
        end
        ...
        if api_version.to_i == 1
          ...
          if user?(channel)
            UI.user_error!(
              'HipChat private message not working with API V1 please use API V2 instead'
            )
          else
          ...
          escaped_channel =
            URI.unescape(channel) == channel ? URI.escape(channel) : channel
          if user?(channel)
            params = {
              'message' => message, 'message_format' => message_format
            }
            json_headers = {
            ...
            uri =
              URI.parse(
                "https://#{api_host}/v2/user/#{escaped_channel}/message"
              )
            ...
            uri =
              URI.parse(
                "https://#{api_host}/v2/room/#{escaped_channel}/notification"
              )
            response =
              Net::HTTP.post_form(
                uri,
      ...
      def self.check_response_code(response, channel)
        ...
        when 401
          ...
          UI.user_error!(
            "Unexpected #{response
              .code} for `#{channel}` with response: #{response.body}"
          )
        end
      end
      ...
      def self.available_options
        [
            ...
            verify_block:
              proc do |value|
                unless value.to_s.length > 0
                  UI.error(
                    "Please add 'ENV[\"HIPCHAT_API_TOKEN\"] = \"your token\"' to your Fastfile's `before_all` section."
                  )
            ...
            verify_block:
              proc do |value|
                if value.nil? || ![1, 2].include?(value.to_i)
                  UI.error(
                    "Please add 'ENV[\"HIPCHAT_API_VERSION\"] = \"1 or 2\"' to your Fastfile's `before_all` section."
                  )
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless %w[html text].include?(value.to_s)
                  UI.error(
                    "Please specify the message format as either 'html' or 'text'."
                  )
