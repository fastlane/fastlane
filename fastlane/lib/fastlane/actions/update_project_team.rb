    ...
    class UpdateProjectTeamAction < Action
      def self.run(params)
        ...
        unless File.exist?(project_path)
          UI.user_error!(
            "Could not find path to xcodeproj '#{project_path}'. Pass the path to your project (not workspace)!"
          )
        end
        ...
        if selected_targets
          ...
          unless diff_targets.empty?
            UI.user_error!(
              "Could not find target(s) in the project '#{project_path}' - #{diff_targets
                .join(',')}"
            )
          end

          targets.select! do |native_target|
            selected_targets.include?(native_target.name)
          end
        end
        ...
        targets.each do |target|
          UI.message(
            "Updating development team (#{params[:teamid]}) for target `#{target
              .name}` in the project '#{project_path}'"
          )
          ...
          UI.success(
            "Successfully updated project settings to use Developer Team ID '#{params[
              :teamid
