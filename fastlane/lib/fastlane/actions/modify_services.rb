    ...
    class ModifyServicesAction < Action
      def self.run(params)
        ...
        Dir.chdir(FastlaneCore::FastlaneFolder.path || Dir.pwd) do
          ...
          enabled_services =
            services.select do |_k, v|
              v == true || v.to_s == 'on'
            end.map { |k, v| [k, 'on'] }.to_h
          disabled_services =
            services.reject do |_k, v|
              v == true || v.to_s == 'on'
            end.map { |k, v| [k, 'off'] }.to_h
          ...
          unless enabled_services.empty?
            Produce::Service.enable(enabled_services_object, nil)
          end
          ...
          unless disabled_services.empty?
            Produce::Service.disable(disabled_services_object, nil)
          end
        end
      end
