    ...
    class UpdateInfoPlistAction < Action
      def self.run(params)
        ...
        if params[:app_identifier] || params[:display_name] || params[:block]
          if (params[:app_identifier] || params[:display_name]) &&
             params[:block]
            UI.important(
              'block parameter can not be specified with app_identifier or display_name'
            )
            return false
          end
          ...
          if folder.nil?
            UI.user_error!(
              'Could not figure out your xcodeproj path. Please specify it using the `xcodeproj` parameter'
            )
          end
          ...
          if params[:scheme]
            ...
            scheme =
              project.native_targets.detect do |target|
                target.name == params[:scheme]
              end
            unless scheme
              UI.user_error!("Couldn't find scheme named '#{params[:scheme]}'")
            end

            params[:plist_path] =
              scheme.build_configurations.first.build_settings['INFOPLIST_FILE']
            unless params[:plist_path]
              UI.user_error!(
                "Scheme named '#{params[:scheme]}' doesn't have a plist file"
              )
            end
          ...
          unless File.exist?(info_plist_path)
            UI.user_error!(
              "Couldn't find info plist file at path '#{info_plist_path}'"
            )
          end
            ...
            block:
              proc do |plist|
                ...
                if params[:display_name]
                  plist['CFBundleDisplayName'] = params[:display_name]
                end
          ...
          UI.important(
            "You haven't specified any parameters to update your plist."
          )
          false
        end
      end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value[-6..-1].casecmp('.plist').zero?
                  UI.user_error!('Invalid plist file')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
