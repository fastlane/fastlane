    ...
    class RubyVersionAction < Action
      def self.run(params)
        ...
        unless defined_version
          UI.user_error!(
            'Please pass minimum ruby version as parameter to ruby_version'
          )
        end

        if Gem::Version.new(RUBY_VERSION) < defined_version
          error_message =
            "The Fastfile requires a ruby version of >= #{defined_version}. You are on #{RUBY_VERSION}."
          UI.user_error!(error_message)
        end

        UI.message(
          "Your ruby version #{RUBY_VERSION} matches the minimum requirement of #{defined_version}  ✅"
        )
      end
