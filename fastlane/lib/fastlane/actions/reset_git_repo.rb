...
module Fastlane
  module Actions
    ...
    class ResetGitRepoAction < Action
      def self.run(params)
        if params[:force] ||
           Actions.lane_context[SharedValues::GIT_REPO_WAS_CLEAN_ON_START]
          paths = params[:files]
          ...
          if paths.nil?
            ...
            unless params[:exclude].count == 0
              clean_command +=
                ' ' +
                  params[:exclude].map do |exclude|
                    '-e ' + exclude.shellescape
                  end.join(' ')
            end
            ...
            UI.success(
              'Git repo was reset and cleaned back to a pristine state.'
            )
          else
            paths.each do |path|
              unless File.exist?(path)
                UI.important("Couldn't find file at path '#{path}'")
              end
          ...
          UI.user_error!(
            "This is a destructive and potentially dangerous action. To protect from data loss, please add the `ensure_git_status_clean` action to the beginning of your lane, or if you're absolutely sure of what you're doing then call this action with the :force option."
          )
        end
      end
            ...
            verify_block:
              proc do |value|
                unless value.kind_of?(Array)
                  UI.user_error!('Please pass an array only')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
