    ...
    class UpdateFastlaneAction < Action
      ...
      def self.run(options)
        ...
        if tools_to_update.count == 0
          UI.error(
            "No tools specified or couldn't find any installed fastlane.tools"
          )
          return
        end
        ...
        if sudo_needed
          UI.important(
            'It seems that your Gem directory is not writable by your current user.'
          )
          UI.important(
            "fastlane would need sudo rights to update itself, however, running 'sudo fastlane' is not recommended."
          )
          UI.important(
            'If you still want to use this action, please read the documentation on how to set this up:'
          )
        ...
        unless updater.respond_to?(:highest_installed_gems)
          ...
          UI.important(
            'Please update your version of ruby gems before proceeding.'
          )
        ...
        highest_versions =
          updater.highest_installed_gems.keep_if do |key|
            tools_to_update.include?(key)
          end
        update_needed =
          updater.which_to_update(highest_versions, tools_to_update)
        ...
        update_needed.each do |tool_info|
          ...
          latest_official_version =
            FastlaneCore::UpdateChecker.fetch_latest(tool)

          if options[:nightly]
            UI.message(
              "Updating #{tool} from #{local_version.to_s
                .yellow} to nightly build #{gem_version.to_s
                .yellow}... (last official release #{latest_official_version
                .to_s
                .yellow}) 🚀"
            )
          else
            UI.message(
              "Updating #{tool} from #{local_version.to_s
                .yellow} to #{latest_official_version.to_s.yellow}... 🚀"
            )
          end
          ...
          requirement_version =
            if options[:nightly]
              ...
              local_version.approximate_recommendation
            end
        ...
        if options[:nightly]
          UI.success(
            "Thanks for using fastlane's nightly builds! This makes it easier for everyone to detect regressions earlier."
          )
          UI.success(
            'Please submit an issue on GitHub if anything behaves differently than it should 🍪'
          )
        else
        ...
        UI.message(
          'fastlane.tools successfully updated! I will now restart myself... 😴'
        )
      ...
      def self.show_information_about_nightly_builds
        ...
        UI.message('')
        UI.message(
          'Nightly builds are reviewed and tested just like the public releases 🚂'
        )
        UI.message('')
      end

      def self.all_installed_tools
        Gem::Specification.select { |s| ALL_TOOLS.include?(s.name) }.map(&:name)
          .uniq
      end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            ...
            description:
              'Comma separated list of fastlane tools to update (e.g. `fastlane,deliver,sigh`)',
            deprecated: true,
            optional: true
          )
        ]
