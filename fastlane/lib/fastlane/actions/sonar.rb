  ...
  module Actions
    class SonarAction < Action
      def self.run(params)
        ...
        sonar_scanner_args = []
        if params[:project_configuration_path]
          sonar_scanner_args <<
            "-Dproject.settings=\"#{params[:project_configuration_path]}\""
        end
        if params[:project_key]
          sonar_scanner_args << "-Dsonar.projectKey=\"#{params[:project_key]}\""
        end
        ...
        if params[:sources_path]
          sonar_scanner_args << "-Dsonar.sources=\"#{params[:sources_path]}\""
        end
        if params[:project_language]
          sonar_scanner_args <<
            "-Dsonar.language=\"#{params[:project_language]}\""
        end
        if params[:source_encoding]
          sonar_scanner_args <<
            "-Dsonar.sourceEncoding=\"#{params[:source_encoding]}\""
        end
        if params[:sonar_login]
          sonar_scanner_args << "-Dsonar.login=\"#{params[:sonar_login]}\""
        end
        if params[:sonar_url]
          sonar_scanner_args << "-Dsonar.host.url=\"#{params[:sonar_url]}\""
        end
        if params[:sonar_runner_args]
          sonar_scanner_args << params[:sonar_runner_args]
        end

        command =
          [command_prefix, 'sonar-scanner', sonar_scanner_args].join(' ')
      ...
      def self.verify_sonar_scanner_binary
        unless `which sonar-scanner`.to_s.length > 0
          UI.user_error!(
            'You have to install sonar-scanner using `brew install sonar-scanner`'
          )
        end
      end
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            key: :project_configuration_path,
            ...
            # default is enforced by sonar-scanner binary
            optional: true,
            verify_block:
              proc do |value|
                unless value.nil? || File.exist?(value)
