    ...
    class AppStoreBuildNumberAction < Action
      ...
      def self.get_build_number(params)
        ...
        if params[:live]
          ...
          unless app.live_version
            UI.user_error!(
              "Could not find a live-version of #{params[
                :app_identifier
              ]} on iTC"
            )
          end
          ...
          UI.message(
            "Fetching the latest build number for version #{version_number}"
          )

          begin
            build_numbers =
              app.all_builds_for_train(
                train: version_number, platform: platform
              )
                .map(&:build_version)
            build_nr = self.order_versions(build_numbers).last
            if build_nr.nil? && params[:initial_build_number]
              UI.message(
                "Could not find a build on iTC. Using supplied 'initial_build_number' option"
              )
              build_nr = params[:initial_build_number]
            end
          rescue StandardError
            unless params[:initial_build_number]
              UI.user_error!(
        ...
        UI.message(
          "Latest upload for version #{version_number} is build: #{build_nr}"
        )
