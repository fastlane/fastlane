    ...
    class ChangelogFromGitCommitsAction < Action
      def self.run(params)
        if params[:commits_count]
          UI.success(
            "Collecting the last #{params[:commits_count]} Git commits"
          )
        else
          if params[:between]
            if params[:between].kind_of?(String) &&
               params[:between].include?(',') # :between is string
            ...
            from =
              Actions.last_git_tag_name(
                params[:match_lightweight_tag],
                params[:tag_match_pattern]
              )
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            optional: true,
            is_string: false,
            conflicting_options: %i[commits_count],
            verify_block:
              proc do |value|
                if value.kind_of?(String)
                  unless value.include?(',')
                    UI.user_error!(':between must contain comma')
                  end
                else
                  unless value.kind_of?(Array)
                    UI.user_error!(':between must be of type array')
                  end
                  if value.any?(&:nil?)
                    UI.user_error!(':between must not contain nil values')
                  end
                  unless (value || []).size == 2
                    UI.user_error!(':between must be an array of size 2')
                  end
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            optional: true,
            is_string: false,
            conflicting_options: %i[between],
            type: Integer,
            verify_block:
              proc do |value|
                unless value.to_i >= 1
                  UI.user_error!(':commits_count must be >= 1')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value.nil?
                  UI.important(
                    'The :include_merges option is deprecated. Please use :merge_commit_filtering instead'
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            description:
              "Controls inclusion of merge commits when collecting the changelog. Valid values: #{GIT_MERGE_COMMIT_FILTERING_OPTIONS
                .map { |o| "`:#{o}`" }.join(', ')}",
            optional: true,
            ...
            verify_block:
              proc do |value|
                matches_option =
                  GIT_MERGE_COMMIT_FILTERING_OPTIONS.any? do |opt|
                    opt.to_s == value
                  end
                unless matches_option
                  UI.user_error!(
                    "Valid values for :merge_commit_filtering are #{GIT_MERGE_COMMIT_FILTERING_OPTIONS
                      .map { |o| "'#{o}'" }.join(', ')}"
                  )
                end
              end
      ...
      def self.author
        %w[mfurtak asfalcone SiarheiFedartsou allewun]
      end
