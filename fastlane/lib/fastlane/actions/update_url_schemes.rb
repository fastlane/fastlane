      ...
      def self.run(params)
        ...
        unless hash['CFBundleURLTypes']
          hash['CFBundleURLTypes'] = [
            { 'CFBundleTypeRole' => 'Editor', 'CFBundleURLSchemes' => [] }
          ]
        end
        ...
        if update_url_schemes
          new_schemes =
            update_url_schemes.call(
              hash['CFBundleURLTypes'].first['CFBundleURLSchemes']
            )

          # Verify array of strings
          string =
            "The URL schemes must be an array of strings, got '#{new_schemes}'."
            ...
            verify_block:
              proc do |path|
                unless File.exist?(path)
                  UI.user_error!("Could not find plist at path '#{path}'")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |url_schemes|
                string =
                  "The URL schemes must be an array of strings, got '#{url_schemes}'."
                verify_schemes!(url_schemes, string)
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :update_url_schemes,
            description:
