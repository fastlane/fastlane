  ...
  module Actions
    class SpmAction < Action
      def self.run(params)
        ...
        cmd <<
          (
            if package_commands.include?(params[:command])
              'package'
            else
              params[:command]
            end
        ...
        if params[:package_path]
          cmd << "--package-path #{params[:package_path]}"
        end
        ...
        cmd << "--xcconfig-overrides #{params[:xcconfig]}" if params[:xcconfig]
        ...
        FastlaneCore::CommandExecutor.execute(
          command: cmd.join(' '), print_all: true, print_command: true
        )
      end
      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            description:
              "The swift command (one of: #{available_commands.join(', ')})",
            default_value: 'build',
            verify_block:
              proc do |value|
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless valid_configurations.include?(value)
                  UI.user_error!(
                    'Please pass a valid configuration: (debug|release)'
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless xcpretty_output_types.include?(value)
                  UI.user_error!(
                    "Please pass a valid xcpretty output type: (#{xcpretty_output_types
                      .join('|')})"
                  )
                end
              end
