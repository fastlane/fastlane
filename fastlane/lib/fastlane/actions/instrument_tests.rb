  ...
  module Actions
    ...
    class InstrumentedTestsAction < Action
      def self.run(params)
        ...
        avd_options = params[:avd_options] if not params[:avd_options].nil? # Make this better!!
        avd_abi = "--abi #{params[:avd_abi]}" if not params[:avd_abi].nil?
        avd_tag = "--tag #{params[:avd_tag]}" if not params[:avd_tag].nil?
        create_avd = ["#{params[:sdk_path]}/tools/android", "create avd", avd_name, target_id, avd_abi, avd_tag, avd_options].join(" ")
        start_avd = ["#{params[:sdk_path]}/tools/emulator", "-avd #{params[:avd_name]}", "-gpu on"]
        devices = `#{params[:sdk_path]}/tools/android list avd`.chomp

        # Delete avd if one already exists
        if not devices.match(/#{params[:avd_name]}/).nil?
        ...
        pid = fork do
          exec Action.sh(start_avd)
        end

        # Wait for device to be fully booted
        Helper.log.info("Waiting for emulator to finish booting.....".yellow)
        while true
          stdout, stdeerr, status = Open3.capture3("#{params[:sdk_path]}/platform-tools/adb shell getprop sys.boot_completed")
        ...
        Helper.log.info('Executing Instrumented Tests!'.green)
        begin
          gradle.trigger(task: params[:task], flags: params[:flags], serial: nil)
        ensure
          Helper.log.info('Shutting down emulator...'.green)
          adb.trigger(command: 'emu kill', serial: 'emulator-5554')

          Helper.log.info('Deleting emulator....'.green)
          `#{params[:sdk_path]}/tools/android delete avd -n #{params[:avd_name]}`.chomp
        end
      end

      def self.description
        'Run android instrumented test againts a newly created avd.'
      end

      def self.details
        [
            'Instrumented tests need a emulator or real device to execute against.',
           ' "This will run against a local avd which will be created/booted/deleted on each run,"'
        ].join("\n")
      end
      ...
      def self.return_value
        "Console output from gradle command"
      end
