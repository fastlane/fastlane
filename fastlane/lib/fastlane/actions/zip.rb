  ...
  module Actions
    class ZipAction < Action
      def self.run(params)
        ...
        Dir.chdir(File.expand_path('..', params[:path])) do
          # required to properly zip
          ...
          if params[:password]
            ...
            Actions.sh(
              "zip -#{zip_options} #{password_option} #{absolute_output_path
                .shellescape} #{File.basename(params[:path]).shellescape}"
            )
          else
            Actions.sh(
              "zip -#{zip_options} #{absolute_output_path.shellescape} #{File
        ...
        UI.success(
          "Successfully generated zip file at path '#{File.expand_path(
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!(
                    "Couldn't find file/folder at path '#{File.expand_path(
                      value
                    )}'"
                  )
                end
