    ...
    class CreateJiraVersionAction < Action
      def self.run(params)
        ...
        unless project_name.nil?
          project = client.Project.find('AECP')
        ...
        [
          FastlaneCore::ConfigItem.new(key: :url,
                                      ...
                                      verify_block: proc do |value|
                                         UI.user_error!("No url for Jira given, pass using `url: 'url'`") unless (value and not value.empty?)
                                       end),
          FastlaneCore::ConfigItem.new(key: :username,
                                       ...
                                       type: String,
                                       verify_block: proc do |value|
                                         UI.user_error!("No username given, pass using `username: 'jira_user'`") unless (value and not value.empty?)
                                       end),
          FastlaneCore::ConfigItem.new(key: :password,
                                       ...
                                       type: String,
                                       verify_block: proc do |value|
                                         UI.user_error!("No password given, pass using `password: 'T0PS3CR3T'`") unless (value and not value.empty?)
                                       end),
          FastlaneCore::ConfigItem.new(key: :project_name,
                                       ...
                                       verify_block: proc do |value|
                                         UI.user_error!("No Project ID given, pass using `project_id: 'PROJID'`") unless (value and not value.empty?)
                                       end),
          ...
          FastlaneCore::ConfigItem.new(key: :name,
                                       ...
                                       type: String,
                                       verify_block: proc do |value|
                                         UI.user_error!("No version name given, pass using `name: '1.0.0'`") unless (value and not value.empty?)
                                       end),
