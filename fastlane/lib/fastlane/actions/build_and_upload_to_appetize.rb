  ...
  module Actions
    class BuildAndUploadToAppetizeAction < Action
      def self.run(params)
        ...
        if params[:scheme].to_s.length > 0
          xcodebuild_configs[:scheme] ||= params[:scheme]
        end
        ...
        zipped_bundle =
          Actions::ZipAction.run(
            path: app_path, output_path: File.join(tmp_path, 'Result.zip')
          )

        other_action.appetize(
          path: zipped_bundle, api_token: params[:api_token]
        )

        public_key = Actions.lane_context[SharedValues::APPETIZE_PUBLIC_KEY]
        UI.success(
          "Generated Public Key: #{Actions.lane_context[
