  ...
  module Actions
    ...
    class DownloadDsymsAction < Action
      ...
      def self.run(params)
        ...
        unless app
          UI.user_error!(
            "Could not find app with bundle identifier '#{params[
              :app_identifier
            ]}' on account #{params[:username]}"
          )
        end
        ...
        if version == 'latest'
          ...
          latest_version =
            app.edit_version(platform: platform) ||
              app.live_version(platform: platform)

          if latest_version.version.nil?
            UI.user_error!(
              'Could not find latest version for your app, please try setting a specific version'
            )
          end

          latest_candidate_build =
            latest_version.candidate_builds.max_by(&:upload_date)
        ...
        app.tunes_all_build_trains(platform: platform).each do |train|
          ...
          if version && version != train.version_string
            UI.verbose(
              "Version #{version} doesn't match: #{train.version_string}"
            )
            next
          end
          ...
          if min_version && min_version > Gem::Version.new(train.version_string)
            UI.verbose(
              "Min version #{min_version} not reached: #{train.version_string}"
            )
            next
          end

          app.tunes_all_builds_for_train(
            train: train.version_string, platform: platform
          )
            .each do |build|
            message = []
            message << "Found build version: #{build.build_version}"
            if build_number
              message << ", comparing to supplied build_number: #{build_number}"
            end
            UI.verbose(message.join(' '))
            ...
            if build_number && build.build_version != build_number
              UI.verbose(
                "build_version: #{build
                  .build_version} doesn't match: #{build_number}"
              )
              next
            end
            ...
            begin
              if build_number
                UI.verbose(
                  "Build_version: #{build
                    .build_version} matches #{build_number}, grabbing dsym_url"
                )
              end

              build_details =
                app.tunes_build_details(
              ...
              UI.error(
                "Error accessing dSYM file for build\n\n#{build}\n\nException: #{ex}"
              )
            end
            ...
            if download_url
              ...
              UI.message(
                "No dSYM URL for #{build.build_version} (#{train
                  .version_string})"
              )
            end
          end
      ...
      def self.download(
        download_url, bundle_id, train_number, build_version, output_directory
      )
        ...
        UI.success(
          "🔑  Successfully downloaded dSYM file for #{train_number} - #{build_version} to '#{path}'"
        )
      ...
      def self.write_dsym(
        data, bundle_id, train_number, build_number, output_directory
      )
        file_name = "#{bundle_id}-#{train_number}-#{build_number}.dSYM.zip"
        file_name = output_directory + file_name if output_directory
      ...
      def self.is_supported?(platform)
        %i[ios appletvos].include?(platform)
      end
