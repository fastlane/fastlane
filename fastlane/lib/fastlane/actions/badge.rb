      ...
      def self.available_options
        [
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value == true
                  UI.user_error!(
                    'dark is only a flag and should always be true'
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && File.exist?(value)
                  UI.user_error!('custom should be a valid file path')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value == true
                  UI.user_error!(
                    'no_badge is only a flag and should always be true'
                  )
                end
              end
          ),
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value == true
                  UI.user_error!(
                    'alpha is only a flag and should always be true'
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                if Dir[value].empty?
                  UI.user_error!('path needs to be a valid directory')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :shield_io_timeout,
            ...
            verify_block:
              proc do |value|
                if value.to_i < 1
                  UI.user_error!('shield_io_timeout needs to be an integer > 0')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value == true
                  UI.user_error!(
                    'alpha_channel is only a flag and should always be true'
                  )
                end
              end
          ),
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value == true
                  UI.user_error!(
                    'shield_no_resize is only a flag and should always be true'
                  )
                end
              end
          )
