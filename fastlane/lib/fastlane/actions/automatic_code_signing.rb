    ...
    class AutomaticCodeSigningAction < Action
      def self.run(params)
        FastlaneCore::PrintTable.print_values(
          config: params, title: 'Summary for Automatic Codesigning'
        )
        path = params[:path]
        ...
        UI.message(
          "Updating the Automatic Codesigning flag to #{if params[
            :use_automatic_signing
          ]
            'enabled'
          else
            'disabled'
          end} for the given project '#{path}'"
        )
        ...
        target_dictionary =
          project.targets.map do |f|
            {
              name: f.name,
              uuid: f.uuid,
              build_configuration_list: f.build_configuration_list
            }
          end
        ...
        target_attributes.each do |target, sett|
          ...
          if params[:targets]
            ...
            unless params[:targets].include?(found_target[:name])
              UI.important(
                "Skipping #{found_target[:name]} not selected (#{params[
                  :targets
                ]
                  .join(',')})"
              )
          ...
          if params[:team_id]
            ...
            UI.important(
              "Set Team id to: #{params[:team_id]} for target: #{found_target[
                :name
              ]}"
            )
          end
          if params[:code_sign_identity]
            ...
            UI.important(
              "Set Code Sign identity to: #{params[
                :code_sign_identity
              ]} for target: #{found_target[:name]}"
            )
          end
          if params[:profile_name]
            ...
            UI.important(
              "Set Provisioning Profile name to: #{params[
                :profile_name
              ]} for target: #{found_target[:name]}"
            )
          end
          ...
          if params[:profile_uuid]
            ...
            UI.important(
              "Set Provisioning Profile UUID to: #{params[
                :profile_uuid
              ]} for target: #{found_target[:name]}"
            )
          end
          if params[:bundle_identifier]
            ...
            UI.important(
              "Set Bundle identifier to: #{params[
                :bundle_identifier
              ]} for target: #{found_target[:name]}"
            )
          end
        ...
        if changed_targets.empty?
          ...
          UI.success(
            "Successfully updated project settings to use Code Sign Style = '#{if params[
              :use_automatic_signing
            ]
              'Automatic'
            else
          ...
          changed_targets.each { |target| UI.success("\t * #{target}") }
        end
      ...
      def self.alias_used(action_alias, params)
        if action_alias == 'enable_automatic_code_signing'
          params[:use_automatic_signing] = true
        end
      end

      def self.aliases
        %w[enable_automatic_code_signing disable_automatic_code_signing]
      end
            ...
            verify_block:
              proc do |value|
                unless File.exist?(File.expand_path(value))
                  UI.user_error!('Path is invalid')
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            key: :use_automatic_signing,
      ...
      def self.authors
        %w[mathiasAichinger hjanuschka p4checo portellaa aeons]
      end
