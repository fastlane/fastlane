module Fastlane
  module Actions
    class SwiftlintAction < Action
      def self.run(params)
        if `which swiftlint`.to_s.length == 0 && params[:executable].nil? &&
           !Helper.test?
          UI.user_error!(
        ...
        if params[:mode] == :autocorrect &&
           ...
           !Helper.test?
          UI.user_error!(
            "Your version of swiftlint (#{version}) does not support autocorrect mode.\nUpdate swiftlint using `brew update && brew upgrade swiftlint`"
          )
        end
        ...
        if params[:config_file]
          command << " --config #{params[:config_file].shellescape}"
        end
        ...
        if params[:files]
          if version < Gem::Version.new('0.5.1')
            UI.user_error!(
              "Your version of swiftlint (#{version}) does not support list of files as input.\nUpdate swiftlint using `brew update && brew upgrade swiftlint`"
            )
          end

          files =
            params[:files].map.with_index(0) do |f, i|
              "SCRIPT_INPUT_FILE_#{i}=#{f.shellescape}"
            end.join(' ')
          command =
            command.prepend(
              "SCRIPT_INPUT_FILE_COUNT=#{params[:files].count} #{files} "
            )
          command << ' --use-script-input-files'
        end

        if params[:output_file]
          command << " > #{params[:output_file].shellescape}"
        end
      ...
      def self.available_options
        [
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!(
                    "Couldn't find path '#{File.expand_path(value)}'"
                  )
                end
              end
          ),
          ...
          FastlaneCore::ConfigItem.new(
            key: :ignore_exit_status,
            description:
              "Ignore the exit status of the SwiftLint command, so that serious violations \
      ...
      def self.handle_swiftlint_error(ignore_exit_status, exit_status)
        if ignore_exit_status
          ...
          secondary_message =
            'fastlane will continue because the `ignore_exit_status` option was used! 🙈'
        else
          failure_suffix = 'which represents a failure.'
          secondary_message =
            'If you want fastlane to continue anyway, use the `ignore_exit_status` option. 🙈'
        end

        UI.important('')
        UI.important(
          "SwiftLint finished with exit code #{exit_status}, #{failure_suffix}"
        )
        UI.important(secondary_message)
        UI.important('')
        unless ignore_exit_status
          UI.user_error!(
            "SwiftLint finished with errors (exit code: #{exit_status})"
          )
        end
      end
    end
