    ...
    class SetBuildNumberRepositoryAction < Action
      ...
      def self.run(params)
        ...
        Fastlane::Actions::IncrementBuildNumberAction.run(
          build_number: build_number, xcodeproj: params[:xcodeproj]
        )
      end
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                ...
                unless Helper.test? || File.exist?(path)
                  UI.user_error!("Could not find Xcode project at #{path}")
                end
              end
          )
        ]
