module Fastlane
  module Actions
    ...
    class TryoutsAction < Action
      ...
      def self.run(params)
        UI.success(
          'Upload to Tryouts has been started. This may take some time.'
        )
        ...
        when 200...300
          Actions.lane_context[SharedValues::TRYOUTS_BUILD_INFORMATION] =
            response.body
          UI.success('Build successfully uploaded to Tryouts!')
          if response.body['download_url']
            UI.message("Release download url: #{response.body['download_url']}")
          end
        else
          UI.user_error!(
            "Error when trying to upload build file to Tryouts: #{response
              .body}"
          )
        end
      end
      ...
      def self.upload_build(params)
        ...
        options = {}
        options[:build] =
          Faraday::UploadIO.new(params[:build_file], 'application/octet-stream')
        ...
        [
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No application identifier for Tryouts given, pass using `app_id: 'application id'`"
                  )
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless value && !value.empty?
                  UI.user_error!(
                    "No API token for Tryouts given, pass using `api_token: 'token'`"
                  )
                end
              end
          ),
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Couldn't find build file at path '#{value}'")
                end
              end
          ),
          FastlaneCore::ConfigItem.new(
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                unless File.exist?(value)
                  UI.user_error!("Couldn't find notes file at path '#{value}'")
                end
              end,
            optional: true
          ),
          ...
          FastlaneCore::ConfigItem.new(
            ...
            verify_block:
              proc do |value|
                available_options = %w[1 2]
                unless available_options.include?(value.to_s)
                  UI.user_error!(
                    "'#{value}' is not a valid 'status' value. Available options are #{available_options
                      .join(', ')}"
                  )
                end
              end,
