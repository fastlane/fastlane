  ...
  module Actions
    ...
    class S3Action < Action
      def self.run(config)
        ...
        unless s3_access_key.to_s.length > 0
          UI.user_error!(
            "No S3 access key given, pass using `access_key: 'key'`"
          )
        end
        unless s3_secret_access_key.to_s.length > 0
          UI.user_error!(
            "No S3 secret access key given, pass using `secret_access_key: 'secret key'`"
          )
        end
        unless s3_bucket.to_s.length > 0
          UI.user_error!("No S3 bucket given, pass using `bucket: 'bucket'`")
        end
        unless ipa_file.to_s.length > 0
          UI.user_error!("No IPA file path given, pass using `ipa: 'ipa path'`")
        end
        ...
        s3_client =
          self.s3_client(s3_access_key, s3_secret_access_key, s3_region)
        bucket = s3_client.buckets[s3_bucket]

        url_part =
          self.expand_path_with_substitutions_from_ipa_plist(ipa_file, s3_path)
        ...
        if dsym_file
          ...
          dsym_url =
            self.upload_file(bucket, dsym_file_name, dsym_file_data, acl)
        ...
        return true if params[:upload_metadata] == false
        ...
        s3_domain =
          AWS::Core::Endpoints.hostname(s3_region, 's3') || 's3.amazonaws.com'
        plist_file_name ||= "#{url_part}#{URI.escape(title)}.plist"
        plist_url =
          URI::HTTPS.build(
            host: s3_domain, path: "/#{s3_bucket}/#{plist_file_name}"
          )
            .to_s
        ...
        version_url =
          self.upload_file(bucket, version_file_name, version_render, acl)
        ...
        UI.success(
          "Successfully uploaded ipa file to '#{Actions.lane_context[
      ...
      def self.v1_sdk_module_present?
        begin
          ...
          UI.verbose(
            "Couldn't find the needed `AWS` module in the 'aws-sdk' gem"
          )
          return false
        end
      ...
      def self.upload_file(bucket, file_name, file_data, acl)
        ...
        obj = obj.object if obj.kind_of?(AWS::S3::ObjectVersion)
      ...
      def self.expand_path_with_substitutions_from_ipa_plist(ipa, path)
        ...
        info = FastlaneCore::IpaFileAnalyser.fetch_info_plist_file(ipa) or
          return path
