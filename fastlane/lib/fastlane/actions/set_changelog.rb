  ...
  module Actions
    class SetChangelogAction < Action
      def self.run(params)
        ...
        app =
          Spaceship::Application.find(params[:app_identifier]) ||
            Spaceship::Application.find(params[:app_identifier], mac: true)
        if app.nil?
          UI.user_error!(
            "Couldn't find app with identifier #{params[:app_identifier]}"
          )
        end
        ...
        unless version_number
          ...
          if app.edit_version(platform: platform) &&
             app.edit_version(platform: platform).version
        ...
        if (v = app.edit_version(platform: platform))
          if v.version != version_number
            ...
            UI.message(
              "Changing existing version number from '#{v
        ...
        UI.message(
          "Found and updated changelog for the following languages: #{v
            .release_notes
            .languages
            .join(', ')}"
        )
