      ...
      class << self
        def discover_crashlytics_path(params)
          ...
          if path
            if File.basename(path) != 'submit'
              ...
              UI.verbose(
                ":crashlytics_path passed through parameters did not point to a submit binary. Using this submit binary on that path instead: '#{path}'"
              )
            else
              UI.verbose(
                "Using :crashlytics_path passed in through parameters: '#{path}'"
              )
            end
          end
          ...
          path ||=
            Dir['./Pods/iOS/Crashlytics/Crashlytics.framework/submit'].last
          path ||= Dir['./**/Crashlytics.framework/submit'].last

          downcase_path = path ? path.downcase : nil
          if downcase_path && downcase_path.include?('pods') &&
             downcase_path.include?('crashlytics.framework')
            UI.deprecated(
        ...
        def generate_ios_command(params)
          ...
          unless submit_binary
            UI.user_error!(
              "Couldn't find Crashlytics' submit binary in current directory. Make sure to add the 'Crashlytics' pod to your 'Podfile' and run `pod update`"
            )
          end
          if File.basename(submit_binary) != 'submit'
            UI.user_error!(
              "Invalid crashlytics path was detected with '#{submit_binary}'. Path must point to the `submit` binary (example: './Pods/Crashlytics/submit')"
            )
          end
          ...
          if params[:notes_path]
            command << "-notesPath '#{params[:notes_path]}'"
          end
        ...
        def generate_android_command(params, android_manifest_path)
          unless params[:crashlytics_path]
            params[:crashlytics_path] = download_android_tools
          end

          unless params[:crashlytics_path].end_with?('.jar') || Helper.test?
            UI.user_error!(
              'The `crashlytics_path` must be a jar file for Android'
            )
          end
          ...
          command <<
            "-uploadDist #{File.expand_path(params[:apk_path]).shellescape}"
          command <<
            "-androidManifest #{File.expand_path(android_manifest_path)
              .shellescape}"

          # Optional
          if params[:emails]
            command << "-betaDistributionEmails #{params[:emails].shellescape}"
          end
          if params[:notes_path]
            command <<
              "-betaDistributionReleaseNotesFilePath #{File.expand_path(
                params[:notes_path]
              )
                .shellescape}"
          end
          if params[:groups]
            command <<
              "-betaDistributionGroupAliases #{params[:groups].shellescape}"
          end
          command <<
            "-betaDistributionNotifications #{(
              params[:notifications] ? 'true' : 'false'
            )}"
        ...
        def download_android_tools
          containing =
            File.join(File.expand_path('~/Library'), 'CrashlyticsAndroid')
          ...
          url =
            'https://ssl-download-crashlytics-com.s3.amazonaws.com/android/ant/crashlytics.zip'
          require 'net/http'
          ...
          begin
            UI.important(
              'Downloading Crashlytics Support Library - this might take a minute...'
            )
            ...
            unless result.kind_of?(Net::HTTPSuccess)
              UI.error!("#{result.message} (#{result.code})")
            end
            ...
            unless File.exist?(jar_path)
              UI.user_error!("Couldn't find 'crashlytics-devtools.jar'")
            end

            UI.success(
              "Successfully downloaded Crashlytics Support Library to '#{jar_path}'"
            )
          rescue => ex
