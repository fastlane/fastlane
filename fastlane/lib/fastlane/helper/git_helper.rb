module Fastlane
  module Actions
    GIT_MERGE_COMMIT_FILTERING_OPTIONS = %i[
      include_merges
      exclude_merges
      only_include_merges
    ].freeze

    def self.git_log_between(
      pretty_format,
      from,
      to,
    ...
    def self.last_git_tag_name(
      match_lightweight = true, tag_match_pattern = nil
    )
      tag_pattern_param =
        tag_match_pattern ? "=#{tag_match_pattern.shellescape}" : ''
    ...
    def self.git_author
      UI.deprecated(
        '`git_author` is deprecated. Please use `git_author_email` instead.'
      )
      git_author_email
    end
    ...
    def self.last_git_commit
      UI.important(
        '`last_git_commit` is deprecated. Please use `last_git_commit_message` instead.'
      )
