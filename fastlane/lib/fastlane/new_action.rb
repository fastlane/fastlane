module Fastlane
  ...
  module NewAction
    def self.run(new_action_name: nil)
      name =
        if new_action_name && check_action_name_from_args(new_action_name)
          new_action_name
        else
          fetch_name
        end
    ...
    def self.fetch_name
      puts(
        "Must be lower case, and use a '_' between words. Do not use '.'".green
      )
      puts("examples: 'testflight', 'upload_to_s3'".green)
      name = UI.input('Name of your action: ')
      until name_valid?(name)
        puts(
          'Name is invalid. Please ensure the name is all lowercase, free of spaces and without special characters! Try again.'
        )
        name = UI.input('Name of your action: ')
      end
      name
