...
module Fastlane
  class SwiftLaneManager < LaneManagerBase
    ...
    def self.cruise_lane(
      ...
      parameters = nil,
      env = nil,
      disable_runner_upgrades: false,
      swift_server_port: nil
    )
      ...
      begin
        ...
        if disable_runner_upgrades
          UI.verbose(
            'disable_runner_upgrades is true, not attempting to update the FastlaneRunner project'
              .yellow
          )
        elsif Helper.ci?
          UI.verbose(
            'Running in CI, not attempting to update the FastlaneRunner project'
              .yellow
          )
        else
      ...
      skip_message = true if exit_reason == :cancelled && e.nil?
      ...
      if display_upgraded_message
        UI.message(
          'We updated your FastlaneRunner project during this run to make it compatible with your current version of fastlane.'
            .yellow
        )
        UI.message(
          'Please make sure to check the changes into source control.'.yellow
        )
      end
    end
    ...
    def self.cruise_swift_lane_in_thread(
      lane, parameters = nil, swift_server_port
    )
      parameters = {} if parameters.nil?

      parameter_string = ''
      parameters.each { |key, value| parameter_string += " #{key} #{value}" }

      parameter_string += ' logMode verbose' if FastlaneCore::Globals.verbose?
      ...
      return Thread.new do
        Actions.sh(
          "#{FastlaneCore::FastlaneFolder
            .swift_runner_path} lane #{lane}#{parameter_string} > /dev/null"
        )
      end
    end
    ...
    def self.swap_paths_in_target(
      target: nil,
      file_refs_to_swap: nil,
      expected_path_to_replacement_path_tuples: nil
    )
      made_project_updates = false
      file_refs_to_swap.each do |file_ref|
        expected_path_to_replacement_path_tuples
          .each do |preinstalled_config_relative_path, user_config_relative_path|
    ...
    def self.collect_tool_paths_for_replacement(
      all_user_tool_file_paths: nil, look_for_new_configs: nil
    )
      new_user_tool_file_paths =
        all_user_tool_file_paths
          .select do |user_config, preinstalled_config_relative_path, user_config_relative_path|
      ...
      new_user_tool_file_paths =
        new_user_tool_file_paths
          .map do |user_config, preinstalled_config_relative_path, user_config_relative_path|
          if look_for_new_configs
    ...
    def self.target_for_fastlane_runner_project(runner_project: nil)
      fastlane_runner_array =
        runner_project.targets.select do |target|
    ...
    def self.first_time_setup
      setup_message = [
        'fastlane is now configured to use a swift-based Fastfile (Fastfile.swift) 🦅'
      ]
      setup_message <<
        "To edit your new Fastfile.swift, type: `open #{FastlaneCore::FastlaneFolder
          .swift_runner_project_path}`"

      # Go through and link up whatever we generated during `fastlane init swift` so the user can edit them easily
      self.link_user_configs_to_project(
        updated_message: setup_message.join("\n")
      )
    end
    ...
    def self.link_user_configs_to_project(updated_message: nil)
      ...
      all_user_tool_file_paths =
        TOOL_CONFIG_FILES.map do |tool_name|
          [
      ...
      new_user_tool_file_paths =
        collect_tool_paths_for_replacement(
          all_user_tool_file_paths: all_user_tool_file_paths,
          look_for_new_configs: true
        )

      # Tool files we provide AND the user doesn't provide
      user_tool_files_possibly_removed =
        collect_tool_paths_for_replacement(
          all_user_tool_file_paths: all_user_tool_file_paths,
          look_for_new_configs: false
        )

      fastlane_runner_project = self.runner_project
      runner_target =
        target_for_fastlane_runner_project(
          runner_project: fastlane_runner_project
        )
      target_file_refs = target_source_file_refs(target: runner_target)

      # Swap in all new user supplied configs into the project
      project_modified =
        swap_paths_in_target(
      ...
      if project_modified
        ...
        updated_message ||=
          "Updated #{FastlaneCore::FastlaneFolder.swift_runner_project_path}"
    ...
    def self.start_socket_thread(port: nil)
      ...
      return Thread.new do
        ...
        server =
          Fastlane::SocketServer.new(
            command_executor: command_executor, port: port
          )
        server.start
      end
    end
    ...
    def self.ensure_runner_built!
      ...
      if FastlaneCore::FastlaneFolder.swift_runner_built?
        runner_last_modified_age =
          File.mtime(FastlaneCore::FastlaneFolder.swift_runner_path).to_i
        fastfile_last_modified_age =
          File.mtime(FastlaneCore::FastlaneFolder.fastfile_path).to_i

        if runner_last_modified_age < fastfile_last_modified_age
          # It's older than the Fastfile, so build it again
          UI.verbose(
            "Found changes to user's Fastfile.swift, setting re-build runner flag"
          )
      ...
      self.build_runner! if runner_needs_building
    end
    ...
    def self.ensure_runner_up_to_date_fastlane!
      ...
      if upgrade_needed
        ...
        user_wants_upgrade =
          UI.confirm(
            'Should we try to upgrade just your `FastlaneSwiftRunner` project?'
          )

        unless user_wants_upgrade
          UI.important(
            "Ok, if things break, you can try to run this lane again and you'll be prompted to upgrade another time"
          )
        end
        ...
        if user_wants_upgrade
          ...
          if upgraded
            UI.success(
              'Updated your FastlaneSwiftRunner project with the newest runner code'
            )
          end
