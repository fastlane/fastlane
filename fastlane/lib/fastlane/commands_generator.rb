...
module Fastlane
  class CommandsGenerator
    ...
    def self.start
      ...
      self.confirm_troubleshoot if ARGV.include?('--troubleshoot')
      ...
      unless ARGV[0] == 'env' || CLIToolsDistributor.running_version_command? ||
             CLIToolsDistributor.running_help_command?
      ...
      if FastlaneCore::Globals.capture_output?
        if $stdout.respond_to?(:string)
          ...
          FastlaneCore::Globals.captured_output =
            Helper.strip_ansi_colors($stdout.string)
        end
        $stdout = STDOUT
        $stderr = STDERR
    ...
    def run
      ...
      command :trigger do |c|
        ...
        c.option(
          '--env STRING[,STRING2]',
          String,
          'Add environment(s) to use with `dotenv`'
        )
        c.option(
        ...
        c.option(
          '--swift_server_port INT',
          'Set specific port to communicate between fastlane and FastlaneRunner'
        )

        c.action do |args, options|
          Fastlane::CommandLineHandler.handle(args, options) if ensure_fastfile
        end
      end
      ...
      command :init do |c|
        ...
        c.option(
          ...
          '--user STRING',
          String,
          'iOS projects only: Your Apple ID'
        )

        c.action do |args, options|
          is_swift_fastfile = args.include?('swift')
          Fastlane::Setup.start(
            user: options.user, is_swift_fastfile: is_swift_fastfile
          )
        end
      end
      ...
      command :socket_server do |c|
        ...
        c.option(
          '-c seconds',
          '--connection_timeout',
          'Sets connection established timeout'
        )
        c.action do |args, options|
          ...
          if stay_alive && options.connection_timeout.nil?
            UI.important(
              "stay_alive is set, but the connection timeout is not, this will give you #{default_connection_timeout} seconds to (re)connect"
            )
          end
      ...
      command :list do |c|
        ...
        c.action do |args, options|
          if ensure_fastfile
            ff =
              Fastlane::FastFile.new(FastlaneCore::FastlaneFolder.fastfile_path)
            UI.message('Available lanes:')
            ff.runner.available_lanes.each { |lane| UI.message("- #{lane}") }
          ...
          if ensure_fastfile
            ff =
              Fastlane::FastFile.new(
                File.join(FastlaneCore::FastlaneFolder.path || '.', 'Fastfile')
              )
            UI.message(
              "You don't need to run `fastlane docs` manually any more, this will be done automatically for you when running a lane."
            )
      ...
      command :actions do |c|
        ...
        c.option(
          '--platform STRING',
          String,
          'Only show actions available on the given platform'
        )

        c.action do |args, options|
          require 'fastlane/documentation/actions_list'
          Fastlane::ActionsList.run(
            filter: args.first, platform: options.platform
          )
        end
      end
      ...
      command :new_plugin do |c|
        ...
        c.action { |args, options| PluginGenerator.new.generate(args.shift) }
      end
      ...
      command :add_plugin do |c|
        ...
        c.action do |args, options|
          ...
          UI.important(
            "Make sure to commit your Gemfile, Gemfile.lock and #{PluginManager::PLUGINFILE_NAME} to version control"
          )
    ...
    def ensure_fastfile
      ...
      create =
        UI.confirm(
          'Could not find fastlane in current directory. Make sure to have your fastlane configuration files inside a folder called "fastlane". Would you like to set fastlane up?'
        )
      Fastlane::Setup.start if create
      return false
    end
  end
end
