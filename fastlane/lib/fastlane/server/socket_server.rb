...
module Fastlane
  class SocketServer
    ...
    def receive_and_process_commands
      loop do
        # no idea how many commands are coming, so we loop until an error or the done command is sent
        ...
        if execution_state == COMMAND_EXECUTION_STATE[:ready]
          ...
          execution_state =
            parse_and_execute_command(command_string: command_string)
        end
    ...
    def handle_control_command(command)
      ...
      UI.important(command.user_message) if command.user_message
    ...
    def listen
      ...
      UI.verbose(
        "Waiting for #{@connection_timeout} seconds for a connection from FastlaneRunner"
      )
      ...
      @client = nil
      begin
        Timeout.timeout(@connection_timeout) { @client = @server.accept } # Wait for a client to connect
      rescue Timeout::Error
        UI.user_error!(
          "fastlane failed to receive a connection from the FastlaneRunner binary after #{@connection_timeout} seconds, shutting down"
        )
      rescue StandardError => e
        UI.user_error!(
          "Something went wrong while waiting for a connection from the FastlaneRunner binary, shutting down\n#{e}"
        )
      end
    ...
    def handle_disconnect(error: false, exit_reason: :error)
      ...
      if exit_reason == :error
        UI.important(
          'Client disconnected, a pipe broke, or received malformed data'
        )
      end
      # clean up
      @client.close
    ...
    def execute_action_command(command: nil)
      command_return =
        @command_executor.execute(command: command, target_object: nil)
      ...
      return_object =
        return_value_processor.prepare_object(
          return_value: return_object, return_value_type: return_value_type
        )
      ...
      if closure_arg.nil?
        ...
        closure_arg =
          return_value_processor.prepare_object(
            return_value: closure_arg, return_value_type: :string # always assume string for closure error_callback
          )
      end
      ...
      payload = {
        payload: {
          status: 'failure', failure_information: exception_array.flatten
        }
      }
      return JSON.generate(payload)
    end
