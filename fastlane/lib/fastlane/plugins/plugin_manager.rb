...
module Fastlane
  class PluginManager
    ...
    def pluginfile_content
      if pluginfile_path && File.exist?(pluginfile_path)
        File.read(pluginfile_path)
      end
    end
    ...
    def self.to_gem_name(plugin_name)
      if plugin_name.start_with?(plugin_prefix)
        plugin_name
      else
    ...
    def plugin_is_added_as_dependency?(plugin_name)
      unless plugin_name.start_with?(self.class.plugin_prefix)
        UI.user_error!(
          "fastlane plugins must start with '#{self.class
    ...
    def add_dependency(plugin_name)
      unless pluginfile_path
        UI.user_error!(
          'fastlane is not setup for this project, make sure you have a fastlane folder'
        )
      end
      unless plugin_name.start_with?(self.class.plugin_prefix)
        plugin_name = self.class.plugin_prefix + plugin_name
      end
    ...
    def gem_dependency_suffix(plugin_name)
      ...
      selection_rubygems =
        "RubyGems.org ('#{plugin_name}' seems to not be available there)"
      ...
      if selection == selection_git_url
        git_url =
          UI.input(
            'Please enter the URL to the plugin, including the protocol (e.g. https:// or git://)'
          )
        return ", git: '#{git_url}'"
      elsif selection == selection_path
        path =
          UI.input(
            'Please enter the relative path to the plugin you want to use. It has to point to the directory containing the .gemspec file'
          )
        return ", path: '#{path}'"
        ...
        source_url =
          UI.input(
            'Please enter the gem source URL which hosts the plugin you want to use, including the protocol (e.g. https:// or git://)'
          )
    ...
    def update_dependencies!
      ...
      UI.user_error!('No plugins are installed') if plugins.empty?
      with_clean_bundler_env do
        cmd = 'bundle update'
        cmd << " #{plugins.join(' ')}"
        cmd << ' --quiet' unless FastlaneCore::Globals.verbose?
    ...
    def with_clean_bundler_env
      ...
      Bundler.with_clean_env { yield if block_given? }
    end
    ...
    def setup
      ...
      if gemfile_content.to_s.length > 0
        UI.important(
          "fastlane will modify your existing Gemfile at path '#{path_to_gemfile}'"
        )
      else
        UI.important(
          "fastlane will create a new Gemfile at path '#{path_to_gemfile}'"
        )
      end
      ...
      unless UI.confirm(
             "Should fastlane modify the Gemfile at path '#{path_to_gemfile}' for you?"
           )
        ...
        UI.user_error!(
          "Please update '#{path_to_gemfile} and run fastlane again"
        )
      end
    ...
    def load_plugins
      ...
      available_plugins.each do |gem_name|
        ...
        begin
          ...
          self.plugin_references[gem_name] = {
            version_number:
              Fastlane::ActionCollector.determine_version(gem_name),
            actions: []
          }
        end
      end
      ...
      if !loaded_plugins &&
         self.pluginfile_content.to_s.include?(PluginManager.plugin_prefix)
        UI.error(
          "It seems like you wanted to load some plugins, however they couldn't be loaded"
        )
        UI.error(
          "Please follow the troubleshooting guide: #{TROUBLESHOOTING_URL}"
        )
      end

      skip_print_plugin_info =
        self.plugin_references.empty? ||
          CLIToolsDistributor.running_version_command? ||
          FastlaneCore::Env.truthy?('FASTLANE_ENV_PRINTER')

      # We want to avoid printing output other than the version number if we are running `fastlane -v`
      unless skip_print_plugin_info
        print_plugin_information(self.plugin_references)
      end
    end
    ...
    def print_plugin_information(references)
      rows =
        references.collect do |current|
          if current[1][:actions].empty?
            # Something is wrong with this plugin, no available actions
            [
              current[0].red,
              current[1][:version_number],
              'No actions found'.red
            ]
          else
            [
              current[0],
              current[1][:version_number],
              current[1][:actions].join("\n")
            ]
          end
        end
        ...
        Terminal::Table.new(
          {
            ...
            headings: %w[Plugin Version Action]
          }
        )
      )
      puts('')
    ...
    def store_plugin_reference(gem_name)
      module_name =
        gem_name.gsub(PluginManager.plugin_prefix, '').fastlane_class
      ...
      references =
        Fastlane.const_get(module_name).all_classes.collect do |path|
      ...
      references.each do |current_ref|
        ...
        class_name =
          (current_ref.to_s.fastlane_class + 'Action').to_sym

        if self.loaded_fastlane_actions.include?(class_name)
          UI.important(
            "Plugin '#{module_name}' overwrites already loaded action '#{current_ref}'"
          )
        end
      end

      self.plugin_references[gem_name] = {
        version_number: version_number, actions: references
      }
    end
  end
end
