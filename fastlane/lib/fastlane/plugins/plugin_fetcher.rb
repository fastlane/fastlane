module Fastlane
  ...
  class PluginFetcher
    ...
    def self.fetch_gems(search_query: nil)
      ...
      loop do
        url =
          "https://rubygems.org/api/v1/search.json?query=#{PluginManager
            .plugin_prefix}&page=#{page}"
        FastlaneCore::UI.verbose("RubyGems API Request: #{url}")
        results = JSON.parse(open(url).read)
        break if results.count == 0

        plugins += results.collect { |current| FastlanePlugin.new(current) }
        page += 1
      end

      return plugins if search_query.to_s.length == 0
      plugins.keep_if do |current|
        current.full_name.include?(search_query) or
          current.info.include?(search_query)
      end
