...
/**
 ...
 - parameters:
   ...
   - apiToken: App Store Connect API Token; it is hash generated by app_store_connect_api_token action
...
public func appStoreBuildNumber(apiKeyPath: OptionalConfigValue<String?> = .fastlaneDefault(nil),
                                ...
                                apiToken: OptionalConfigValue<[String: Any]?> = .fastlaneDefault(nil),
...
/**
 ...
 - parameters:
   - tokenText: The App Store Connect API JWT token text
   - tokenFilepath: The path to JWT token file
   - inHouse: Is App Store or Enterprise (in house) team? App Store Connect API cannot determine this on its own (yet)
   - setSpaceshipToken: Authorizes all Spaceship::ConnectAPI requests by automatically setting Spaceship::ConnectAPI.token

 Load the App Store Connect API token to use in other fastlane tools and actions
 */
public func appStoreConnectApiToken(tokenText: OptionalConfigValue<String?> = .fastlaneDefault(nil),
                                    tokenFilepath: OptionalConfigValue<String?> = .fastlaneDefault(nil),
                                    inHouse: OptionalConfigValue<Bool> = .fastlaneDefault(false),
                                    setSpaceshipToken: OptionalConfigValue<Bool> = .fastlaneDefault(true))
{
    let tokenTextArg = tokenText.asRubyArgument(name: "token_text", type: nil)
    let tokenFilepathArg = tokenFilepath.asRubyArgument(name: "token_filepath", type: nil)
    let inHouseArg = inHouse.asRubyArgument(name: "in_house", type: nil)
    let setSpaceshipTokenArg = setSpaceshipToken.asRubyArgument(name: "set_spaceship_token", type: nil)
    let array: [RubyCommand.Argument?] = [tokenTextArg,
                                          tokenFilepathArg,
                                          inHouseArg,
                                          setSpaceshipTokenArg]
    let args: [RubyCommand.Argument] = array
        .filter { $0?.value != nil }
        .compactMap { $0 }
    let command = RubyCommand(commandID: "", methodName: "app_store_connect_api_token", className: nil, args: args)
    _ = runner.executeCommand(command)
}
...
public func deliver(apiKeyPath: OptionalConfigValue<String?> = .fastlaneDefault(deliverfile.apiKeyPath),
                    ...
                    apiToken: OptionalConfigValue<[String: Any]?> = .fastlaneDefault(deliverfile.apiToken),
...
@discardableResult public func precheck(apiKeyPath: OptionalConfigValue<String?> = .fastlaneDefault(precheckfile.apiKeyPath),
                                        ...
                                        apiToken: OptionalConfigValue<[String: Any]?> = .fastlaneDefault(precheckfile.apiToken),
