...
describe Fastlane do
  describe Fastlane::EnvironmentPrinter do
    ...
    it 'anonymizes a path containing the user’s home' do
      expect(
        Fastlane::EnvironmentPrinter.anonymized_path(
          '/Users/john/.fastlane/bin/bundle/bin/fastlane',
          '/Users/john'
        )
      ).to eq('~/.fastlane/bin/bundle/bin/fastlane')
      expect(
        Fastlane::EnvironmentPrinter.anonymized_path(
          '/Users/john',
          '/Users/john'
      ...
      ).to eq('/workspace/project/test')
    end

    context 'FastlaneCore::Helper.xcode_version cannot be obtained' do
      before do
        allow(FastlaneCore::Helper).to receive(:xcode_version).and_raise(
                   'Boom!'
                 )
      end

      it 'contains stack information other than Xcode Version',
