  ...
  describe Fastlane::Action do
    describe '#action_name' do
      it 'converts the :: format to a readable one' do
        ...
        expect(Fastlane::Actions::IncrementBuildNumberAction.action_name).to eq(
    ...
    describe 'Call another action from an action' do
      it 'allows the user to call it using `other_action.rocket`' do
        ...
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileActionFromAction'
          )
        Fastlane::Actions.executed_actions.clear

        response = { rocket: '🚀', pwd: Dir.pwd }
        expect(ff.runner.execute(:something, :ios)).to eq(response)
        expect(Fastlane::Actions.executed_actions.map { |a| a[:name] }).to eq(
                    %w[from]
                  )
      end
      ...
      it 'shows only actions called from Fastfile' do
        ...
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileActionFromActionWithOtherAction'
          )
        Fastlane::Actions.executed_actions.clear
      ...
      it 'shows an appropriate error message when trying to directly call an action' do
        ...
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileActionFromActionInvalid'
          )
        expect { ff.runner.execute(:something, :ios) }.to raise_error(
                    'To call another action from an action use `other_action.rocket` instead'
                  )
      end
    end
    ...
    describe 'Action.sh' do
      it 'delegates to Actions.sh_control_output' do
        ...
        expect(Fastlane::Actions).to receive(:sh_control_output).with(
                    'ls',
                    '-la'
                  )
                    .and_yield(mock_status, 'Command output')
