...
describe Fastlane do
  describe Fastlane::LaneList do
    ...
    it '#generate_json' do
      result =
        Fastlane::LaneList.generate_json(
          './fastlane/spec/fixtures/fastfiles/FastfileGrouped'
        )
      expect(result).to eq(
                  {
                    ...
                    ios: {
                      beta: {
                        description: 'Submit a new version to the App Store'
                      },
                      release: { description: '' }
                    },
                    android: {
    ...
    describe '#lane_name_from_swift_line' do
      ...
      let(:testLaneWithOptions) do
        'func testLane(withOptions: [String: String]?) {'
      end
      let(:testNoLane) { 'func test() {' }
      let(:testNoLaneWithOptions) do
        'func test(withOptions: [String: String]?) {'
      end

      it 'finds lane name without options' do
        name =
          Fastlane::LaneList.lane_name_from_swift_line(
            potential_lane_line: testLane
          )
      ...
      it 'finds lane name with options' do
        name =
          Fastlane::LaneList.lane_name_from_swift_line(
            potential_lane_line: testLaneWithOptions
          )
        expect(name).to eq('testLane')
      end
      ...
      it "doesn't find lane name without options" do
        name =
          Fastlane::LaneList.lane_name_from_swift_line(
            potential_lane_line: testNoLane
          )
      ...
      it "doesn't find lane name with options" do
        name =
          Fastlane::LaneList.lane_name_from_swift_line(
            potential_lane_line: testNoLaneWithOptions
          )
        expect(name).to eq(nil)
      end
    end
