  ...
  describe Fastlane::LaneManager do
    describe '#init' do
      it 'raises an error on invalid platform' do
        expect { Fastlane::LaneManager.cruise_lane(123, nil) }.to raise_error(
                    'platform must be a string'
                  )
      end
      it 'raises an error on invalid lane' do
        expect { Fastlane::LaneManager.cruise_lane(nil, 123) }.to raise_error(
                    'lane must be a string'
                  )
      end

      describe 'dotenv' do
        it 'Finds the dotenv in the parent' do
          ensure_dot_env_value_from_fastlane_or_parent(
            'withFastfiles/parentonly',
            nil,
            { DOTENV: 'parent' }
          )
          ensure_dot_env_value_from_parent_only(
            'withoutFastfiles/parentonly',
            nil,
            { DOTENV: 'parent' }
          )
        end
        ...
        it 'Finds the dotenv in the fastlane dir' do
          ensure_dot_env_value_from_fastlane_or_parent(
            'withFastfiles/fastlaneonly',
            nil,
            { DOTENV: 'fastlane' }
          )
          ensure_dot_env_value_from_parent_only(
            'withoutFastfiles/fastlaneonly',
            nil,
            { DOTENV: 'fastlane' }
          )
        end
        ...
        it 'Finds the dotenv in the fastlane dir when in both parent and fastlane' do
          ensure_dot_env_value_from_fastlane_or_parent(
            'withFastfiles/parentandfastlane',
            nil,
            { DOTENV: 'fastlane' }
          )
          ensure_dot_env_value_from_parent_only(
            'withoutFastfiles/parentandfastlane',
            nil,
            { DOTENV: 'fastlane' }
          )
        end
        ...
        def ensure_dot_env_value_from_fastlane_or_parent(
          parent_dir, envs, expected_values
        )
          project_dir =
            File.absolute_path('./fastlane/spec/fixtures/dotenvs/' + parent_dir)
          fastlane_dir = File.absolute_path(project_dir + '/fastlane')
          ensure_dot_env_value_from_folders(
            [project_dir, fastlane_dir],
            envs,
            expected_values
          )
        end
        ...
        def ensure_dot_env_value_from_parent_only(
          parent_dir, envs, expected_values
        )
          project_dir =
            File.absolute_path('./fastlane/spec/fixtures/dotenvs/' + parent_dir)
        ...
        def ensure_dot_env_value_from_folders(folders, envs, expected_values)
          folders.each do |dir|
            expected_values.each { |k, v| ENV.delete(k.to_s) }
            Dir.chdir(dir) do
              ff = Fastlane::Helper::DotenvHelper.load_dot_env(envs)
              expected_values.each { |k, v| expect(ENV[k.to_s]).to eq(v) }
            end
          end
        end
      end
      ...
      describe 'successful init' do
        before do
          allow(FastlaneCore::FastlaneFolder).to receive(:path).and_return(
                     File.absolute_path('./fastlane/spec/fixtures/fastfiles/')
                   )
        end
        ...
        it 'Uses the default platform if given' do
          ...
          expect(lanes[:ios][:crashy].description).to eq(
                      ['This action does nothing', 'but crash']
                    )
        ...
        it 'Supports running a lane with custom Fastfile path' do
          ...
          lanes = ff.runner.lanes
          expect(lanes[nil][:test].description).to eq(
                      ['test description for cruise lanes']
                    )
