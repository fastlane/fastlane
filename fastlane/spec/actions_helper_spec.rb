describe Fastlane do
  describe Fastlane::FastFile do
    ...
    it '#action_class_ref' do
      expect(Fastlane::Actions.action_class_ref('gym')).to eq(
                  Fastlane::Actions::GymAction
                )
      expect(Fastlane::Actions.action_class_ref(:cocoapods)).to eq(
    ...
    describe '#load_external_actions' do
      it 'can load custom paths' do
        Fastlane::Actions.load_external_actions(
          './fastlane/spec/fixtures/actions'
        )
      ...
      it 'throws an error if plugin is damaged' do
        expect(UI).to receive(:user_error!).with(
                    "Action 'broken_action' is damaged!",
                    { show_github_issues: true }
                  )
        Fastlane::Actions.load_external_actions(
          './fastlane/spec/fixtures/broken_actions'
        )
      end
      ...
      it 'throws errors when syntax is incorrect' do
        ...
        expect(UI).to receive(:user_error!).with(
                    'Syntax error in broken_file.rb'
                  )
        Fastlane::Actions.load_external_actions(
          './fastlane/spec/fixtures/broken_files'
        )
      end
    end
    ...
    describe '#deprecated_actions' do
      it 'is class action' do
        ...
        # An action
        example_action_ref =
          Fastlane::Actions.action_class_ref('example_action')
        expect(Fastlane::Actions.is_class_action?(example_action_ref)).to eq(
                    true
                  )

        # Not an action
        broken_action_ref = Fastlane::Actions::BrokenAction
        expect(Fastlane::Actions.is_class_action?(broken_action_ref)).to eq(
                    false
                  )
      ...
      it 'is action deprecated' do
        ...
        example_action_ref =
          Fastlane::Actions.action_class_ref('example_action')
        expect(Fastlane::Actions.is_deprecated?(example_action_ref)).to eq(
                    false
                  )

        # An action (depreated)
        deprecated_action_ref =
          Fastlane::Actions.action_class_ref('deprecated_action')
        expect(Fastlane::Actions.is_deprecated?(deprecated_action_ref)).to eq(
                    true
                  )
