describe Fastlane do
  describe Fastlane::FastFile do
    describe 'SwiftLint' do
      ...
      before :each do
        allow(Fastlane::Actions::SwiftlintAction).to receive(:swiftlint_version)
                   .and_return(swiftlint_gem_version)
      end
      ...
      context 'when specify strict option' do
        ...
        it 'adds strict option for custom executable' do
          ...
          allow(Fastlane::Actions::SwiftlintAction).to receive(
                     :swiftlint_version
                   ) do |params|
      ...
      context 'when specify output_file, config_file and strict options' do
        it 'adds config option, strict option and redirect file' do
          ...
          expect(result).to eq(
                      "swiftlint lint --strict --config #{config_file} > #{output_file}"
                    )
        end
      end
      ...
      context 'when specify list of files to process' do
        it 'adds use script input files option and environment variables' do
          ...
          expect(result).to eq(
                      'SCRIPT_INPUT_FILE_COUNT=3 SCRIPT_INPUT_FILE_0=AppDelegate.swift SCRIPT_INPUT_FILE_1=path/to/project/src/Model.swift SCRIPT_INPUT_FILE_2=path/to/project/test/Test.swift swiftlint lint --use-script-input-files'
                    )
        end
      end
      ...
      context 'when file paths contain spaces' do
        it 'escapes spaces in output and config file paths' do
          ...
          expect(result).to eq(
                      "swiftlint lint --config #{config_file
                        .shellescape} > #{output_file.shellescape}"
                    )
        end
        ...
        it 'escapes spaces in list of files to process' do
          ...
          expect(result).to eq(
                      "SCRIPT_INPUT_FILE_COUNT=1 SCRIPT_INPUT_FILE_0=#{file
                        .shellescape} swiftlint lint --use-script-input-files"
                    )
        end
      end
      ...
      context 'when specify quiet option' do
        ...
        it 'omits the switch if swiftlint version is too low' do
          allow(Fastlane::Actions::SwiftlintAction).to receive(
                     ...
                     .and_return(Gem::Version.new('0.8.0'))
