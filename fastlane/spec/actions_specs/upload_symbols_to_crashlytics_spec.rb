    ...
    describe 'upload_symbols_to_crashlytics' do
      ...
      it 'extracts zip files' do
        ...
        expect(Fastlane::Actions).to receive(:sh).with(
                    "unzip -qo #{File.expand_path(dsym_path).shellescape}"
                  )
      ...
      it 'uploads dSYM files with api_token' do
        ...
        command = []
        command <<
          File.expand_path(File.join('fastlane', binary_path)).shellescape
        command << '-a something123'
        command << '-p ios'
        command <<
          File.expand_path(File.join('fastlane', dsym_path)).shellescape
      ...
      it 'uploads dSYM files with gsp_path' do
        ...
        command <<
          File.expand_path(File.join('fastlane', binary_path)).shellescape
        command <<
          "-gsp #{File.expand_path(File.join('fastlane', gsp_path))
            .shellescape}"
        command << '-p ios'
        command <<
          File.expand_path(File.join('fastlane', dsym_path)).shellescape
      ...
      it 'raises exception if no api access is given' do
        allow(Fastlane::Actions::UploadSymbolsToCrashlyticsAction).to receive(
                   :find_gsp_path
                 )
                   .and_return(nil)
      ...
      context 'with dsym_paths' do
        before :each do
          ...
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::DSYM_PATHS
          ] =
            nil
        ...
        it 'uploads dSYM files with gsp_path' do
          ...
          command_1 =
            command +
              [File.expand_path(File.join('fastlane', dsym_1_path)).shellescape]
          command_2 =
            command +
              [File.expand_path(File.join('fastlane', dsym_2_path)).shellescape]
