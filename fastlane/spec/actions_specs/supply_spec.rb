describe Fastlane do
  describe Fastlane::FastFile do
    describe 'supply' do
      ...
      let(:apk_paths) { %w[app/my1.apk app/my2.apk] }
      let(:wrong_apk_paths) { %w[wrong.apk nope.apk] }
      let(:aab_path) { 'app/bundle.aab' }
      let(:aab_paths_unique) { %w[app/bundle1.aab] }
      let(:aab_paths_multiple) { %w[app/bundle1.aab app/bundle2.aab] }
      ...
      describe 'single APK path handling' do
        ...
        it 'ignores the lane context APK paths if the APK param is present' do
          ...
          with_action_context_values(
            Fastlane::Actions::SharedValues::GRADLE_ALL_APK_OUTPUT_PATHS =>
              wrong_apk_paths
          ) do
            Fastlane::FastFile.new.parse(
              "lane :test do
      ...
      describe 'multiple APK path handling' do
        ...
        it 'ignores the lane context APK paths if the APK paths param is present' do
          with_action_context_values(
            Fastlane::Actions::SharedValues::GRADLE_ALL_APK_OUTPUT_PATHS => %w[
              wrong.apk
              nope.apk
            ]
          ) do
      ...
      describe 'multiple AAB path handling' do
        before :each do
          allow(File).to receive(:exist?).with(aab_path).and_return(true)
          allow(File).to receive(:exist?).with(aab_paths_unique).and_return(
                     true
                   )
        ...
        it 'uses the lane context AAB paths if no other AAB info is present' do
          with_action_context_values(
            Fastlane::Actions::SharedValues::GRADLE_ALL_AAB_OUTPUT_PATHS =>
              aab_paths_unique
          ) do
            Fastlane::FastFile.new.parse(
              'lane :test do
