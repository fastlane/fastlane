describe Fastlane::Actions::EnsureXcodeVersionAction do
  describe 'matching versions' do
    ...
    let(:matching_response_extra_output) do
      "Couldn't verify that spaceship is up to date\nXcode 8.0\nBuild version 8A218a"
    end

    it 'is successful when the version matches' do
      expect(Fastlane::Actions::EnsureXcodeVersionAction).to receive(:sh)
    ...
    it 'matches even when there is extra output' do
      expect(Fastlane::Actions::EnsureXcodeVersionAction).to receive(:sh)
                  .and_return(matching_response_extra_output)
      expect(UI).to receive(:success).with(/Driving the lane/)
      expect(UI).to receive(:success).with(/Selected Xcode version is correct/)
    ...
    describe 'loads a .xcode-version file if it exists' do
      ...
      before do
        expect(Fastlane::Actions::EnsureXcodeVersionAction).to receive(:sh)
                    .and_return(matching_response)
        expect(Dir).to receive(:glob).with('.xcode-version').and_return(
                    [xcode_version_path]
                  )
      end
      ...
      it 'succeeds if the numbers match' do
        ...
        expect(File).to receive(:read).with(xcode_version_path).and_return(
                    '8.0'
                  )
