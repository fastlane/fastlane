describe Fastlane::Actions::CommitVersionBumpAction do
  ...
  describe 'settings_plists_from_param' do
    it 'returns the param in an array if a String' do
      ...
      expect(settings_plists).to eq(%w[About.plist])
    end

    it 'returns the param if an Array' do
      settings_plists =
        action.settings_plists_from_param(%w[About.plist Root.plist])
    ...
    it 'returns ["Root.plist"] for any other input' do
      ...
      expect(settings_plists).to eq(%w[Root.plist])
    end
  end
  ...
  describe 'settings_bundle_file_path' do
    it 'returns the path of a file in the settings bundle from an Xcodeproj::Project' do
      settings_bundle =
        double(
          'file',
          path: 'Settings.bundle', real_path: '/path/to/MyApp/Settings.bundle'
        )
  ...
  describe 'modified_files_relative_to_repo_root' do
    it 'returns an empty array if modified_files is nil' do
      Fastlane::Actions.lane_context[
        Fastlane::Actions::SharedValues::MODIFIED_FILES
      ] =
        nil
      expect(
        action.modified_files_relative_to_repo_root('/path/to/repo/root')
      ).to be_empty
    end
    ...
    it 'removes duplicates' do
      ...
      relative_paths =
        action.modified_files_relative_to_repo_root('/path/to/repo/root')
      expect(relative_paths).to eq(%w[relative/path])
    end
  end
  ...
  describe 'Actions::add_modified_files' do
    it 'adds an array of paths to the list, initializing to [] if nil' do
      Fastlane::Actions.lane_context[
        ...
        nil
      Fastlane::Actions.add_modified_files(
        %w[relative/path1 /path/to/repo/root/relative/path2]
      )
    ...
    it 'ignores duplicates' do
      Fastlane::Actions.lane_context[
        ...
        Set.new(%w[relative/path])
      Fastlane::Actions.add_modified_files(%w[relative/path])
      expected = Set.new(%w[relative/path])
      expect(
        Fastlane::Actions.lane_context[
          Fastlane::Actions::SharedValues::MODIFIED_FILES
        ]
  ...
  describe 'build_git_command' do
    ...
    it 'creates a commit message containing the build number if no message is provided' do
      Fastlane::Actions.lane_context[
        Fastlane::Actions::SharedValues::BUILD_NUMBER
      ] =
        '123'
    ...
    it 'appends the --no-verify if required' do
      command =
        action.build_git_command(message: 'my commit message', no_verify: true)
