describe Fastlane do
  describe Fastlane::FastFile do
    describe 'verify_build' do
      ...
      let(:not_an_ipa) do
        File.expand_path('./fastlane_core/spec/fixtures/ipas/not-an-ipa.ipa')
      end
      let(:correctly_signed_ipa) do
        File.expand_path(
          './fastlane_core/spec/fixtures/ipas/very-capable-app.ipa'
        )
      end
      let(:correctly_signed_ipa_with_spaces) do
        File.expand_path(
          './fastlane_core/spec/fixtures/ipas/very capable app.ipa'
        )
      end
      let(:correctly_signed_xcarchive) do
        File.expand_path(
          './fastlane_core/spec/fixtures/archives/very-capable-app.xcarchive'
        )
      end
      let(:correctly_signed_xcarchive_with_spaces) do
        File.expand_path(
          './fastlane_core/spec/fixtures/archives/very capable app.xcarchive'
        )
      end
      let(:correctly_signed_app) do
        File.expand_path(
          './fastlane_core/spec/fixtures/archives/very-capable-app.xcarchive/Products/Applications/very-capable-app.app'
        )
      end
      let(:correctly_signed_app_with_spaces) do
        File.expand_path(
          './fastlane_core/spec/fixtures/archives/very capable app.xcarchive/Products/Applications/very capable app.app'
        )
      end
      let(:incorrectly_signed_ipa) do
        File.expand_path(
          './fastlane_core/spec/fixtures/ipas/ContainsWatchApp.ipa'
        )
      end
      let(:ipa_with_no_app) do
        File.expand_path('./fastlane_core/spec/fixtures/ipas/no-app-bundle.ipa')
      end
      let(:simulator_app) do
        File.expand_path(
          './fastlane_core/spec/fixtures/bundles/simulator-app.app'
        )
      end
      let(:not_an_app) do
        File.expand_path('./fastlane_core/spec/fixtures/bundles/not-an-app.txt')
      end
      let(:archive_with_no_app) do
        File.expand_path(
          './fastlane_core/spec/fixtures/archives/no-app-bundle.xcarchive'
        )
      end
      ...
      before(:each) do
        Fastlane::Actions.lane_context[
          ...
          nil
        allow(FastlaneCore::UI).to receive(:success).with(
                   "Driving the lane 'test' 🚀"
                 )
      end
      ...
      if FastlaneCore::Helper.mac?
        it 'uses the ipa output path from lane context' do
          Fastlane::Actions.lane_context[
            ...
            correctly_signed_ipa

          expect(FastlaneCore::PrintTable).to receive(:print_values).with(
                      config: expected_app_info, title: expected_title
                    )
          expect(FastlaneCore::UI).to receive(:success).with(
                      'Build is verified, have a 🍪.'
                    )
        ...
        it 'raises an error if app is built for iOS simulator' do
          expect(FastlaneCore::UI).to receive(:user_error!).with(
      ...
      describe 'Conflicting options' do
        it 'raises an error if both build_path and ipa_path options are specified' do
          expect(FastlaneCore::UI).to receive(:user_error!).with(
                      "Unresolved conflict between options: 'build_path' and 'ipa_path'"
                    )
                      .and_call_original
          expect do
            Fastlane::FastFile.new.parse(
        ...
        it 'raises an error if both ipa_path and build_path options are specified' do
          expect(FastlaneCore::UI).to receive(:user_error!).with(
                      "Unresolved conflict between options: 'ipa_path' and 'build_path'"
                    )
                      .and_call_original
          expect do
            Fastlane::FastFile.new.parse(
      ...
      describe 'Missing ipa file' do
        it 'raises an error if ipa file path set via lane context does not exist' do
          ...
          expect(FastlaneCore::UI).to receive(:user_error!).with(
                      "Unable to find file '#{no_such_file}'"
                    )
                      .and_call_original
          expect do
            Fastlane::FastFile.new.parse(
