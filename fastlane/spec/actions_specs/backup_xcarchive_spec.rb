    ...
    describe 'Backup xcarchive Integration' do
      ...
      context 'plain backup' do
        ...
        it 'copy xcarchive to destination' do
          ...
          expect(
            File.exist?(File.join(destination_path, xcarchive_file))
          ).to eq(true)
          expect(
            Fastlane::Actions.lane_context[
              Fastlane::Actions::SharedValues::BACKUP_XCARCHIVE_FILE
            ]
          ).to eq(File.join(destination_path, xcarchive_file))
        end
      ...
      context 'zipped backup' do
        before :each do
          ...
          expect(Dir).to receive(:mktmpdir).with('backup_xcarchive').and_yield(
                      tmp_path
                    )
        end
        ...
        it 'make and copy zip to destination' do
          ...
          ).to eq(File.join(destination_path, zip_file))
        end
      ...
      context 'zipped backup with custom filename' do
        ...
        it 'make zip with custom filename and copy to destination' do
          allow(Fastlane::Actions).to receive(:sh).with(
                     "cd \"#{source_path}\" && zip -r -X \"#{tmp_path}/#{custom_zip_filename}.xcarchive.zip\" \"#{xcarchive_file}\" > /dev/null"
                   )
                     .and_return('')
          result =
            Fastlane::FastFile.new.parse(
