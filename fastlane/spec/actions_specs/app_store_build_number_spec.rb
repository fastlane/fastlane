  ...
  describe Fastlane::FastFile do
    describe 'app_store_build_number' do
      it 'orders versions array of integers' do
        versions = [3, 5, 1, 0, 4]
        result =
          Fastlane::Actions::AppStoreBuildNumberAction.order_versions(versions)

        expect(result).to eq(%w[0 1 3 4 5])
      end
      ...
      it 'orders versions array of integers, string integers, floats, and semantic versions string' do
        ...
        expect(result).to eq(%w[1 2.3 3 5.6 6.5.4 9 11.4.6])
      end

      it 'returns value as string (with build number as version string)' do
        allow(Fastlane::Actions::AppStoreBuildNumberAction).to receive(
                   :get_build_number
                 )
                   .and_return('1.2.3')
