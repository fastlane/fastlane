describe Fastlane do
  describe Fastlane::FastFile do
    describe 'make_jenkins_changelog' do
      before { ENV['BUILD_URL'] = 'https://jenkinsurl.com/JOB/' }

      after { ENV['BUILD_URL'] = nil }

      it "returns the fallback if it wasn't able to communicate with the server" do
        stub_request(
          :get,
          'https://jenkinsurl.com/JOB/api/json\?wrapper\=changes\&xpath\=//changeSet//comment'
        )
      ...
      it 'correctly parses the data if it was able to retrieve it and does not include the commit body' do
        stub_request(
          ...
          body:
            File.read(
              './fastlane/spec/fixtures/requests/make_jenkins_changelog.json'
            ),
          headers: {}
        )
        ...
        expect(result).to eq(
                    "Disable changelog generation from Jenkins until we sort it out\n"
                  )
      end
