    ...
    describe 'Create keychain Integration' do
      it 'works with name and password' do
        ...
        expect(result[0]).to eq(
                    'security create-keychain -p testpassword ~/Library/Keychains/test.keychain'
                  )
        ...
        expect(result[2]).to end_with(
                    File.expand_path('~/Library/Keychains/test.keychain').to_s
                  )
      end
      ...
      it 'works with name and password that contain spaces or `\"`' do
        ...
        expect(result[0]).to eq(
                    "security create-keychain -p #{password
                      .shellescape} ~/Library/Keychains/test.keychain"
                  )
      end
      ...
      it 'works with default_keychain and name and password' do
        ...
        expect(result[1]).to eq(
                    'security default-keychain -s ~/Library/Keychains/test.keychain'
                  )
      ...
      it 'works with unlock and name and password' do
        ...
        expect(result[1]).to eq(
                    'security unlock-keychain -p testpassword ~/Library/Keychains/test.keychain'
                  )
      ...
      it 'works with :path param' do
        ...
        expect(result.size).to eq(4)
        expect(result[0]).to eq(
                    'security create-keychain -p testpassword /tmp/test.keychain'
                  )

        expect(result[1]).to eq(
                    'security default-keychain -s /tmp/test.keychain'
                  )
        expect(result[2]).to eq(
                    'security unlock-keychain -p testpassword /tmp/test.keychain'
                  )
