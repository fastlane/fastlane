    ...
    describe 'Create dSYM zip' do
      ...
      before(:each) do
        ...
        Fastlane::Actions.lane_context[
          Fastlane::Actions::SharedValues::XCODEBUILD_ARCHIVE
        ] =
          xcodebuild_archive
      end
      ...
      context "when there's no custom zip path" do
        ...
        it 'creates a zip file with default archive_path from xcodebuild' do
          ...
          dsym_folder_path =
            File.join(root_path, File.join(xcodebuild_archive, 'dSYMs'))
          zipped_dsym_path =
            File.join(root_path, "#{file_basename}.app.dSYM.zip")

          expect(result).to eq(
                      "cd \"#{dsym_folder_path}\" && zip -r \"#{zipped_dsym_path}\" \"MyApp.app.dSYM\""
                    )
        end
      end
      ...
      context "when there's a custom zip path" do
        ...
        it 'creates a zip file with a custom archive path' do
          ...
          dsym_folder_path =
            File.join(root_path, File.join(custom_app_path.to_s, 'dSYMs'))
          zipped_dsym_path =
            File.join(root_path, "#{file_basename}.app.dSYM.zip")
      ...
      context "when there's a custom dsym path" do
        ...
        it 'creates a zip file with a custom dsym path' do
          ...
          zipped_dsym_path =
            File.join(
              File.join(root_path, 'CustomPath'),
              "#{file_basename}.app.dSYM.zip"
            )
