  ...
  describe Fastlane::FastFile do
    describe 'download_dsyms' do
      ...
      before do
        ...
        # trains
        allow(app).to receive(:tunes_all_build_trains).and_return(
                   [train, train2]
                 )
      ...
      context 'with no special options' do
        it 'downloads all dsyms of all builds in all trains' do
          expect(app).to receive(:tunes_all_builds_for_train).and_return(
                      [build, build2]
                    )
          expect(app).to receive(:tunes_all_builds_for_train).and_return(
                      [build, build2]
                    )
          expect(app).to receive(:tunes_build_details).with(
                      train: '1.0.0', build_number: '1', platform: :ios
                    )
                      .and_return(build_detail)
          expect(app).to receive(:tunes_build_details).with(
                      train: '1.0.0', build_number: '2', platform: :ios
                    )
                      .and_return(build_detail)
          expect(app).to receive(:tunes_build_details).with(
                      train: '2.0.0', build_number: '1', platform: :ios
                    )
                      .and_return(build_detail)
          expect(app).to receive(:tunes_build_details).with(
                      train: '2.0.0', build_number: '2', platform: :ios
                    )
                      .and_return(build_detail)
          expect(Fastlane::Actions::DownloadDsymsAction).to receive(:download)
                      .with(
                      download_url,
                      app.bundle_id,
      ...
      context 'when version is latest' do
        before do
          ...
          allow(version).to receive(:candidate_builds).and_return(
                     [build2, build3]
                   )
