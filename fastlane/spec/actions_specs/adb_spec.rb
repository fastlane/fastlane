  ...
  describe Fastlane::FastFile do
    describe 'adb' do
      ...
      it 'generates a valid command for commands with multiple parts' do
        ...
        expect(result).to eq(
                    './fastlane/README.md test command with multiple parts'
                  )
      end
      ...
      it 'generates a valid command when a non-empty serial is passed' do
        ...
        expect(result).to eq(
                    'ANDROID_SERIAL=emulator-1234 ./fastlane/README.md test command with non-empty serial'
                  )
      end
      ...
      it 'generates a valid command when an empty serial is passed' do
        ...
        expect(result).to eq(
                    './fastlane/README.md test command with empty serial'
                  )
      end
      ...
      it 'picks up path from ANDROID_HOME environment variable and handles path that has to be made safe' do
        stub_const(
          'ENV',
          { 'ANDROID_HOME' => '/usr/local/android-sdk/with space' }
        )
        result =
          Fastlane::FastFile.new.parse(
            "lane :test do
        ...
        path =
          '/usr/local/android-sdk/with space/platform-tools/adb'.shellescape
