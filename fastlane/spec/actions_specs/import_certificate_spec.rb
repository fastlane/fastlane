  ...
  describe Fastlane::FastFile do
    describe 'Import certificate Integration' do
      ...
      it 'works with certificate and password' do
        ...
        keychain_path =
          File.expand_path(File.join('~', 'Library', 'Keychains', keychain))
        expected_command =
          "security import #{cert_name} -k '#{keychain_path}' -P #{password} -T /usr/bin/codesign -T /usr/bin/security 1> /dev/null"
        ...
        allowed_command =
          "security set-key-partition-list -S apple-tool:,apple: -k #{''
            .shellescape} #{keychain_path.shellescape} 1> /dev/null"
      ...
      it "works with certificate and password that contain spaces, special chars, or ''" do
        ...
        keychain_path =
          File.expand_path(File.join('~', 'Library', 'Keychains', keychain))
        expected_security_import_command =
          "security import #{cert_name.shellescape} -k '#{keychain_path
            .shellescape}' -P #{password
            .shellescape} -T /usr/bin/codesign -T /usr/bin/security 1> /dev/null"

        # this command is also sent on macOS Sierra and we need to allow it or else the test will fail
        expected_set_key_partition_list_command =
          "security set-key-partition-list -S apple-tool:,apple: -k #{password
            .shellescape} #{keychain_path.shellescape} 1> /dev/null"
