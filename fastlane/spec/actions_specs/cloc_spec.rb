  ...
  describe Fastlane::FastFile do
    describe 'CLOC Integration' do
      it 'does run cloc using only default options' do
        ...
        expect(result).to eq(
                    '/usr/local/bin/cloc  --by-file --xml  --out=build/cloc.xml'
                  )
      end
      ...
      it 'does set the exclude directories' do
        ...
        expect(result).to eq(
                    '/usr/local/bin/cloc --exclude-dir=test1,test2,build --by-file --xml  --out=build/cloc.xml'
                  )
      end
      ...
      it 'does set the output directory' do
        ...
        expect(result).to eq(
                    '/usr/local/bin/cloc  --by-file --xml  --out=/tmp/cloc.xml'
                  )
      end
      ...
      it 'does set the source directory' do
        ...
        expect(result).to eq(
                    '/usr/local/bin/cloc  --by-file --xml  --out=build/cloc.xml MyCoolApp'
                  )
      end
      ...
      it 'does switch to plain text when xml is toggled off' do
        ...
        expect(result).to eq(
                    '/usr/local/bin/cloc  --by-file  --out=build/cloc.txt'
                  )
      end
    end
  end
