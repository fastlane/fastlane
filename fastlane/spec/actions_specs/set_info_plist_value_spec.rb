  ...
  describe Fastlane::FastFile do
    describe 'set_info_plist' do
      ...
      it 'stores changes in the output plist file' do
        ...
        Fastlane::FastFile.new.parse(
          "lane :test do
          set_info_plist_value(path: '#{plist_path}', key: 'CFBundleIdentifier', value: '#{new_value}', output_file_name:'#{File
            .join(test_path, output_path)}')
        end"
        )
          .runner
          .execute(:test)
        ...
        value =
          Fastlane::FastFile.new.parse(
            "lane :test do
          get_info_plist_value(path: '#{File.join(
              test_path,
              output_path
