describe Fastlane do
  describe Fastlane::FastFile do
    describe 'git_add' do
      before { allow(File).to receive(:exist?).with(anything).and_return(true) }
      ...
      context 'as array' do
        let(:path) { %w[myfile.txt yourfile.txt] }

        it 'executes the correct git command' do
          allow(Fastlane::Actions).to receive(:sh).with(
                     "git add #{path[0]} #{path[1]}",
                     anything
                   )
                     .and_return('')
          result =
      ...
      context 'as array with spaces in name and directory' do
        ...
        it 'executes the correct git command' do
          allow(Fastlane::Actions).to receive(:sh).with(
                     "git add #{path[0].shellescape} #{path[1].shellescape}",
                     anything
                   )
                     .and_return('')
          result =
      ...
      context 'without parameters' do
        it 'executes the correct git command' do
          allow(Fastlane::Actions).to receive(:sh).with('git add .', anything)
                     .and_return('')
          result =
            Fastlane::FastFile.new.parse(
              'lane :test do
      ...
      it 'logs the command if verbose' do
        with_verbose(true) do
          allow(Fastlane::Actions).to receive(:sh).with(anything, { log: true })
                     .and_return('')
          result =
            Fastlane::FastFile.new.parse(
              "lane :test do
