    ...
    describe 'OCLint Integration' do
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    "Could not find json compilation database at path 'compile_commands.json'"
                  )
      end
      ...
      it 'works given the path to compile_commands.json' do
        ...
        expect(result).to match(
                    %r{cd .* && oclint -report-type=html -o=oclint_report.html -p ./fastlane/spec/fixtures/oclint \".*}
                  )
      end
      ...
      it 'works with all parameters' do
        ...
        expect(result).to include(
                    ' oclint -report-type=pmd -o=report_path.xml '
                  )
        ...
        expect(result).to include(
                    ' -disable-rule GotoStatement -disable-rule ShortVariableName '
                  )
      ...
      it 'works with select regex' do
        ...
        expect(result).to include(
                    '"fastlane/spec/fixtures/oclint/src/AppDelegate.m"'
                  )
      end
      ...
      it 'works with exclude regex' do
        ...
        expect(result).not_to(
          include('"fastlane/spec/fixtures/oclint/src/Test.m"')
        )
      end
      ...
      context 'with valid path to compile_commands.json' do
        context 'with no path to oclint' do
          ...
          let(:command) do
            "cd #{File.expand_path('.')
              .shellescape} && oclint -report-type=html -o=oclint_report.html"
          end
        ...
        context 'with given path to oclint' do
          ...
          let(:command) do
            "cd #{File.expand_path('.')
              .shellescape} && test/bin/oclint -report-type=html -o=oclint_report.html"
          end
