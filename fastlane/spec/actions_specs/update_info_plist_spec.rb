      ...
      describe 'with existing xcodeproj file' do
        before do
          ...
          File.write(
            ...
            '<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd"><plist version="1.0"><dict><key>CFBundleDisplayName</key><string>empty</string><key>CFBundleIdentifier</key><string>empty</string></dict></plist>'
          )
        end
        ...
        it 'throws an error when the info plist file does not exist' do
          ...
          full_path =
            [test_path, proj_file, '..', "NOEXIST-#{plist_path}"].join(
              File::SEPARATOR
            )
          ...
          expect do
            Fastlane::FastFile.new.parse(
              ...
              .execute(:test)
                      ...
                      "Couldn't find info plist file at path '#{full_path}'"
                    )
        end
