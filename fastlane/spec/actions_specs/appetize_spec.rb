describe Fastlane do
  describe Fastlane::FastFile do
    ...
    let(:params) { { token: api_token, url: url, platform: 'ios' } }
    ...
    before do
      ...
      allow(request).to receive(:body=).with(kind_of(String)).and_return(
                 response
               )
      ...
      it 'works with valid parameters' do
        ...
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::APPETIZE_PUBLIC_KEY
          ]
        ).to eql('sKdfjL')
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::APPETIZE_APP_URL
          ]
        ).to eql('https://appetize.io/app/sKdfjL')
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::APPETIZE_MANAGE_URL
          ]
        ).to eql('https://appetize.io/manage/private_Djksfj')
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::APPETIZE_API_HOST
          ]
        ).to eql('api.appetize.io')
      end
