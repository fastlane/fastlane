  ...
  describe Fastlane::FastFile do
    describe 'Update App Identifier Integration' do
      ...
      def create_plist_with_identifier(bundle_identifier)
        File.write(
          File.join(test_path, plist_path),
          "<?xml version=\"1.0\" encoding=\"UTF-8\"?><!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\"><plist version=\"1.0\"><dict><key>CFBundleIdentifier</key><string>#{bundle_identifier}</string></dict></plist>"
        )
      end
      ...
      if FastlaneCore::Helper.mac?
        ...
        it 'updates the xcode project when product bundle identifier in use' do
          ...
          stub_object = %w[object]
          stub_settings_1 =
            Hash['PRODUCT_BUNDLE_IDENTIFIER', 'com.something.else']
          stub_settings_1['INFOPLIST_FILE'] = plist_path
          stub_settings_2 =
            Hash['PRODUCT_BUNDLE_IDENTIFIER', 'com.something.entirely.else']
          stub_settings_2['INFOPLIST_FILE'] = 'Other-Info.plist'

          expect(Xcodeproj::Project).to receive(:open).with(
                      '/tmp/fastlane/tests/fastlane/bundle.xcodeproj'
                    )
                      .and_return(stub_project)
          expect(stub_project).to receive(:objects).and_return(stub_object)
          expect(stub_object).to receive(:select).and_return(
                      [stub_configuration_1, stub_configuration_2]
                    )
          expect(stub_configuration_1).to receive(:build_settings).twice
                      .and_return(stub_settings_1)
          expect(stub_configuration_2).to receive(:build_settings).and_return(
                      stub_settings_2
                    )
          ...
          expect(stub_settings_1['PRODUCT_BUNDLE_IDENTIFIER']).to eq(
                      'com.test.plist'
                    )
          expect(stub_settings_2['PRODUCT_BUNDLE_IDENTIFIER']).to_not(
            eq('com.test.plist')
          )
        end
        ...
        it "should raise an exception when PRODUCT_BUNDLE_IDENTIFIER in info plist but project doesn't use this info plist" do
          ...
          expect(stub_configuration).to receive(:build_settings).and_return(
                      stub_settings
                    )
          ...
          expect do
            Fastlane::FastFile.new.parse(
              ...
              .execute(:test)
                      ...
                      "Xcodeproj doesn't have configuration with info plist #{plist_path}."
                    )
        end
          ...
          expect do
            ...
            update_app_identifier({
              ...
              .execute(:test)
                      ...
                      "Info plist uses $(#{identifier_key}), but xcodeproj does not"
                    )
        end
      end
