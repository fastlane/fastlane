describe Fastlane do
  describe Fastlane::FastFile do
    before do
      ...
      @output_path_with_zip =
        './fastlane/spec/fixtures/actions/archive_file.zip'
    ...
    describe 'zip' do
      it 'generates a valid zip command' do
        expect(Fastlane::Actions).to receive(:sh).with(
                    "zip -r #{File.expand_path(@path)}.zip archive.rb"
                  )
      ...
      it 'generates a valid zip command without verbose output' do
        expect(Fastlane::Actions).to receive(:sh).with(
                    "zip -rq #{File.expand_path(@path)}.zip archive.rb"
                  )
      ...
      it 'encrypts the contents of the zip archive using a password' do
        ...
        expect(Fastlane::Actions).to receive(:sh).with(
                    "zip -rq -P '#{password}' #{File.expand_path(
                      @path
                    )}.zip archive.rb"
                  )
