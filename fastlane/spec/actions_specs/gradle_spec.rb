    ...
    describe 'gradle' do
      ...
      describe 'output controls' do
        let(:expected_command) do
          "#{File.expand_path('README.md').shellescape} tasks -p ."
        end
        ...
        it "prints the command text and suppresses the command's output" do
          expect(Fastlane::Actions).to receive(:sh_control_output).with(
                      expected_command,
                      print_command: true, print_command_output: false
                    )
                      .and_call_original
      ...
      it 'generates a valid command' do
        ...
        expect(result).to eq(
                    "#{File.expand_path('README.md')
                      .shellescape} assembleWorldDominationRelease -p . -PversionCode=200"
                  )
      end
      ...
      it 'correctly escapes the gradle path' do
        ...
        expect(result).to eq(
                    "ANDROID_SERIAL=abc123 #{gradle_path
                      .shellescape} assembleWorldDominationRelease -p . -PversionCode=200"
                  )
      end
      ...
      it 'correctly escapes multiple properties and types' do
        ...
        expect(result).to eq(
                    "#{File.expand_path('README.md')
                      .shellescape} assembleWorldDominationRelease -p . -PversionCode=200 -P#{notes_key
                      .shellescape}=#{notes_result
                      .shellescape} -Dorg.gradle.daemon=true"
                  )
      end
      ...
      it 'correctly uses the serial' do
        ...
        expect(result).to eq(
                    "ANDROID_SERIAL=abc123 #{File.expand_path('README.md')
                      .shellescape} assembleWorldDominationRelease -p . -PversionCode=200"
                  )
      end
      ...
      it 'supports multiple build types' do
        ...
        expect(result).to eq(
                    "#{File.expand_path('README.md')
                      .shellescape} assembleWorldDomination -p ."
                  )
      end
      ...
      it 'supports multiple flavors and build types' do
        ...
        expect(result).to eq(
                    "#{File.expand_path('README.md').shellescape} assemble -p ."
                  )
      end
      ...
      it 'supports the backwards compatible syntax' do
        ...
        expect(result).to eq(
                    "#{File.expand_path('README.md')
                      .shellescape} assembleWorldDominationRelease -p ."
                  )
      end
    end
  end
