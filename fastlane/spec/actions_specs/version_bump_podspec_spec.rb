describe Fastlane do
  describe Fastlane::FastFile do
    describe 'version_podspec_file' do
      before { @version_podspec_file = Fastlane::Helper::PodspecHelper.new }

      it 'raises an exception when an incorrect path is given' do
        expect do
          Fastlane::Helper::PodspecHelper.new('invalid_podspec')
        end.to raise_error(
                    "Could not find podspec file at path 'invalid_podspec'"
                  )
      end

      it 'raises an exception when there is no version in podspec' do
        expect { Fastlane::Helper::PodspecHelper.new.parse('') }.to raise_error(
      ...
      it 'raises an exception when the version is commented-out in podspec' do
        ...
        expect do
          Fastlane::Helper::PodspecHelper.new.parse(test_content)
                    ...
                    "Could not find version in podspec content '#{test_content}'"
                  )
      end
      ...
      context 'when semantic version' do
        ...
        it 'appears to do nothing if reinjecting the same version number' do
          ...
          @version_podspec_file.parse(test_content)
          expect(@version_podspec_file.update_podspec).to eq(
                      'Pod::Spec.new do |s|
          s.version = "1.3.2"
        end'
                    )
        ...
        it 'allows to set a specific version' do
          ...
          @version_podspec_file.parse(test_content)
          expect(@version_podspec_file.update_podspec('2.0.0')).to eq(
                      'Pod::Spec.new do |s|
          s.version = "2.0.0"
        end'
                    )
      ...
      context 'when not semantic version' do
        ...
        it 'allows to set a specific version' do
          ...
          @version_podspec_file.parse(test_content)
          expect(@version_podspec_file.update_podspec('2.0.0.6')).to eq(
                      'Pod::Spec.new do |s|
          s.version = "2.0.0.6"
        end'
                    )
    ...
    describe 'version_bump_podspec' do
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    'Please pass a path to the `version_bump_podspec` action'
                  )
      end
      ...
      context 'when not semantic version' do
        before do
          allow(FastlaneCore::FastlaneFolder).to receive(:path).and_return(nil)
          @podspec_path =
            './fastlane/spec/fixtures/podspecs/test_not_semantic.podspec'
        end
