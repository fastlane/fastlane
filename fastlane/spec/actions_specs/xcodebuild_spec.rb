  ...
  describe Fastlane::FastFile do
    build_log_path =
      File.expand_path(
        "#{FastlaneCore::Helper.buildlog_path}/fastlane/xcbuild/#{Time.now
          .strftime('%F')}/#{Process.pid}/xcodebuild.log"
      )
    ...
    describe 'Xcodebuild Integration' do
      ...
      it 'when archiving, should cache the archive path for a later export step' do
        ...
        expect(Fastlane::Actions.lane_context[:XCODEBUILD_ARCHIVE]).to eq(
      ...
      it 'when exporting, should cache the ipa path for a later deploy step' do
        ...
        expect(Fastlane::Actions.lane_context[:IPA_OUTPUT_PATH]).to eq(
                    './build-dir/MyApp.ipa'
                  )
      end
    ...
    describe 'xcbuild without xpretty and with test and reports' do
      it "is equivalent to 'xcodebuild build'" do
        ...
        expect(result).to eq(
                    'set -o pipefail && ' + "cat '#{build_log_path}' " +
