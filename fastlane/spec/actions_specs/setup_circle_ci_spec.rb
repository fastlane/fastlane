describe Fastlane do
  describe Fastlane::Actions::SetupCircleCiAction do
    describe '#setup_output_paths' do
      before { stub_const('ENV', { 'FL_OUTPUT_DIR' => '/dev/null' }) }
    ...
    describe '#should_run' do
      context 'when running on CI' do
        before { expect(Fastlane::Helper).to receive(:ci?).and_return(true) }
      ...
      context 'when not running on CI' do
        before { expect(Fastlane::Helper).to receive(:ci?).and_return(false) }
    ...
    describe '#setup_keychain' do
      ...
      describe 'Setting up the environment' do
        before do
          stub_const('ENV', {})
          allow(Fastlane::Actions::CreateKeychainAction).to receive(:run)
                     .and_return(nil)
        end
