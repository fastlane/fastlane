  ...
  describe Fastlane::FastFile do
    describe 'Cocoapods Integration' do
      ...
      it 'error_callback is executed on failure' do
        ...
        allow(Fastlane::Actions).to receive(:sh_control_output) do
                   expect(Fastlane::Actions).to have_received(
                               :sh_control_output
                             )
                               .with(
                               kind_of(String),
      ...
      it 'retries with --repo-update on error if try_repo_update_on_error is true' do
        allow(Fastlane::Actions).to receive(
                   :sh_control_output
                 ) do |command, params|
        ...
        expect(Fastlane::Actions).to have_received(:sh_control_output).with(
                    'bundle exec pod install',
                    hash_including(print_command: true)
                  )
                    .ordered
        expect(Fastlane::Actions).to have_received(:sh_control_output).with(
      ...
      it 'doesnt retry with --repo-update if there were no errors' do
        ...
        expect(Fastlane::Actions).to have_received(:sh_control_output).with(
                    ...
                    .ordered
        expect(Fastlane::Actions).to_not(
          have_received(:sh_control_output).with(
            'bundle exec pod install --repo-update',
            hash_including(print_command: true)
          )
            .ordered
      ...
      it 'calls error_callback if retry with --repo-update finished with error' do
        ...
        allow(Fastlane::Actions).to receive(:sh_control_output) do |_, params|
                   error_callback = params[:error_callback]
                   error_callback.call('error')
                 end
        expect do
