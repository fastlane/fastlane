  ...
  describe Fastlane::FastFile do
    describe 'Nexus Upload integration' do
      it 'sets verbosity correctly' do
        expect(
          Fastlane::Actions::NexusUploadAction.verbose(verbose: true)
        ).to eq('--verbose')
        expect(
          Fastlane::Actions::NexusUploadAction.verbose(verbose: false)
        ).to eq('--silent')
      end

      it 'sets upload url correctly for Nexus 2' do
        expect(
          Fastlane::Actions::NexusUploadAction.upload_url(
        ...
        expect(
          Fastlane::Actions::NexusUploadAction.upload_url(
            endpoint: 'http://localhost:8081',
            mount_path: '/custom-nexus',
            nexus_version: 2
          )
        ).to eq(
                    'http://localhost:8081/custom-nexus/service/local/artifact/maven/content'
                  )
        expect(
          Fastlane::Actions::NexusUploadAction.upload_url(
            endpoint: 'http://localhost:8081', mount_path: '', nexus_version: 2
          )
        ).to eq('http://localhost:8081/service/local/artifact/maven/content')
      end
        ...
        expect(
          Fastlane::Actions::NexusUploadAction.upload_url(
            ...
            file: file_path.to_s
                    ...
                    'http://localhost:8081/nexus/repository/artefacts/com/fastlane/myproject/1.12/myproject-1.12.ipa'
                  )
      end
        ...
        expect(
          Fastlane::Actions::NexusUploadAction.upload_url(
            ...
            repo_classifier: 'ipa'
                    ...
                    'http://localhost:8081/nexus/repository/artefacts/com/fastlane/myproject/1.12/myproject-1.12-ipa.ipa'
                  )
      end
      ...
      it 'sets upload options correctly for Nexus 2 with all required parameters' do
        ...
        result =
          Fastlane::Actions::NexusUploadAction.upload_options(
            nexus_version: 2,
            repo_id: 'artefacts',
            repo_group_id: 'com.fastlane',
            repo_project_name: 'myproject',
      ...
      it 'sets ssl option correctly' do
        expect(
          Fastlane::Actions::NexusUploadAction.ssl_options(ssl_verify: false)
        ).to eq(%w[--insecure])
        expect(
          Fastlane::Actions::NexusUploadAction.ssl_options(ssl_verify: true)
        ).to eq([])
      end

      it 'sets proxy options correctly' do
        expect(
          Fastlane::Actions::NexusUploadAction.proxy_options(
            proxy_address: '',
            proxy_port: nil,
            proxy_username: nil,
            proxy_password: nil
        ...
        ).to eq(['-x http://1:2', '--proxy-user 3:4'])
      end
      ...
      it 'uses optional options correctly' do
        ...
        expect(result).to include('--verbose')
        expect(result).to include(
                    'http://localhost:8081/my-nexus/service/local/artifact/maven/content'
                  )
