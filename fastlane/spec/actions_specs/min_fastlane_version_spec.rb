    ...
    describe 'min_fastlane_version action' do
      ...
      it "raises an exception if it's an old version" do
        expect do
          ...
          expect(FastlaneCore::Changelog).to receive(:show_changes).with(
                      'fastlane',
                      Fastlane::VERSION,
                      update_gem_command: 'bundle update fastlane'
                    )
          Fastlane::FastFile.new.parse(
            "lane :test do
      ...
      it "raises an exception if it's an old version in a non-bundler environement" do
        expect do
          ...
          expect(FastlaneCore::Changelog).to receive(:show_changes).with(
                      'fastlane',
                      Fastlane::VERSION,
                      update_gem_command: 'sudo gem install fastlane'
                    )
          ...
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    "The Fastfile requires a fastlane version of >= 9999. You are on #{Fastlane::VERSION}."
                  )
      end
