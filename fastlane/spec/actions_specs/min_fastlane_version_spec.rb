    ...
    describe "min_fastlane_version action" do
      ...
      it "raises an exception if it's an old version" do
        expect do
          ...
          expect(FastlaneCore::Changelog).to receive(:show_changes).with("fastlane", Fastlane::VERSION, "bundle update fastlane")
          Fastlane::FastFile.new.parse("lane :test do
      ...
      it "raises an exception if it's an old version in a non-bundler environement" do
        expect do
          ...
          stub_const('ENV', {})
          expect(FastlaneCore::Changelog).to receive(:show_changes).with("fastlane", Fastlane::VERSION, "sudo gem install fastlane")
