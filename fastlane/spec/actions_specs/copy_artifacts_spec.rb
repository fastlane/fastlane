  ...
  describe Fastlane::FastFile do
    describe 'Copy Artifacts Integration' do
      ...
      it 'Copies a directory and subfiles to target path' do
        ...
        expect(File.exist?("#{@tmp_path}/target/dir/copy_artifacts")).to eq(
                    true
                  )
      end
      ...
      it 'Copies a file with a space' do
        ...
        expect(
          File.exist?("#{@tmp_path}/target/copy_artifacts\ with\ a\ space")
        ).to eq(true)
      end
      ...
      it 'Copies a file with verbose' do
        ...
        expect(UI).to receive(:verbose).once.ordered.with(
                    "Copying artifacts #{source_path} to #{target_path}"
                  )
        expect(UI).to receive(:verbose).once.ordered.with(
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    "Not all files were present in copy artifacts. Missing #{source_path}"
                  )
