  ...
  describe Fastlane::FastFile do
    describe 'changelog_from_git_commits' do
      it 'Collects messages from the last tag to HEAD by default' do
        ...
        pseudocommand =
          "git log --pretty=\"%B\" #{inner_command.shellescape}...HEAD"
      ...
      it 'Uses the provided pretty format to collect log messages' do
        ...
        inner_command =
          'git describe --tags `git rev-list --tags --max-count=1`'
        pseudocommand =
          "git log --pretty=\"%s%n%b\" #{inner_command.shellescape}...HEAD"
        expect(result).to eq(pseudocommand)
      end
      ...
      it 'Uses the provided date format to collect log messages if specified' do
        ...
        inner_command =
          'git describe --tags `git rev-list --tags --max-count=1`'
        pseudocommand =
          "git log --pretty=\"%s%n%b\" --date=\"short\" #{inner_command
            .shellescape}...HEAD"
        expect(result).to eq(pseudocommand)
      end
      ...
      it 'Handles tag names with characters that need shell escaping' do
        ...
        expect(result).to eq(
                    "git log --pretty=\"%B\" #{tag.shellescape}...HEAD"
                  )
      end
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    "Unresolved conflict between options: 'commits_count' and 'between'"
                  )
      end
      ...
      it 'Does not accept an invalid value for :merge_commit_filtering' do
        values =
          Fastlane::Actions::GIT_MERGE_COMMIT_FILTERING_OPTIONS.map do |o|
            "'#{o}'"
          end.join(', ')
      ...
      it 'Does not include merge commits in the list of commits' do
        ...
        inner_command =
          'git describe --tags `git rev-list --tags --max-count=1`'
        pseudocommand =
          "git log --pretty=\"%B\" #{inner_command
            .shellescape}...HEAD --no-merges"
        expect(result).to eq(pseudocommand)
      end
      ...
      it 'Only include merge commits if merge_commit_filtering is only_include_merges' do
        ...
        inner_command =
          'git describe --tags `git rev-list --tags --max-count=1`'
        pseudocommand =
          "git log --pretty=\"%B\" #{inner_command.shellescape}...HEAD --merges"
        expect(result).to eq(pseudocommand)
      end
      ...
      it 'Uses pattern matching for tag name if requested' do
        ...
        inner_command =
          "git describe --tags `git rev-list --tags=#{tag_match_pattern
            .shellescape} --max-count=1`"
        pseudocommand =
          "git log --pretty=\"%B\" #{inner_command.shellescape}...HEAD"
        expect(result).to eq(pseudocommand)
      end
      ...
      it 'Runs between option from command line' do
        expect(
          system('fastlane run changelog_from_git_commits between:123456,HEAD')
        ).to be
      end
