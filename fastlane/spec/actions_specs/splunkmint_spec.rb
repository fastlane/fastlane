  ...
  describe Fastlane::FastFile do
    describe 'Splunk MINT integration' do
      it 'verbosity is set correctly' do
        expect(
          Fastlane::Actions::SplunkmintAction.verbose(verbose: true)
        ).to eq('--verbose')
        expect(
          Fastlane::Actions::SplunkmintAction.verbose(verbose: false)
        ).to eq('')
      end

      it 'upload url is set correctly' do
        expect(Fastlane::Actions::SplunkmintAction.upload_url).to eq(
      ...
      it 'raises an error if no dsym source has been found' do
        ...
        expect do
          ...
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::DSYM_OUTPUT_PATH
          ] =
            nil
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::DSYM_ZIP_PATH
          ] =
            nil
      ...
      it 'proxy options are set correctly' do
        expect(
          Fastlane::Actions::SplunkmintAction.proxy_options(
            proxy_address: '',
            proxy_port: nil,
            proxy_username: nil,
            proxy_password: nil
      ...
      it 'mandatory options are used correctly' do
        ...
        expect(result).to include(
                    "--header 'X-Splunk-Mint-Auth-Token: e05ba40754c4869fb7e0b61'"
                  )
