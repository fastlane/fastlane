  ...
  describe 'Automatic Code Signing' do
    ...
    it 'enable_automatic_code_signing' do
      ...
      expect(UI).to receive(:success).with(
                  "Successfully updated project settings to use Code Sign Style = 'Automatic'"
                )
      result =
      ...
      project =
        Xcodeproj::Project.open(
          './fastlane/spec/fixtures/xcodeproj/automatic_code_signing.xcodeproj'
        )
      root_attrs = project.root_object.attributes['TargetAttributes']
      expect(root_attrs['771D79501D9E69C900D840FA']['ProvisioningStyle']).to eq(
                  'Automatic'
                )
      expect(root_attrs['77C503031DD3175E00AC8FF0']['ProvisioningStyle']).to eq(
                  'Automatic'
                )

      expect(root_attrs['771D79501D9E69C900D840FA']['DevelopmentTeam']).to eq(
                  'XXXX'
                )
      expect(root_attrs['77C503031DD3175E00AC8FF0']['DevelopmentTeam']).to eq(
                  'XXXX'
                )

      project.targets.each do |target|
        target.build_configuration_list.get_setting('CODE_SIGN_STYLE')
          .map { |build_config, value| expect(value).to eq('Automatic') }
      end
    end
    ...
    it 'disable_automatic_code_signing for specific target' do
      allow(UI).to receive(:success)
      expect(UI).to receive(:success).with(
                  "Successfully updated project settings to use Code Sign Style = 'Manual'"
                )
      expect(UI).to receive(:success).with("\t * today")
      expect(UI).to receive(:important).with(
                  'Skipping demo not selected (today)'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
    ...
    it 'disable_automatic_code_signing' do
      ...
      project.targets.each do |target|
        target.build_configuration_list.get_setting('CODE_SIGN_STYLE')
          .map { |build_config, value| expect(value).to eq('Manual') }
      end
    end
    ...
    it 'sets team id' do
      ...
      expect(UI).to receive(:important).with(
                  'Set Team id to: G3KGXDXQL9 for target: demo'
                )
      expect(UI).to receive(:important).with(
                  'Set Team id to: G3KGXDXQL9 for target: today'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
    ...
    it 'sets code sign identity' do
      ...
      expect(UI).to receive(:important).with(
                  'Set Code Sign identity to: iPhone Distribution for target: demo'
                )
      expect(UI).to receive(:important).with(
                  'Set Code Sign identity to: iPhone Distribution for target: today'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
      ...
      project.targets.each do |target|
        target.build_configuration_list.get_setting('CODE_SIGN_STYLE')
          .map { |build_config, value| expect(value).to eq('Manual') }
        target.build_configuration_list.get_setting('CODE_SIGN_IDENTITY')
          .map do |build_config, value|
          expect(value).to eq('iPhone Distribution')
        end
        next unless target.name == 'demo'
        target.build_configuration_list.get_setting(
    ...
    it 'sets profile name' do
      ...
      expect(UI).to receive(:important).with(
                  'Set Provisioning Profile name to: Mindera for target: demo'
                )
      expect(UI).to receive(:important).with(
                  'Set Provisioning Profile name to: Mindera for target: today'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
      ...
      project.targets.each do |target|
        ...
        target.build_configuration_list.get_setting(
          ...
          .map { |build_config, value| expect(value).to eq('Mindera') }
      end
    end
    ...
    it 'sets profile uuid' do
      ...
      expect(UI).to receive(:important).with(
                  'Set Provisioning Profile UUID to: 1337 for target: demo'
                )
      expect(UI).to receive(:important).with(
                  'Set Provisioning Profile UUID to: 1337 for target: today'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
      ...
      project.targets.each do |target|
        target.build_configuration_list.get_setting('CODE_SIGN_STYLE')
          .map { |build_config, value| expect(value).to eq('Manual') }
        target.build_configuration_list.get_setting('PROVISIONING_PROFILE')
          .map { |build_config, value| expect(value).to eq('1337') }
      end
    end
    ...
    it 'sets bundle identifier' do
      ...
      expect(UI).to receive(:important).with(
                  'Set Bundle identifier to: com.fastlane.mindera.cosigner for target: demo'
                )
      expect(UI).to receive(:important).with(
                  'Set Bundle identifier to: com.fastlane.mindera.cosigner for target: today'
                )
      result =
        Fastlane::FastFile.new.parse(
          "lane :test do
      ...
      project.targets.each do |target|
        target.build_configuration_list.get_setting('CODE_SIGN_STYLE')
          .map { |build_config, value| expect(value).to eq('Manual') }
        target.build_configuration_list.get_setting('PRODUCT_BUNDLE_IDENTIFIER')
          .map do |build_config, value|
