describe Fastlane do
  describe Fastlane::FastFile do
    before do
      ...
      allow(FastlaneCore::CommandExecutor).to receive(:execute)
    ...
    describe 'SPM Integration' do
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    'Please pass a valid command. Use one of the following: build, test, clean, reset, update, resolve, generate-xcodeproj, init'
                  )
      end
      ...
      context 'when command is package related' do
        ...
        it 'adds configuration flag to package command if configuration is set' do
          ...
          expect(result).to eq(
                      "swift package --configuration release #{command}"
                    )
        end
        ...
        it 'set pipefail with xcpretty' do
          ...
          expect(result).to eq(
                      "set -o pipefail && swift package #{command} 2>&1 | xcpretty --simple"
                    )
        end
      end
      ...
      context 'when command is generate-xcodeproj' do
        ...
        it 'adds --verbose and xcpretty options correctly as well' do
          ...
          expect(result).to eq(
                      'set -o pipefail && swift package --verbose generate-xcodeproj --xcconfig-overrides Package.xcconfig 2>&1 | xcpretty --simple'
                    )
        end
      end
    end
