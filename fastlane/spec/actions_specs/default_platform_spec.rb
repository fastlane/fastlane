  ...
  describe Fastlane::FastFile do
    describe 'Default Platform Action' do
      it 'stores the default platform and converts to a symbol' do
        Fastlane::Actions::DefaultPlatformAction.run(%w[ios])
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::DEFAULT_PLATFORM
          ]
        ).to eq(:ios)
      end
    ...
    describe 'Extra platforms' do
      ...
      it 'displays a warning if a platform is not supported' do
        expect(FastlaneCore::UI).to receive(:important).with(
                    "Platform 'notSupportedOS' is not officially supported. Currently supported platforms are [:ios, :mac, :android]."
                  )
        Fastlane::Actions::DefaultPlatformAction.run(%w[notSupportedOS])
      end

      it "doesn't display a warning at every run if a platform has been added to extra" do
        Fastlane::SupportedPlatforms.extra = %i[notSupportedOS]
