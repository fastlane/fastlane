describe Fastlane do
  describe Fastlane::FastFile do
    context 'SVN repository' do
      before do
        expect(Fastlane::Actions::GetBuildNumberRepositoryAction).to receive(
                    :is_svn?
                  )
                    .and_return(true)
        allow(Fastlane::Actions::GetBuildNumberRepositoryAction).to receive(
                   :is_git_svn?
                 )
                   .and_return(false)
        allow(Fastlane::Actions::GetBuildNumberRepositoryAction).to receive(
      ...
      it 'get SVN build number' do
        ...
        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::BUILD_NUMBER_REPOSITORY
          ]
        ).to eq('svn info | grep Revision | egrep -o "[0-9]+"')
      end
    end
    ...
    context 'GIT-SVN repository' do
      ...
      it 'get GIT-SVN build number' do
        ...
        ).to eq('git svn info | grep Revision | egrep -o "[0-9]+"')
      end
    end
    ...
    context 'GIT repository' do
      ...
      it 'get GIT build number' do
        ...
        ).to eq('git rev-parse --short HEAD')
      end
    end
    ...
    context 'Mercurial repository' do
      ...
      it 'get HG build number' do
        ...
        ).to eq('hg parent --template "{node|short}"')
      end
