...
describe Fastlane::CLIToolsDistributor do
  describe 'command handling' do
    it 'runs the lane instead of the tool when there is a conflict' do
      FastlaneSpec::Env.with_ARGV(%w[sigh])
      require 'fastlane/commands_generator'
      expect(FastlaneCore::FastlaneFolder).to receive(:fastfile_path)
                  .and_return(
                  './fastlane/spec/fixtures/fastfiles/FastfileUseToolNameAsLane'
                )
                  .at_least(:once)
    ...
    it 'runs a separate tool when the tool is available and the name is not used in a lane' do
      FastlaneSpec::Env.with_ARGV(%w[gym])
      require 'gym/options'
  ...
  describe 'update checking' do
    it 'checks for updates when running a lane' do
      ...
      expect(FastlaneCore::FastlaneFolder).to receive(:fastfile_path)
                  ...
                  .at_least(:once)
      expect(FastlaneCore::UpdateChecker).to receive(:start_looking_for_update)
                  .with('fastlane')
      expect(Fastlane::CommandsGenerator).to receive(:start).and_return(nil)
      expect(FastlaneCore::UpdateChecker).to receive(:show_update_status).with(
                  'fastlane',
                  Fastlane::VERSION
                )
      Fastlane::CLIToolsDistributor.take_off
    ...
    it 'checks for updates even if the lane has an error' do
      FastlaneSpec::Env.with_ARGV(%w[beta])
      expect(FastlaneCore::FastlaneFolder).to receive(:fastfile_path)
                  .and_return(
      ...
      expect_any_instance_of(Commander::Runner).to receive(:abort).with(
                                  ...
                                  .and_raise(SystemExit) # mute console output from `abort`
      expect { Fastlane::CLIToolsDistributor.take_off }.to raise_error(
                  SystemExit
                )
    end
  end
