...
describe Fastlane::Actions do
  describe '#sh' do
    ...
    context 'external commands are failed' do
      context 'with error_callback' do
        it "doesn't raise shell_error" do
          ...
          Fastlane::Actions.sh(
            'exit 1',
            error_callback: ->(_) { called = true }
          )

          expect(called).to be(true)
          expect(FastlaneCore::UI).to have_received(:error).with(
                      "Exit status of command 'exit 1' was 1 instead of 0.\n"
                    )
        end
      end
      ...
      context 'without error_callback' do
        it 'raise shell_error' do
          ...
          expect(UI).to have_received(:shell_error!).with(
                      "Exit status of command 'exit 1' was 1 instead of 0.\n"
                    )
        end
      end
    ...
    context 'passing command arguments to the system' do
      ...
      it 'allows override of argv[0]' do
        expect_command(%w[/usr/local/bin/git git], 'commit', '-m', 'A message')
        Fastlane::Actions.sh(
          %w[/usr/local/bin/git git],
          'commit',
          '-m',
          'A message'
        )
    ...
    context 'with a postfix block' do
      ...
      it 'yields command output' do
        expect_command(
          'ls',
          '-la',
          exitstatus: 1, output: 'Heeeelp! Something went wrong.'
        )
        Fastlane::Actions.sh('ls', '-la') do |status, result|
          expect(status.exitstatus).to eq(1)
      ...
      it 'returns the return value of the block if present' do
        expect_command('ls', '-la')
        return_value = Fastlane::Actions.sh('ls', '-la') { |status, result| 42 }
  ...
  describe 'shell_command_from_args' do
    ...
    it 'raises when no argument passed' do
      expect { command_from_args }.to raise_error(ArgumentError)
    end
    ...
    it 'adds an environment Hash at the beginning' do
      ...
      expect(command).to eq(
                  "PATH=/usr/local/bin git commit -m #{message.shellescape}"
                )
    end
    ...
    it 'shell-escapes environment variable values' do
      ...
      command =
        command_from_args({ 'PATH' => path }, 'git', 'commit', '-m', message)
      expect(command).to eq(
                  "PATH=#{path.shellescape} git commit -m #{message
                    .shellescape}"
                )
    end

    it 'recognizes an array as the only element of a command' do
      command = command_from_args(%w[/usr/local/bin/git git])
    ...
    it 'recognizes an array as the first element of a command' do
      message = 'A message'
      command =
        command_from_args(%w[/usr/local/bin/git git], 'commit', '-m', message)
      expect(command).to eq(
                  "/usr/local/bin/git commit -m #{message.shellescape}"
                )
    end
  end
end
...
def expect_command(*command, exitstatus: 0, output: '')
  ...
  expect(Open3).to receive(:popen2e).with(*command).and_yield(
              mock_input,
              mock_output,
              mock_thread
            )
