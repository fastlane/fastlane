describe Fastlane::PluginInfoCollector do
  ...
  before do
    [
      ...
      'fastlane-whatever',
      'whatever'
    ].each { |current| stub_plugin_exists_on_rubygems(current, false) }
  end
  ...
  describe '#collect_plugin_name' do
    ...
    describe 'with invalid initial input' do
      it 'offers a correction and uses it if accepted' do
        ...
        expect(collector.collect_plugin_name('fastlane-whatever')).to eq(
                    'whatever'
                  )
      end
    ...
    it "handles plugin name starting with '#{Fastlane::PluginManager::FASTLANE_PLUGIN_PREFIX}'" do
      expect(
        collector.plugin_name_valid?(
          "#{Fastlane::PluginManager::FASTLANE_PLUGIN_PREFIX}test_name"
        )
      ).to be_falsey
    end
  ...
  describe '#fix_plugin_name' do
    ...
    it "handles plugin name with characters we don't want" do
      expect(collector.fix_plugin_name('T!EST-na$me ple&ase')).to eq(
                  'test_name_please'
                )
    end
  end
  ...
  describe 'author collection' do
    ...
    it 'has an author name from git config' do
      expect(test_ui).not_to(receive(:input))
      expect(collector.collect_author('Fabricio Devtoolio')).to eq(
                  'Fabricio Devtoolio'
                )
    end
  end
  ...
  describe 'email collection' do
    ...
    it 'has an email from git config' do
      expect(test_ui).not_to(receive(:input))
      expect(collector.collect_email('fabric.devtools@gmail.com')).to eq(
  ...
  describe 'email detection' do
    it 'has an email from git config' do
      expect(FastlaneCore::Helper).to receive(:backticks).with(
                  ...
                  .and_return('fabric.devtools@gmail.com')
