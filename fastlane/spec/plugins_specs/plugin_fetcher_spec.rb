  ...
  describe Fastlane::PluginFetcher do
    describe '#fetch_gems' do
      before do
        ...
        stub_request(
          :get,
          "https://rubygems.org/api/v1/search.json?page=1&query=fastlane-plugin-#{current_gem}"
        )
          .to_return(
          status: 200,
          body:
            File.read(
              './fastlane/spec/fixtures/requests/rubygems_plugin_query.json'
            ),
          headers: {}
        )
        ...
        stub_request(
          :get,
          "https://rubygems.org/api/v1/search.json?page=2&query=fastlane-plugin-#{current_gem}"
        )
          .to_return(status: 200, body: [].to_json, headers: {})

        stub_request(
          :get,
          'https://rubygems.org/api/v1/search.json?page=1&query=fastlane-plugin-'
        )
          .to_return(
          status: 200,
          body:
        ...
        stub_request(
          :get,
          'https://rubygems.org/api/v1/search.json?page=2&query=fastlane-plugin-'
        )
          .to_return(status: 200, body: [].to_json, headers: {})
      end
      ...
      it 'returns all available plugins if no search query is given' do
        ...
        expect(plugin1.homepage).to eq(
                    'https://github.com/suculent/fastlane-plugin-apprepo'
                  )
