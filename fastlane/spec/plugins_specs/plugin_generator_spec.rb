...
describe Fastlane::PluginGenerator do
  describe '#generate' do
    let(:plugin_info) do
      Fastlane::PluginInfo.new(
        'tester_thing',
        'Fabricio Devtoolio',
        'fabric.devtools@gmail.com',
        'summary',
        'details'
      )
    end
    ...
    before(:each) do
      ...
      unless initialized
        ...
        allow(test_ui).to receive(:input).and_raise(
                   ':input call was not mocked!'
                 )
        allow(test_ui).to receive(:confirm).and_raise(
                   ':confirm call was not mocked!'
                 )
        ...
        generator =
          Fastlane::PluginGenerator.new(ui: test_ui, dest_root: tmp_dir)
    ...
    it 'creates a .gitignore file' do
      ...
      %w[
        *.gem\n
        ...
        /doc/\n
        /rdoc/\n
      ].each { |item| expect(dot_gitignore_lines).to include(item) }
    end
    ...
    it 'creates a plugin.rb file for the plugin' do
      ...
      Dir.chdir(gem_name) do
        ...
        # the behavior!
        action_class =
          Object.const_get("Fastlane::#{plugin_name.fastlane_class}")

        all_classes = action_class.all_classes
        expect(all_classes).to contain_exactly(
                    File.expand_path(
                      "lib/#{plugin_info.actions_path}/#{plugin_name}_action.rb"
                    ),
                    File.expand_path(
                      "lib/#{plugin_info.helper_path}/#{plugin_name}_helper.rb"
                    )
                  )

        # Get the relative paths to require, check that they have already been required.
        require_paths =
          all_classes.map { |c| c.gsub(lib + '/', '').gsub('.rb', '') }
    ...
    it 'creates a module for the VERSION' do
      ...
      expect(
        File.exist?(
          File.join(tmp_dir, gem_name, 'lib', require_path, 'version.rb')
        )
      ).to be(true)
    end
    ...
    it 'creates a Action class' do
      ...
      action_class =
        Object.const_get(
          "Fastlane::Actions::#{plugin_name.fastlane_class}Action"
        )
    ...
    it 'creates a complete, valid gemspec' do
      ...
      Dir.chdir(gem_name) do
        ...
        expect(gemspec.development_dependencies).to contain_exactly(
                    ...
                    Gem::Dependency.new(
                      'rubocop',
                      Gem::Requirement.new([Fastlane::RUBOCOP_REQUIREMENT]),
                      :development
                    ),
                    Gem::Dependency.new(
                    ...
                    Gem::Dependency.new(
                      'fastlane',
                      Gem::Requirement.new([">= #{Fastlane::VERSION}"]),
                      :development
                    )
                  )
      end
    ...
    it 'creates a valid helper class' do
      helper_file =
        File.join(
          ...
          gem_name,
          'lib',
          plugin_info.helper_path,
          "#{plugin_info.plugin_name}_helper.rb"
        )
      ...
      helper_class =
        Object.const_get(
          "Fastlane::Helper::#{plugin_name.fastlane_class}Helper"
        )
    ...
    it 'creates a action_spec.rb file' do
      action_spec_file =
        File.join(tmp_dir, gem_name, 'spec', "#{plugin_name}_action_spec.rb")
    ...
    it 'creates a Rakefile' do
      ...
      expect(rakefile_contents).to eq(
