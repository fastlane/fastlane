  ...
  describe Fastlane::PluginManager do
    ...
    describe '#gemfile_path' do
      ...
      it 'returns nil if no Gemfile available' do
        expect(Bundler::SharedHelpers).to receive(:default_gemfile).and_raise(
    ...
    describe '#available_gems' do
      it 'returns [] if no Gemfile is available' do
        allow(Bundler::SharedHelpers).to receive(:default_gemfile).and_raise(
                   Bundler::GemfileNotFound
                 )
      ...
      it 'returns all fastlane plugins with no fastlane_core' do
        allow(Bundler::SharedHelpers).to receive(:default_gemfile).and_return(
                   './fastlane/spec/fixtures/plugins/Pluginfile1'
                 )
        expect(plugin_manager.available_gems).to eq(
                    %w[fastlane-plugin-xcversion fastlane_core hemal]
                  )
      end
    end
    ...
    describe '#plugin_is_added_as_dependency?' do
      ...
      it 'returns true if a plugin is available' do
        expect(
          plugin_manager.plugin_is_added_as_dependency?(
      ...
      it 'returns false if a plugin is available' do
        expect(
          plugin_manager.plugin_is_added_as_dependency?('fastlane-plugin-hemal')
        ).to eq(false)
      end
      ...
      it "raises an error if parameter doesn't start with fastlane plugin prefix" do
        expect do
          plugin_manager.plugin_is_added_as_dependency?('hemal')
                    ...
                    "fastlane plugins must start with 'fastlane-plugin-' string"
                  )
      end
    end
    ...
    describe '#plugins_attached?' do
      it 'returns true if plugins are attached' do
        allow(Bundler::SharedHelpers).to receive(:default_gemfile).and_return(
                   './fastlane/spec/fixtures/plugins/GemfileWithAttached'
                 )
      ...
      it 'returns false if plugins are not attached' do
        allow(Bundler::SharedHelpers).to receive(:default_gemfile).and_return(
                   './fastlane/spec/fixtures/plugins/GemfileWithoutAttached'
                 )
    ...
    describe '#install_dependencies!' do
      it 'execs out the correct command' do
        ...
        expect(plugin_manager).to receive(:exec).with(
                    "bundle install --quiet && echo 'Successfully installed plugins'"
                  )
    ...
    describe '#update_dependencies!' do
      ...
      it 'execs out the correct command' do
        expect(plugin_manager).to receive(:ensure_plugins_attached!)
        expect(plugin_manager).to receive(:exec).with(
                    "bundle update fastlane-plugin-xcversion --quiet && echo 'Successfully updated plugins'"
                  )
    ...
    describe '#gem_dependency_suffix' do
      it 'default to RubyGems if gem is available' do
        expect(Fastlane::PluginManager).to receive(
                    :fetch_gem_info_from_rubygems
                  )
                    .and_return({ anything: :really })
      ...
      describe 'Gem is not available on RubyGems.org' do
        ...
        it 'supports specifying a custom local path' do
          expect(FastlaneCore::UI.ui_object).to receive(:select).and_return(
                      'Local Path'
                    )
          expect(FastlaneCore::UI.ui_object).to receive(:input).and_return(
                      '../yoo'
                    )
          expect(plugin_manager.gem_dependency_suffix('fastlane')).to eq(
                      ", path: '../yoo'"
        ...
        it 'supports specifying a custom git URL' do
          ...
          expect(plugin_manager.gem_dependency_suffix('fastlane')).to eq(
                      ", git: 'https://github.com/fastlane/fastlane'"
                    )
        end

        it 'supports falling back to RubyGems' do
          expect(FastlaneCore::UI.ui_object).to receive(:select).and_return(
                      "RubyGems.org ('fastlane' seems to not be available there)"
                    )
          expect(plugin_manager.gem_dependency_suffix('fastlane')).to eq('')
        end
    ...
    describe 'Previously bundled action' do
      ...
      it 'shows how to install a plugin if you want to use a previously bundled action' do
        ...
        expect(Fastlane::Actions).to receive(:formerly_bundled_actions)
                    .and_return([deprecated_action])
          ...
          result =
            ...
            #{deprecated_action}
              ...
              .execute(:test)
                    ...
                    "The action '#{deprecated_action}' is no longer bundled with fastlane. You can install it using `fastlane add_plugin deprecated`"
                  )
      end
    ...
    describe '#add_dependency' do
      it 'shows an error if a dash is used' do
        ...
        expect { pm.add_dependency('ya-tu_sabes') }.to raise_error(
                    "Plugin name must not contain a '-', did you mean '_'?"
                  )
      end
      ...
      it 'works with valid parameters' do
        pm = Fastlane::PluginManager.new
        expect(pm).to receive(:pluginfile_path).and_return('.')
        expect(pm).to receive(:plugin_is_added_as_dependency?).with(
    ...
    describe 'Overwriting plugins' do
      it 'shows a warning if a plugin overwrites an existing action' do
        module Fastlane::Crashlytics; end

        pm = Fastlane::PluginManager.new
        plugin_name = 'crashlytics'
        expect(pm).to receive(:available_plugins).and_return([plugin_name])
        expect(Fastlane::FastlaneRequire).to receive(:install_gem_if_needed)
                    .with(gem_name: plugin_name, require_gem: true)
        expect(Fastlane::Crashlytics).to receive(:all_classes).and_return(
                    ["/actions/#{plugin_name}.rb"]
                  )
        expect(UI).to receive(:important).with(
                    "Plugin 'Crashlytics' overwrites already loaded action '#{plugin_name}'"
                  )
        pm.load_plugins
      end
    end
    ...
    describe 'Error handling of invalid plugins' do
      it 'shows an appropriate error message when an action is not available, even though a plugin was added' do
        expect do
          expect_any_instance_of(Fastlane::PluginManager).to receive(
                                      :available_plugins
                                    )
                                      .and_return(
                                      %w[fastlane-plugin-my_custom_plugin]
                                    )
          result =
            Fastlane::FastFile.new.parse(
              'lane :test do
            my_custom_plugin
              ...
              .execute(:test)
                    ...
                    "Plugin 'my_custom_plugin' was not properly loaded, make sure to follow the plugin docs for troubleshooting: https://docs.fastlane.tools/plugins/plugins-troubleshooting/"
                  )
      end
          ...
          result =
            ...
            my_custom_plugin
              ...
              .execute(:test)
                    ...
                    "Could not find action, lane or variable 'my_custom_plugin'. Check out the documentation for more details: https://docs.fastlane.tools/actions"
                  )
      end
      ...
      it 'shows an appropriate error message when a plugin is really broken' do
        ...
        expect(UI).to receive(:error).with(
                    "Error loading plugin '#{plugin_name}': #{ex}"
                  )
        pm.load_plugins
