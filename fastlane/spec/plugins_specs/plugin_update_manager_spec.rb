describe Fastlane::PluginUpdateManager do
  describe '#show_update_status' do
    before(:each) { ENV.delete('FASTLANE_SKIP_UPDATE_CHECK') }
    ...
    it 'prints out a table if updates are available' do
      ...
      Fastlane::PluginUpdateManager.server_results[plugin_name] =
        Gem::Version.new('1.1.0')
      allow(Fastlane::PluginUpdateManager).to receive(:plugin_references)
                 .and_return({ plugin_name => { version_number: '0.1.1' } })
