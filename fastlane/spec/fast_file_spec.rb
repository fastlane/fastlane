  ...
  describe Fastlane::FastFile do
    describe '#initialize' do
      it 'raises an error if file does not exist' do
        expect do
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/fastfileNotHere'
          )
        end.to raise_exception(
                    "Could not find Fastfile at path './fastlane/spec/fixtures/fastfiles/fastfileNotHere'"
                  )
      end
      ...
      it 'raises an error if unknow method is called' do
        expect do
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileInvalid'
          )
        end.to raise_exception(
                    "Could not find action, lane or variable 'laneasdf'. Check out the documentation for more details: https://docs.fastlane.tools/actions"
                  )
      end
    end
    ...
    describe '#is_platform_block?' do
      ...
      it "raises an exception if key doesn't exist at all" do
        expect(UI).to receive(:user_error!).with(
                    "Could not find 'asdf'. Available lanes: test, anotherroot, mac beta, ios beta, ios release, android beta, android witherror, android unsupported_action"
                  )
        @ff.is_platform_block?('asdf')
      end

      it 'has an alias for `update`, if there is no lane called `update`' do
        expect(Fastlane::OneOff).to receive(:run).with(
                    { action: 'update_fastlane', parameters: {} }
                  )
    ...
    describe '#lane_name' do
      before do
        @ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile1')
      end

      it 'raises an error if block is missing' do
        expect(UI).to receive(:user_error!).with(
                    "You have to pass a block using 'do' for lane 'my_name'. Make sure you read the docs on GitHub."
                  )
        @ff.lane(:my_name)
      end
      ...
      it 'raises an error if name contains spaces' do
        expect(UI).to receive(:user_error!).with(
                    'lane name must not contain any spaces'
                  )
        @ff.lane(:"my name") {  }
      end
    ...
    describe 'Grouped fastlane for different platforms' do
      ...
      it 'calls all error blocks if multiple are given (android - witherror)' do
        expect { @ff.runner.execute('witherror', 'android') }.to raise_error(
                    'my exception'
                  )
        ...
        expect(File.exist?('/tmp/fastlane/before_all.txt')).to eq(true)
        expect(File.exist?('/tmp/fastlane/before_each_witherror.txt')).to eq(
                    true
                  )
        expect(File.exist?('/tmp/fastlane/after_each_witherror.txt')).to eq(
                    false
                  )

        expect(
          Fastlane::Actions.lane_context[
            Fastlane::Actions::SharedValues::PLATFORM_NAME
          ]
        ).to eq(:android)
      end
      ...
      it 'allows calls without a platform (nil - anotherroot)' do
        ...
        expect(File.exist?('/tmp/fastlane/before_each_anotherroot.txt')).to eq(
                    true
                  )
        expect(File.exist?('/tmp/fastlane/after_each_anotherroot.txt')).to eq(
                    true
                  )
      ...
      it 'logs a warning if and unsupported action is called on an non officially supported platform' do
        expect(FastlaneCore::UI).to receive(:important).with(
                    "Action 'frameit' isn't known to support operating system 'android'."
                  )
    ...
    describe 'Different Fastfiles' do
      ...
      it 'prints a warning if a lane is called like an action' do
        ...
        expect(UI).to receive(:error).with(
                    "Name of the lane 'cocoapods' is already taken by the action named 'cocoapods'"
                  )
        expect(UI).to receive(:error).with(
                    '------------------------------------------------'
                  )
        Fastlane::FastFile.new(
          './fastlane/spec/fixtures/fastfiles/FastfileLaneNameEqualsActionName'
        )
      end
      ...
      it 'Parameters are also passed to the before_all, after_all' do
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileConfigs'
          )
      ...
      it 'allows the user to invent a new platform' do
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileNewPlatform'
          )
        expect { ff.runner.execute(:crash, :windows) }.to raise_error(
                    ':windows crash'
                  )
      end

      it 'allows the user to set the platform in their Fastfile', focus: true do
        expect(UI).to receive(:important).with(
                    "Setting '[:windows, :neogeo]' as extra SupportedPlatforms"
                  )
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileAddNewPlatform'
          )
      ...
      it 'before_each and after_each are called every time' do
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileLaneBlocks'
          )
      ...
      it 'Exception in error block are swallowed and shown, and original exception is re-raised' do
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileErrorInError'
          )
        ...
        expect { ff.runner.execute(:beta, nil, {}) }.to raise_error(
                    'Original error'
                  )
      end
      ...
      describe 'supports switching lanes' do
        it 'use case 1: passing parameters to another lane and getting the result' do
          ff =
            Fastlane::FastFile.new(
              './fastlane/spec/fixtures/fastfiles/SwitcherFastfile'
            )
        ...
        it "calling a lane that doesn't exist" do
          ...
          expect { ff.runner.execute(:invalid, :ios) }.to raise_error(
                      "Could not find action, lane or variable 'wrong_platform'. Check out the documentation for more details: https://docs.fastlane.tools/actions"
                    )
        end
      ...
      it 'collects the lane description for documentation' do
        ...
        expect(ff.runner.lanes[nil][:deploy].description).to eq(
                    ['My Deploy', 'description']
                  )
        expect(ff.runner.lanes[:mac][:specific].description).to eq(
                    ['look at my mac, my mac is amazing']
                  )
      end
      ...
      it 'execute different envs with lane in before block' do
        ...
        ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile2')
        ff.runner.execute(:deploy)
      ...
      it 'automatically converts invalid quotations' do
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileInvalidQuotation'
          )
        # No exception :)
      end
...
lane :beta do
  sigh(app_identifier: \"hi\"
        ...
        expect(UI).to receive(:user_error!).with(
                    "Syntax error in your Fastfile on line 17: fastlane/spec/fixtures/fastfiles/FastfileSytnaxError:17: syntax error, unexpected keyword_end, expecting ')'"
                  )
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileSytnaxError'
          )
      end
      ...
      it 'properly shows an error message when there is a syntax error in the imported Fastfile' do
        allow(FastlaneCore::FastlaneFolder).to receive(:path).and_return(nil)
        ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile')
        expect(UI).to receive(:content_error).with(
                    '
...
lane :beta do
  sigh(app_identifier: \"hi\"
      ...
      it 'imports actions associated with a Fastfile before their Fastfile' do
        ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile')
        expect { ff.import('./import1/Fastfile') }.not_to(raise_error)
      end

      it 'raises an error if lane is not available' do
        ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile1')
        expect { ff.runner.execute(:not_here) }.to raise_exception(
                    "Could not find lane 'not_here'. Available lanes: test, deploy, error_causing_lane, mac specific"
                  )
      end
      ...
      it 'raises an error if the lane name contains spaces' do
        ...
        ff =
          Fastlane::FastFile.new(
            './fastlane/spec/fixtures/fastfiles/FastfileInvalidName'
          )
      end
      ...
      it 'calls the error block when an error occurs' do
        ...
        ff =
          Fastlane::FastFile.new('./fastlane/spec/fixtures/fastfiles/Fastfile1')
        expect { ff.runner.execute(:error_causing_lane) }.to raise_exception(
                    'divided by 0'
                  )
