...
describe Cert do
  describe Cert::Runner do
    ...
    it 'Successful run' do
      ...
      allow(Spaceship.certificate.production).to receive(:all).and_return(
                 [certificate]
               )
    ...
    describe ':filename option handling' do
      ...
      let(:filepath) do
        filename_ext =
          File.extname(filename) == '.cer' ? filename : "#{filename}.cer"
        if File.extname(filename) == '.cer'
          "#{temp}/#{filename}"
        else
          "#{temp}/#{filename}.cer"
        end
      ...
      before do
        ...
        allow(FastlaneCore::CertChecker).to receive(:installed?).and_return(
                   true
                 )
      end
