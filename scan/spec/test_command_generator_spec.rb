...
'
    ...
    allow(Open3).to receive(:popen3).with('xcrun simctl list devices')
               .and_yield(nil, response, nil, nil)

    rt_response = ''
    allow(rt_response).to receive(:read).and_return("no\n")
    allow(Open3).to receive(:popen3).with('xcrun simctl list runtimes')
               .and_yield(nil, rt_response, nil, nil)

    allow(FastlaneCore::CommandExecutor).to receive(:execute).with(
  ...
  describe Scan::TestCommandGenerator do
    before(:each) { @test_command_generator = Scan::TestCommandGenerator.new }
        ...
        expect do
          Fastlane::FastFile.new.parse(
            ...
            .execute(:test)
                    ...
                    "Unresolved conflict between options: 'toolchain' and 'xctestrun'"
                  )
      end

      it 'passes the toolchain option to xcodebuild',
    ...
    it 'supports custom xcpretty formatter as a gem name',
       requires_xcodebuild: true do
      options = {
    ...
    it 'supports custom xcpretty formatter as a path to a ruby file',
       requires_xcodebuild: true do
      options = {
        formatter: 'custom-formatter.rb',
        project: './scan/examples/standard/app.xcodeproj',
        sdk: '9.0'
      }
      Scan.config =
    ...
    describe 'Standard Example' do
      ...
      it 'uses the correct build command with the example project with no additional parameters',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
      ...
      it '#buildlog_path is not used when not provided',
         requires_xcodebuild: true do
        result = @test_command_generator.xcodebuild_log_path
        expect(result.to_s).to include(
                    File.expand_path(
                      "#{FastlaneCore::Helper.buildlog_path}/scan"
                    )
                  )
      end
    end
    ...
    describe 'Derived Data Example' do
      before do
        options = {
          project: './scan/examples/standard/app.xcodeproj',
          derived_data_path: '/tmp/my/derived_data'
        }
        Scan.config =
          FastlaneCore::Configuration.create(
            Scan::Options.available_options,
      ...
      it 'uses the correct build command with the example project',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
    ...
    describe 'Test Max Concurrent Simulators' do
      ...
      it 'uses the correct number of concurrent simulators',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
    ...
    describe 'with Scan option :include_simulator_logs' do
      context 'extract system.logarchive' do
        it 'copies all device logs to the output directory',
           requires_xcodebuild: true do
          Scan.config =
            FastlaneCore::Configuration.create(
              Scan::Options.available_options,
          ...
          expect(FastlaneCore::CommandExecutor).to receive(:execute).with(
                      command:
                        "xcrun simctl spawn 021A465B-A294-4D9E-AD07-6BDC8E186343 log collect --output /tmp/scan_results/system_logs-iPhone\\ 6s_iOS_10.0.logarchive 2>/dev/null",
                      print_all: false,
                      print_command: true
                    )
          ...
          expect(FastlaneCore::CommandExecutor).to receive(:execute).with(
                      command:
                        "xcrun simctl spawn 2ABEAF08-E480-4617-894F-6BAB587E7963 log collect --output /tmp/scan_results/system_logs-iPad\\ Air_iOS_10.0.logarchive 2>/dev/null",
                      print_all: false,
                      print_command: true
                    )

          mock_test_result_parser = Object.new
          allow(Scan::TestResultParser).to receive(:new).and_return(
                     mock_test_result_parser
                   )
          allow(mock_test_result_parser).to receive(:parse_result).and_return(
                     { tests: 100, failures: 0 }
                   )

          mock_slack_poster = Object.new
          allow(Scan::SlackPoster).to receive(:new).and_return(
                     mock_slack_poster
                   )
          allow(mock_slack_poster).to receive(:run)

          mock_test_command_generator = Object.new
          allow(Scan::TestCommandGenerator).to receive(:new).and_return(
                     mock_test_command_generator
                   )
          allow(mock_test_command_generator).to receive(:xcodebuild_log_path)
                     .and_return('./scan/spec/fixtures/boring.log')
    ...
    describe 'Test Exclusion Example' do
      it 'only tests the test bundle/suite/cases specified in only_testing when the input is an array',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
      ...
      it 'only tests the test bundle/suite/cases specified in only_testing when the input is a string',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
      ...
      it 'does not the test bundle/suite/cases specified in skip_testing when the input is an array',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
      ...
      it 'does not the test bundle/suite/cases specified in skip_testing when the input is a string',
         requires_xcodebuild: true do
        log_path = File.expand_path('~/Library/Logs/scan/app-app.log')
    ...
    it 'uses a device without version specifier', requires_xcodebuild: true do
      options = {
        project: './scan/examples/standard/app.xcodeproj', device: 'iPhone 6s'
      }
      Scan.config =
        FastlaneCore::Configuration.create(
          Scan::Options.available_options,
    ...
    it 'rejects devices with versions below deployment target',
       requires_xcodebuild: true do
      options = {
        project: './scan/examples/standard/app.xcodeproj',
        device: 'iPhone 5 (8.4)'
    ...
    describe 'test-without-building and build-for-testing' do
      ...
      it 'should raise an exception if two build_modes are set',
         requires_xcodebuild: true do
        allow(FastlaneCore::FastlaneFolder).to receive(:path).and_return('.')
        expect do
          Fastlane::FastFile.new.parse(
    ...
    describe 'Multiple devices example' do
      ...
      it 'uses multiple devices', requires_xcodebuild: true do
        options = {
          project: './scan/examples/standard/app.xcodeproj',
          devices: ['iPhone 6s (9.3)', 'iPad Air (9.3)']
        }
        Scan.config =
          FastlaneCore::Configuration.create(
            Scan::Options.available_options,
      ...
      it 'de-duplicates devices matching same simulator',
         requires_xcodebuild: true do
        options = {
          project: './scan/examples/standard/app.xcodeproj',
          devices: ['iPhone 5 (9.0)', 'iPhone 5 (9.0.0)']
        }
        Scan.config =
          FastlaneCore::Configuration.create(
            Scan::Options.available_options,
      ...
      it 'raises an error if multiple devices are specified for `device`' do
        expect do
          options = {
            project: './scan/examples/standard/app.xcodeproj',
            device: ['iPhone 6s (9.3)', 'iPad Air (9.3)']
          }
          Scan.config =
            FastlaneCore::Configuration.create(
              Scan::Options.available_options,
              options
                    ...
                    "'device' value must be a String! Found Array instead."
                  )
      end
