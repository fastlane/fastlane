...
module Scan
  ...
  class Options
    def self.verify_type(item_name, acceptable_types, value)
      ...
      unless type_ok
        UI.user_error!(
          "'#{item_name}' should be of type #{acceptable_types.join(
            ' or '
    ...
    def self.available_options
      ...
      [
        ...
        FastlaneCore::ConfigItem
          ...
          verify_block:
            proc do |value|
              ...
              unless File.exist?(v)
                UI.user_error!("Workspace file not found at path '#{v}'")
              end
              UI.user_error!('Workspace file invalid') unless File.directory?(v)
              unless v.include?('.xcworkspace')
                UI.user_error!(
                  'Workspace file is not a workspace, must end with .xcworkspace'
                )
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              v = File.expand_path(value.to_s)
              unless File.exist?(v)
                UI.user_error!("Project file not found at path '#{v}'")
              end
              UI.user_error!('Project file invalid') unless File.directory?(v)
              unless v.include?('.xcodeproj')
                UI.user_error!(
        ...
        FastlaneCore::ConfigItem
          ...
          conflicting_options: %i[devices],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'device' and 'devices' options in one run"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem
          ...
          is_string: false,
          verify_block:
            proc { |value| verify_type('only_testing', [Array, String], value) }
        ),
        FastlaneCore::ConfigItem.new(
          key: :skip_testing,
          ...
          is_string: false,
          verify_block:
            proc { |value| verify_type('skip_testing', [Array, String], value) }
        ),
        # other test options
        FastlaneCore::ConfigItem
          .new(
          ...
          conflicting_options: %i[build_for_testing],
          is_string: true,
          optional: true
        ),
        FastlaneCore::ConfigItem.new(
          ...
          conflicting_options: %i[xctestrun],
          description:
        ...
        FastlaneCore::ConfigItem.new(
          key: :address_sanitizer,
          description: 'Should the address sanitizer be turned on?',
          is_string: false,
          type: Boolean,
          optional: true,
          conflicting_options: %i[thread_sanitizer],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'address_sanitizer' and 'thread_sanitizer' options in one run"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          key: :thread_sanitizer,
          description: 'Should the thread sanitizer be turned on?',
          is_string: false,
          type: Boolean,
          optional: true,
          conflicting_options: %i[address_sanitizer],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'thread_sanitizer' and 'address_sanitizer' options in one run"
              )
            end
        ),
        # output
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless %w[standard basic rspec raw].include?(value)
                UI.user_error!("Invalid output_style #{value}")
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          ...
          conflicting_options: %i[custom_report_file_name],
          optional: true,
          default_value: nil
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          key: :disable_concurrent_testing,
          type: Boolean,
          default_value: false,
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "File not found at path '#{File.expand_path(value)}'"
                )
              end
            end
        ),
        ...
        FastlaneCore::ConfigItem.new(
          key: :slack_use_webhook_configured_username_and_icon,
        ...
        FastlaneCore::ConfigItem.new(
          key: :custom_report_file_name,
          env_name: 'SCAN_CUSTOM_REPORT_FILE_NAME',
          description:
            'Sets custom full report file name when generating a single report',
          deprecated: 'Use `--output_files` instead',
          conflicting_options: %i[output_files],
          optional: true,
          is_string: true
        )
      ]
