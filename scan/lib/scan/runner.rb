  ...
  class Runner
        ...
        error:
          proc do |error_output|
            begin
              ...
              SlackPoster.new.run({ build_errors: 1 })
              raise ex
            end
          end
      )
    ...
    def handle_results(tests_exit_status)
      ...
      unless tests_exit_status == 0
        UI.test_failure!(
          "Test execution failed. Exit status: #{tests_exit_status}"
        )
      end
    ...
    def zip_build_products
      ...
      FastlaneCore::Helper.zip_directory(
        path,
        output_path,
        contents_only: true, overwrite: true, print: false
      )
    ...
    def test_results
      ...
      if temp_junit_report && File.file?(temp_junit_report)
        return File.read(temp_junit_report)
      end
      ...
      reporter_options_generator =
        XCPrettyReporterOptionsGenerator.new(false, [], [], '', false, nil)
      reporter_options = reporter_options_generator.generate_reporter_options
      xcpretty_args_options =
        reporter_options_generator.generate_xcpretty_args_options
      cmd =
        "cat #{@test_command_generator.xcodebuild_log_path
          .shellescape} | xcpretty #{reporter_options.join(
          ' '
        )} #{xcpretty_args_options} &> /dev/null"
      system(cmd)
    ...
    def copy_simulator_logs
      ...
      Scan.devices.each do |device|
        ...
        FastlaneCore::Simulator.copy_logs(
          device,
          log_identity,
