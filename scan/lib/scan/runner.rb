...
module Scan
  class Runner
    ...
    def test_results
      ...
      reporter_xcpretty_args = reporter_options_generator.generate_xcpretty_args
      cmd = "cat #{@test_command_generator.xcodebuild_log_path.shellescape} | xcpretty #{reporter_options.join(' ')} #{reporter_xcpretty_args.join(' ')} &> /dev/null"
      system(cmd)
