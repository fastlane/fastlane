  ...
  class SlackPoster
    def run(results)
      ...
      if Scan.config[:slack_channel].to_s.length > 0
        ...
        channel = ('#' + channel) unless %w[# @].include?(channel[0]) # send message to channel by default
      end

      username =
        if Scan.config[:slack_use_webhook_configured_username_and_icon]
          nil
        else
          Scan.config[:slack_username]
        end
      icon_url =
        if Scan.config[:slack_use_webhook_configured_username_and_icon]
          nil
        else
          Scan.config[:slack_icon_url]
        end
      fields = []
      ...
      if results[:failures]
        fields <<
          {
            title: 'Test Failures', value: results[:failures].to_s, short: true
          }
      end
      ...
      Fastlane::Actions::SlackAction.run(
        {
          message:
            "#{Scan.project.app_name} Tests:\n#{Scan.config[:slack_message]}",
          channel: channel,
          slack_url: Scan.config[:slack_url].to_s,
          success:
            results[:build_errors].to_i == 0 && results[:failures].to_i == 0,
          username: username,
          icon_url: icon_url,
          payload: {},
