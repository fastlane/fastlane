...
module Precheck
  ...
  class RuleProcessor
    def self.process_app_and_version(app: nil, app_version: nil, rules: nil)
      items_to_check = []
      items_to_check +=
        generate_text_items_to_check(app: app, app_version: app_version)
      items_to_check +=
        generate_url_items_to_check(app: app, app_version: app_version)
    ...
    def self.process_rules(items_to_check: nil, rules: nil)
      ...
      rules.each do |rule|
        ...
        if rule_level == RULE_LEVELS[:skip]
          skipped_rules << rule
          UI.message(
            "Skipped: #{rule.class.friendly_name}-> #{rule.description}".yellow
          )
          next
        end
        ...
        if rule.needs_customization?
          if rule_config.nil? || rule_config[:data].nil?
            UI.verbose(
              "#{rule
                .key} excluded because no data was passed to it e.g.: #{rule
                .key}(data: <data here>)"
            )
            next
          end
          ...
          if rule_level == RULE_LEVELS[:error]
            error_results =
              add_new_result_to_rule_hash(
                rule_hash: error_results, result: result
              )
          end
          if rule_level == RULE_LEVELS[:warn]
            warning_results =
              add_new_result_to_rule_hash(
                rule_hash: warning_results, result: result
              )
          end
        ...
        if rule_failed_at_least_once
          message =
            "😵  Failed: #{rule.class.friendly_name}-> #{rule.description}"
          if rule_level == RULE_LEVELS[:error]
            UI.error(message)
          else
    ...
    def self.add_new_result_to_rule_hash(rule_hash: nil, result: nil)
      rule_hash[result.rule] = [] unless rule_hash.include?(result.rule)
    ...
    def self.generate_url_items_to_check(app: nil, app_version: nil)
      ...
      items +=
        collect_urls_from_hash(
          hash: app_version.support_url,
          item_name: :support_url,
          friendly_name_postfix: 'support URL'
        )
      items +=
        collect_urls_from_hash(
          hash: app_version.marketing_url,
    ...
    def self.collect_urls_from_hash(
      hash: nil, item_name: nil, friendly_name_postfix: nil, is_optional: false
    )
      items = []
    ...
    def self.generate_text_items_to_check(app: nil, app_version: nil)
      ...
      items <<
        TextItemToCheck.new(app_version.copyright, :copyright, 'copyright')

      items +=
        collect_text_items_from_language_item(
          hash: app_version.keywords,
          item_name: :keywords,
      ...
      items +=
        collect_text_items_from_language_item(
          hash: app_version.release_notes,
      ...
      items +=
        collect_text_items_from_language_item(
          hash: app.details.apple_tv_privacy_policy,
      ...
      if should_include_iap
        UI.message(
          'Reading in-app purchases. If you have a lot, this might take a while'
        )
        UI.message(
        ...
        in_app_purchases.each do |purchase|
          items +=
            collect_iap_language_items(
              purchase_edit_versions: purchase.edit.versions
            )
        end
    ...
    def self.collect_iap_language_items(
      purchase_edit_versions: nil, is_optional: false
    )
      items = []
      purchase_edit_versions.each do |language_key, hash|
        ...
        items <<
          TextItemToCheck.new(
            name,
            :in_app_purchase,
            "in-app purchase name: #{name}: (#{language_key})",
            is_optional
          )
        items <<
          TextItemToCheck.new(
            description,
            :in_app_purchase,
            "in-app purchase desc: #{description}: (#{language_key})",
            is_optional
          )
      end
      return items
