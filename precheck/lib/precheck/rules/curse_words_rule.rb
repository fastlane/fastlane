  ...
  class CurseWordsRule < TextRule
    ...
    def rule_block
      return lambda do |text|
        ...
        all_metadata_words_list =
          (split_words + split_words_without_punctuation).uniq
        metadata_word_hashes =
          all_metadata_words_list.map { |word| Digest::SHA256.hexdigest(word) }
        ...
        if found_words.length > 0
          ...
          UI.verbose(
            "#{self.class.name.split('::').last ||=
              self.class.name} found potential curse words 😬"
          )
          UI.verbose(
            'Keep in mind, these words might be ok given the context they are used in'
          )
          UI.verbose("Matched: \"#{friendly_found_words}\"")
          return RuleReturn.new(
            validation_state: VALIDATION_STATES[:failed],
            failure_data: "found: #{friendly_found_words}"
          )
        else
    ...
    def hashed_curse_word_set
      curse_hashes = []
      File.open(
        File.dirname(__FILE__) + '/rules_data/curse_word_hashes/en_us.txt'
      )
        .each { |line| curse_hashes << line.to_s.strip }
