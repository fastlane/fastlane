...
module Precheck
  class UnreachableURLRule < URLRule
    ...
    def rule_block
      return lambda do |url|
        url = url.to_s.strip
        if url.empty?
          return RuleReturn.new(
            validation_state: Precheck::VALIDATION_STATES[:failed],
            failure_data: 'empty url'
          )
        end
        ...
        begin
          ...
          uri.fragment = nil
          request =
            Faraday.new(uri.normalize.to_s) do |connection|
              connection.use(FaradayMiddleware::FollowRedirects)
              connection.adapter(:net_http)
            end
          unless request.head.status == 200
            return RuleReturn.new(
              validation_state: Precheck::VALIDATION_STATES[:failed],
              failure_data: url
            )
