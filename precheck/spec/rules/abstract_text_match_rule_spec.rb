...
module Precheck
  ...
  class PassOnExclusionTestingTextMatchRule < TestingRule
    def lowercased_words_to_look_for
      %w[tacos puppies taquitos].map(&:downcase)
    end
  ...
  describe Precheck do
    describe Precheck::AbstractTextMatchRule do
      ...
      let(:pass_on_exclusion_or_empty_rule) do
        PassOnExclusionOrEmptyTestingTextMatchRule.new
      end
      ...
      it 'ensures string is flagged if words found in exclusion rule' do
        item =
          TextItemToCheck.new(
            "tacos are really delicious and puppies shouldn't eat them.",
            :description,
            'description'
          )
      ...
      it 'ensures string is not flagged if all words are found in fail_on_exclusion rule' do
        item =
          TextItemToCheck.new(
            "tacos and taquitos are really delicious, too bad I'm vegetarian now",
            :description,
            'description'
          )
      ...
      it 'ensures string is flagged if missing some words in fail_on_exclusion rule' do
        item =
          TextItemToCheck.new(
            "tacos are really delicious, too bad I'm vegetarian now",
            :description,
            'description'
          )
        result = fail_on_exclusion_rule.check_item(item)
