  ...
  describe Precheck do
    describe Precheck::UnreachableURLRule do
      ...
      def setup_url_rule_mock(url: 'http://fastlane.tools', return_status: 200)
        ...
        allow(Faraday).to receive(:new).with(uri.normalize.to_s).and_return(
                   request
                 )
      end
      ...
      it 'passes for 200 status URL' do
        setup_url_rule_mock
        item =
          URLItemToCheck.new('http://fastlane.tools', 'some_url', 'test URL')
      ...
      it 'passes for valid non encoded URL' do
        setup_url_rule_mock(
          url: 'http://fastlane.tools/%E3%83%86%E3%82%B9%E3%83%88'
        )

        item =
          URLItemToCheck.new(
            'http://fastlane.tools/テスト',
            'some_url',
            'test URL'
          )
        result = rule.check_item(item)
