...
| --- | --- | --- | --- | --- | --- | --- |
| A | 16 | 1 | `spaceship/spec/spaceship_spec.rb` (10), `spaceauth_spec.rb` (6) | `WebMock::NetConnectNotAllowedError` on `POST https://idmsa.apple.com/appleauth/auth/signin/init` | A login ran without the SIRP stub, so a real SRP value was computed and matched none of the recorded request bodies | **fixed**, confirmed at seed 48174: all 16 gone, no `signin/init` request blocked anywhere in the run |
...
Moving the SIRP stubbing to every spaceship example appears to have cleared row A: that run blocked no `signin/init` request at all. Two blocked requests remained in `portal/certificate_spec.rb`, and the diagnostics showed them to be certificate submission rather than login, which is now row I.
...
| M | `fastlane/spec/ruby_version_warning_spec.rb:10` | **fixed, trigger now identified**. `fastlane_core/spec/update_checker_spec.rb` deleted `BUNDLE_BIN_PATH` and `BUNDLE_GEMFILE` in a `before` and never put them back. Those two are all `Helper.bundler?` looks at, so every later example in the process looked as though it were not running under bundler, and `print_bundle_exec_warning` then found the repository's own Gemfile and emitted `fastlane detected a Gemfile in the current directory` through `UI.important`. That is the extra call that broke the `.once` constraint. Confirmed directly rather than by inference: with the two variables removed, `Helper.bundler?` returns false and the warning fires. Now scoped with `with_env_values`, so it holds with the environment guard off. Original note follows. The examples constrain every call to `UI.important`, and `take_off` also emits the bundle exec warning, which failed them. Stubbed, in keeping with the rest of the file, which already mocks most of `take_off`. What made that warning fire is still unknown, see below |
...
| P | `gym/spec/platform_detection_spec.rb:32` | **closed, no longer reproduces**. Passes standalone at defined order and at seeds 1 and 2, and has not appeared in any local full run since, nor in the CI runs on this branch. Recorded as environmental when raised, and nothing since contradicts that. Stays in the list so later orderings keep sampling it |
...
Seeds 48174, 1150, 21323 and 40083 were each pinned in turn while the failures they exposed were worked through, and all four are green. The workflow samples a fresh order per run again; the first batch of five turned up one new failure, row S at seed 53367.
...
## plugin_generator_spec, resolved

Two examples in `fastlane/spec/plugins_specs/plugin_generator_spec.rb` failed in three different ways. One was ordering, one was a stale file, and the story I first wrote about the third was wrong twice over.

Running the group on its own gave `Errno::ENOENT` for the plugin directory. RSpec runs an inner `before(:all)` ahead of an outer `before(:each)`, so the group's `bundle install` ran before the plugin above had been generated, and it only worked because an earlier example in the file had generated it already. Row D and row S shape. It is a guarded `before(:each)` now and the group runs standalone.

The rubocop and rake examples, `expect($?.exitstatus).to eq(0)`, were the interesting ones. `plugin_sh` throws the command's output away and the example asserts only on the exit status, so for a long time nobody had read what rubocop actually said. It says:

```
Gemspec/RequiredRubyVersion: required_ruby_version and TargetRubyVersion (2.6) should be equal
```

The generated plugin's gemspec requires Ruby >= 3.1 and its `.rubocop.yml` targeted 2.6. That looked like a defect in the plugin template, and it is not: `fastlane/lib/fastlane/plugins/template/.rubocop.yml` is generated by `rake prepare_rubocop_config` from fastlane's own config, and gitignored, because it is a build artifact. A working copy can be left holding one from an older fastlane, and mine was. Regenerating it fixes the failure.

So it was environmental after all, as first recorded, but not for the reason assumed, and not something a developer can be expected to work out from `expected 0, got 1`. `prepare_rubocop_config` is now a prerequisite of `test_all` and `test_parallel` as well as of the packaging tasks, so the run says the same thing on any machine.

With all three addressed the file is green at defined order, at random, and under a four worker split with nothing excluded, which is what let the `EXCLUDE` come back out of the CI job.
