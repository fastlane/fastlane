...
| E | 2 | 2 | `fastlane_core/spec/command_executor_spec.rb` | `Exit status: 42` for commands that cannot fail | Two unrelated causes sharing a count. The examples mocked `Process.wait`, which is what fills in `$?`, while `FastlanePty` reads `$?` for the exit status, so they reported the previous example's status | **mostly fixed** by letting the mocks reap for real, 3,1,2,3 failures across seeds to 1,0,1,1,0,1. One left, see row J |
...
| J | 1 | 1 | `fastlane_core/spec/command_executor_spec.rb` | `expected "log\n", got "Logging disabled while running tests...\nlog\n"` | A one shot banner. FastlaneCore prints it the first time logging is touched in the process, so whichever example runs first absorbs it into its own stdout assertion. Nothing to do with exit status, it only shared row E's count | open |
