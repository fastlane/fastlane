This Xcode project is not directly used by the _trainer_ specs suite as a fixture (i.e. the specs suite don't reference that fixture directly), but it is the Xcode project that was used to generate the `Xcode26-Mixed-XCTest-SwiftTesting-Project.xcresult` file.

## Why is this Xcode project committed to the repo?

The only reason it is committed to the repo is to:

 - Provide a reference to what tests were used to generate the `Xcode26-Mixed-XCTest-SwiftTesting-Project.xcresult` file.
...
To add more tests to be then reported in the `.xcresult` fixture so that the _trainer_ specs suite can test more cases:
 ...
 - Copy the `$XCRESULT_PATH/{Info.plist,database.sqlite3}` files in the `spec/fixtures/Xcode26-Mixed-XCTest-SwiftTesting-Project.xcresult` directory.
   - Since the `database.sqlite3` file represents the same data as what's serialized in `$XCRESULT_PATH/Data/*`, but takes way less disk space (since the database can optimize storage), the idea is to only commit that database file instead of the `Data/` directory to save disk space and make the git logs/diffs easier to work with.
    ...
    # cp "$XCRESULT_PATH"/{Info.plist,database.sqlite3} trainer/spec/fixtures/Xcode26-Mixed-XCTest-SwiftTesting.xcresult
    ```
