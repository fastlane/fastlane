  ...
  class Runner
    ...
    def run
      FastlaneCore::PrintTable.print_values(
        config: @config,
        hide_keys: [],
        title: "Summary for screengrab #{Fastlane::VERSION}"
      )
      ...
      apk_paths_provided =
        app_apk_path && !app_apk_path.empty? && tests_apk_path &&
          !tests_apk_path.empty?

      unless apk_paths_provided || discovered_apk_paths.any?
        UI.error('No APK paths were provided and no APKs could be found')
        UI.error(
          "Please provide APK paths with 'app_apk_path' and 'tests_apk_path' and make sure you have assembled APKs prior to running this command."
        )
        return
      end
      ...
      if test_classes_to_use && test_classes_to_use.any? &&
         test_packages_to_use &&
         test_packages_to_use.any?
        UI.error(
          "'use_tests_in_classes' and 'use_tests_in_packages' can not be combined. Please use one or the other."
        )
        return
      end

      if (test_classes_to_use.nil? || test_classes_to_use.empty?) &&
         (test_packages_to_use.nil? || test_packages_to_use.empty?)
        UI.important(
          'Limiting the test classes run by `screengrab` to just those that generate screenshots can make runs faster.'
        )
        UI.important(
          'Consider using the :use_tests_in_classes or :use_tests_in_packages option, and organize your tests accordingly.'
        )
      end
    ...
    def select_device
      devices =
        run_adb_command('adb devices -l', print_all: true, print_command: true)
          .split("\n")
      # the first output by adb devices is "List of devices attached" so remove that and any adb startup output
      devices.reject! { |device| [].any? { |status| device.include?(status) } }
      ...
      if @config[:specific_device]
        devices.select! { |d| d.include?(@config[:specific_device]) }
      end

      if devices.empty?
        UI.user_error!(
          "No connected devices matched your criteria: #{@config[
            :specific_device
          ]}"
        )
      end
      ...
      if devices.length > 1
        ...
        UI.important(
          'To specify which connected device to use, use the -s (specific_device) config option'
        )
      end
    ...
    def select_app_apk(discovered_apk_paths)
      UI.important(
        "To not be asked about this value, you can specify it using 'app_apk_path'"
      )
    ...
    def select_tests_apk(discovered_apk_paths)
      UI.important(
        "To not be asked about this value, you can specify it using 'tests_apk_path'"
      )
    ...
    def clear_local_previous_screenshots(device_type_dir_name)
      if @config[:clear_previous_screenshots]
        UI.message(
          "Clearing #{device_type_dir_name} within #{@config[
            :output_directory
          ]}"
        )
    ...
    def screenshot_file_names_in(output_directory, device_type)
      Dir.glob(
        File.join(output_directory, '**', device_type, '*.png'),
        File::FNM_CASEFOLD
      )
    end
    ...
    def clear_device_previous_screenshots(
      device_serial, device_screenshots_paths
    )
      ...
      device_screenshots_paths.each do |device_path|
        if_device_path_exists(device_serial, device_path) do |path|
          run_adb_command(
            "adb -s #{device_serial} shell rm -rf #{path}",
            print_all: true, print_command: true
          )
        end
      end
    ...
    def validate_apk(app_apk_path)
      ...
      apk_permissions =
        @executor.execute(
          command: "#{@android_env.aapt_path} dump permissions #{app_apk_path}",
          print_all: true,
          print_command: true
        )

      missing_permissions =
        NEEDED_PERMISSIONS.reject { |needed| apk_permissions.include?(needed) }

      if missing_permissions.any?
        UI.user_error!(
          "The needed permission(s) #{missing_permissions.join(
            ', '
          )} could not be found in your app APK"
        )
      end
    end
    ...
    def install_apks(device_serial, app_apk_path, tests_apk_path)
      ...
      apk_install_output =
        run_adb_command(
          "adb -s #{device_serial} install -t -r #{app_apk_path.shellescape}",
          print_all: true, print_command: true
        )
      if apk_install_output.include?('Failure [')
        UI.user_error!('App APK could not be installed')
      end

      UI.message('Installing tests APK')
      apk_install_output =
        run_adb_command(
          "adb -s #{device_serial} install -t -r #{tests_apk_path.shellescape}",
          print_all: true, print_command: true
        )
      if apk_install_output.include?('Failure [')
        UI.user_error!('Tests APK could not be installed')
      end
    end
    ...
    def uninstall_apks(device_serial, app_package_name, tests_package_name)
      ...
      if packages.include?(tests_package_name.to_s)
        ...
        run_adb_command(
          "adb -s #{device_serial} uninstall #{tests_package_name}",
          print_all: true, print_command: true
        )
      end
    end
    ...
    def grant_permissions(device_serial)
      UI.message(
        'Granting the permission necessary to change locales on the device'
      )
      run_adb_command(
        "adb -s #{device_serial} shell pm grant #{@config[
          :app_package_name
        ]} android.permission.CHANGE_CONFIGURATION",
        print_all: true, print_command: true
      )
      ...
      if device_api_version >= 23
        UI.message(
          'Granting the permissions necessary to access device external storage'
        )
        run_adb_command(
          "adb -s #{device_serial} shell pm grant #{@config[
            :app_package_name
    ...
    def run_tests_for_locale(
      ...
      if test_classes_to_use
        instrument_command << "-e class #{test_classes_to_use.join(',')}"
      end
      if test_packages_to_use
        instrument_command << "-e package #{test_packages_to_use.join(',')}"
      end
      if launch_arguments
        instrument_command <<
          launch_arguments.map { |item| '-e ' + item }.join(' ')
      end
      instrument_command <<
        "#{@config[:tests_package_name]}/#{@config[
          :test_instrumentation_runner
        ]}"

      test_output =
        run_adb_command(
          instrument_command.join(" \\\n"),
          print_all: true, print_command: true
        )
      ...
      if test_output.include?('FAILURES!!!')
        if @config[:exit_on_test_failure]
          UI.test_failure!(
            "Tests failed for locale #{locale} on device #{device_serial}"
          )
        else
    ...
    def pull_screenshots_from_device(
      device_serial, device_screenshots_paths, device_type_dir_name
    )
      UI.message('Pulling captured screenshots from the device')
      starting_screenshot_count =
      ...
      Dir.mktmpdir do |tempdir|
        device_screenshots_paths.each do |device_path|
          if_device_path_exists(device_serial, device_path) do |path|
            run_adb_command(
              "adb -s #{device_serial} pull #{path} #{tempdir}",
              print_all: false, print_command: true
            )
          end
        end
    ...
    def move_pulled_screenshots(pull_dir, device_type_dir_name)
      ...
      Dir.glob(screenshots_dir_pattern, File::FNM_CASEFOLD)
        .each do |screenshots_dir|
        src_screenshots =
          Dir.glob(File.join(screenshots_dir, '*.png'), File::FNM_CASEFOLD)
    ...
    def if_device_path_exists(device_serial, device_path)
      if run_adb_command(
         ...
         print_all: false, print_command: false
       )
