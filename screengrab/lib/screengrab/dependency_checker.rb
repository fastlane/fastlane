...
module Screengrab
  class DependencyChecker
    ...
    def self.check_adb(android_env)
      ...
      warn_if_command_path_not_relative_to_android_home(
        'adb',
        android_home,
        adb_path
      )
    ...
    def self.raise_missing_adb(android_home)
      if android_home
        UI.error(
          "The `adb` command could not be found relative to your provided ANDROID_HOME at #{android_home}"
        )
        UI.error(
          'Please ensure that the Android SDK is installed and the platform-tools directory is present'
        )
      else
    ...
    def self.warn_missing_aapt(android_home)
      if android_home
        UI.important(
          "The `aapt` command could not be found relative to your provided ANDROID_HOME at #{android_home}"
        )
        UI.important(
          'Please ensure that the Android SDK is installed and you have the build tools downloaded'
        )
      else
        UI.important('The `aapt` command could not be found on your PATH')
        UI.important(
          'Please ensure that the Android SDK is installed and you have the build tools downloaded and present on your PATH'
        )
      end
    end

    def self.warn_if_command_path_not_relative_to_android_home(
      cmd_name, android_home, cmd_path
    )
      if android_home && cmd_path && !cmd_path.start_with?(android_home)
        UI.important(
          "Using `#{cmd_name}` found at #{cmd_path} which is not within the specified ANDROID_HOME at #{android_home}"
        )
      end
    end
  end
