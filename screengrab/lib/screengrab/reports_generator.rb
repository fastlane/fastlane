  ...
  class ReportsGenerator
    ...
    def generate
      ...
      Dir[File.join(screens_path, '*')].sort.each do |language_folder|
        ...
        Dir[File.join(language_folder, 'images', '*', '*.png')].sort
          .each do |screenshot|
      ...
      UI.success(
        "Successfully created HTML file with an overview of all the screenshots: '#{export_path}'"
      )
      unless Screengrab.config[:skip_open_summary]
