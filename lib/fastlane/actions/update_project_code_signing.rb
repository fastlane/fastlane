  ...
  module Actions
    ...
    class UpdateProjectCodeSigningAction < Action
      def self.run(params)
        ...
        fail "Could not find path to project config '#{path}'. Pass the path to your project (not workspace)!".red unless File.exist?(path)
        ...
        [
          FastlaneCore::ConfigItem.new(key: :path,
                                       ...
                                       verify_block: proc do |value|
                                         fail "Path is invalid".red unless File.exist?(value)
                                       end),
