        ...
        [
          FastlaneCore::ConfigItem.new(key: :api_token,
                                       ...
                                       verify_block: proc do |value|
                                         fail "No API Token for DeployGate given, pass using `api_token: 'token'`".red unless value.to_s.length > 0
                                       end),
          FastlaneCore::ConfigItem.new(key: :user,
                                       env_name: "DEPLOYGATE_USER",
                                       description: "Target username or organization name",
                                       verify_block: proc do |value|
                                         fail "No User for app given, pass using `user: 'user'`".red unless value.to_s.length > 0
                                       end),
