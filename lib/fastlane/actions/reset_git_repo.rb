    ...
    class ResetGitRepoAction < Action
      def self.run(params)
        if params[:force] || params[:force] || Actions.lane_context[SharedValues::GIT_REPO_WAS_CLEAN_ON_START]
          ...
          fail 'This is a destructive and potentially dangerous action. To protect from data loss, please add the `ensure_git_status_clean` action to the beginning of your lane, or if you\'re absolutely sure of what you\'re doing then call this action with the :force option.'.red
        end
      end
        ...
        [
          FastlaneCore::ConfigItem.new(key: :files,
                                       ...
                                       verify_block: proc do |value|
                                         fail "Please pass an array only" unless value.is_a? Array
                                       end),
