      ...
      def self.run(params)
        ...
        if devices
          device_objs = devices.map do |k, v|
            fail "Passed invalid UDID: #{v} for device: #{k}".red unless UDID_REGEXP =~ v
          ...
          fail "Please provide a file according to the Apple Sample UDID file (https://devimages.apple.com.edgekey.net/downloads/devices/Multiple-Upload-Samples.zip)".red unless devices_file.first == ["Device ID", "Device Name"]

          device_objs = devices_file.drop(1).map do |device|
            fail "Invalid device line, please provide a file according to the Apple Sample UDID file (https://devimages.apple.com.edgekey.net/downloads/devices/Multiple-Upload-Samples.zip)".red unless device.count == 2
            fail "Passed invalid UDID: #{device[0]} for device: #{device[1]}".red unless UDID_REGEXP =~ device[0]

            Cupertino::ProvisioningPortal::Device.new(device[1], device[0])
          end
        else
          fail "You must pass either a valid `devices` or `devices_file`. Please check the readme.".red
        end
        ...
        [
          ...
          FastlaneCore::ConfigItem.new(key: :devices_file,
                                       ...
                                       verify_block: proc do |value|
                                         fail "Could not find file '#{value}'".red unless File.exist?(value)
                                       end),
