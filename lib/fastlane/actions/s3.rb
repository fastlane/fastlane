  ...
  module Actions
    ...
    class S3Action < Action
      def self.run(config)
        ...
        fail "No S3 access key given, pass using `access_key: 'key'`".red unless s3_access_key.to_s.length > 0
        fail "No S3 secret access key given, pass using `secret_access_key: 'secret key'`".red unless s3_secret_access_key.to_s.length > 0
        fail "No S3 bucket given, pass using `bucket: 'bucket'`".red unless s3_bucket.to_s.length > 0
        fail "No IPA file path given, pass using `ipa: 'ipa path'`".red unless ipa_file.to_s.length > 0
      ...
      def self.get_ipa_info(ipa_file)
        ...
        [bundle_id, bundle_version, title]
      end
