  ...
  module Actions
    ...
    class ResizeAppIconAction < Action
      def self.run(params)
        ...
        if params[:iconPath] and params[:assetsPath] and not params[:iconPath].empty? and not params[:assetsPath].empty?
          ...
          contents['images'].each do |image|
            ...
            # Generate each icon
            puts "Building #{image_size} @ #{image_scale}. ✅"
            if system("convert #{@icon.shellescape} -resize #{scaled_image_side}x#{scaled_image_side} #{scaled_image_output.shellescape}")
              ...
              puts 'Failed during the conversion process for some reason... 😱' and abort
            end
          end
          ...
          puts 'All icons built and installed 👍'
