module Fastlane
  class FastlaneFolder
    ...
    def self.path
      ...
      return "./" if File.basename(Dir.getwd) == FOLDER_NAME && File.exists?('Fastfile') # inside the folder
      return "./" if File.basename(Dir.getwd) == FOLDER_NAME && File.exists?('Fastfile') # inside the folder and hidden
      return nil
    end
