...
module Snapshot
  class SimulatorLauncherXcode8 < SimulatorLauncherBase
    def take_screenshots_one_simulator_at_a_time
      ...
      launcher_config.devices.each_with_index do |device, device_index|
        ...
        launcher_config
          .launch_args_set
          .each do |launch_args|
          launcher_config.languages
            .each_with_index do |language, language_index|
            locale = nil
            ...
            current_run =
              device_index * launcher_config.languages.count + language_index +
                1
            number_of_runs =
              launcher_config.languages.count * launcher_config.devices.count
            UI.message("snapshot run #{current_run} of #{number_of_runs}")
            results[device][language] =
              run_for_device_and_language(language, locale, device, launch_args)
    ...
    def run_for_device_and_language(
      language, locale, device, launch_arguments, retries = 0
    )
      ...
      if retries < launcher_config.number_of_retries
        UI.important(
          "Tests failed, re-trying #{retries + 1} out of #{launcher_config
            .number_of_retries +
            1} times"
        )
    ...
    def launch_one_at_a_time(language, locale, device_type, launch_arguments)
      ...
      if launcher_config.add_photos
        add_media([device_type], :photo, launcher_config.add_photos)
      end
      if launcher_config.add_videos
        add_media([device_type], :video, launcher_config.add_videos)
      end

      open_simulator_for_device(device_type)

      command =
        TestCommandGeneratorXcode8.generate(
          device_type: device_type, language: language, locale: locale
        )
      ...
      raw_output =
        File.read(
          TestCommandGeneratorXcode8.xcodebuild_log_path(
            device_type: device_type, language: language, locale: locale
          )
        )
    ...
    def open_simulator_for_device(device_name)
      unless FastlaneCore::Env.truthy?('FASTLANE_EXPLICIT_OPEN_SIMULATOR')
        return
      end
