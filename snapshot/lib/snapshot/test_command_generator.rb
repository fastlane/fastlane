    ...
    class << self
      ...
      def destination(devices)
        unless verify_devices_share_os(devices)
          UI.user_error!(
            'All devices provided to snapshot should run the same operating system'
          )
        end
      ...
      def get_device_type_with_simctl(device_names)
        ...
        return device_names.map do |device_name|
          ...
          device.devicetype.name if device
        end.compact
      end
    end
  end
