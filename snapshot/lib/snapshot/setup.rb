...
module Snapshot
  class Setup
    ...
    def self.create(
      path, is_swift_fastfile: false, print_instructions_on_failure: false
    )
      ...
      if File.exist?(snapfile_path)
        if print_instructions_on_failure
          print_instructions(
            snapshot_helper_filename: snapshot_helper_filename,
            snapfile_path: snapfile_path
          )
          return
        else
          UI.user_error!(
            "Snapfile already exists at path '#{snapfile_path}'. Run 'fastlane snapshot' to generate screenshots."
          )
        end
      end
      ...
      File.write(
        File.join(path, snapshot_helper_filename),
        File.read("#{Snapshot::ROOT}/lib/assets/#{snapshot_helper_filename}")
      )

      puts(
        "✅  Successfully created #{snapshot_helper_filename} '#{File.join(
          path,
          snapshot_helper_filename
        )}'"
          .green
      )
    ...
    def self.print_instructions(
      snapshot_helper_filename: nil, snapfile_path: nil
    )
      ...
      puts(
        "2) Add the ./fastlane/#{snapshot_helper_filename} to your UI Test target"
          .yellow
      )
      ...
      puts(
        '5) Add a new Xcode scheme for the newly created UITest target'.yellow
      )
      puts(
