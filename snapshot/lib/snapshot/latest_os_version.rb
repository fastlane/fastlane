...
module Snapshot
  class LatestOsVersion
    def self.ios_version
      if FastlaneCore::Env.truthy?('SNAPSHOT_IOS_VERSION')
        return ENV['SNAPSHOT_IOS_VERSION']
      end
      self.version('iOS')
    end
    ...
    def self.version_for_os(os)
      ...
      if matched.nil?
        FastlaneCore::UI.user_error!(
          "Could not determine installed #{os} SDK version. Try running the _xcodebuild_ command manually to ensure it works."
        )
      elsif matched.length > 1
        return matched[1]
      else
        FastlaneCore::UI.user_error!(
          "Could not determine installed #{os} SDK version. Please pass it via the environment variable 'SNAPSHOT_IOS_VERSION'"
        )
      end
    end
  end
