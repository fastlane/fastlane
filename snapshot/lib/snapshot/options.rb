...
module Snapshot
  class Options
    def self.available_options
      output_directory =
        (File.directory?('fastlane') ? 'fastlane/screenshots' : 'screenshots')
      ...
      @options ||=
        [
            ...
            verify_block:
              proc do |value|
                ...
                unless File.directory?(v)
                  UI.user_error!('Workspace file invalid')
                end
            ...
            verify_block:
              proc do |value|
                ...
                value.each do |current|
                  ...
                  unless available.any? { |d| d.name.strip == device } ||
                         device == 'Mac'
                    UI.user_error!(
                      "Device '#{device}' not in list of available simulators '#{available
                        .join(', ')}'"
                    )
                  end
                end
          ...
          # optional true because we offer a picker to the user
          FastlaneCore::ConfigItem
            .new(
