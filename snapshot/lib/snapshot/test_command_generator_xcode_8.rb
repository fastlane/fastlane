  ...
  class TestCommandGeneratorXcode8 < TestCommandGeneratorBase
    class << self
      ...
      def pipe(device_type, language, locale)
        ...
        return [
          "| tee #{log_path.shellescape} | xcpretty #{Snapshot.config[
            :xcpretty_args
          ]}"
        ]
      end
      ...
      def destination(device_name)
        ...
        os_version =
          Snapshot.config[:ios_version] || Snapshot::LatestOsVersion.version(os)

        device = find_device(device_name, os_version)
        if device.nil?
          UI.user_error!(
            "No device found named '#{device_name}' for version '#{os_version}'"
          )
        elsif device.os_version != os_version
          UI.important(
            "Using device named '#{device_name}' with version '#{device
              .os_version}' because no match was found for version '#{os_version}'"
          )
        end
        value =
          "platform=#{os} Simulator,id=#{device.udid},OS=#{device.os_version}"
