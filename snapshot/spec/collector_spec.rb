  ...
  describe Snapshot::Collector do
    describe '#attachments_in_file for Xcode 7.2.1 plist output' do
      it 'finds attachments and returns filenames' do
        ...
        expect(
          Snapshot::Collector.attachments_in_file(
            'snapshot/spec/fixtures/Xcode-7_2_1-TestSummaries.plist'
          )
        ).to contain_exactly(*expected_files)
      end
    end
    ...
    describe '#attachments_in_file for Xcode 7.3 plist output' do
      it 'finds attachments and returns filenames' do
        ...
        expect(
          Snapshot::Collector.attachments_in_file(
            'snapshot/spec/fixtures/Xcode-7_3-TestSummaries.plist'
          )
        ).to contain_exactly(*expected_files)
      end
    end
