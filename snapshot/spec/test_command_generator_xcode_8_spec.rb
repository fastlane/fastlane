describe Snapshot do
  describe Snapshot::TestCommandGeneratorXcode8 do
    ...
    before do
      allow(Snapshot::LatestOsVersion).to receive(:version).and_return(
                 os_version
               )
    ...
    describe '#destination' do
      it 'returns the highest version available for device if no match for the specified/latest os_version' do
        ...
        expect(result).to eq(
                    ["-destination 'platform=iOS Simulator,id=4444,OS=9.0'"]
                  )
      end
    end
    ...
    describe 'copy_simulator_logs' do
      ...
      it 'copies all device log archives to the output directory on macOS 10.12 (Siera)',
         requires_xcode: true do
        Snapshot.config = @config
        launcher_config =
          Snapshot::SimulatorLauncherConfiguration.new(
        ...
        expect(FastlaneCore::CommandExecutor).to receive(:execute).with(
                    command:
                      'xcrun simctl spawn 33333 log collect --output /tmp/scan_results/de-DE/system_logs-cfcd208495d565ef66e7dff9f98764da.logarchive 2>/dev/null',
                    print_all: false,
                    print_command: true
                  )
        ...
        expect(FastlaneCore::CommandExecutor).to receive(:execute).with(
                    command:
                      'xcrun simctl spawn 98765 log collect --output /tmp/scan_results/en-US/system_logs-cfcd208495d565ef66e7dff9f98764da.logarchive 2>/dev/null',
                    print_all: false,
                    print_command: true
                  )

        Snapshot::SimulatorLauncherXcode8.new(
          launcher_configuration: launcher_config
        )
          .copy_simulator_logs(['iPhone 6 (10.1)'], 'de-DE', nil, 0)
        Snapshot::SimulatorLauncherXcode8.new(
          launcher_configuration: launcher_config
        )
          .copy_simulator_logs(['iPhone 6s (10.1)'], 'en-US', nil, 0)
      end

      it 'copies all iOS 9 device log files to the output directory on macOS 10.12 (Sierra)',
         requires_xcode: true do
        Snapshot.config = @config
        launcher_config =
          Snapshot::SimulatorLauncherConfiguration.new(
        ...
        expect(FileUtils).to receive(:rm_f).with(
                    %r{#{Snapshot.config[
                      :output_directory
                    ]}/de-DE/system-cfcd208495d565ef66e7dff9f98764da\.log}
                  )
                    .and_return(true)
        ...
        expect(FileUtils).to receive(:rm_f).with(
                    %r{#{Snapshot.config[
                      :output_directory
                    ]}/en-US/system-cfcd208495d565ef66e7dff9f98764da\.log}
                  )
                    .and_return(true)
        expect(FileUtils).to receive(:cp).with(
                    /.*/,
        ...
        Snapshot::SimulatorLauncherXcode8.new(
          ...
          .copy_simulator_logs(['iPhone 6s'], 'de-DE', nil, 0)
        Snapshot::SimulatorLauncherXcode8.new(
          launcher_configuration: launcher_config
        )
          .copy_simulator_logs(['iPhone 6'], 'en-US', nil, 0)
      end

      it 'copies all device log files to the output directory on macOS 10.11 (El Capitan)',
         requires_xcode: true do
        Snapshot.config = @config
        launcher_config =
          Snapshot::SimulatorLauncherConfiguration.new(
        ...
        Snapshot::SimulatorLauncherXcode8.new(
          ...
          .copy_simulator_logs(['iPad Air'], 'en-US', nil, 0)
      end
    end
    ...
    describe 'Valid iOS Configuration' do
      ...
      context 'default options' do
        it 'uses the default parameters', requires_xcode: true do
          configure(options)
          expect(Dir).to receive(:mktmpdir).with('snapshot_derived').and_return(
                      '/tmp/path/to/snapshot_derived'
                    )
          command =
            Snapshot::TestCommandGeneratorXcode8.generate(
              device_type: 'iPhone 6', language: 'en', locale: nil
            )
          ...
          expect(command).to eq(
                      [
                        ...
                        :build,
                        :test,
                        "| tee #{File.expand_path(
                          "#{FastlaneCore::Helper
                            .buildlog_path}/snapshot/Example-ExampleUITests-iPhone\\ 6-en.log"
                        )} | xcpretty "
                      ]
                    )
        end
        ...
        it 'allows to supply custom xcargs', requires_xcode: true do
          ...
          expect(command).to eq(
                      [
                        ...
                        :test,
                        "| tee #{File.expand_path(
                          '~/Library/Logs/snapshot/Example-ExampleUITests-iPhone\\ 6-en.log'
                        )} | xcpretty "
                      ]
                    )
        end
        ...
        it 'uses the default parameters on tvOS too', requires_xcode: true do
          ...
          command =
            Snapshot::TestCommandGeneratorXcode8.generate(
              device_type: 'Apple TV 1080p', language: 'en', locale: nil
            )
          id = command.join('').match(/id=(.+?),/)[1]
          ...
          expect(command).to eq(
                      [
                        ...
                        "| tee #{File.expand_path(
                          "#{FastlaneCore::Helper
                            .buildlog_path}/snapshot/Example-ExampleUITests-Apple\\ TV\\ 1080p-en.log"
                        )} | xcpretty "
                      ]
                    )
        end
      ...
      context 'fixed derivedDataPath' do
        ...
        it 'uses the fixed derivedDataPath if given', requires_xcode: true do
          ...
          expect(command.join('')).to include(
    ...
    describe 'Valid macOS Configuration' do
      ...
      it 'uses default parameters on macOS', requires_xcode: true do
        ...
        command =
          Snapshot::TestCommandGeneratorXcode8.generate(
            device_type: 'Mac', language: 'en', locale: nil
          )
        expect(command).to eq(
                    [
                      'set -o pipefail &&',
                      ...
                      "| tee #{File.expand_path(
                        "#{FastlaneCore::Helper
                          .buildlog_path}/snapshot/ExampleMacOS-ExampleMacOS-Mac-en.log"
                      )} | xcpretty "
                    ]
                  )
      end
    ...
    describe 'Unique logs' do
      ...
      it 'uses correct name and language', requires_xcode: true do
        ...
        log_path =
          Snapshot::TestCommandGeneratorXcode8.xcodebuild_log_path(
            device_type: 'iPhone 6', language: 'pt', locale: nil
          )
        expect(log_path).to eq(
                    File.expand_path(
                      "#{FastlaneCore::Helper
                        .buildlog_path}/snapshot/Example-ExampleUITests-iPhone 6-pt.log"
                    )
                      .to_s
                  )
      end
      ...
      it 'uses includes locale if specified', requires_xcode: true do
        ...
        log_path =
          Snapshot::TestCommandGeneratorXcode8.xcodebuild_log_path(
            device_type: 'iPhone 6', language: 'pt', locale: 'pt_BR'
          )
        expect(log_path).to eq(
                    File.expand_path(
                      "#{FastlaneCore::Helper
                        .buildlog_path}/snapshot/Example-ExampleUITests-iPhone 6-pt-pt_BR.log"
                    )
                      .to_s
                  )
      end
      ...
      it 'can work without parameters', requires_xcode: true do
        ...
        expect(log_path).to eq(
                    File.expand_path(
                      "#{FastlaneCore::Helper
                        .buildlog_path}/snapshot/Example-ExampleUITests.log"
                    )
                      .to_s
                  )
      end
