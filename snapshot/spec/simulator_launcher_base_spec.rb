describe Snapshot do
  describe Snapshot::SimulatorLauncherBase do
    ...
    let(:paths) { %w[./logo.png] }

    describe '#add_media' do
      it 'should call simctl addmedia' do
        allow(Snapshot::TestCommandGenerator).to receive(:device_udid)
                   .and_return(device_udid)
        ...
        expect(Fastlane::Helper).to receive(:backticks).with(
                    "xcrun simctl addmedia #{device_udid} #{paths.join(
                      ' '
                    )} &> /dev/null"
                  )
                    .and_return('')
        ...
        expect(Fastlane::Helper).to receive(:backticks).with(anything)
                    .and_return(anything)
                    .exactly(0)
                    .times

        launcher = Snapshot::SimulatorLauncherBase.new
        launcher.add_media(%w[phone], 'photo', paths)
      end
