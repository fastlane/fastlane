...
def colorize_rows(rows)
  rows.map do |row|
    should_row_be_red?(row) ? row.map { |item| item.to_s.red } : row.map(&:to_s)
  end
end
...
desc(
  'Print stats about how much time has passed and work has happened since the last release of each tool'
)
task(:release_stats) do
  ...
  GEMS.each do |repo|
    Dir.chdir(repo) do
      ...
      commit_count =
        `
          git log
          #{last_tag_name.shellescape}
          ...HEAD --no-merges --oneline .
        `.chomp
          .split("\n")
          .count
      commit_time =
        DateTime.parse(`git show -s --format=%ci #{last_tag_sha}`.chomp).to_time
  ...
  puts(
    Terminal::Table.new(
      title: 'How Long Since the Last Release?'.green,
      headings: %w[Tool Commits Days],
      rows: rows
    )
  )
end
