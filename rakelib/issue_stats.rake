...
def get_all(conn, url, params)
  ...
  loop do
    ...
    next_url_match =
      (resp.headers['Link'] || '').match(/\<(.+)\>; rel="next"/) || []
...
def colorized_row(row)
  ...
  colorized_row[3] =
    colorized_row[3].to_s.send(colorized_row[3] > 0 ? :red : :green)
  colorized_row
end
...
task(:issue_stats) do
  ...
  if GITHUB_TOKEN.to_s.empty?
    raise 'Please set GITHUB_SCRIPT_TOKEN in your environment with a GitHub personal access token value'
            .red
  end
  ...
  all_issues_since =
    get_all(
      conn,
      '/repos/fastlane/fastlane/issues',
      state: 'all',
      sort: 'update',
      direction: 'desc',
      per_page: 100,
  ...
  all_issues_since.reject! do |issue|
    FASTLANE_MEMBERS.include?(issue['user']['login'])
  end
  ...
  issue_counts_by_label =
    issues_by_labels.map do |label, issues|
      return [label, 0, 0, 0] unless issues

      opened_issues =
        issues.select do |issue|
          convert_time(issue, 'created_at') > QUERY_START_TIME
        end.count
      closed_issues =
        issues.select do |issue|
          issue['closed_at'] &&
            (convert_time(issue, 'closed_at') > QUERY_START_TIME)
        end.count
  ...
  sorted_issues =
    issue_counts_by_label.sort { |row_1, row_2| row_1[3] <=> row_2[3] }.reverse
  # rubocop: enable Performance/CompareWithBlock
  pretty_issues = sorted_issues.map { |issue_row| colorized_row(issue_row) }

  totals =
    issue_counts_by_label.each_with_object(
      ['total', 0, 0, 0]
    ) { |row, total_row| (1..3).each { |i| total_row[i] += row[i] } }

  table =
    Terminal::Table.new(
      title: "Issue statistics over the past #{QUERY_DAYS} days".green,
