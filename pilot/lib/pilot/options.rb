    ...
    def self.available_options
      ...
      [
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless %w[ios appletvos osx].include?(value)
                UI.user_error!(
          ...
          verify_block:
            proc do |value|
              ...
              unless File.exist?(value)
                UI.user_error!("Could not find ipa file at path '#{value}'")
              end
          ...
          verify_block:
            proc do |values|
              ...
              values.keys.each do |value|
                unless valid_keys.include?(value.to_s)
                  UI.user_error!("Invalid key '#{value}'")
                end
              end
            end
        ),
        ...
        FastlaneCore::ConfigItem.new(
          ...
          description:
            "Don't wait for the build to process. If set to true, the changelog won't be set, `distribute_external` option won't work and no build will be distributed to testers. (You might want to use this option if you are using this action on CI and have to pay for 'minutes used' on your CI plan)",
          is_string: false,
          default_value: false
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless value.include?('@')
                UI.user_error!('Please pass a valid email address')
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          ...
          description:
            'Use version info from uploaded ipa file to determine what build to use for distribution. If set to false, latest processing or any latest build will be used',
          is_string: false,
          default_value: false
        ),
        FastlaneCore::ConfigItem.new(
