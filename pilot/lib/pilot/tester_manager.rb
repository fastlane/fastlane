...
module Pilot
  class TesterManager < Manager
    def add_tester(options)
      ...
      unless app
        UI.user_error!(
          'You must provide either a Apple ID for the app (with the `:apple_id` option) or app identifier (with the `:app_identifier` option)'
        )
      end
      ...
      unless groups_param
        UI.user_error!(
          'You must provide 1 or more groups (with the `:groups` option)'
        )
      end
      ...
      begin
        ...
        groups =
          Spaceship::TestFlight::Group.add_tester_to_groups!(
            tester: tester, app: app, groups: config[:groups]
          )
        group_names = groups.map(&:name).join(', ')
        UI.success(
          "Successfully added tester to group(s): #{group_names} in app: #{app
            .name}"
        )
      rescue => ex
    ...
    def remove_tester(options)
      ...
      unless app
        tester.delete!
        UI.success(
          "Successfully removed tester #{tester.email} from Users and Roles"
        )
        return
      end
      ...
      begin
        ...
        if config[:groups].nil?
          test_flight_testers =
            Spaceship::TestFlight::Tester.search(
              app_id: app.apple_id,
              text: tester.email,
              is_email_exact_match: true
            )

          if test_flight_testers.length > 1
            UI.user_error!(
              "Could not remove #{tester.email} from app: #{app
                .name}, reason: too many matches: #{test_flight_testers}"
            )
          elsif test_flight_testers.length == 0
            UI.user_error!(
              "Could not remove #{tester.email} from app: #{app
                .name}, reason: unable to find tester on app"
            )
          end
          test_flight_tester = test_flight_testers.first
          test_flight_tester.remove_from_app!(app_id: app.apple_id)
          UI.success(
            "Successfully removed tester, #{test_flight_tester
              .email}, from app: #{app.name}"
          )
        else
          groups =
            Spaceship::TestFlight::Group.remove_tester_from_groups!(
              tester: tester, app: app, groups: config[:groups]
            )
          group_names = groups.map(&:name).join(', ')
          UI.success(
            "Successfully removed tester #{tester.email} from app #{app
              .name} in group(s) #{group_names}"
          )
        end
      rescue => ex
    ...
    def find_app_tester(email: nil, app: nil)
      ...
      if current_user.admin?
        tester =
          Spaceship::TestFlight::Tester.find(app_id: app_apple_id, email: email)
      elsif current_user.app_manager?
        unless app_apple_id
          UI.user_error!(
            "Account #{current_user
              .email_address} is only an 'App Manager' and therefore you must also define what app this tester (#{email}) should be added to"
          )
        end
        tester =
          Spaceship::TestFlight::Tester.find(app_id: app_apple_id, email: email)
        ...
        UI.user_error!(
          "Account #{current_user
            .email_address} doesn't have a role that is allowed to administer app testers, current roles: #{current_user
            .roles}"
        )
        tester = nil
      end

      UI.success("Found existing tester #{email}") if tester
    ...
    def find_current_user
      ...
      unless current_user
        UI.user_error!(
          "Unable to find a member for AppleID: #{current_user_apple_id}, email: #{current_user_email}"
        )
      end
      return current_user
    end
    ...
    def create_tester(email: nil, first_name: nil, last_name: nil, app: nil)
      ...
      if current_user.admin? || current_user.app_manager?
        ...
        return Spaceship::TestFlight::Tester.find(
          app_id: app.apple_id, email: email
        )
      else
        UI.user_error!(
          "Current account doesn't have permission to create a tester"
        )
      end
    rescue => ex
    ...
    def describe_tester(tester)
      ...
      if tester.latest_installed_date
        rows <<
          [
            'Latest Version',
            "#{tester.latest_install_info[
              'latestInstalledShortVersion'
            ]} (#{tester.latest_install_info['latestInstalledVersion']})"
          ]
