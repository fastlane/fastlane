describe Pilot::Options do
  before(:each) { ENV.delete('FASTLANE_TEAM_ID') }

  after(:each) { ENV.delete('FASTLANE_TEAM_ID') }

  it 'accepts a developer portal team ID' do
    FastlaneCore::Configuration.create(
      Pilot::Options.available_options,
      { dev_portal_team_id: 'ABCD1234' }
    )
    ...
    it 'accepts valid values' do
      options = {
        beta_app_review_info: {
          ...
          notes:
            'this is review note for the reviewer <3 thank you for reviewing'
        }
      }
      expect do
        FastlaneCore::Configuration.create(
  ...
  context 'localized_build_info' do
    it 'accepts valid values' do
      options = {
        localized_build_info: {
          'default' => { whats_new: 'Default changelog' },
          'en-GB' => { whats_new: 'en-gb changelog' }
        }
      }
      expect do
        FastlaneCore::Configuration.create(
    ...
    it 'throws errors for invalid keys' do
      options = {
        localized_build_info: {
          'default' => { whats_new: 'Default changelog' },
          'en-GB' => { whats_new: 'en-gb changelog', buffalo_chicken: 'pizza' }
        }
      }
      expect do
        FastlaneCore::Configuration.create(
