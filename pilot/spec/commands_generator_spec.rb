...
describe Pilot::CommandsGenerator do
  ...
  {
    upload: :upload, distribute: :distribute, builds: :list
    ...
    describe ":#{command} option handling" do
      ...
      it 'can use the app_identifier flag from tool options' do
        stub_commander_runner_args(
          [command.to_s, '--app_identifier', 'your.awesome.App']
        )
  ...
  def expect_tester_manager_call_with(method_sym, expected_options_sets)
    unless expected_options_sets.kind_of?(Array)
      expected_options_sets = [expected_options_sets]
    end

    fake_tester_manager = 'tester_manager'
    expect(Pilot::TesterManager).to receive(:new).and_return(
                fake_tester_manager
              )
  ...
  describe ':list option handling' do
    it 'can use the username short flag from tool options' do
      ...
      stub_commander_runner_args(%w[list -u me@it.com])
    ...
    it 'can use the app_identifier flag from tool options' do
      stub_commander_runner_args(%w[list --app_identifier your.awesome.App])
  ...
  {
    add: :add_tester, remove: :remove_tester, find: :find_tester
    ...
    describe ":#{command} option handling" do
      ...
      it 'can provide multiple emails as args' do
        stub_commander_runner_args(
          [command.to_s, 'you@that.com', 'another@that.com']
        )
        ...
        expected_options2 =
          FastlaneCore::Configuration.create(
            available_options,
            { email: 'another@that.com' }
          )
        expect_tester_manager_call_with(
          tester_manager_method,
          [expected_options1, expected_options2]
        )
  ...
  describe ':export option handling' do
    def expect_tester_exporter_export_testers_with(expected_options)
      ...
      expect(Pilot::TesterExporter).to receive(:new).and_return(
                  fake_tester_exporter
                )
      expect(fake_tester_exporter).to receive(
                  :export_testers
                ) do |actual_options|
        expect(actual_options._values).to eq(expected_options._values)
      end
    ...
    it 'can use the testers_file_path short flag from tool options' do
      stub_commander_runner_args(%w[export -c file/path])

      expected_options =
        FastlaneCore::Configuration.create(
          available_options,
          { testers_file_path: 'file/path' }
        )
    ...
    it 'can use the app_identifier flag from tool options' do
      stub_commander_runner_args(%w[export --app_identifier your.awesome.App])
  ...
  describe ':import option handling' do
    def expect_tester_importer_import_testers_with(expected_options)
      ...
      expect(Pilot::TesterImporter).to receive(:new).and_return(
                  fake_tester_importer
                )
      expect(fake_tester_importer).to receive(
                  :import_testers
                ) do |actual_options|
        expect(actual_options._values).to eq(expected_options._values)
      end
    ...
    it 'can use the testers_file_path short flag from tool options' do
      stub_commander_runner_args(%w[import -c file/path])
    ...
    it 'can use the app_identifier flag from tool options' do
      stub_commander_runner_args(%w[import --app_identifier your.awesome.App])
