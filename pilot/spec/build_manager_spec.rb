describe 'Build Manager' do
  describe '.truncate_changelog' do
    it 'Truncates Changelog' do
      changelog =
        File.read('./pilot/spec/fixtures/build_manager/changelog_long')
      changelog = Pilot::BuildManager.truncate_changelog(changelog)
      expect(changelog).to eq(
                  File.read(
                    './pilot/spec/fixtures/build_manager/changelog_long_truncated'
                  )
                )
    end
  ...
  describe 'distribute submits the build for review' do
    ...
    let(:ready_to_submit_mock_build) do
      Spaceship::TestFlight::Build.new(
        ...
        'appAdamId' => 1,
        'externalState' =>
          Spaceship::TestFlight::Build::BUILD_STATES[:ready_to_submit],
        'exportCompliance' => {
          'usesEncryption' => true, 'encryptionUpdated' => false
        },
        'betaReviewInfo' => {
    ...
    let(:approved_mock_build) do
      Spaceship::TestFlight::Build.new(
        ...
        'appAdamId' => 1,
        'externalState' =>
          Spaceship::TestFlight::Build::BUILD_STATES[:approved],
        'exportCompliance' => {
          'usesEncryption' => true, 'encryptionUpdated' => false
        },
        'betaReviewInfo' => {
    ...
    let(:mock_api_client_builds_with_nil_encryption) { [{ 'id' => '123' }] }
    let(:mock_api_client_build_beta_details) { [{ 'id' => '321' }] }
    ...
    describe 'distribute failures' do
      before(:each) do
        ...
        allow(mock_base_client).to receive(:get_build).and_return(
                   ready_to_submit_mock_build
                 )
        allow(mock_base_client).to receive(:add_group_to_build)
        allow(Spaceship::TestFlight::Group).to receive(:default_external_group)
                   .and_return(mock_default_external_group)

        allow(Spaceship::ConnectAPI::Base).to receive(:client).and_return(
                   mock_base_api_client
                 )
        allow(mock_base_api_client).to receive(:get_builds).and_return(
                   mock_api_client_builds
                 )
        allow(mock_base_api_client).to receive(:patch_beta_app_review_detail)
                   .and_return(mock_api_client_builds)
        allow(mock_base_api_client).to receive(:get_build_beta_details)
                   .and_return(mock_api_client_build_beta_details)
        allow(mock_base_api_client).to receive(:get_beta_app_localizations)
      ...
      it 'doesnt recover if there is a 504 and the build is not approved' do
        allow(mock_base_api_client).to receive(
                   :post_beta_app_review_submissions
                 )
                   .and_raise(
                   Spaceship::Client::InternalServerError,
                   'Server error got 504'
                 )
        allow(Spaceship::TestFlight::Build).to receive(:find).and_return(
                   ready_to_submit_mock_build
                 )
        expect(FastlaneCore::UI).to receive(:message).with(
                    'Distributing new build to testers:  - '
                  )
        expect(FastlaneCore::UI).to receive(:message).with(
                    'Submitting the build for review timed out, trying to recover.'
                  )
        expect do
          fake_build_manager.distribute(
            distribute_options,
            build: ready_to_submit_mock_build
          )
        end.to raise_error(
                    Spaceship::Client::InternalServerError,
                    'Server error got 504'
    ...
    describe 'distribute success' do
      ...
      it 'updates non-localized  demo_account_required, notify_external_testers, beta_app_feedback_email, and beta_app_description' do
        ...
        expect(mock_base_api_client).to receive(:get_builds).with(
                    {
                      filter: {
                        ...
                        version: ready_to_submit_mock_build.build_version,
                        app: ready_to_submit_mock_build.app_id
                      }
                    }
                  )
                    .and_return(builds)
        ...
        # Demo account
        expect(mock_base_api_client).to receive(:patch_beta_app_review_detail)
                    .with(
                    {
                      app_id: ready_to_submit_mock_build.app_id,
                      attributes: {
                        demoAccountRequired: options[:demo_account_required]
                      }
                    }
                  )
        ...
        expect(mock_base_api_client).to receive(:patch_build_beta_details).with(
                    {
                      build_beta_details_id:
                        mock_api_client_build_beta_details.first['id'],
                      attributes: {
                        autoNotifyEnabled: options[:notify_external_testers]
                      }
                    }
                  )

        # Feedback email and marketing url set for all localizations
        expect(mock_base_api_client).to receive(:get_beta_app_localizations)
                    .with(
                    { filter: { app: ready_to_submit_mock_build.app_id } }
                  )
                    .and_return(mock_api_client_beta_app_localizations)
        mock_api_client_beta_app_localizations.each do |localization|
          expect(mock_base_api_client).to receive(:patch_beta_app_localizations)
                      .with(
                      {
        ...
        expect(FastlaneCore::UI).to receive(:success).with(
        ...
        expect(mock_base_api_client).to receive(:patch_builds).with(
                    ...
                    .and_return(mock_api_client_beta_app_localizations)
        expect(fake_build_manager).to receive(
                    :wait_for_build_processing_to_be_complete
                  )

        expect(FastlaneCore::UI).to receive(:success).with(
                    /Successfully distributed build to/
                  )
