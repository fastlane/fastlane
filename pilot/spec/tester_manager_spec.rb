...
describe Pilot::TesterManager do
  describe 'Manages adding/removing/displaying testers' do
    ...
    before(:each) do
      ...
      allow(fake_client).to receive(:user_email).and_return(
                 'taquitos@fastlane.tools'
               )
    ...
    describe 'when invoked from the context of an app' do
      it 'prints a table without columns showing device and version info' do
        allow(Spaceship::TestFlight::Tester).to receive(:all).and_return(
                   app_context_testers
                 )

        headings = %w[First Last Email Groups]
        rows =
          app_context_testers.map do |tester|
            [
              tester.first_name,
              tester.last_name,
              tester.email,
    ...
    describe 'when admin asks to create new tester to a specific existing custom group' do
      it 'creates a new tester and adds it to the default group' do
        allow(current_user).to receive(:roles).and_return(%w[admin])
        allow(tester_manager).to receive(:find_app_tester).and_return(
                   fake_tester
                 )
        ...
        expect(Spaceship::TestFlight::Group).to receive(:add_tester_to_groups!)
                    .and_return([custom_tester_group])
        expect(FastlaneCore::UI).to receive(:success).with(
                    'Successfully added tester to group(s): Test Group in app: My Fake App'
                  )
    ...
    describe 'when asked to add an existing external tester to a specific existing custom group' do
      it 'adds the tester to the custom group' do
        allow(current_user).to receive(:roles).and_return(%w[appmanager])
        allow(tester_manager).to receive(:find_app_tester).and_return(
                   fake_tester
                 )

        expect(Spaceship::TestFlight::Tester).to_not(
          receive(:create_app_level_tester)
        )
        expect(Spaceship::TestFlight::Group).to receive(:add_tester_to_groups!)
                    .and_return([custom_tester_group])
        expect(FastlaneCore::UI).to receive(:success).with(
    ...
    describe 'when asked to add an existing internal tester to a specific existing custom group' do
      it 'adds the tester to the custom group' do
        ...
        expect(Spaceship::TestFlight::Tester).to receive(:find).and_return(
                    fake_tester
                  )
        expect(Spaceship::TestFlight::Tester).to_not(
          receive(:create_app_level_tester)
        ...
        expect(Spaceship::TestFlight::Group).to receive(:add_tester_to_groups!)
                    .and_return([custom_tester_group])
        expect(FastlaneCore::UI).to receive(:success).with(
                    'Found existing tester fabric-devtools@gmail.com+fake@gmail.com'
                  )
        expect(FastlaneCore::UI).to receive(:success).with(
                    'Successfully added tester to group(s): Test Group in app: My Fake App'
                  )
    ...
    describe 'when external tester is removed without providing app' do
      it 'removes the tester without error' do
        ...
        expect(Spaceship::TestFlight::Group).to_not(
          receive(:remove_tester_from_groups!)
        )
        expect(FastlaneCore::UI).to receive(:success).with(
                    'Found existing tester fabric-devtools@gmail.com+fake@gmail.com'
                  )
        expect(FastlaneCore::UI).to receive(:success).with(
                    'Successfully removed tester fabric-devtools@gmail.com+fake@gmail.com from Users and Roles'
                  )
