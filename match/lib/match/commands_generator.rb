    ...
    def run
        ...
        c.action do |args, options|
          if args.count > 0
            FastlaneCore::UI.user_error!(
              'Please run `fastlane match [type]`, allowed values: development, adhoc, enterprise  or appstore'
            )
          end
      ...
      command :init do |c|
        ...
        c.action do |args, options|
          ...
          if File.exist?(path)
            FastlaneCore::UI.user_error!(
              "You already have a Matchfile in this directory (#{path})"
            )
            return 0
          end
      ...
      command :change_password do |c|
        ...
        c.action do |args, options|
          ...
          UI.success(
            'Successfully changed the password. Make sure to update the password on all your clients and servers by running `fastlane match [environment]`'
          )
        end
      end
      ...
      command :migrate do |c|
        ...
        c.action { |args, options| Match::Migrate.new.migrate(args, options) }
      end
      ...
      %w[development distribution enterprise].each do |type|
        command "nuke #{type}" do |c|
          c.syntax = "fastlane match nuke #{type}"
          c.description =
            "Delete all certificates and provisioning profiles from the Apple Dev Portal of the type #{type}"
