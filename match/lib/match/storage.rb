    ...
    class << self
      def backends
        @backends ||=
          {
            'git' =>
              lambda { |params| return Storage::GitStorage.configure(params) },
            'google_cloud' =>
              lambda do |params|
      ...
      def register_backend(type: nil, storage_class: nil, &configurator)
        ...
        if configurator
          ...
          @backends[normalized_name] =
            ->(params) { return storage_class.configure(params) }
        else
          UI.user_error!(
            'Specify either a `storage_class` or a configuration block when registering a storage backend'
          )
        end
      end
