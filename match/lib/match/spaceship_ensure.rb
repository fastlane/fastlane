  ...
  class SpaceshipEnsure
    def initialize(user, team_id, team_name)
      ...
      if keychain_entry.password(ask_if_missing: false).to_s.length == 0
        UI.important(
          'You can also run `fastlane match` in readonly mode to not require any access to the'
        )
        UI.important(
          'Developer Portal. This way you only share the keys and credentials'
        )
        UI.command('fastlane match --readonly')
        UI.important(
          'More information https://docs.fastlane.tools/actions/match/#access-control'
        )
      end
    ...
    def bundle_identifier_exists(username: nil, app_identifier: nil)
      ...
      Sigh::Runner.new.print_produce_command(
        { username: username, app_identifier: app_identifier }
      )
      UI.error(
      ...
      available_apps =
        Spaceship.app.all.collect { |a| "#{a.bundle_id} (#{a.name})" }
      UI.message("Available apps:\n- #{available_apps.join("\n- ")}")
      UI.error(
        'Make sure to run `fastlane match` with the same user and team every time.'
      )
      UI.user_error!(
        "Couldn't find bundle identifier '#{app_identifier}' for the user '#{username}'"
      )
    end

    def certificate_exists(username: nil, certificate_id: nil)
      found =
        Spaceship.certificate.all.find { |cert| cert.id == certificate_id }
      return if found

      UI.error(
        "Certificate '#{certificate_id}' (stored in your storage) is not available on the Developer Portal"
      )
      UI.error("for the user #{username}")
      UI.error(
        "Make sure to use the same user and team every time you run 'match' for this"
      )
      UI.error(
        'Git repository. This might be caused by revoking the certificate on the Dev Portal'
      )
      UI.user_error!(
    ...
    def profile_exists(username: nil, uuid: nil)
      ...
      unless found
        UI.error(
          "Provisioning profile '#{uuid}' is not available on the Developer Portal for the user #{username}, fixing this now for you 🔨"
        )
        return false
      end
      ...
      if found.valid?
        ...
        UI.important(
          "'#{found
            .name}' is available on the Developer Portal, however it's 'Invalid', fixing this now for you 🔨"
        )
