...
module Match
  class Nuke
    ...
    def run(params, type: nil)
      ...
      if (self.certs + self.profiles + self.files).count > 0
        unless params[:skip_confirmation]
          UI.error('---')
          UI.error(
            'Are you sure you want to completely delete and revoke all the'
          )
          UI.error('certificates and provisioning profiles listed above? (y/n)')
          if type == 'distribution'
            UI.error(
              'Warning: By nuking distribution, both App Store and Ad Hoc profiles will be deleted'
            )
          end
          if had_app_identifier
            UI.error(
              'Warning: The :app_identifier value will be ignored - this will delete all profiles for all your apps!'
            )
          end
          UI.error('---')
        end
        if params[:skip_confirmation] ||
           UI.confirm('Do you really want to nuke everything listed above?')
          nuke_it_now!
        ...
        UI.success(
          'No relevant certificates or provisioning profiles found, nothing to nuke here :)'
        )
      end
    end
    ...
    def prepare_list
      ...
      prov_types = []
      prov_types = %i[development] if cert_type == :development
      prov_types = %i[appstore adhoc] if cert_type == :distribution
      prov_types = %i[enterprise] if cert_type == :enterprise

      Spaceship.login(params[:username])
      Spaceship.select_team

      if Spaceship.client.in_house? &&
         (type == 'distribution' || type == 'enterprise')
        UI.error('---')
        UI.error('⚠️ Warning: This seems to be an Enterprise account!')
        if type == 'distribution'
          UI.error(
            "By nuking your account's distribution, all your apps deployed via ad-hoc will stop working!"
          )
        end
        if type == 'enterprise'
          UI.error(
            "By nuking your account's enterprise, all your in-house apps will stop working!"
          )
        end
        UI.error('---')

        unless UI.confirm('Do you really want to nuke your Enterprise account?')
          UI.user_error!('Enterprise account nuke cancelled')
        end
      end
      ...
      certs =
        Dir[
          File.join(
            self.storage.working_directory,
            '**',
            cert_type.to_s,
            '*.cer'
          )
    ...
    def print_tables
      ...
      if self.certs.count > 0
        rows =
          self.certs.collect do |cert|
            cert_expiration =
              cert.expires.nil? ? 'Unknown' : cert.expires.strftime('%Y-%m-%d')
            [
              cert.name,
              cert.id,
              cert.class.to_s.split('::').last,
              cert_expiration
            ]
          end
        puts(
          Terminal::Table.new(
            {
              ...
              headings: %w[Name ID Type Expires],
          ...
          Terminal::Table.new(
            {
              ...
              headings: %w[Name ID Status Type Expires],
              rows: FastlaneCore::PrintTable.transform_output(rows)
            }
          )
        )
      ...
      if self.files.count > 0
        ...
        puts(
          Terminal::Table.new(
            {
              title:
                'Files that are going to be deleted'.green + "\n" +
    ...
    def nuke_it_now!
      unless self.profiles.count == 0
        UI.header("Deleting #{self.profiles.count} provisioning profiles...")
      end
      ...
      unless self.certs.count == 0
        UI.header("Revoking #{self.certs.count} certificates...")
      end
