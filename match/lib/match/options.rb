    ...
    def self.available_options
      ...
      [
        ...
        FastlaneCore::ConfigItem
          ...
          env_name: 'MATCH_TYPE',
          description:
            "Define the profile type, can be #{Match.environments.join(', ')}",
          is_string: true,
          short_option: '-y',
          ...
          verify_block:
            proc do |value|
              unless Match.environments.include?(value)
                UI.user_error!(
                  "Unsupported environment #{value}, must be in #{Match
                    .environments
                    .join(', ')}"
                )
              end
          ...
          verify_block:
            proc do |value|
              unless Match.storage_modes.include?(value)
                UI.user_error!(
                  "Unsupported storage_mode #{value}, must be in #{Match
                    .storage_modes
                    .join(', ')}"
                )
              end
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Could not find keys file at path '#{File.expand_path(
                    value
                  )}'"
                )
              end
        ...
        FastlaneCore::ConfigItem
          ...
          default_value: false,
          verify_block:
            proc { |value| FastlaneCore::Globals.verbose = true if value }
        )
      ]
    end
  end
