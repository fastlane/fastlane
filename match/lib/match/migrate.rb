...
module Match
  class Migrate
    def migrate(args, options)
      ...
      UI.message(
        "Detected team ID '#{team_id}' to use for Google Cloud Storage..."
      )
      ...
      Dir.chdir(git_storage.working_directory) do
        Dir[File.join('**', '*')].each do |current_file|
          ...
          to_path =
            File.join(
              google_cloud_storage.working_directory,
              team_id,
              current_file
            )
          ...
          if File.exist?(to_path)
            UI.user_error!(
              "Looks like file already exists on Google Cloud Storage at path '#{to_path}', stopping the migration process. Please make sure the bucket is empty, or at least doesn't contain any files related to the same Team ID"
            )
          end
      ...
      UI.success(
        'Successfully migrated your code signing certificates and provisioning profiles to Google Cloud Storage'
      )
      UI.success(
        'Make sure to update your configuration to specify the `storage_mode`, as well as the bucket to use.'
      )
      UI.message('')
      if loaded_matchfile
        UI.message(
          "Update your Matchfile at path '#{loaded_matchfile.configfile_path}':"
        )
        UI.message('')
        UI.command_output("\t\tstorage_mode \"google_cloud\"")
        UI.command_output(
          "\t\tgoogle_cloud_bucket_name \"#{google_cloud_storage.bucket_name}\""
        )
      else
        ...
        UI.command_output("\t\tstorage_mode: \"google_cloud\",")
        UI.command_output(
          "\t\tgoogle_cloud_bucket_name: \"#{google_cloud_storage
            .bucket_name}\","
        )
      end
      UI.message('')
      UI.success(
        'You can also remove the `git_url`, as well as any other git related configurations from your Fastfile and Matchfile'
      )
      UI.message('')
    ...
    def ensure_parameters_are_valid(params)
      if params[:readonly]
        UI.user_error!(
          "`fastlane match migrate` doesn't work in `readonly` mode"
        )
      end

      if params[:storage_mode] != 'git'
        UI.user_error!(
          "`fastlane match migrate` only allows migration from `git` to `google_cloud` right now, looks like your currently selected `storage_mode` is '#{params[
            :storage_mode
          ]}'"
        )
      end
