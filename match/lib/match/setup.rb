  ...
  class Setup
    def run(path, is_swift_fastfile: false)
      if is_swift_fastfile
        template =
          File.read("#{Match::ROOT}/lib/assets/MatchfileTemplate.swift")
      else
      ...
      if specific_content.nil?
        UI.crash!(
          "Looks like `generate_matchfile_content` was `nil` for `#{storage_mode}`"
        )
      end
      ...
      UI.success(
        "Successfully created '#{path}'. You can open the file using a code editor."
      )
      ...
      UI.message(
        'For more information visit https://docs.fastlane.tools/actions/match/'
      )
    end
