  ...
  class Utils
    def self.import(item_path, keychain, password: '')
      ...
      FastlaneCore::KeychainImporter.import_file(
        item_path,
        keychain_path,
        keychain_password: password, output: FastlaneCore::Globals.verbose?
      )
    end
    ...
    def self.fill_environment(key, value)
      if FastlaneCore::Globals.verbose?
        UI.important("Setting environment variable '#{key}' to '#{value}'")
      end
      ENV[key] = value
    end

    def self.environment_variable_name(
      app_identifier: nil, type: nil, platform: :ios
    )
      base_environment_variable_name(
        app_identifier: app_identifier, type: type, platform: platform
      )
        .join('_')
    end

    def self.environment_variable_name_team_id(
      app_identifier: nil, type: nil, platform: :ios
    )
      (
        base_environment_variable_name(
    ...
    def self.environment_variable_name_profile_name(
      app_identifier: nil, type: nil, platform: :ios
    )
      (
        base_environment_variable_name(
    ...
    def self.environment_variable_name_profile_path(
      app_identifier: nil, type: nil, platform: :ios
    )
      (
        base_environment_variable_name(
    ...
    def self.get_cert_info(cer_certificate_path)
      ...
      return out_array.map { |x| x.split(/=+/) if x.include?('=') }.compact
        .map { |k, v| [openssl_keys_to_readable_keys.fetch(k, k), v] }.push(
        [openssl_keys_to_readable_keys.fetch('notBefore'), cert.not_before]
      )
    ...
    def self.base_environment_variable_name(
      app_identifier: nil, type: nil, platform: :ios
    )
