...
module Match
  ...
  class Generator
    def self.generate_certificate(params, cert_type, working_directory)
      ...
      arguments =
        FastlaneCore::Configuration.create(
          Cert::Options.available_options,
          {
            ...
            force: true,
            username:
              # we don't need a certificate without its private key, we only care about a new certificate
              params[
                :username
              ],
      ...
      begin
        ...
        if ex.to_s.include?('You already have a current')
          UI.user_error!(
            'Could not create a new certificate as you reached the maximum number of certificates for this account. You can use the `fastlane match nuke` command to revoke your existing certificates. More information https://docs.fastlane.tools/actions/match/'
          )
        else
          raise ex
        end
      ...
      Dir[File.join(working_directory, '**', '*.certSigningRequest')]
    ...
    def self.generate_provisioning_profile(
      params: nil,
      prov_type: nil,
      certificate_id: nil,
      app_identifier: nil,
      ...
      names << params[:platform] if params[:platform].to_s != :ios.to_s # For ios we do not include the platform for backwards compatibility
