    ...
    class OpenSSL < Interface
      ...
      def encrypt_files
        ...
        iterate(self.working_directory) do |current|
          ...
          if FastlaneCore::Globals.verbose?
            UI.success("🔒  Encrypted '#{File.basename(current)}'")
          end
        end
        ...
        iterate(self.working_directory) do |current|
          ...
          begin
            ...
            if ENV['MATCH_PASSWORD']
              UI.user_error!("Invalid password passed via 'MATCH_PASSWORD'")
            end
            clear_password
          ...
          if FastlaneCore::Globals.verbose?
            UI.success("🔓  Decrypted '#{File.basename(current)}'")
          end
        end
      ...
      def store_password(password)
        Security::InternetPassword.add(
          server_name(self.keychain_name),
          '',
          password
        )
      end
      ...
      def iterate(source_path)
        Dir[File.join(source_path, '**', '*.{cer,p12,mobileprovision}')]
          .each do |path|
      ...
      def password
        ...
        unless password
          if !UI.interactive?
            UI.error(
              'Neither the MATCH_PASSWORD environment variable nor the local keychain contained a password.'
            )
            UI.error(
              'Bailing out instead of asking for a password, since this is non-interactive mode.'
            )
            UI.user_error!(
              'Try setting the MATCH_PASSWORD environment variable, or temporarily enable interactive mode to store a password.'
            )
          else
            UI.important(
      ...
      def encrypt_specific_file(path: nil, password: nil)
        ...
        encrypted_data =
          'Salted__' + salt + cipher.update(data_to_encrypt) + cipher.final
      ...
      def decrypt_specific_file(path: nil, password: nil, hash_algorithm: 'MD5')
        ...
        if hash_algorithm != fallback_hash_algorithm
          decrypt_specific_file(
            path: path,
            password: password,
            hash_algorithm: fallback_hash_algorithm
          )
        else
          UI.error(error.to_s)
