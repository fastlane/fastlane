describe Match do
  describe Match::Runner do
    ...
    before do
      ...
      allow(ENV).to receive(:[]).with('MATCH_KEYCHAIN_NAME').and_return(
                 keychain
               )
      allow(ENV).to receive(:[]).with('MATCH_KEYCHAIN_PASSWORD').and_return(nil)
    end

    it "creates a new profile and certificate if it doesn't exist yet",
       requires_security: true do
      ...
      destination =
        File.expand_path(
          '~/Library/MobileDevice/Provisioning Profiles/98264c6b-5151-4349-8d0f-66691e48ae35.mobileprovision'
        )
      ...
      expect(Match::Generator).to receive(:generate_certificate).with(
                  config,
                  :distribution,
                  fake_storage.working_directory
                )
                  .and_return(cert_path)
      expect(Match::Generator).to receive(:generate_provisioning_profile).with(
                  params: config,
                  prov_type: :appstore,
      ...
      expect(FastlaneCore::ProvisioningProfile).to receive(:install).with(
                  profile_path,
                  keychain_path
                )
      ...
      expect(fake_storage).to receive(:save_changes!).with(
                  files_to_commit: [
                    ...
                    # this is important, as a cert consists out of 2 files
      ...
      expect(
        ENV[
          Match::Utils.environment_variable_name(
            app_identifier: 'tools.fastlane.app', type: 'appstore'
          )
        ]
      ).to eql('98264c6b-5151-4349-8d0f-66691e48ae35')
      expect(
        ENV[
          Match::Utils.environment_variable_name_team_id(
            app_identifier: 'tools.fastlane.app', type: 'appstore'
          )
        ]
      ).to eql('439BBM9367')
      ...
      ).to eql('tools.fastlane.app AppStore')
      profile_path =
        File.expand_path(
          '~/Library/MobileDevice/Provisioning Profiles/98264c6b-5151-4349-8d0f-66691e48ae35.mobileprovision'
        )
      expect(
        ENV[
          Match::Utils.environment_variable_name_profile_path(
            app_identifier: 'tools.fastlane.app', type: 'appstore'
          )
        ]
      ).to eql(profile_path)
    ...
    it 'uses existing certificates and profiles if they exist',
       requires_security: true do
      git_url = 'https://github.com/fastlane/fastlane/tree/master/certificates'
      values = {
        app_identifier: 'tools.fastlane.app',
      ...
      cert_path =
        './match/spec/fixtures/existing/certs/distribution/E7P4EE896K.cer'
      key_path =
        './match/spec/fixtures/existing/certs/distribution/E7P4EE896K.p12'
      ...
      expect(Match::Encryption::OpenSSL).to receive(:new).with(
                  keychain_name: fake_storage.git_url,
                  working_directory: fake_storage.working_directory
                )
                  .and_return(fake_encryption)
      expect(fake_encryption).to receive(:decrypt_files).and_return(nil)

      expect(Match::Utils).to receive(:import).with(
                  key_path,
                  keychain,
                  password: nil
                )
      ...
      expect(FastlaneCore::CertChecker).to receive(:installed?).with(
                  cert_path,
                  in_keychain: nil
                )
                  .and_return(false)
      ...
      ).to eql('736590c3-dfe8-4c25-b2eb-2404b8e65fb8')
      expect(
        ENV[
          Match::Utils.environment_variable_name_team_id(
            app_identifier: 'tools.fastlane.app', type: 'appstore'
      ...
      ).to eql('match AppStore tools.fastlane.app 1449198835')
      profile_path =
        File.expand_path(
          '~/Library/MobileDevice/Provisioning Profiles/736590c3-dfe8-4c25-b2eb-2404b8e65fb8.mobileprovision'
        )
      expect(
        ENV[
          Match::Utils.environment_variable_name_profile_path(
    ...
    it 'fails because of an outdated certificate', requires_security: true do
      ...
      expect { Match::Runner.new.run(config) }.to raise_error(
                  "Your certificate 'E7P4EE896K.cer' is not valid, please check end date and renew it if necessary"
                )
    end
  end
end
