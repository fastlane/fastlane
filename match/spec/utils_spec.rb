describe Match do
  describe Match::Utils do
    before(:each) do
      allow(FastlaneCore::Helper).to receive(:backticks).with(
    ...
    describe 'import' do
      it 'finds a normal keychain name relative to ~/Library/Keychains' do
        expected_command =
          "security import item.path -k '#{Dir
            .home}/Library/Keychains/login.keychain' -P #{''
            .shellescape} -T /usr/bin/codesign -T /usr/bin/security 1> /dev/null"
        ...
        allowed_command =
          "security set-key-partition-list -S apple-tool:,apple: -k #{''
            .shellescape} #{Dir
            .home}/Library/Keychains/login.keychain 1> /dev/null"

        allow(File).to receive(:file?).and_return(false)
        expect(File).to receive(:file?).with(
                    "#{Dir.home}/Library/Keychains/login.keychain"
                  )
                    .and_return(true)
      ...
      it 'treats a keychain name it cannot find in ~/Library/Keychains as the full keychain path' do
        ...
        expected_command =
          "security import item.path -k '#{keychain}' -P #{''
            .shellescape} -T /usr/bin/codesign -T /usr/bin/security 1> /dev/null"

        # this command is also sent on macOS Sierra and we need to allow it or else the test will fail
        allowed_command =
          "security set-key-partition-list -S apple-tool:,apple: -k #{''
            .shellescape} #{keychain} 1> /dev/null"
      ...
      it 'tries to find the macOS Sierra keychain too' do
        expected_command =
          "security import item.path -k '#{Dir
            .home}/Library/Keychains/login.keychain-db' -P #{''
            .shellescape} -T /usr/bin/codesign -T /usr/bin/security 1> /dev/null"
        ...
        allowed_command =
          "security set-key-partition-list -S apple-tool:,apple: -k #{''
            .shellescape} #{Dir
            .home}/Library/Keychains/login.keychain-db 1> /dev/null"
