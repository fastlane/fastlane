describe Match do
  describe Match::Storage::GitStorage do
    describe '#generate_commit_message' do
      it 'works' do
        storage =
          Match::Storage::GitStorage.new(type: 'appstore', platform: 'ios')
    ...
    describe '#download' do
      it 'clones the repo' do
        ...
        shallow_clone = true
        command =
          "git clone #{git_url.shellescape} #{path
            .shellescape} --depth 1 --no-single-branch"
        to_params = { command: command, print_all: nil, print_command: nil }
        ...
        storage =
          Match::Storage::GitStorage.new(
            git_url: git_url, shallow_clone: shallow_clone
          )
        storage.download

        expect(File.directory?(storage.working_directory)).to eq(true)
        expect(
          File.exist?(File.join(storage.working_directory, 'README.md'))
        ).to eq(false) # because the README is being added when committing the changes
      end
      ...
      it 'checks out a branch' do
        ...
        expected_commands.each do |command|
          expect(FastlaneCore::CommandExecutor).to receive(:execute).once.with(
                      { command: command, print_all: nil, print_command: nil }
                    )
                      .and_return('')
        end
        ...
        storage =
          Match::Storage::GitStorage.new(
            git_url: git_url, shallow_clone: shallow_clone, branch: git_branch
          )
        storage.download
    ...
    describe '#save_changes' do
      it 'skips README file generation if so requested' do
        ...
        expected_commands = [
          ...
          'git commit -m ' +
            '[fastlane] Updated appstore and platform ios'.shellescape,
        ...
        expect(
          File.read(File.join(storage.working_directory, 'match_version.txt'))
        ).to eq(Fastlane::VERSION)
      end
    end
  end
end
