describe Match do
    describe Match::Storage::AWSSecretsManagerStorage do
      ...
      describe '#download' do
        ...
        let(:secrets_list_to_download) { instance_double('Types::ListSecretsResponse', secret_list: secrets_to_download) }
        
        let(:aws_sm_client) { instance_double('Aws::SecretsManager::Client', 
        ...
        it 'downloads to correct working directory' do
            expect(aws_sm_client).to receive(:list_secrets).with({:filters=>[{:key=>"name", :values=>["test_team_id"]}]})
        ...
        it 'downloads secrets and strips the aws_secrets_manager_prefix for working_directory path' do
          ...
          expect(aws_sm_client).to receive(:list_secrets).with({:filters=>[{:key=>"name", :values=>["123456/test_team_id"]}]})
