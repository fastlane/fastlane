describe Match do
  describe Match::Encryption::OpenSSL do
    before do
      ...
      @e =
        Match::Encryption::OpenSSL.new(
          keychain_name: @git_url, working_directory: @directory
        )
    end
    ...
    it 'raises an exception if invalid password is passed' do
      ...
      expect { @e.decrypt_files }.to raise_error(
    ...
    it 'raises an exception if no password is supplied' do
      ENV['MATCH_PASSWORD'] = ''
      expect { @e.encrypt_files }.to raise_error('No password supplied')
    end
  end
end
