...
describe Match::CommandsGenerator do
    ...
    describe ":#{env} option handling" do
      it 'can use the git_url short flag from tool options' do
        stub_commander_runner_args(
          [env, '-r', 'git@github.com:you/your_repo.git']
        )

        expected_options =
          FastlaneCore::Configuration.create(
            available_options,
            { git_url: 'git@github.com:you/your_repo.git', type: env }
          )
  ...
  describe ':change_password option handling' do
    ...
    it 'can use the git_url short flag from tool options' do
      stub_commander_runner_args(
        %w[change_password -r git@github.com:you/your_repo.git]
      )
    ...
    it 'can use the shallow_clone flag from tool options' do
      stub_commander_runner_args(%w[change_password --shallow_clone true])
  ...
  describe ':decrypt option handling' do
    def expect_githelper_clone_with(git_url, shallow_clone, git_branch)
      ...
      allow(fake_storage).to receive(:keychain_name).and_return(
                 'https://github.com/fastlane/certs'
               )
    ...
    it 'can use the git_url short flag from tool options' do
      stub_commander_runner_args(
        %w[decrypt -r git@github.com:you/your_repo.git]
      )

      expect_githelper_clone_with(
        'git@github.com:you/your_repo.git',
        false,
        { branch: 'master', clone_branch_directly: false }
      )
    ...
    it 'can use the shallow_clone flag from tool options' do
      stub_commander_runner_args(
        %w[decrypt -r git@github.com:you/your_repo.git --shallow_clone true]
      )
  ...
  %w[development distribution].each do |type|
    describe "nuke #{type} option handling" do
      it 'can use the git_url short flag from tool options' do
        stub_commander_runner_args(
          ['nuke', type, '-r', 'git@github.com:you/your_repo.git']
        )
