...
describe Deliver::SubmitForReview do
  ...
  def make_fake_builds(number_of_builds)
    (0...number_of_builds).map do |num|
      OpenStruct.new(
        { upload_date: Time.now.utc + 60 * num, processing: false }
      ) # minutes_from_now
    end
  end
  ...
  describe :find_build do
    ...
    context 'no builds' do
      ...
      it 'throws a UI error' do
        expect { review_submitter.find_build(fake_builds) }.to raise_error(
    ...
    describe :wait_for_build do
      context 'no candidates' do
        ...
        before do
          allow(Time).to receive(:now).and_return(time_now, (time_now + 60 * 6))
        end
        it 'throws a UI error' do
          ...
          expect { review_submitter.wait_for_build(fake_app) }.to raise_error(
                      FastlaneCore::Interface::FastlaneError,
                      'Could not find any available candidate builds on App Store Connect to submit'
                    )
        end
