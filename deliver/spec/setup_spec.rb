  ...
  describe Deliver::Setup do
    ...
    describe '#generate_metadata_files' do
      context 'with review_information' do
        ...
        before do
          allow(version).to receive_message_chain('application.details')
                     .and_return(application)
          allow(version).to receive_message_chain('description.languages')
                     .and_return([])
          allow(version).to receive_message_chain('large_app_icon.asset_token')
                     .and_return(nil)
          allow(version).to receive_message_chain('watch_app_icon.asset_token')
                     .and_return(nil)
          stub_const(
            'Deliver::UploadMetadata::NON_LOCALISED_VERSION_VALUES',
            []
          )
          stub_const('Deliver::UploadMetadata::NON_LOCALISED_APP_VALUES', [])
          stub_const(
            'Deliver::UploadMetadata::TRADE_REPRESENTATIVE_CONTACT_INFORMATION_VALUES',
            {}
          )
        end
        ...
        it 'generates review information' do
          ...
          %w[
            ...
            expect(
              File.exist?(File.join(base_dir, "#{filename}.txt"))
            ).to be_truthy
          end
        end

        after { FileUtils.remove_entry_secure(tempdir) }
      end
      context 'with trade_representative_contact_information' do
        ...
        it 'generates trade representative contact information' do
          setup.generate_metadata_files(version, tempdir)
          base_dir =
            File.join(tempdir, 'trade_representative_contact_information')
          %w[
            trade_name
            first_name
            last_name
