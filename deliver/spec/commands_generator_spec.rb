...
describe Deliver::CommandsGenerator do
  ...
  describe ':run option handling' do
    it 'can use the username short flag from tool options' do
      ...
      stub_commander_runner_args(
        ['--description', 'My description', '-u', 'me@it.com']
      )

      expected_options =
        FastlaneCore::Configuration.create(
          Deliver::Options.available_options,
          { description: 'My description', username: 'me@it.com' }
        )
    ...
    it 'can use the app_identifier flag from tool options' do
      # leaving out the command name defaults to 'run'
      stub_commander_runner_args(
        ['--description', 'My description', '--app_identifier', 'abcd']
      )
  ...
  describe ':submit_build option handling' do
    it 'can use the username short flag from tool options' do
      stub_commander_runner_args(
        ['submit_build', '--description', 'My description', '-u', 'me@it.com']
      )
  ...
  describe ':init option handling' do
    ...
    it 'can use the username short flag from tool options' do
      stub_commander_runner_args(
        ['init', '--description', 'My description', '-u', 'me@it.com']
      )
    ...
    it 'can use the app_identifier flag from tool options' do
      stub_commander_runner_args(
        ['init', '--description', 'My description', '--app_identifier', 'abcd']
      )
  ...
  describe ':generate_summary option handling' do
    def expect_generate_summary_run_with(expected_options)
      ...
      expect(Deliver::GenerateSummary).to receive(:new).and_return(
    ...
    it 'can use the username short flag from tool options' do
      ...
      expected_options =
        FastlaneCore::Configuration.create(
          Deliver::Options.available_options,
          { description: 'My description', username: 'me@it.com', force: true }
        )
    ...
    it 'can use the app_identifier flag from tool options' do
      ...
      expected_options =
        FastlaneCore::Configuration.create(
          Deliver::Options.available_options,
          { description: 'My description', app_identifier: 'abcd', force: true }
        )
  ...
  describe ':download_screenshots option handling' do
    def expect_download_screenshots_run_with(expected_options)
      expect(Deliver::DownloadScreenshots).to receive(
                  :run
                ) do |actual_options, screenshots_path|
        expect(expected_options._values).to eq(actual_options._values)
  ...
  describe ':download_metadata option handling' do
    it 'can use the app_identifier flag from tool options' do
      ...
      expect(Deliver::Setup).to receive(:new).and_return(fake_setup)
      expect(fake_setup).to receive(
                  :generate_metadata_files
                ) do |version, metadata_path|
    ...
    describe 'force overwriting metadata' do
      it 'forces overwriting metadata if force is set' do
        ...
        expect(
          Deliver::CommandsGenerator.force_overwrite_metadata?(
            options,
            'an/ignored/path'
          )
        ).to be_truthy
