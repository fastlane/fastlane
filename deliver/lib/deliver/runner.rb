  ...
  class Runner
    ...
    def initialize(options, skip_auto_detection = {})
      ...
      FastlaneCore::PrintTable.print_values(
        config: options,
        hide_keys: %i[app],
        mask_keys: %w[app_review_information.demo_password],
        title: "deliver #{Fastlane::VERSION} Summary"
      )
    end
    ...
    def run
      if options[:app_version].to_s.length > 0 &&
         !options[:skip_app_version_update]
        verify_version
      end
      upload_metadata
      ...
      unless options[:skip_binary_upload]
        UI.success('Finished the upload to App Store Connect')
      end
    ...
    def verify_version
      ...
      changed =
        options[:app].ensure_version!(app_version, platform: options[:platform])

      if changed
        UI.success("Successfully set the version to '#{app_version}'")
      else
        UI.success(
          "'#{app_version}' is the latest version on App Store Connect"
        )
      end
    end
    ...
    def prepare_app_icons(options = {})
      ...
      default_app_icon_path =
        Dir[File.join(options[:metadata_path], 'app_icon.{png,jpg}')].first
      if default_app_icon_path && File.exist?(default_app_icon_path)
        options[:app_icon] ||= default_app_icon_path
      end

      default_watch_icon_path =
        Dir[File.join(options[:metadata_path], 'watch_icon.{png,jpg}')].first
      if default_watch_icon_path && File.exist?(default_watch_icon_path)
        options[:apple_watch_app_icon] ||= default_watch_icon_path
      end
    end
    ...
    def upload_binary
      ...
      if options[:ipa]
        ...
        package_path =
          FastlaneCore::PkgUploadPackageBuilder.new.generate(
            app_id: options[:app].apple_id,
