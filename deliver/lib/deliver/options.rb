...
module Deliver
  ...
  class Options
    def self.available_options
      user =
        CredentialsManager::AppfileConfig.try_fetch_value(:itunes_connect_id)
      ...
      [
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Could not find ipa file at path '#{File.expand_path(value)}'"
                )
              end
              unless value.end_with?('.ipa')
                UI.user_error!("'#{value}' doesn't seem to be an ipa file")
              end
            end,
          conflicting_options: %i[pkg],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'ipa' and '#{value.key}' options in one run."
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Could not find pkg file at path '#{File.expand_path(value)}'"
                )
              end
              unless value.end_with?('.pkg')
                UI.user_error!("'#{value}' doesn't seem to be a pkg file")
              end
            end,
          conflicting_options: %i[ipa],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'pkg' and '#{value.key}' options in one run."
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          optional: true,
          conflicting_options: %i[ipa pkg],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'build_number' and '#{value
                  .key}' options in one run."
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless "'ios', 'appletvos', 'osx'".include?(value)
                UI.user_error!(
                  'The platform can only be ios, appletvos, or osx'
                )
              end
            end
        ),
        ...
        FastlaneCore::ConfigItem.new(
          key: :auto_release_date,
          env_name: 'DELIVER_AUTO_RELEASE_DATE',
          description:
            'Date in milliseconds for automatically releasing on pending approval',
          is_string: false,
          optional: true,
          conflicting_options: %i[automatic_release],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can't use 'auto_release_date' and '#{value
                  .key}' options together."
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Could not find config file at path '#{File.expand_path(
                    value
                  )}'"
                )
              end
        ...
        FastlaneCore::ConfigItem
          ...
          optional: true,
          is_string: false,
          # as we also allow integers, which we convert to strings anyway
          code_gen_sensitive: true,
          default_value:
            CredentialsManager::AppfileConfig.try_fetch_value(:itc_team_id),
          default_value_dynamic: true,
          verify_block:
            proc { |value| ENV['FASTLANE_ITC_TEAM_ID'] = value.to_s }
        ),
        FastlaneCore::ConfigItem.new(
          key: :team_name,
          short_option: '-e',
          ...
          code_gen_sensitive: true,
          default_value:
            CredentialsManager::AppfileConfig.try_fetch_value(:itc_team_name),
          default_value_dynamic: true,
          verify_block:
            proc { |value| ENV['FASTLANE_ITC_TEAM_NAME'] = value.to_s }
        ),
        FastlaneCore::ConfigItem.new(
        ...
        FastlaneCore::ConfigItem.new(
          ...
          default_value_dynamic: true,
          verify_block: proc { |value| ENV['FASTLANE_TEAM_NAME'] = value.to_s }
        ),
        ...
        FastlaneCore::ConfigItem
          .new(
          key: :run_precheck_before_submit,
          short_option: '-x',
        ...
        FastlaneCore::ConfigItem.new(
          key: :precheck_default_rule_level,
          short_option: '-d',
        ...
        FastlaneCore::ConfigItem
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Could not find png file at path '#{File.expand_path(value)}'"
                )
              end
              unless Deliver::UploadAssets::SUPPORTED_ICON_EXTENSIONS.include?(
                     File.extname(value).downcase
                   )
                UI.user_error!(
                  "'#{value}' doesn't seem to be one of the supported files. supported: #{Deliver::UploadAssets::SUPPORTED_ICON_EXTENSIONS
                    .join(',')}"
                )
              end
            end
        ...
        FastlaneCore::ConfigItem.new(
          key: :primary_first_sub_category,
          description:
        ...
        FastlaneCore::ConfigItem.new(
          key: :primary_second_sub_category,
          description:
        ...
        FastlaneCore::ConfigItem.new(
          key: :secondary_first_sub_category,
          description:
        ...
        FastlaneCore::ConfigItem.new(
          key: :secondary_second_sub_category,
          description:
        ...
        FastlaneCore::ConfigItem.new(
          key: :trade_representative_contact_information,
          description:
          ...
          verify_block:
            proc do |value|
              unless value.kind_of?(Hash)
                UI.user_error!(
                  ':subtitle must be a hash, with the language being the key'
                )
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless value.kind_of?(Hash)
                UI.user_error!(
                  ':keywords must be a hash, with the language being the key'
                )
              end
              value.each do |language, keywords|
                ...
                unless keywords.kind_of?(String)
                  UI.user_error!(
                    'keywords must be a hash with all values being strings'
                  )
                end
              end
            end
        ...
        FastlaneCore::ConfigItem.new(
          key: :ignore_language_directory_validation,
        ...
        FastlaneCore::ConfigItem
          ...
          is_string: false
        ) # don't add any verification here, as it's used to store a spaceship ref
      ]
    end
  end
