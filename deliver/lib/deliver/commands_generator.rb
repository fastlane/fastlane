...
module Deliver
  class CommandsGenerator
    ...
    def deliverfile_options(skip_verification: false)
      ...
      irrelevant_options_keys = %i[ipa pkg app_rating_config_path]
    ...
    def self.force_overwrite_metadata?(options, path)
      ...
      if UI.interactive?
        res ||=
          UI.confirm(
            "Do you want to overwrite existing metadata on path '#{File
              .expand_path(path)}'?"
          )
      end
      res
    ...
    def run
      ...
      command :run do |c|
        ...
        c.action do |args, options|
          ...
          if options[:description] || options[:ipa] || options[:pkg]
            loaded = true
          end
          if File.exist?(
             File.join(FastlaneCore::FastlaneFolder.path || '.', 'metadata')
           )
            loaded = true
          end
          unless loaded
            if UI.confirm(
               'No deliver configuration found in the current directory. Do you want to setup deliver?'
             )
              is_swift =
                UI.confirm('Would you like to use Swift instead of Ruby?')
      ...
      command :submit_build do |c|
        ...
        c.description =
          'Submit a specific build-nr for review, use latest for the latest build'
      ...
      command :init do |c|
        ...
        c.description =
          'Create the initial `deliver` configuration based on an existing app'
        ...
        c.action do |args, options|
          if File.exist?('Deliverfile') || File.exist?('fastlane/Deliverfile')
            UI.important(
              'You already have a running deliver setup in this directory'
            )
            return 0
          end
      ...
      command :generate_summary do |c|
        ...
        c.description =
          'Generate HTML Summary without uploading/downloading anything'
      ...
      command :download_screenshots do |c|
        ...
        c.description =
          'Downloads all existing screenshots from App Store Connect and stores them in the screenshots folder'
        ...
        c.action do |args, options|
          options =
            FastlaneCore::Configuration.create(
              deliverfile_options(skip_verification: true),
              options.__hash__
            )
          options.load_configuration_file('Deliverfile')
      ...
      command :download_metadata do |c|
        ...
        c.description =
          'Downloads existing metadata and stores it locally. This overwrites the local files.'
        ...
        c.action do |args, options|
          ...
          res =
            Deliver::CommandsGenerator.force_overwrite_metadata?(options, path)
          return 0 unless res
          ...
          if options[:app_version].to_s.length > 0
            ...
            if v.version != options[:app_version]
              raise "Neither the current nor live version match specified app_version \"#{options[
                      :app_version
                    ]}\""
            end
          end
