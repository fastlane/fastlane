  ...
  module Loader
    ...
    def self.language_folders(root, ignore_validation)
      ...
      allowed_directory_names =
        allowed_directory_names_with_case.map(&:downcase).freeze

      selected_folders =
        folders.select do |path|
          File.directory?(path) &&
            allowed_directory_names.include?(File.basename(path).downcase)
        end.sort
      ...
      rejected_folders =
        folders.select do |path|
          ...
          File.directory?(path) &&
            !allowed_directory_names.include?(normalized_path) &&
            !EXCEPTION_DIRECTORIES.include?(normalized_path)
        end.sort

      if !ignore_validation && !rejected_folders.empty?
        rejected_folders = rejected_folders.map { |path| File.basename(path) }
        UI.user_error!(
          "Unsupported directory name(s) for screenshots/metadata in '#{root}': #{rejected_folders
            .join(', ')}" \
