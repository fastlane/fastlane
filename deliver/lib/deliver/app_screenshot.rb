  ...
  class AppScreenshot
    ...
    def initialize(path, language, screen_size = nil)
      ...
      unless self.is_valid?
        UI.error(
          "Looks like the screenshot given (#{path}) does not match the requirements of #{screen_size}"
        )
      end
    end
    ...
    def is_valid?
      unless %w[png PNG jpg JPG jpeg JPEG].include?(self.path.split('.').last)
        return false
      end
    ...
    def self.device_messages
      return {
        ScreenSize::IOS_65_MESSAGES => [[1242, 2688]],
        ScreenSize::IOS_61_MESSAGES => [[828, 1792]],
        ScreenSize::IOS_58_MESSAGES => [[1125, 2436]],
        ScreenSize::IOS_55_MESSAGES => [[1080, 1920], [1242, 2208]],
        ScreenSize::IOS_47_MESSAGES => [[750, 1334]],
        ScreenSize::IOS_40_MESSAGES => [[640, 1136], [640, 1096], [1136, 600]], # landscape status bar is smaller
        ScreenSize::IOS_IPAD_MESSAGES => [
          [1024, 748],
          [1024, 768],
          [2048, 1496],
        ...
        ScreenSize::IOS_IPAD_10_5_MESSAGES => [[1668, 2224], [2224, 1668]],
        ScreenSize::IOS_IPAD_11 => [[1668, 2388], [2388, 1668]],
        ScreenSize::IOS_IPAD_PRO_MESSAGES => [[2732, 2048], [2048, 2732]]
      }
    end
    ...
    def self.devices
      return {
        ScreenSize::IOS_65 => [[1242, 2688]],
        ScreenSize::IOS_61 => [[828, 1792]],
        ScreenSize::IOS_58 => [[1125, 2436]],
        ScreenSize::IOS_55 => [[1080, 1920], [1242, 2208]],
        ScreenSize::IOS_47 => [[750, 1334]],
        ScreenSize::IOS_40 => [[640, 1136], [640, 1096], [1136, 600]], # landscape without status bar
        ScreenSize::IOS_35 => [[640, 960], [640, 920], [960, 600]], # landscape without status bar
        ScreenSize::IOS_IPAD => [
          [],
          [1024, 768],
          [2048, 1496],
        ...
        ScreenSize::IOS_IPAD_10_5 => [[1668, 2224], [2224, 1668]],
        ScreenSize::IOS_IPAD_11 => [[1668, 2388], [2388, 1668]],
        ScreenSize::IOS_IPAD_PRO => [[2732, 2048], [2048, 2732]],
        ScreenSize::MAC => [
          [1280, 800],
          [1440, 900],
          [2560, 1600],
        ...
        ScreenSize::IOS_APPLE_WATCH => [[312, 390]],
        ScreenSize::IOS_APPLE_WATCH_SERIES4 => [[368, 448]],
        ScreenSize::APPLE_TV => [[1920, 1080], [3840, 2160]]
      }
    end
    ...
    def self.calculate_screen_size(path)
      ...
      if size.nil? || size.count == 0
        UI.user_error!("Could not find or parse file at path '#{path}'")
      end
      ...
      skip_landscape = true if path_component.eql?('appleTV')

      # iMessage screenshots have same resolution as app screenshots so we need to distinguish them
      devices =
        path_component.eql?('iMessage') ? self.device_messages : self.devices
