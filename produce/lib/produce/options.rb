...
module Produce
  class Options
    def self.available_options
      user =
        CredentialsManager::AppfileConfig.try_fetch_value(:apple_dev_portal_id)
      ...
      [
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless "'ios', 'osx'".include?(value)
                UI.user_error!('The platform can only be ios or osx')
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              ...
              unless value.kind_of?(Hash)
                UI.user_error!('enabled_features has to be of type Hash')
              end
              value.each do |key, v|
                unless allowed_keys.include?(key.to_sym)
                  UI.user_error!(
                    "The key: '#{key}' is not supported in `enabled_features' - following keys are available: [#{allowed_keys
                      .join(',')}]"
                  )
                end
              end
        ...
        FastlaneCore::ConfigItem.new(
          ...
          description:
            "Array with Spaceship App Services (e.g. #{allowed_services_description})",
          is_string: false,
          default_value: {},
          verify_block:
            proc do |value|
              allowed_keys = Produce::DeveloperCenter::ALLOWED_SERVICES.keys
              unless value.kind_of?(Hash)
                UI.user_error!('enable_services has to be of type Hash')
              end
              value.each do |key, v|
                unless allowed_keys.include?(key.to_sym)
                  UI.user_error!(
                    "The key: '#{key}' is not supported in `enable_services' - following keys are available: [#{allowed_keys
                      .join(',')}]"
                  )
                end
              end
