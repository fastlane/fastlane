...
module Produce
  class ItunesConnect
    def run
      ...
      if wildcard_bundle?
        @full_bundle_identifier.gsub!(
          '*',
          Produce.config[:bundle_identifier_suffix].to_s
        )
      end
    ...
    def create_new_app
      ...
      if application
        UI.success(
          "App '#{Produce.config[
            :app_identifier
          ]}' already exists (#{application
            .apple_id}), nothing to do on App Store Connect"
        )
        # Nothing to do here
      else
        UI.success(
          "Creating new app '#{Produce.config[:app_name]}' on App Store Connect"
        )
        ...
        if generated_app['adamId'].to_s.empty?
          UI.crash!('Something went wrong when creating the new app on iTC')
        end
        ...
        while application.nil?
          ...
          if counter == 200
            UI.crash!(
              "Couldn't find newly created app on App Store Connect - please check the website for more information"
            )
          end
          ...
          UI.message(
            'Waiting for the newly created application to be available on App Store Connect...'
          )
          sleep(15)
          application = fetch_application
        end
        ...
        unless application
          UI.crash!(
            "Something went wrong when creating the new app - it's not listed in the App's list"
          )
        end

        UI.message('Ensuring version number')
        if Produce.config[:app_version]
          application.ensure_version!(
            Produce.config[:app_version],
            platform: Produce.config[:platform]
          )
        end

        UI.success(
          "Successfully created new app '#{Produce.config[
            :app_name
          ]}' on App Store Connect with ID #{application.apple_id}"
        )
      end

      return Spaceship::Tunes::Application.find(
    ...
    def language
      ...
      converted =
        Spaceship::Tunes::LanguageConverter.from_itc_readable_to_itc(@language)
