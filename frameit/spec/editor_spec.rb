describe Frameit do
  describe Frameit::Editor do
    describe 'frame!' do
      before do
        allow_any_instance_of(MiniMagick::Tool::Mogrify).to receive(:call) do
                                   ''
                                 end
      ...
      it 'properly frame screenshots with captions that include apostrophes' do
        expect_any_instance_of(MiniMagick::Tool::Mogrify).to receive(:draw)
                                    .with(
                                    "text 0,0 'Don\\'t forget the apostrophes'"
                                  )
        screenshot =
          Frameit::Screenshot.new(
            './frameit/spec/fixtures/editor/apostrophes.png',
            Frameit::Color::BLACK
          )
      ...
      it 'does not double escape apostrophes' do
        ...
        screenshot =
          Frameit::Screenshot.new(
            './frameit/spec/fixtures/editor/escaped-apostrophes.png',
            Frameit::Color::BLACK
          )
        Frameit::Editor.new(screenshot).frame!
      end
    ...
    describe 'should_skip?' do
      it 'returns true with no matching filter in Framefile.json' do
        screenshot =
          Frameit::Screenshot.new(
            './frameit/spec/fixtures/editor/ignore.png',
            Frameit::Color::BLACK
          )
