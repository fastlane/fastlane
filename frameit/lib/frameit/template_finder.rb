...
module Frameit
  ...
  class TemplateFinder
    ...
    def self.get_template(screenshot)
      ...
      UI.verbose(
        "Looking for #{filename} and found #{templates.count} template(s)"
      )

      if templates.count == 0
        if screenshot.screen_size == Deliver::AppScreenshot::ScreenSize::IOS_35
          UI.important(
            "Unfortunately 3.5\" device frames were discontinued. Skipping screen '#{screenshot
              .path}'"
          )
          UI.error("Looked for: '#{filename}.png'")
        elsif screenshot.color == Frameit::Color::ROSE_GOLD ||
              screenshot.color == Frameit::Color::GOLD
          ...
          UI.important(
            "Unfortunately device type '#{screenshot
              .device_name}' is not available in #{screenshot
              .color}, falling back to silver..."
          )
