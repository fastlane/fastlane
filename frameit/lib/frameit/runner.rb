...
module Frameit
  class Runner
    def initialize
      ...
      downloader.download_frames unless downloader.frames_exist?
    end

    def run(path, color = nil)
      unless color
        color = Frameit::Color::BLACK
        if Frameit.config[:white] || Frameit.config[:silver]
      ...
      if screenshots.count > 0
        screenshots.each do |full_path|
          ...
          begin
            ...
            if editor.should_skip?
              UI.message(
                "Skipping framing of screenshot #{screenshot
                  .path}.  No title provided in your Framefile.json or title.strings."
              )
            else
        ...
        UI.error(
          "Could not find screenshots in current directory: '#{File.expand_path(
            path
          )}'"
        )
      end
