...
module Frameit
  ...
  class Trimbox
    ...
    def initialize(identify_string)
      unless identify_string.length > 0
        UI.user_error!(
          "Trimbox can not be initialised with an empty 'identify_string'."
        )
      end
      ...
      unless trim_values.length == 4
        UI.user_error!(
          "Trimbox is initialised with an invalid value for 'identify_string'."
        )
      end
