describe Gym do
  describe Gym::Options do
    ...
    it 'removes the `ipa` from the output name if given',
    ...
    it 'automatically chooses an existing scheme if the the defined one is not available',
       requires_xcodebuild: true do
      options = {
        project: './gym/examples/standard/Example.xcodeproj', scheme: 'NotHere'
      }
      Gym.config =
        FastlaneCore::Configuration.create(
          Gym::Options.available_options,
    ...
    it 'replaces SEPARATOR (i.e. /) character with underscore(_) in output name',
       requires_xcodebuild: true do
      options = {
        output_name: 'feature' + File::SEPARATOR + 'Example.ipa',
        project: './gym/examples/standard/Example.xcodeproj'
      }
      Gym.config =
        FastlaneCore::Configuration.create(
