...
%"
  ...
  describe Gym::ErrorHandler, requires_xcodebuild: true do
    ...
    def mock_gym_path(content)
      ...
      allow(Gym::BuildCommandGenerator).to receive(:xcodebuild_log_path)
                 .and_return(log_path)
    ...
    it 'raises build error with error_info' do
      ...
      expect(UI).to receive(:build_failure!).with(
...
"
      ...
      expect(UI).to receive(:build_failure!).with(
                  'Error building the application - see the log above',
                  error_info: code_signing_output
                )
      expect(UI).to receive(:command_output).with(
                  "No profile for team 'N8X438SEU2' matching 'match AppStore me.themoji.app.beta' found:  Xcode couldn't find any provisioning profiles matching 'N8X438SEU2/match AppStore me.themoji.app.beta'. " \
    ...
    it 'prints mismatch between the export_method and the selected profiles only once' do
      ...
      expect(UI).to receive(:error).with(/There seems to be a mismatch between/)
                  .once
