describe Gym do
  ...
  before(:each) { allow(Gym).to receive(:project).and_return(@project) }
  ...
  describe Gym::PackageCommandGeneratorXcode7, requires_xcodebuild: true do
    it 'passes xcargs through to xcode build wrapper ' do
      ...
      expect(result).to eq(
                  [
                    "/usr/bin/xcrun #{Gym::PackageCommandGeneratorXcode7
                      .wrap_xcodebuild
                      .shellescape} -exportArchive",
                    "-exportOptionsPlist '#{Gym::PackageCommandGeneratorXcode7
                      .config_path}'",
                    "-archivePath #{Gym::BuildCommandGenerator.archive_path
                      .shellescape}",
                    "-exportPath '#{Gym::PackageCommandGeneratorXcode7
    ...
    it 'works with spaces in path name' do
      ...
      allow(Gym::PackageCommandGeneratorXcode7).to receive(:wrap_xcodebuild)
                 .and_return('/tmp/path with spaces')
    ...
    it 'generates a valid plist file we need' do
      ...
      expect(Plist.parse_xml(config_path)).to eq({ 'method' => 'app-store' })
    end
    ...
    it 'defaults to the correct export type if :export_options parameter is provided' do
      options = {
        project: './gym/examples/standard/Example.xcodeproj',
        export_options: { include_symbols: true }
      }
      ...
      expect(Plist.parse_xml(config_path)).to eq(
                  {
                    ...
                    'manifest' => {
                      ...
                      'displayImageURL' =>
                        'https://www.example.com/display%20image.png',
                      'fullSizeImageURL' =>
                        'https://www.example.com/fullSize%20image.png'
                    },
                    'method' => 'app-store',
                    'uploadSymbols' => true,
                    'uploadBitcode' => false,
    ...
    it "doesn't store bitcode/symbols information for non app-store builds" do
      ...
      expect(Plist.parse_xml(config_path)).to eq({ 'method' => 'ad-hoc' })
    end
    ...
    it 'uses a temporary folder to store the resulting ipa file' do
      ...
      result = Gym::PackageCommandGeneratorXcode7.generate
      expect(Gym::PackageCommandGeneratorXcode7.temporary_output_path).to match(
                  %r{#{Dir.tmpdir}/gym_output.+}
                )
      expect(Gym::PackageCommandGeneratorXcode7.manifest_path).to match(
                  %r{#{Dir.tmpdir}/gym_output.+/manifest.plist}
                )
      expect(Gym::PackageCommandGeneratorXcode7.app_thinning_path).to match(
                  %r{#{Dir.tmpdir}/gym_output.+/app-thinning.plist}
                )
      expect(
        Gym::PackageCommandGeneratorXcode7.app_thinning_size_report_path
      ).to match(%r{#{Dir.tmpdir}/gym_output.+/App Thinning Size Report.txt})
      expect(Gym::PackageCommandGeneratorXcode7.apps_path).to match(
                  %r{#{Dir.tmpdir}/gym_output.+/Apps}
                )
    end
  end
end
