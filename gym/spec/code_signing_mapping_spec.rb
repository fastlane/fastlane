describe Gym::CodeSigningMapping do
  describe '#app_identifier_contains?' do
    ...
    it "returns true if it doesn't contain it" do
      csm = Gym::CodeSigningMapping.new(project: nil)
      return_value =
        csm.app_identifier_contains?('FuLL-StRing Yo', 'fullstringyo')
    ...
    it 'Replace the inhouse keyword for enterprise profiles' do
      csm = Gym::CodeSigningMapping.new(project: nil)
      return_value =
        csm.app_identifier_contains?('match InHouse bundle', 'enterprise')
      expect(return_value).to eq(true)
    end
  end

  describe '#detect_project_profile_mapping' do
    it 'returns the mapping of the selected provisioning profiles',
       requires_xcode: true do
      workspace_path =
        'gym/spec/fixtures/projects/cocoapods/Example.xcworkspace'
      project = FastlaneCore::Project.new({ workspace: workspace_path })
      csm = Gym::CodeSigningMapping.new(project: project)
      expect(csm.detect_project_profile_mapping).to eq(
                  {
                    'family.wwdc.app' => 'match AppStore family.wwdc.app',
                    'family.wwdc.app.watchkitapp' =>
                      'match AppStore family.wwdc.app.watchkitapp',
                    'family.wwdc.app.watchkitapp.watchkitextension' =>
                      'match AppStore family.wwdc.app.watchkitappextension'
                  }
                )
    end

    it 'detects the build configuration from selected scheme',
       requires_xcode: true do
      workspace_path =
        'gym/spec/fixtures/projects/cocoapods/Example.xcworkspace'
      project = FastlaneCore::Project.new({ workspace: workspace_path })
      ...
      expect(csm.detect_project_profile_mapping).to eq(
                  {
                    'family.wwdc.app' => 'match Development family.wwdc.app',
                    'family.wwdc.app.watchkitapp' =>
                      'match Development family.wwdc.app.watchkitapp',
                    'family.wwdc.app.watchkitapp.watchkitextension' =>
                      'match Development family.wwdc.app.watchkitappextension'
                  }
                )
    end
  end
  ...
  describe '#detect_project_profile_mapping_for_tv_os' do
    it 'returns the mapping of the selected provisioning profiles for tv_os',
       requires_xcode: true do
      workspace_path =
        'gym/spec/fixtures/projects/cocoapods/Example.xcworkspace'
      project = FastlaneCore::Project.new({ workspace: workspace_path })
      ...
      expect(csm.detect_project_profile_mapping).to eq(
                  { 'family.wwdc.app' => 'match AppStore family.wwdc.app.tvos' }
                )
    end
  end
  ...
  describe '#merge_profile_mapping' do
    ...
    it 'only mapping from match (user) is available' do
      result =
        csm.merge_profile_mapping(
          primary_mapping: { 'identifier.1' => 'value.1' },
          secondary_mapping: {},
          export_method: 'app-store'
        )
    ...
    it 'accesses the Xcode profile mapping, if nothing else is given' do
      ...
      result =
        csm.merge_profile_mapping(
          primary_mapping: {}, export_method: 'app-store'
        )
    ...
    describe 'handle conflicts' do
      it 'Both primary and secondary are available, and both match the export method, it should prefer the primary mapping' do
        result =
          csm.merge_profile_mapping(
            primary_mapping: { 'identifier.1' => 'Ap-pStoreValue2' },
      ...
      it 'Both primary and secondary are available, and the secondary is the only one that matches the export type' do
        result =
          csm.merge_profile_mapping(
            primary_mapping: { 'identifier.1' => 'Ap-p StoreValue1' },
      ...
      it 'both primary and secondary are available, and neither of them match the export type, it should choose the secondary_mapping' do
        result =
          csm.merge_profile_mapping(
            primary_mapping: { 'identifier.1' => 'AppStore' },
      ...
      context 'when both primary and secondary are available and same value' do
        let(:result) do
          csm.merge_profile_mapping(
            primary_mapping: { primary_key => 'AppStore' },
