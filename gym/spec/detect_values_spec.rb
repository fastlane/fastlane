  ...
  describe Gym::DetectValues do
    ...
    describe 'Xcode config handling', :stuff, requires_xcodebuild: true do
      it 'fetches the custom build path from the Xcode config' do
        expect(Gym::DetectValues).to receive(:has_xcode_preferences_plist?)
                    .and_return(true)
        expect(Gym::DetectValues).to receive(:xcode_preferences_dictionary)
                    .and_return(
                    { 'IDECustomDistributionArchivesLocation' => '/test/path' }
                  )

        options = {
          project: './gym/examples/multipleSchemes/Example.xcodeproj'
        }
        Gym.config =
