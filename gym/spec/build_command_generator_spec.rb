  ...
  describe Gym::BuildCommandGenerator do
    ...
    it 'supports additional parameters', requires_xcodebuild: true do
      log_path =
        File.expand_path(
          "#{FastlaneCore::Helper
            .buildlog_path}/gym/ExampleProductName-Example.log"
        )
    ...
    describe 'Standard Example' do
      ...
      it '#buildlog_path is not used when not provided',
        ...
        expect(result.to_s).to include(
                    File.expand_path(
                      "#{FastlaneCore::Helper.buildlog_path}/gym"
                    )
                  )
      end
    end
    ...
    describe 'Analyze Build Time Example' do
      ...
      it 'uses the correct build command with the example project when option is enabled',
         requires_xcodebuild: true do
        options = {
          project: './gym/examples/standard/Example.xcodeproj',
        ...
        expect(result).to eq(
                    [
                      "grep -E '^[0-9.]+ms' #{@log_path
                        .shellescape} | grep -vE '^0\.[0-9]' | sort -nr > culprits.txt"
                    ]
                  )
      end

      it 'uses the correct build command with the example project when option is disabled',
         requires_xcodebuild: true do
        options = {
          project: './gym/examples/standard/Example.xcodeproj',
