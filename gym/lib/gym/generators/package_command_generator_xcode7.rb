      ...
      def ipa_path
        unless Gym.cache[:ipa_path]
          ...
          if path
            ...
            FileUtils.mv(path, Gym.cache[:ipa_path]) unless Helper.compare_paths(path, Gym.cache[:ipa_path])
            ...
            Gym.cache[:ipa_path] = File.join(temporary_output_path, "#{Gym.config[:output_name]}.ipa")
            FileUtils.cp(path, Gym.cache[:ipa_path]) unless Helper.compare_paths(path, Gym.cache[:ipa_path])
          else
