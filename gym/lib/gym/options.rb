    ...
    def self.plain_options
      [
        FastlaneCore::ConfigItem.new(
          ...
          conflicting_options: %i[project],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can only pass either a 'workspace' or a '#{value
                  .key}', not both"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          conflicting_options: %i[workspace],
          conflict_block:
            proc do |value|
              UI.user_error!(
                "You can only pass either a 'project' or a '#{value
                  .key}', not both"
              )
            end
        ),
          ...
          verify_block:
            proc do |value|
              ...
              unless av.include?(value)
                UI.user_error!(
                  "Unsupported export_method '#{value}', must be: #{av}"
                )
              end
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          conflict_block:
            proc do |value|
              UI.user_error!(
                "'#{value.key}' must be false to use 'export_options'"
              )
            end
        ),
        FastlaneCore::ConfigItem.new(
          ...
          conflict_block:
            proc do |value|
              UI.user_error!(
                "'#{value.key}' must be false to use 'export_xcargs'"
              )
            end,
          type: :shell_string
        ),
        ...
        FastlaneCore::ConfigItem.new(
          ...
          verify_block:
            proc do |value|
              unless File.exist?(value)
                UI.user_error!(
                  "Formatter file not found at path '#{File.expand_path(
                    value
                  )}'"
                )
              end
