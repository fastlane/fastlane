    ...
    class << self
      ...
      def handle_build_error(output)
        ...
        when /Your build settings specify a provisioning profile with the UUID/
          ...
          print(
            "Your project defines a provisioning profile which doesn't exist on your local machine"
          )
          print(
            'You can use sigh (https://docs.fastlane.tools/actions/sigh/) to download and install the provisioning profile'
          )
          print(
            'Follow this guide: https://docs.fastlane.tools/codesigning/GettingStarted/'
          )
        when /Provisioning profile does not match bundle identifier/
          print('Invalid code signing settings')
          print(
            "Your project defines a provisioning profile that doesn't match the bundle identifier of your app"
          )
          print(
            'Make sure you use the correct provisioning profile for this app'
          )
          print('Take a look at the ouptput above for more information')
          print(
            'You can follow this guide: https://docs.fastlane.tools/codesigning/GettingStarted/'
          )
        when
          print(
            "You don't have the provisioning profile for '#{$1}' installed on the local machine"
          )
          print(
            "Make sure you have the profile on this computer and it's properly installed"
          )
          print(
            'You can use sigh (https://docs.fastlane.tools/actions/sigh/) to download and install the provisioning profile'
          )
        ...
        when
          print(
            "You don't have a provisioning profile for the bundle identifier '#{$1}' installed on the local machine"
          )
          print(
            "Make sure you have the profile on this computer and it's properly installed"
          )
        ...
        when /code signing is required/
          ...
          print(
            'To generate an ipa file you need to enable code signing for your project'
          )
        ...
        when /Code signing is required for product/
          ...
          print(
            'The very bottom of the file will tell you the raw Xcode error message'
          )
      ...
      def handle_package_error(output)
        ...
        when /single\-bundle/
          print(
            'Your project does not contain a single–bundle application or contains multiple products'
          )
          print(
            'Please read the documentation provided by Apple: https://developer.apple.com/library/ios/technotes/tn2215/_index.html'
          )
        ...
        when /mismatch between specified provisioning profile and signing identity/
          print(
            'Mismatch between provisioning profile and code signing identity'
          )
          print(
            'This means, the specified provisioning profile was not created using'
          )
          print('the specified certificate.')
          print(
            'Run cert and sigh before gym to make sure to have all signing resources ready'
          )
        ...
        when /Codesign check fails/
          print(
            'A general code signing error occurred. Make sure you passed a valid'
          )
      ...
      def handle_empty_archive
        ...
        print(
          'Also, make sure to have a valid code signing identity and provisioning profile installed'
        )
        print(
          'Follow this guide to setup code signing https://docs.fastlane.tools/codesigning/GettingStarted/'
        )
        print(
          'If your intention was only to export an ipa be sure to provide a valid archive at the archive path.'
        )
        print(
          'This error might also happen if your workspace/project file is not in the root directory of your project.'
        )
        ...
        print(
          'https://github.com/artsy/emission-nebula/commit/44fe51a7fea8f7d52f0f77d6c3084827fe5dd59e'
        )
      ...
      def print_full_log_path
        ...
        log_content.each { |row| UI.command_output(row) }
      ...
      def print_xcode_version
        ...
        FastlaneCore::PrintTable.print_values(
          config: values, hide_keys: [], title: 'Build environment'.yellow
        )
      end
      ...
      def print_xcode_path_instructions
        ...
        xcode_installations_in_default_path =
          Dir[File.join(default_xcode_path, 'Xcode*.app')]
        ...
        UI.important(
          "Found multiple versions of Xcode in '#{default_xcode_path}'"
        )
        UI.important(
      ...
      def print_environment_information
        if Gym.config[:export_method].to_s == 'development'
          ...
          selected_provisioning_profiles
            .each do |current_bundle_identifier, current_profile_name|
            available_export_types.each do |current_to_try, matching_type|
              unless current_profile_name.to_s.downcase.include?(
                     current_to_try.to_s.downcase
                   )
                next
              end
